...
Med hjälp av omfång kan du hantera behörigheter för skyddade resurser. När en åtkomsttoken begärs måste klient programmet ange önskade behörigheter i **scope** -parametern för begäran. Om du till exempel vill ange **omfattning svärdet** för `read` för API: et som har **app-ID-URI: n** för `https://contoso.onmicrosoft.com/api`, skulle omfånget `https://contoso.onmicrosoft.com/api/read`.
...
Om parametern **response_type** i en `/authorize`-begäran innehåller `token`måste **omfattnings** parametern innehålla minst en annan resurs omfattning än `openid` och `offline_access` som ska beviljas. Annars Miss lyckas `/authorize`-begäran.
...
När du använder https://jwt.ms för att kontrol lera den åtkomsttoken som returnerades, bör du se något som liknar följande exempel:
