<properties
    pageTitle="Lägga till inloggning till en Node.js-webbapp för Azure B2C Preview | Microsoft Azure"
...
# B2C Preview: Lägga till inloggning till en Node.js-webbapp
...
> [AZURE.NOTE] Den här artikeln innehåller inte information om hur du implementerar inloggning, registrering och profilhantering med hjälp av Azure Active Directory B2C. Den fokuserar på att anropa webb-API:er när användaren har autentiserats. Börja med [självstudierna för att komma igång med .NET-webbappar](active-directory-b2c-devquickstarts-web-dotnet.md) om du vill lära dig grunderna i Azure Active Directory B2C.
...
> [AZURE.WARNING]   För den här B2C-förhandsversionen måste du använda samma **klient-ID**,/**program-ID** och principer för både servern som kör webb-API-uppgiften och klienten som ansluter till den. Det här gäller även självstudierna för iOS och Android. Om du tidigare har skapat ett program i någon av dessa snabbstarter använder du dessa värden. Skapa inte nya.
...
- Ange `http://localhost/TodoListService` som en **svars-URL**. Den är standard-URL:en för det här kodexemplet.
...
- Kopiera **namnet** för varje princip när du har skapat den. Det bör ha prefixet `b2c_1_`.  Du behöver principnamnen senare.
...
## Gör nödvändiga förberedelser för katalogen
...
## Konfigurera din app att använda Passport-Node.js-strategin
...
- `clientID`: **Program-ID:t** som din app tilldelats på registreringsportalen.
- `returnURL`: **Omdirigerings-URI:n** som du angav på portalen.
...
## Använda Passport för att utfärda inloggnings- och utloggningsförfrågningar till Azure AD
...
## Skapa vyerna och vägarna i Express för att anropa dina principer
...
Nu kan du gå vidare till mer avancerade avsnitt. Du kanske vill prova följande:
