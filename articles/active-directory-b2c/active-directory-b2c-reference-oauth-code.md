...
1. Ersätt `{policy}` med namnet på en princip som du har skapat i din klient organisation, till `b2c_1_sign_in`exempel.
...
| visas |Valfritt |Typ av användar interaktion som krävs. För närvarande är `login`det enda giltiga värdet, vilket tvingar användaren att ange sina autentiseringsuppgifter för begäran. Enkel inloggning träder inte i kraft. |
...
Ett lyckat svar som `response_mode=query` använder ser ut så här:
...
| Parameter | Beskrivning |
| --- | --- |
| error |En fel kods sträng som du kan använda för att klassificera de typer av fel som inträffar. Du kan också använda strängen för att reagera på fel. |
...
Nu när du har skaffat en auktoriseringskod kan du lösa in `code` för en token till den avsedda resursen genom att skicka en post-begäran `/token` till slut punkten. I Azure AD B2C kan du [begära åtkomsttoken för andra API:](active-directory-b2c-access-tokens.md#request-a-token) er som vanligt genom att ange deras omfång i begäran.
...
| code |Obligatorisk |Den auktoriseringskod som du förvärvade i den första delen av flödet. |
...
Åtkomsttoken och ID-token är korta. När de har gått ut måste du uppdatera dem för att fortsätta att få åtkomst till resurser. Det gör du genom att skicka en ny post- `/token` begäran till slut punkten. Den här gången ska du `refresh_token` ange i stället `code`för:
...
| client_id |Obligatorisk |Det program-ID som har tilldelats din app i [Azure Portal](https://portal.azure.com). |
| client_secret |Obligatorisk |Client_secret som är kopplad till din client_id i [Azure Portal](https://portal.azure.com). |
...
| scope |De omfattningar som token är giltigt för. Du kan också använda omfattningarna för att cachelagra tokens för senare användning. |
| expires_in |Den tids längd som token är giltig (i sekunder). |
...
| Parameter | Beskrivning |
| --- | --- |
| error |En fel kods sträng som du kan använda för att klassificera typer av fel som inträffar. Du kan också använda strängen för att reagera på fel. |
| error_description |Ett fel meddelande som kan hjälpa dig att identifiera rotor saken till ett autentiseringsfel. |
