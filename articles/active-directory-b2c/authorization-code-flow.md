...
Nu när du har skaffat en auktoriseringskod kan du lösa in `code` för en token till den avsedda resursen genom att skicka en POST-begäran till `/token`-slutpunkten. I Azure AD B2C kan du [begära åtkomsttoken för andra API:](access-tokens.md#request-a-token) er som vanligt genom att ange deras omfång i begäran.
