...
1. Kör PowerShell-kommandot för att generera ett självsignerat certifikat. Ändra `-Subject` argumentet efter behov för ditt program och Azure AD B2C klient namn. Du kan också justera `-NotAfter` datum för att ange ett annat förfallo datum för certifikatet.
...
1. Öppna **hantera användar certifikat** > **Aktuell användare** > **personliga** > **certifikat** > *yourappname.yourtenant.onmicrosoft.com*
1. Välj **åtgärden** för certifikat > > **alla aktiviteter** > **Exportera**
...
Leta upp avsnittet `<ClaimsProviders>` och Lägg till följande XML-kodfragment.
...
Metadata för Azure AD B2Cs principen är tillgängliga på följande URL. Ersätt `tenant-name` med namnet på Azure AD B2C klienten och `policy-name` med namnet (ID) för principen:
...
`identifierUris` är en sträng samling som innehåller användardefinierade URI: er som unikt identifierar en webbapp i den Azure AD B2C klienten. Tjänste leverantören måste ange det här värdet i `Issuer`-elementet i en SAML-begäran.
...
Om du inte anger en URI för metadata kan du uttryckligen ange svars-URL: en. Den här valfria egenskapen representerar `AssertionConsumerServiceUrl` (`SingleSignOnService`-URL i metadata för tjänst leverantören) och `BindingType` antas vara `HTTP POST`.
...
I den här självstudien, där du använder SAML-testprogrammet, anger du egenskapen `url` för `replyUrlsWithType` till värdet som visas i följande JSON-kodfragment.
...
Välj **inloggning** och du bör visas en inloggnings skärm för slutanvändare. Vid inloggning utfärdas en SAML-kontroll tillbaka till exempel programmet.
