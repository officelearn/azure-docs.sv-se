---
title: Anpassa användar gränssnittet (UI) dynamiskt med anpassade principer
...
Den här artikeln fokuserar på hur du anpassar Azure AD B2C användar gränssnitt med *dynamiskt innehåll* med hjälp av anpassade principer. För att komma igång, se [UI-anpassning i en anpassad princip](custom-policy-ui-customization.md).

>[!NOTE]
>Azure AD B2C artikeln, [Konfigurera UI-anpassning i en anpassad princip](custom-policy-ui-customization.md), diskuterar följande grundläggande:
...
Avsnittet `ContentDefinitions` innehåller en serie med `ContentDefinition` XML-element. Attributet ID i `ContentDefinition`-elementet anger vilken typ av sida som relaterar till innehålls definitionen. Det innebär att elementet definierar den kontext som en anpassad HTML5/CSS-mall kommer att använda. I följande tabell beskrivs de innehålls Definitions-ID: n som identifieras av IEF-motorn och vilka sidtyper som är relaterade till dem.
...
I artikeln [Konfigurera UI-anpassning i en anpassad princip](custom-policy-ui-customization.md) , laddar du upp HTML5-filer till Azure Blob Storage. Dessa HTML5-filer är statiska och återger samma HTML-innehåll för varje begäran.
...
* Åsidosätt `LoadUri`-elementen så att de pekar på din HTML5-fil.
...
2. I fönstret **nytt projekt** väljer du **visuella C#**  > **webb** > **ASP.net Core webb program (.net Core)** .

3. Ge programmet namnet (till exempel *contoso. AADB2C. UI*) och välj sedan **OK**.

    ![Skapa nytt Visual Studio-projekt](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project1.png)

4. Välj mallen för **webb program** .

5. Ange autentiseringen till **Ingen autentisering**.

    ![Välj mall för webb program](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project2.png)

6. Klicka på **OK** för att skapa projektet.

## <a name="step-2-create-mvc-view"></a>Steg 2: skapa MVC-vy
### <a name="step-21-download-the-b2c-built-in-html5-template"></a>Steg 2,1: hämta den inbyggda HTML5-mallen B2C
...
### <a name="step-22-add-the-mvc-view"></a>Steg 2,2: Lägg till MVC-vyn
1. Högerklicka på mappen vyer/hem och **Lägg sedan till** > **nytt objekt**.

    ![Meny alternativet Lägg till nytt objekt i Visual Studio](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view1.png)
...
5. Välj **Lägg till**.
    ...
    ![Dialog rutan Lägg till nytt objekt i Visual Studio med MVC View-sidan markerad](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view2.png)
...
10. Kopiera innehållet i filen och klistra in det under layout-definitionen. Koden bör se ut så här:
    ...
    ![enhetlig. cshtml-fil efter tillägg av HTML5](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-edit-view1.png)

### <a name="step-23-change-the-background-image"></a>Steg 2,3: ändra bakgrunds bilden

Leta upp det `<img>`-element som innehåller värdet för `ID` *background_background_image*och ersätt sedan `src`-värdet med **https://kbdevstorage1.blob.core.windows.net/asset-blobs/19889_en_1** eller någon annan bakgrunds bild som du vill använda.

![img-element med anpassat background_background_image src-värde](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-static-background.png)
...
1. Öppna **Controllers\HomeController.cs**och Lägg till följande metod:
    ...
    ![Ändra kontroll enheten så att den återger vyn](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-controller-view.png)
...
1. I **Solution Explorer**högerklickar du på projektet **contoso. AADB2C. UI** och väljer sedan **publicera**.
    ...
    ![Publicera till Microsoft Azure App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish1.png)

2. Välj panelen **Microsoft Azure App Service** och välj sedan **publicera**.

    ![Skapa nytt Microsoft Azure App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish2.png)
...
4. Välj **Skapa** för att börja skapa Azure-resurser.
    ...
    ![Ange App Service egenskaper](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish3.png)

    När processen har skapats publicerar guiden ASP.NET-webbappen till Azure och startar sedan appen i standard webbläsaren.

5. Kopiera URL: en för den _enhetliga_ sidan (till exempel _https://< APP_NAME >. azurewebsites. net/Home/Unified_).
...
1. I [Azure Portal](https://portal.azure.com/)väljer du **app Services**och väljer sedan namnet på din API-app.
    ...
    ![Välj API-app i Azure Portal](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS1.png)

2. I avsnittet **Inställningar** under **API** -avsnittet väljer du **CORS**.

    ![Meny alternativet CORS markerat i App Service menyn i Azure Portal](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS2.png)
...
4. Välj **Spara**.
    ...
    ![Sidan CORS-inställningar med asterisk markerat i tillåtna ursprung](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS3.png)

    När du har valt **Spara**accepterar API-appen JavaScript-anrop från de angivna URL: erna.

## <a name="step-4-html5-template-validation"></a>Steg 4: val av HTML5-mall
Din HTML5-mall är klar att använda. Det är dock inte tillgängligt i `ContentDefinition`s koden. Innan du kan lägga till `ContentDefinition` i den anpassade principen kontrollerar du att:
...
Gör så här för att konfigurera `ContentDefinition`:
1. Öppna bas filen för principen (till exempel *TrustFrameworkBase. XML*).

2. Sök efter `<ContentDefinitions>`-elementet och kopiera sedan hela innehållet i noden `<ContentDefinitions>`.
...
4. Klistra in hela innehållet i `<ContentDefinitions>`-noden som du kopierade som underordnad till `<BuildingBlocks>`-elementet.
...
6. Ändra värdet för `LoadUri` från _~/Tenant/default/Unified_ till _https://< APP_NAME >. azurewebsites. net/Home/Unified_.
    ...
    ![Exempel på XML-kodfragment med LoadUri-element markerat](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-content-definition.png)
...
2. Öppna **B2C_1A_signup_signin**, den förlitande parten (RP) anpassad princip som du överförde och välj sedan **Kör nu**.
    ...
    ![Din registrerings-eller inloggnings princip](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo1.png)
...
2. Lägg till noden `UserJourneyBehaviors` under `<DefaultUserJourney>`-noden:
...
### <a name="step-82-change-your-code-to-accept-a-query-string-parameter-and-replace-the-background-image"></a>Steg 8,2: ändra koden till att godkänna en frågesträngparametern och ersätt bakgrunds bilden
Ändra HomeController-`unified`-metoden för att godkänna parametern campaignId. Metoden kontrollerar sedan parameterns värde och anger variabeln `ViewData["background"]`.

1. Öppna filen *Controllers\HomeController.cs* och ändra sedan `unified`-metoden genom att lägga till följande kodfragment:
...
2. Leta upp `<img>`-elementet med ID `background_background_image`och ersätt `src`-värdet med `@ViewData["background"]`.

    ![img-element med src-värde markerat ](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-dynamic-background.png)
...
5. Lägg till frågesträngparametern _campaignId_ i URI: n. Lägg exempelvis till `&campaignId=hawaii`, som du ser i följande bild:

    ![URI med en campaignId-parameter för frågesträng markerad](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-campaignId-param.png)

6. Välj **RETUR** för att Visa Hawaii-bakgrunds bilden.

    ![Registrera inloggnings sidan med en egen Hawaii-bild bakgrund](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo2.png)

7. Ändra värdet till *Tokyo*och välj sedan **RETUR**.
    Webbläsaren visar Tokyo-bakgrunden.

    ![Registrerings sidan för registrering med anpassad bild bakgrund för Tokyo-bilder](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo3.png)
...
2. Gå tillbaka till "steg 4" och gör följande:
    ...
    a. I principen för tillägg letar du reda på `<ContentDefinition>`-noden som innehåller `Id="api.selfasserted"`, `Id="api.localaccountsignup"`och `Id="api.localaccountpasswordreset"`.

    b. Ange `LoadUri`-attributet till din *selfasserted* -URI.
...
* När du har slutfört guiden [komma igång med anpassade principer](custom-policy-get-started.md) rekommenderar vi att du skapar ditt scenario genom att använda dina egna anpassade principfiler. Vi har angett [exempel på principfiler](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-ui-customization)för din referens.
