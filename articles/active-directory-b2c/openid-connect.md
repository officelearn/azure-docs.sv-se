...
Du kan lösa in den auktoriseringskod som du har köpt (genom att använda `response_type=code+id_token`) för en token till önskad resurs genom att skicka en `POST` begäran till `/token` slut punkten. I Azure AD B2C kan du [begära åtkomsttoken för andra API: er](access-tokens.md#request-a-token) som vanligt genom att ange deras omfång i begäran.
