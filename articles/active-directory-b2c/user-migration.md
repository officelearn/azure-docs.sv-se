...
Det sömlösa migreringsjobbet kräver fortfarande Mass migrering av användar konton, men använder sedan en [anpassad princip](restful-technical-profile.md) för att fråga en [REST API](rest-api-claims-exchange-dotnet.md) (som du skapar) för att ange varje användares lösen ord vid första inloggningen.
...
1. Kontrol lera om kontot är flaggat för migrering genom att utvärdera ett booleskt tilläggs attribut.
    - Om attributet Extension returnerar `True`, anropa REST API för att verifiera lösen ordet mot den äldre identitets leverantören.
      - Om REST API anger att lösen ordet är felaktigt, returnerar du ett eget fel för användaren.
      - Om REST API fastställer att lösen ordet är korrekt, skriver du lösen ordet till Azure AD B2C-kontot och ändrar attributet Boolean-tillägg till `False`.
    - Om attributet Boolean-tillägg returnerar `False`fortsätter du med inloggnings processen som vanligt.
...
diagram över ![flödes schema för sömlös migrering till användar migrering](./media/user-migration/diagram-01-seamless-migration.png)<br />*Diagram: sömlöst migreringsjobb*
