...
- **Före migrering och lösen ords återställning**: det här flödet gäller när en användares lösen ord inte är tillgänglig. Ett exempel:
...
![Del av UsersData. JSON-filen som visar JSON-block av två användare](./media/user-migration/pre-migration-data-file.png)
...
![Kommando tolks fönstret visar utdata från kommandot UserMigration. exe](./media/user-migration/pre-migration-demo.png)
...
- Om du vill hämta en användare via inloggnings-e-postadress använder du exempel programmet:
   ...
   1. Öppna filen UserProfile. json i en JSON-redigerare för att se användarens information.
      ...
      ![Filen UserProfile. JSON öppnas i Visual Studio Code Editor](./media/user-migration/pre-migration-get-by-email2.png)
...
Följ dessa steg om du vill hämta en länk till din princip för lösen ords återställning. Den här proceduren förutsätter att du tidigare har skapat en [anpassad princip](custom-policy-get-started.md)för lösen ords återställning.
...
1. Kopiera den URL som visas i text rutan **Kör nu-slut punkt** och skicka den sedan till användarna.
    ...
    ![Sidan princip för lösen ords återställning med slut punkten för kör nu markerad](./media/user-migration/pre-migration-policy-uri.png)
...
När användarna försöker logga in utan att återställa lösen ordet först bör principen returnera ett eget fel meddelande. Ett exempel:
...
1. Ange autentiseringsuppgifterna för en av de migrerade användarna och välj sedan **Logga in**. Ditt REST API bör utlösa följande fel meddelande:
    ...
    ![Inloggnings registrerings sidan som visar fel meddelandet ändra lösen ord](./media/user-migration/pre-migration-error-message.png)
...
1. Välj **Spara**
    ...
    ![Konfigurations sidan för diagnostikloggar i Azure Portal](./media/user-migration/pre-migration-diagnostic-logs.png)
