---
title: Gå med i en virtuell RHEL-dator till Azure AD Domain Services | Microsoft-dokument
description: Lär dig hur du konfigurerar och ansluter till en virtuell Red Hat Enterprise Linux-dator till en hanterad Azure AD Domain Services-domän.
...
Den här artikeln visar hur du ansluter en Virtuelld red hat Enterprise Linux (RHEL) till en Azure AD DS-hanterad domän.
...
## <a name="create-and-connect-to-a-rhel-linux-vm"></a>Skapa och ansluta till en virtuell RHEL Linux-dator

Om du har en befintlig RHEL Linux-virtuell dator i Azure ansluter du till den med SSH och fortsätter sedan till nästa steg för att [börja konfigurera den virtuella datorn](#configure-the-hosts-file).

Om du behöver skapa en virtuell RHEL Linux-dator eller vill skapa en test-VM som ska användas med den här artikeln kan du använda någon av följande metoder:
...
* *rhel* är värdnamnet för din virtuella RHEL-dator som du ansluter till den hanterade domänen.
...
Den virtuella datorn behöver ytterligare paket för att ansluta till den virtuella datorn till azure AD DS-hanterad domän. Om du vill installera och konfigurera dessa paket uppdaterar `yum`och installerar du domänkopplingsverktygen med . Det finns vissa skillnader mellan RHEL 7.x och RHEL 6.x, så använd lämpliga kommandon för din distributionsversion i de återstående avsnitten i den här artikeln.
...
Nu när de nödvändiga paketen är installerade på den virtuella datorn ansluter du den virtuella datorn till den Azure AD DS-hanterade domänen. Återigen, använd lämpliga steg för din RHEL distributionsversion.
...
1. Använd `adcli info` kommandot för att identifiera den hanterade Azure AD DS-domänen. I följande exempel upptäcks sfären *ADDDSCONTOSO.COM*. Ange ditt eget Azure AD DS-hanterade domännamn i VERSALER:
...
1. Först ansluter du till `adcli join` domänen med kommandot, det här kommandot skapar också keytab för att autentisera datorn. Använd ett användarkonto som är en del av azure AD DS-hanterad domän.

    ```console
    sudo adcli join aaddscontoso.com -U contosoadmin
    ```

1. Konfigurera nu `/ect/krb5.conf` och `/etc/sssd/sssd.conf` skapa filerna `aaddscontoso.com` så att de använder Active Directory-domänen.
   Kontrollera att `AADDSCONTOSO.COM` den ersätts av ditt eget domännamn:
...
1. Kontrollera `/etc/sssd/sssd.conf` att behörigheterna är 600 och ägs av rotanvändare:
...
1. Används `authconfig` för att instruera den virtuella datorn om AD Linux-integreringen:
...
Kontrollera nu om du kan fråga ad-information för användare med`getent`
...
1. Om du vill använda ändringarna och låta användare logga in med ett lösenord startar du om SSH-tjänsten för din RHEL-distributionsversion:
...
Om du vill bevilja medlemmar i *aad DC-administratörsgruppens* administrativa behörigheter på den virtuella RHEL-datorn lägger du till en post i */etc/sudoers*. När medlemmar i gruppen *AAD DC-administratörer* har lagts till kan de `sudo` använda kommandot på den virtuella RHEL-datorn.
...
1. Skapa en ny SSH-anslutning från konsolen. Använd ett domänkonto som tillhör den `ssh -l` hanterade domänen `contosoadmin@aaddscontoso.com` med kommandot, till exempel och ange sedan adressen till den virtuella datorn, till exempel *rhel.aaddscontoso.com*. Om du använder Azure Cloud Shell använder du den offentliga IP-adressen för den virtuella datorn i stället för det interna DNS-namnet.
