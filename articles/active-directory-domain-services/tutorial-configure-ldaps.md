...
1. Öppna dialog rutan *Kör* genom att välja **Windows** -och **R** -nycklar.
1. Öppna Microsoft Management Console (MMC) genom att ange **MMC** i dialog rutan *Kör* och välj sedan **OK**.
1. I dialog rutan **User Account Control** klickar du på **Ja** för att starta MMC som administratör.
1. Från menyn **Arkiv** klickar du på **Lägg till/ta bort snapin-modul...**
...
1. I dialog rutan **Lägg till eller ta bort snapin-moduler** klickar du på **OK** för att lägga till snapin-modulen certifikat i MMC.
...
1. Lämna de förifyllda fälten inställda och välj sedan **Kör**. Resultatet av frågan visas i det högra fönstret.

    ![Sök efter objekt i din Azure AD DS-hanterade domän med hjälp av LDP. exe](./media/tutorial-configure-ldaps/ldp-query.png)

Om du vill fråga efter en speciell behållare direkt från **trädvyn > träd** -menyn kan du ange en **baserad** , till exempel *OU = AADDC-användare, DC = contoso, DC = com* eller *OU = AADDC Computers, DC = contoso, DC = com*. Mer information om hur du formaterar och skapar frågor finns i [grunderna för LDAP-frågor][ldap-query-basics].
