...
Weitere Informationen finden Sie unter [Willkommen bei Visual Basic für Applikationen-Sprachreferenz für Office 2013](https://msdn.microsoft.com/library/gg264383.aspx).
...
- Zeichenfolgenkonstanten sind in Anführungszeichen eingeschlossen. Beispiel: "Contoso" (Hinweis: gerade Anführungszeichen müssen verwenden "" und nicht typografische Anführungszeichen "")
...
Die vollständige Liste der Funktionen finden Sie der [-Funktion Referenz](active-directory-aadconnectsync-functions-reference.md).
...
Operatoren werden von links nach rechts ausgewertet und haben bei der Auswertung die gleiche Priorität. Das heißt: die \ * (Multiplikator) wird nicht ausgewertet, bevor - (Subtraktion). 2\*(5+3) entspricht nicht dem 2\ * 5 + 3. Die Klammern werden verwendet, um die Reihenfolge der Auswertung zu ändern, wenn die Auswertungsreihenfolge von links nach rechts nicht geeignet ist.
...
Bei eingehenden Synchronisierungsregeln, die Konstante **NULL** sollte stets verwendet werden. Dies weist darauf hin, dass der Attributfluss keine Werte beiträgt und eine andere Regel einen Wert beitragen kann. Wenn keine Regel einen Wert beiträgt, wird das Metaverseattribut entfernt.
