...
Använda Azure Active Directory (Azure AD) för att ange separata administratörer för olika funktioner. Dessa administratörer har åtkomst till de valda funktionerna i Azure-portalen eller den klassiska Azure-portalen och, beroende på deras roll, kommer att kunna skapa eller redigera användare, tilldela administrativa roller till andra, återställa användarlösenord, hantera användarlicenser och hantera domäner, bland annat. En användare som är tilldelad en administratörsroll har samma behörigheter för alla molntjänster som din organisation prenumererar på, oavsett om du tilldelar rollen i Office 365-portalen eller den klassiska Azure-portalen eller med hjälp av Azure AD-modulen för Microsoft PowerShell.

> [!IMPORTANT]
> Microsoft rekommenderar att du hanterar Azure AD via [Azure AD administratörscenter](https://aad.portal.azure.com) på Azure Portal istället för via den klassiska Azure-portalen som nämns i den här artikeln. Att tilldela administratörsroller i Azure AD-administrationscentret, se [Tilldela administratörsroller i Azure Active Directory](active-directory-assign-admin-roles-azure-portal.md).
