...
-   Abrufen der Zugriffstoken zum Aufrufen einer Aufgabenlisten-API mit dem [OAuth 2.0-Authentifizierungsprotokoll](https://msdn.microsoft.com/library/azure/dn645545.aspx).
-   Abrufen der Aufgabenliste eines Benutzers
-   Abmelden von Benutzern

Zum Starten benötigen Sie einen Azure AD-Mandanten, in dem Sie Benutzer erstellen und eine Anwendung registrieren können.  Wenn Sie noch keinen Mandanten haben [erfahren Sie, wie eine](active-directory-howto-tenant.md).
...
11. **Suchen Sie den Client-ID-Wert, und kopieren Sie diese**, Sie benötigen diese später zum Konfigurieren Ihrer Anwendung.
...
Für die Erstellung mit Maven können Sie die Datei „pom.xml“ auf der obersten Ebene verwenden.
  ...
  * Führen Sie die Schritte unter [Voraussetzungen zum Abschnitt zum Einrichten der von Maven für Android](https://github.com/MSOpenTech/azure-activedirectory-library-for-android/wiki/Setting-up-maven-environment-for-Android)
...
* Wenn Sie Android Studio verwenden, können Sie *Aar* Paketformat und auf die Binärdateien verweisen.

####Option 1: ZIP-Datei mit Quellcode

Um eine Kopie des Quellcodes herunterzuladen, klicken Sie auf der rechten Seite der Seite auf "Download ZIP", oder klicken Sie auf [hier](https://github.com/AzureAD/azure-activedirectory-library-for-android/archive/v1.0.9.tar.gz).
...
Sie können der JAR-Datei dem Repository von Maven abrufen und Ablegen der *Libs* Ordner des Projekts. Außerdem müssen Sie die erforderlichen Ressourcen in Ihr Projekt kopieren, da sie in den JAR-Paketen nicht enthalten sind.


### Schritt 5: Hinzufügen von Verweisen auf die ADAL für Android zu Ihrem Projekt


2. Fügen Sie einen Verweis auf Ihr Projekt hinzu, und geben Sie ihn als Android-Bibliothek an. Wenn Sie unsicher sind, wie Sie dazu [Klicken hier weitere Informationen] (http://developer.android.com/tools/projects/projects-eclipse.html)
...
7. Erstellen Sie eine Instanz von AuthenticationContext als Hauptaktivität. Die Details dieses Aufrufs sind Gegenstand dieser Readme-Datei, aber Sie erhalten einen guten Einstieg anhand der [Android Native Client-Beispiel](https://github.com/AzureADSamples/NativeClient-Android). Unten ist ein Beispiel aufgeführt:
...
Rufen Sie **AcquireTokenSilent** das caching und token aktualisieren. Außerdem wird darüber die Synchronisierungsversion bereitgestellt. „userid“ wird als Parameter akzeptiert.
...
11. **Broker**:
  Microsoft Intune Unternehmensportal-app wird die brokerkomponente bereitstellen. ADAL verwendet das Brokerkonto, falls unter diesem Authentifikator ein Benutzerkonto erstellt wurde und der Entwickler sich nicht für das Überspringen entschieden hat. Der Entwickler kann den Brokerbenutzer wie folgt überspringen:

    ```java
     AuthenticationSettings.Instance.setSkipBroker(true);
    ```

 Der Entwickler muss ein spezielles redirectUri-Element für die Brokernutzung registrieren. "Redirecturi" liegt im Format msauth://packagename/Base64UrlencodedSignature. Sie können das redirectUri-Element für Ihre App mit dem Skript „brokerRedirectPrint.ps1“ abrufen oder den API-Aufruf „mContext.getBrokerRedirectUri“ verwenden. Die Signatur bezieht sich auf Ihre Signaturzertifikate.
 ...
 ```
 Brokerbenutzer wird zurückgegeben, wenn das Konto gültig ist.
...
Autoritäts-Url sind STS-Instanz und der Mandantenname erforderlich: https://login.windows.net/yourtenant.onmicrosoft.com
...
```
Sie können auch die Cacheimplementierung bereitstellen, wenn Sie sie anpassen möchten.
```Java
...
Sie können die Bibliothek so konfigurieren, dass Protokollmeldungen generiert werden, die Sie zum Diagnostizieren von Problemen verwenden können. Sie können die Protokollierung konfigurieren, indem Sie den folgenden Aufruf verwenden. Hiermit können Sie einen Rückruf konfigurieren, der von der ADAL zum Weitergeben der Protokollmeldungen genutzt wird, wenn diese erzeugt werden.
 ...
 ```
Nachrichten können in eine benutzerdefinierte Protokolldatei geschrieben werden, wie unten dargestellt. Leider gibt es keine standardmäßige Möglichkeit, Protokolle von einem Gerät abzurufen. Es sind einige Dienste verfügbar, die Ihnen hierbei behilflich sein können. Sie können auch eigene Wege erfinden, z. B. das Senden einer Datei an einen Server.
...
In Android Webview werden Sitzungscookies nach dem Schließen der App nicht gelöscht. Sie können dies mit den unten angegebenen Beispielcode behandeln:
```java
