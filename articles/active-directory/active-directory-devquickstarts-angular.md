...
-   Folgen Sie den Assistenten, und erstellen Sie ein neues **Web Application and/or WebAPI**.
    -   Die **Namen** der Anwendung beschreiben Anwendung ablesen können.
    -   Die **Redirect Uri** ist der Speicherort, an dem AAD die Token zurückgibt.  Der Standardspeicherort für dieses Beispiel lautet `https://localhost:44326/`.
-   Nach Abschluss der Registrierung, weist AAD Ihrer app eine eindeutige **Client-ID**.  Sie benötigen diesen Wert in den nächsten Abschnitten, kopieren Sie ihn daher aus dem **konfigurieren** Registerkarte.
- Für „adal.js“ wird der implizite OAuth-Fluss genutzt, um mit Azure AD zu kommunizieren.  Sie müssen den impliziten Fluss für Ihre Anwendung wie folgt aktivieren:
    - Laden Sie das Anwendungsmanifest, indem Sie auf **Manifest verwalten**.
    ...
    - Speichern und Laden Sie durch Klicken auf das Anwendungsmanifest **Manifest verwalten** erneut.
...
-   Beginnen Sie, indem Sie „adal.js“ über die Paket-Manager-Konsole dem Projekt mit der einseitigen Aufgabenlisten-Anwendung (TodoSPA) hinzufügen:
  - Herunterladen [adal.js](https://raw.githubusercontent.com/AzureAD/azure-activedirectory-library-for-js/master/lib/adal.js) und fügen sie der `App/Scripts/` Projektverzeichnis.
  - Herunterladen [adal-angular.js](https://raw.githubusercontent.com/AzureAD/azure-activedirectory-library-for-js/master/lib/adal-angular.js) und fügen sie der `App/Scripts/` Projektverzeichnis.
...
-   Damit die Back-End-Aufgabenlisten-API der einseitigen Anwendung Token aus dem Browser akzeptieren kann, benötigt das Back-End Konfigurationsinformationen zur App-Registrierung. Öffnen Sie `web.config` im Projekt mit der einseitigen Aufgabenlisten-Anwendung.  Ersetzen Sie die Werte der Elemente im Abschnitt `<appSettings>` durch die Werte, die Sie im Azure-Verwaltungsportal eingegeben haben.  Sobald Ihr Code ADAL verwendet, verweist er auf diese Werte.
    ...
    -   Die `ida:Audience` muss der **Client-ID** Ihrer Anwendung, die Sie aus dem Portal kopiert haben.

## *3.  Verwenden von ADAL zum Schützen von Seiten in der SPA*
...
„adal.js“ erleichtert Ihnen die Integration all dieser allgemeinen Identitätsfunktionen in Ihre Anwendung.  Es übernimmt die unangenehmen Verwaltungsarbeiten für Sie – die Cacheverwaltung, die Unterstützung des OAuth-Protokolls, die Anzeige einer Anmeldeschnittstelle für den Benutzer, die Aktualisierung abgelaufener Tokens und vieles mehr.

Das vollständige Beispiel (ohne Ihre Konfigurationswerte) dient als Referenz [hier](https://github.com/AzureADQuickStarts/SinglePageApp-AngularJS-DotNet/archive/complete.zip).  Sie können sich nun weiteren Szenarien zuwenden.  Sie können beispielsweise Folgendes testen:
