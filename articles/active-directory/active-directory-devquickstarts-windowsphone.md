...
-   Folgen Sie den Assistenten, und erstellen Sie ein neues **systemeigene Clientanwendung**.
    ...
    -   Die **Redirect Uri** ist eine Schema und einer Zeichenfolge Kombination, die Azure AD für die Rückgabe der tokenantworten verwendet.  Geben Sie einen vorläufigen Platzhalterwert ein, zum Beispiel `http://DirectorySearcher`.  Wir werden diesen Wert später ersetzen.
...
- Auf der **konfigurieren** Registerkarte Ihrer Anwendung in Azure-Verwaltungsportal, ersetzen Sie den Wert von der **RedirectUri** mit diesem Wert.  
...
```
- Interaktive Authentifizierung erforderlich ist, wird die ADAL Verwenden des Windows Phone Web Authentication Broker (WAB) und [fortsetzungsmodell](http://www.cloudidentity.com/blog/2014/06/16/adal-for-windows-phone-8-1-deep-dive/) die Azure AD-Anmeldeseite an.  Bei der Anmeldung eines Benutzers muss Ihre Anwendung die Ergebnisse der WAB-Aktivität an ADAL übergeben.  Dies ist so einfach wie die Implementierung der `ContinueWebAuthentication`-Schnittstelle:
...
Das vollständige Beispiel (ohne Ihre Konfigurationswerte) dient als Referenz [hier](https://github.com/AzureADQuickStarts/NativeClient-WindowsPhone/archive/complete.zip).  Sie können sich nun weiteren Identitätsszenarien zuwenden.  Sie können beispielsweise Folgendes testen:
