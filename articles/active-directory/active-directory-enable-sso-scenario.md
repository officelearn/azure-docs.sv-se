...
2. Om du vill aktivera säkerhet och styrning organisationen som behöver kontroll och tillsyn över vem som kan komma åt och som faktiskt har åtkomst till varje program

I en värld av molnprogram detta uppnås bäst identiteten för att styra ”*som är tillåten att göra vad*”.
...
* Användarna behöver komma ihåg flera autentiseringsuppgifter för att komma åt de program som de behöver för att arbeta med. Därför tenderar användare att skriva ned sina lösenord eller använda andra lösningar för hantering av lösenord. Dessa alternativ säkerhetsrisker andra data.
* Redundant tidskrävande aktiviteter minska den tid som användarna och administratörerna ägnar åt att arbeta med affärsverksamhet som ökar din verksamhet slutresultatet.
...
* Central hantering och, enkel inloggning (SSO) rapportering 
...
Värdet för Azure AD är mer omfattande än ”bara” molnprogram. Du kan också använda den med lokala program genom att tillhandahålla säker fjärråtkomst. Med säker fjärråtkomst kan du behöver för VPN-anslutningar eller andra traditionella fjärråtkomst management-implementeringar.

Azure AD innehåller lösningen huvudsakliga datasäkerhet och produktivitet problem genom att tillhandahålla central hantering och enkel inloggning (SSO) för alla program.
...
* Auktorisering körs på Azure AD tilldelning och skydd principen för att säkerställa konsekvent användarupplevelse och gör att du kan lägga till tilldelning, platser och MFA villkor på alla program, oavsett dess interna funktioner.
...
Inbyggda application proxy kan du publicera dina lokala program till användarna som resulterar i åtkomst både konsekvent upplevelse med moderna molnapp och fördelarna från funktionerna i Azure AD övervakning, rapportering och säkerhet.
