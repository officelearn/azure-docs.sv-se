...
Weitere Informationen zu app-Eigenschaften finden Sie unter [Anwendungsobjekte und Dienstprinzipalobjekte](active-directory-application-objects.md)um den Brandingrichtlinien zu erfahren, sollten Sie beim Entwickeln von Anwendungen mit Azure Active Directory finden Sie unter verwenden [Brandingrichtlinien für integrierte Apps](active-directory-branding-guidelines.md);-Anwendung Manifeste werden in erklärt [Grundlegendes zu Azure Active Directory Anwendungsmanifest](active-directory-application-manifest.md).
...
Ausführlichere Informationen zum Consent Framework finden Sie unter [OAuth 2.0 in Azure AD](https://msdn.microsoft.com/library/azure/dn645545.aspx), [Authentifizierungsszenarien für Azure AD](active-directory-authentication-scenarios.md), und Office 365 Thema [Authentifizierung und Autorisierung mithilfe des allgemeinen Consent Frameworks](https://msdn.microsoft.com/library/office/dn605895(v=office.15).aspx).
...
>[AZURE.NOTE] Klicken auf die Schaltfläche "Speichern" automatisch auch legt die Berechtigungen für die Anwendung in Ihrem Verzeichnis anhand der Berechtigungen für andere Anwendungen, die Sie konfiguriert.  Sie können diese Anwendungsberechtigungen auf der Registerkarte mit den Anwendungseigenschaften anzeigen.

### Bereitstellen von Web-APIs für andere Anwendungen

Sie können eine Web-API entwickeln und für andere Unternehmen zur Verfügung stellen, indem Sie Ihre Berechtigungsbereiche für andere Entwickler verfügbar machen. Eine ordnungsgemäß konfigurierte Web-API wird auf die gleiche Weise wie andere Microsoft-Web-APIs (einschließlich der Graph-API und der Office 365-APIs) zur Verfügung gestellt. Ihre Web-API, die Konfiguration zur Verfügung gestellt wird eine [Anwendungsmanifest](active-directory-application-manifest.md), also eine JSON-Datei, die die Konfiguration der Anwendung Identität darstellt. Sie können Ihre Berechtigungsbereiche bereitstellen, indem Sie zu Ihrer Anwendung im Azure-Verwaltungsportal navigieren und dann auf der Befehlsleiste auf die Schaltfläche "Anwendungsmanifest" klicken.  Weitere Informationen finden Sie unter [Grundlegendes zu Azure Active Directory Anwendungsmanifest](active-directory-application-manifest.md).
...
Für vorhandene Benutzer im Azure-Verwaltungsportal entspricht das Festlegen der Anwendungsberechtigungen "Lesen von Verzeichnisdaten" und "Lesen und Schreiben von Verzeichnisdaten" über das neue Steuerelement "Berechtigungen für andere Anwendungen" dem früher verwendeten Assistenten zum Verwalten des Zugriffs.  Um die Berechtigung finden Sie unter Bereiche verfügbar gemacht werden, indem Office 365 finden Sie unter der [Authentifizierung und Autorisierung mithilfe des allgemeinen Consent Frameworks](https://msdn.microsoft.com/office/office365/howto/common-app-authentication-tasks) Thema.

>[AZURE.NOTE] Eine aktuelle Einschränkung können systemeigene Clientanwendungen in der Azure AD Graph-API nur aufrufen, wenn sie die Berechtigung "Zugriff auf das Verzeichnis der Organisation" verwenden.  Diese Einschränkung gilt nicht für Webanwendungen.
...
Zum Gewähren des Zugriffs mit dem Consent Framework muss die Clientanwendung mithilfe von OAuth 2.0 eine Autorisierung anfordern. [Die Codebeispiele](https://github.com/AzureADSamples) stehen Ihnen zeigen, wie eine Webanwendung, die systemeigene Anwendung oder den Server-Daemon-Anwendung Autorisierungscodes und Zugriffstoken zum Aufrufen von Web-APIs anfordert.
...
1. Speichern Sie die aktualisierte JSON-Datei und Laden Sie es durch Klicken auf die **Manifest verwalten** Schaltfläche in der Befehlsleiste auswählen **Upload Manifest**, navigieren Sie zur aktualisierten Manifestdatei, und wählen Sie sie. Nachdem das Manifest hochgeladen wurde, ist Ihre Web-API für die Verwendung der impliziten OAuth 2.0-Gewährung zum Authentifizieren von Benutzern konfiguriert.
...
Sie können die Schritte in der [entwickeln mehrinstanzenfähiger Webanwendungen mit Azure AD](https://msdn.microsoft.com/library/azure/dn151789.aspx) gewähren Sie Zugriff für neue Anwendungen in Azure AD registriert. Beachten Sie unbedingt, dass das neue Consent Framework viel leistungsfähigere Anwendungen ermöglicht und darüber hinaus Benutzern (nicht nur Administratoren) die Möglichkeit bietet, ihre Zustimmung zu diesen Anwendungen zu erteilen.

#### Erstellen des Links, der Zugriff für externe Benutzer erteilt (Legacy)

Damit sich externe Benutzer mithilfe ihrer Organisationskonten für Ihre App registrieren können, müssen Sie Ihre App so aktualisieren, dass Sie eine Schaltfläche anzeigt, mit der die Benutzer auf die Azure AD-Seite zur Erteilung von Zugriff weitergeleitet werden. Brandingleitfaden für diese Registrierungsschaltfläche, in erläutert wird der [Brandingrichtlinien für integrierte Anwendungen](active-directory-branding-guidelines.md) Thema. Nachdem der Benutzer den Zugriff gewährt oder verweigert hat, leitet die Azure AD-Seite zum Erteilen des Zugriffs den Browser mit einer Antwort wieder an Ihre App um. Weitere Informationen zu Eigenschaften finden Sie unter [Anwendungsobjekte und Service-Prinzipien](active-directory-application-objects.md).
...
>[AZURE.NOTE] Nur der Unternehmensadministrator der externen Organisation kann Zugriff auf Ihre app erteilen. Wenn der Benutzer kein Unternehmensadministrator ist, wird eine Option zum Senden einer Nachrichten an den Unternehmensadministrator bereitgestellt, damit Zugriff für die App angefordert werden kann.
...
>[AZURE.NOTE] Die Antwort zum Erteilen von Zugriff enthält kein Sicherheitstoken für den Benutzer. die app muss den Benutzer separat anmelden.
...
1. Kehren Sie zu Ihrer App zurück, und aktualisieren Sie die Konfiguration, um mit der Verwendung des neuen Kennwortschlüssels zu beginnen. Finden Sie unter [mithilfe der Graph-API auf Azure AD-Abfragen](https://msdn.microsoft.com/library/azure/dn151791.aspx) für ein Beispiel dafür, wo diese Aktualisierung stattfinden sollte.
...
1. Für die Anwendung ist Multi-Tenant-Abschnitt, klicken Sie auf Nein. Dadurch wird die Anwendung mit einem Mandanten, aber die Anwendung bleibt weiterhin in einer Organisation, die bereits zugestimmt hat.

1. Klicken Sie auf der Befehlsleiste auf die Schaltfläche "Löschen".

1. Klicken Sie in der Bestätigungsmeldung auf "Ja".

Damit ein Unternehmensadministrator den Zugriff einer App auf das Verzeichnis entfernen kann (nachdem die Zustimmung erteilt wurde), muss der Unternehmensadministrator über ein Azure-Abonnement zum Entziehen des Zugriffs über das Azure-Verwaltungsportal verfügen. Der Unternehmensadministrator können auch die [Azure AD PowerShell-Cmdlets](http://go.microsoft.com/fwlink/?LinkId=294151) den Zugriff entfernen.

## Nächste Schritte

- Finden Sie unter der [Brandingrichtlinien für integrierte Apps](active-directory-branding-guidelines.md)

- Erfahren Sie mehr über [Anwendungsobjekte und Dienstprinzipalobjekte](active-directory-application-objects.md)

- Verstehen der [Azure Active Directory-Anwendungsmanifest](active-directory-application-manifest.md)

- Besuchen Sie die [Active Directory-Entwicklerhandbuch](active-directory-developers-guide.md)
