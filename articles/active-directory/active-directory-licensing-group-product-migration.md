...
Den här artikeln beskriver den rekommenderade metoden för att flytta användare mellan produktlicenser när du använder gruppbaserade licensiering. Målet med den här metoden är att säkerställa att det finns ingen förlust av tjänsten eller data under migreringen: användaren bör växla mellan produkter sömlöst. Två varianter av migreringsprocessen beskrivs:
...
Innan du påbörjar migreringen är det viktigt att kontrollera vissa antaganden är true för alla användare som ska migreras. Om förutsättningarna inte gäller för alla användare, misslyckas migreringen för några. Därför kan vissa av användarna förlora åtkomsten till tjänster eller data. Följande antaganden bör verifieras:
...
-   Användarna har inte andra tilldelade licenser som kan hamna i konflikt med den *mål licens* eller förhindra borttagning av den *källkodslicens*. Till exempel en licens från en tilläggsprodukt som arbetsplats Analytics eller Project Online, som är beroende av andra produkter.

-   Förstå hur grupper hanteras i din miljö. Till exempel om du hanterar grupper lokala och synkronisera dem i Azure Active Directory (AD Azure) via Azure AD Connect sedan du Lägg till/ta bort användare med hjälp av lokalt system. Det tar tid för att ändringarna ska synkronisera till Azure AD och hämta nå fram gruppbaserade licensiering. Om du använder Azure AD dynamiska gruppmedlemskap kan du lägga till/ta bort användare genom att ändra deras attribut i stället. Den övergripande migreringsprocessen är densamma. Den enda skillnaden är hur du lägger till/ta bort användare för gruppmedlemskap.

## <a name="migrate-users-between-products-that-dont-have-conflicting-service-plans"></a>Migrera användare mellan produkter som inte är i konflikt service-planer
Migrering målet är att använda gruppbaserade licensiering för att ändra användarlicenser från en *källkodslicens* (i det här exemplet: Office 365 Enterprise E3) till en *mål licens* (i det här exemplet: Office 365 Enterprise E5). De här två produkterna i det här scenariot innehåller inte står i konflikt serviceplaner så att de kan tilldelas fullständigt samtidigt utan att en konflikt. På någon punkt under migreringen bör användare förlorar åtkomst till tjänster eller data. Migreringen utförs i små ”batchar”. Du kan verifiera för varje grupp och minimera omfånget för eventuella problem som kan uppstå under processen. Generellt sett är processen följande:

1.  Användarna är medlemmar i gruppen för en datakälla och ärver de den *källkodslicens* från gruppen.

2.  Skapa en målgrupp med den *mål licens* men utan några medlemmar.
...
6.  Ta bort batch för samma användare från gruppen. Gruppbaserade licensiering svarar på ändringen och *datakällan licenser* tas bort från användarna.

7.  Upprepa processen för efterföljande grupper med användare.

### <a name="migrate-a-single-user-by-using-the-azure-portal"></a>Migrera en enskild användare med hjälp av Azure portal
Det här är en enkel genomgång för hur du migrerar en enskild användare.

**STEG 1**: användaren har en *källkodslicens* som ärvs från gruppen. Det finns inga direkta tilldelningar för licensen:

![Användare med en källkodslicens som ärvts från gruppen](media/active-directory-licensing-group-product-migration/UserWithSourceLicenseInherited.png)

**STEG 2**: användaren har lagts till i målgruppen och gruppbaserade licensiering bearbetar ändringen. Användaren har nu både den *källkodslicens* och *mål licens* som ärvs från grupper:

![Användare med licens för både källa och mål som ärvts från grupper](media/active-directory-licensing-group-product-migration/UserWithBothSourceAndTargetLicense.png)

**STEG 3**: användaren tas bort från gruppen och gruppbaserade licensiering bearbetar ändringen. Användaren har nu endast den *mål licens*:

![Användare med en mål-licens som ärvts från gruppen](media/active-directory-licensing-group-product-migration/UserWithTargetLicenseAssigned.png)

### <a name="automate-migration-by-using-azure-powershell"></a>Automatisera migrering med hjälp av Azure PowerShell
Följande utdrag visar hur du automatisera migreringsprocessen för storskaliga åtgärden.

> [!NOTE]
> Exempelkoden använder PowerShell-funktioner som ingår i den [sista avsnittet](#powershell-automation-of-migration-and-verification-steps) i den här artikeln.
...
6.  Ta bort batch för samma användare från gruppen. Gruppbaserade licensiering svarar på ändringen och *källkodslicens* tas bort från varje användare. Fel vid konfliktlösning tas bort samtidigt (när inga andra produktlicensen bidrar till fel) och *mål licens* är tilldelad. Den här processen säkerställer att det inte finns någon dataförlust eller under övergången.
...
![Användare med en källkodslicens som ärvts från gruppen](media/active-directory-licensing-group-product-migration/UserWithSourceLicenseInheritedConflictScenario.png)

**STEG 2**: användaren har lagts till i målgruppen och gruppbaserade licensiering bearbetar ändringen. Eftersom användaren fortfarande har den *källkodslicens*, *mål licens* är i ett feltillstånd på grund av konflikten:

![Användare med en källkodslicens som ärvts från grupp och mål för licensen i ett feltillstånd](media/active-directory-licensing-group-product-migration/UserWithSourceLicenseAndTargetLicenseInConflict.png)

**STEG 3**: användaren tas bort från gruppen och gruppbaserade licensiering bearbetar ändringen. Den *mål licens* tillämpas för användaren:

![Användare med en mål-licens som ärvts från gruppen](media/active-directory-licensing-group-product-migration/UserWithTargetLicenseAssignedConflictScenario.png)
...
Det här avsnittet innehåller en PowerShell-kod som krävs för att köra skript som beskrivs i den här artikeln.
...
Mer information om andra scenarier för hantering av programvarulicenser genom grupper i följande artiklar:

* [Identifiera och lösa problem med licens för en grupp i Azure Active Directory](active-directory-licensing-group-problem-resolution-azure-portal.md)
* [PowerShell-exemplen för gruppbaserade licensiering i Azure Active Directory](active-directory-licensing-ps-examples.md)
