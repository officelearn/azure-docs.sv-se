...
Der Code für dieses Lernprogramm [auf GitHub](https://github.com/AzureADQuickStarts/AppModelv2-WebAPI-nodejs).  Um folgen zu können, können Sie [Anwendungsgerüst als einer ZIP-Datei herunterladen](https://github.com/AzureADQuickStarts/AppModelv2-WebAPI-nodejs/archive/skeleton.zip) oder das Skelett Klonen:
...
> [AZURE.NOTE] In dieser exemplarischen Vorgehensweise wird davon ausgegangen, die Standard und die standardserverendpunkte für MongoDB verwenden, die zum Zeitpunkt der Erstellung dieses Dokuments ist: Mongodb://localhost
...
Anschließend fügen wir die OAuth-Strategie mit Passport-Azuread, eine Reihe von Strategien, die Azure Active Directory mit Passport verbunden. Wir verwenden diese Strategie für Bearertoken in diesem Rest-API-Beispiel.

> [AZURE.NOTE] Obwohl OAuth2 ein Framework bereitstellt, in dem alle bekannten Tokentypen ausgegeben werden kann, haben nur bestimmte Typen von Sicherheitstoken weitverbreitetes verwenden gewonnen. Für den Schutz von Endpunkten sind dies in der Regel Bearer-Tokens. Bearer-Tokens sind die in OAuth2 am häufigsten ausgegebenen Tokens, und viele Implementierungen gehen sogar davon aus, dass nur dieser Tokentyp ausgestellt wird.
...
*IdentityMetadata*: Dies ist, wo der Passport-Azure-Ad für die Konfigurationsdaten für die IdP als auch die Schlüssel zum Überprüfen der JWT-Token sucht. Wahrscheinlich nehmen Sie hier keine Änderungen vor, wenn Sie Azure Active Directory verwenden.

*Zielgruppe*: die URI-Umleitung aus dem Portal.
...
Wenn Sie aus der Datei config.js wir in Schritt 11 erstellt haben erinnern, wir unsere Datenbank genannt *Tasklist*, wie das war, was wir am Ende der Verbindungs-URL Mogoose_auth_local platziert. Sie müssen diese Datenbank nicht im Voraus in MongoDB erstellen, denn sie wird, wenn sie noch nicht vorhanden ist, automatisch bei der ersten Ausführung der Serveranwendung erstellt.
...
Als Referenz das vollständige Beispiel (ohne Ihre Konfigurationswerte) [als einer ZIP-Datei hier bereitgestellte](https://github.com/AzureADQuickStarts/AppModelv2-WebAPI-nodejs/archive/complete.zip), oder Sie können von GitHub Klonen:
