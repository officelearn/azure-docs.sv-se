...
| 32002| ServiceBusError| Den här händelsen indikerar att ett fel uppstod vid anslutning till klientens Service Bus instans. Detta kan inträffa om du blockerar utgående anslutningar i din lokala miljö. Kontrol lera brand väggen för att se till att du tillåter anslutningar över TCP 443 och https://ssprsbprodncu-sb.accesscontrol.windows.net/ och försök sedan igen. Om du fortfarande har problem kan du prova med att inaktivera och sedan återaktivera tillbakaskrivning av lösen ord.|
...
| 32011| OnBoardingServiceError| Den här händelsen indikerar att den lokala tjänsten inte kunde kommunicera med webb tjänsten för lösen ords återställning för att initiera onboarding-processen. Detta kan inträffa till följd av en brand Väggs regel eller om det är problem med att hämta en autentiseringstoken för din klient. Du kan åtgärda det här problemet genom att se till att du inte blockerar utgående anslutningar via TCP 443 och TCP 9350-9354 eller till https://ssprsbprodncu-sb.accesscontrol.windows.net/. Se också till att det Azure AD-administratörskonto som du använder för att publicera inte är federerad.|
| 32013| OffBoardingError| Den här händelsen indikerar att den lokala tjänsten inte kunde kommunicera med webb tjänsten för lösen ords återställning för att initiera offboarding-processen. Detta kan inträffa till följd av en brand Väggs regel eller om det är problem med att hämta en autentiseringstoken för din klient. Du kan åtgärda det här problemet genom att se till att du inte blockerar utgående anslutningar över 443 eller till https://ssprsbprodncu-sb.accesscontrol.windows.net/, och att det Azure Active Directory administratörs konto som du använder till avpublicera inte är federerad.|
...
> [!NOTE]
> SSPR kan också inte köras om kontot om "lösen ordet aldrig upphör att gälla" eller "användaren inte kan ändra lösen ord" har kon figurer ATS för kontot i AD DS lokalt. 
...
1. På fliken **anslutningar** väljer du den lokala **Active Directory Domain Services** anslutningen och väljer sedan **Egenskaper**.  
   ![Synchronization Service Manager som visar hur du redigerar egenskaper @ no__t-1  
...
Om du har en allmän fråga om Azure AD och lösen ords återställning via självbetjäning kan du be communityn om hjälp i [Azure AD-forumen](https://social.msdn.microsoft.com/Forums/en-US/home?forum=WindowsAzureAD). Medlemmar i communityn är tekniker, produkt chefer, MVP: er och andra IT-proffs.
