...
## <a name="troubleshoot-the-password-reset-configuration-in-the-azure-portal"></a>Felsöka konfigurationen på Återställ lösenord i Azure Portal
...
| Det finns ett fel vid bearbetning av begäran. | Detta kan orsakas av många problem, men vanligtvis felet orsakas av ett avbrott eller konfigurationsproblem. Om du ser detta fel och den påverkar ditt företag, kan du kontakta Microsoft support för ytterligare hjälp. |
| Lokala principöverträdelsen | Lösenordet uppfyller inte den lokala Active Directory-lösenordsprincipen. |
| Lösenordet uppfyller inte fuzzy princip | Lösenordet som användes för visas i lösenordslistan förbjudna och får inte användas. |
...
| Lösenordsåtgärder misslyckas med ett konfigurationsfel. Programmets händelselogg innehåller Azure AD Connect fel 6329 med texten ”0x8023061f (åtgärden misslyckades eftersom synkronisering av lösenord inte är aktiverat på den här hanteringsagenten)”. | Det här felet uppstår om Azure AD Connect-konfigurationen har ändrats för att lägga till en ny Active Directory-skog (eller om du vill ta bort och readd en befintlig skog) efter funktionen tillbakaskrivning av lösenord har redan aktiverats. Lösenordsåtgärder för användare i dessa nyligen lagt till skogar misslyckas. Lös problemet, inaktivera och sedan återaktivera funktionen tillbakaskrivning av lösenord efter konfigurationsändringar skogen har slutförts. |
...
| 31006 | ChangePasswordStart | Den här händelsen indikerar att tjänsten lokalt identifierats en begäran om lösenordsändring för en federerad, direkt-autentisering eller lösenord-hash-synkroniserade användare som kommer från molnet. Den här händelsen är den första händelsen i tillbakaskrivningen varje ändring av lösenord. |
...
För mer detaljerad refererar till den uppdaterade listan med [IP-intervall i Microsoft Azure Datacenter](https://www.microsoft.com/download/details.aspx?id=41653) uppdateras varje onsdag och träder i kraft nästa måndag.
