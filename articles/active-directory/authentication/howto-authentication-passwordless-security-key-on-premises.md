---
title: Lösen ords lös säkerhets nyckel logga in till lokala resurser (för hands version) – Azure Active Directory
description: Aktivera lösen ords lös säkerhets nyckel logga in till lokala resurser (förhands granskning)
...
ms.locfileid: "73164866"
---
# <a name="enable-passwordless-security-key-sign-in-to-on-premises-resources-preview"></a>Aktivera lösen ords lös säkerhets nyckel logga in till lokala resurser (förhands granskning)
...
Azure Active Directory (AD) kan utfärda Kerberos Ticket Granting Tick (TGT) för en eller flera av dina Active Directory-domäner. Med den här funktionen kan användarna logga in i Windows med moderna autentiseringsuppgifter som FIDO2-säkerhetsnycklar och komma åt traditionella Active Directory baserade resurser. Biljetter och auktorisering i Kerberos-tjänsten kommer att fortsätta att kontrol leras av dina lokala Active Directory domänkontrollanter.

Ett Azure AD Kerberos-serverobjektet kommer att skapas i din lokala Active Directory och sedan publiceras på ett säkert sätt till Azure Active Directory. Objektet är inte associerat med några fysiska servrar. Det är bara en resurs som kan användas av Azure Active Directory för att generera Kerberos biljett beviljande biljetter (TGT) för din Active Directory-domän.

![Hämta en TGT-och PRT från Azure AD och AD DS](./media/howto-authentication-passwordless-on-premises/fido2-tgt-exchange-process.png)
...
Organisationer måste följa stegen i artikeln [Aktivera lösen ords lös säkerhets nyckel logga till Windows 10-enheter (för hands version)](howto-authentication-passwordless-security-key.md) innan du slutför stegen i den här artikeln.

Organisationerna måste uppfylla följande program varu krav.

- Enheter måste köra Windows 10 Insider build 18945 eller senare
- Uppgradera till den senaste versionen av [Azure AD Connect](../hybrid/how-to-connect-install-roadmap.md#install-azure-ad-connect)
- Fullständigt korrigerade domänkontrollanter i Windows Server 2016/2019 för att hantera belastningen på den lokala autentiseringsbegäran.

### <a name="supported-scenarios"></a>Scenarier som stöds

- Scenariot stöder enkel inloggning (SSO) till båda:
   - Moln resurser som Office 365 och andra SAML-aktiverade program.
   - Lokala resurser och Windows-integrerad autentisering till webbplatser, inklusive webbplatser och SharePoint-webbplatser som kräver IIS-autentisering och/eller resurser som använder NTLM-autentisering.

### <a name="unsupported-scenarios"></a>Scenarier som inte stöds

Följande scenarier stöds inte:

- En domän som är ansluten till Windows Server Active Directory Domain Services (AD DS) (endast lokala enheter) **stöds inte**.
...
Administratörer kommer att använda PowerShell-verktyg från sin Azure AD Connect-Server för att skapa ett Azure AD Kerberos-serverobjektet i sina lokala kataloger. Du måste köra de här stegen i varje domän och skog i din organisation som innehåller Azure AD-användare.
...
Du kan visa och verifiera den nyligen skapade Azure AD Kerberos-servern med hjälp av följande kommando. 

```PowerShell
Get-AzureADKerberosServer -Domain $domain -CloudCredential $cloudCred -DomainCredential $domainCred
```

Det här kommandot kommer att skriva ut egenskaperna för Azure AD Kerberos-servern. Du kan granska egenskaperna för att kontrol lera att allt är i rätt ordning.

| Egenskap | Beskrivning |
| --- | --- |
| ID | Unikt ID för AD-domänkontrollantens objekt. Detta ID kallas ibland "fack" eller "gren-ID". |
...
| UserAccount | Det inaktiverade objektet User Account som innehåller Azure AD Kerberos-serverns TGT-krypterings nyckel. Kontots DN är: <br> CN = krbtgt_AzureAD, CN = användare, < domän-DN > |
| Version | Nyckel versionen av Azure AD Kerberos-serverns TGT-krypterings nyckel. Versionen tilldelas när nyckeln skapas. Versionen ökas sedan varje gång nyckeln roteras. Ökningarna baseras på metadata för replikering och kommer troligen att vara större än ett. Den ursprungliga versionen av versionen kan till exempel vara 192272. Första gången som nyckeln roterades kan versionen gå vidare till 212621. Det är viktigt att kontrol lera att versionen för det lokala objektet och CloudKeyVersion för moln objekt är desamma. |
| KeyUpdatedOn | Det datum och den tid som Azure AD Kerberos-servern TGT-krypteringsnyckeln uppdaterades/skapades. |
| KeyUpdatedFrom | Domänkontrollanten där Azure AD Kerberos-servern TGT-krypteringsnyckeln senast uppdaterades. |
...
Din organisation har till exempel en Active Directory skog med två domäner, contoso.com och fabrikam.com. Om du väljer att låta Azure AD utfärda Kerberos biljett beviljande biljetter (TGT) för hela skogen, kommer det att finnas två KerberosDomain-objekt i Azure AD. Ett KerberosDomain-objekt för contoso.com och ett för fabrikam.com. Om du har flera Active Directory-skogar får du ett KerberosDomain-objekt för varje domän i varje skog.

Du måste köra stegen i [skapa Kerberos-serverobjektet](#create-kerberos-server-object) i varje domän och skog i din organisation som innehåller Azure AD-användare.

## <a name="known-behavior"></a>Känt beteende

Inloggning med FIDO kommer att blockeras om ditt lösen ord har upphört att gälla. Förväntat att användaren återställer sitt lösen ord innan de kan logga in med FIDO.
...
Om du vill installera en hybrid Azure AD-ansluten dator, publicera domän anslutning och starta om måste du logga in med ett lösen ord och vänta tills principen har synkroniserats innan du kan använda FIDO för att logga in.

- Kontrol lera din aktuella status genom att skriva `dsregcmd /status` i ett kommando fönster och kontrol lera att både `AzureAdJoined` och `DomainJoined` visar `YES`.
- Den här fördröjningen är en känd begränsning för domänanslutna enheter och inte FIDO.
...
Kontrol lera att det finns tillräckligt många domänkontrollanter som ska svara i tid för att betjäna din resurs förfrågan. Om du vill kontrol lera om du kan se en domänkontrollant som kör funktionen kontrollerar du utdata från Nltest/dsgetdc: contoso/keylist/KDC.
