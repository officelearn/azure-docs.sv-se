---
title: Remote Desktop Gateway-integrering med Azure MFA NPS - tillägget i Azure Active Directory
...
Du måste ha en fungerande Remote Desktop Services (RDS)-infrastruktur på plats. Om du inte gör det, kan du snabbt skapa den här infrastrukturen i Azure med hjälp av följande quickstart-mallen: [Skapa Sessionssamling för Remote Desktop distribution](https://github.com/Azure/azure-quickstart-templates/tree/ad20c78b36d8e1246f96bb0e7a8741db481f957f/rds-deployment).
...
1. I bladet egenskaper bredvid katalog-ID klickar du på den **kopia** ikonen som visas nedan, för att kopiera ID: T till Urklipp.
   ...
   ![Hämta katalog-ID från Azure portal](./media/howto-mfa-nps-extension-rdg/image1.png)
...
1. Typ `.\AzureMfaNpsExtnConfigSetup.ps1`, och tryck på **RETUR**. Skriptet kontrollerar om Azure Active Directory PowerShell-modulen är installerad. Om inte har installerats installerar skriptet modulen åt dig.
   ...
   ![Kör AzureMfaNpsExtnConfigSetup.ps1 i Azure AD PowerShell](./media/howto-mfa-nps-extension-rdg/image4.png)
...
1. När skriptet har kontrollerat installationen av PowerShell-modulen, visas dialogrutan för Azure Active Directory PowerShell-modulen. I dialogrutan anger du dina autentiseringsuppgifter för Azure AD-administratör och lösenord och klicka på **logga In**.
   ...
   ![Autentiserar till Azure AD i PowerShell](./media/howto-mfa-nps-extension-rdg/image5.png)

1. När du uppmanas, klistra in katalog-ID som du tidigare kopierade till Urklipp och tryck på **RETUR**.
...
1. Skriptet skapar ett självsignerat certifikat och utför andra ändringar i konfigurationen. Resultatet ska se ut som på bilden nedan.
   ...
   ![Utdata från PowerShell visar självsignerat certifikat](./media/howto-mfa-nps-extension-rdg/image7.png)
...
1. I den **ange namn eller IP-adressen för servern som kör NPS** skriver IP-adress eller namnet på den server där du installerade NPS-tillägget.
   ...
   ![Ange namn eller IP-adressen för NPS-Server](./media/howto-mfa-nps-extension-rdg/image10.png)
...
1. I den **delad hemlighet** dialogrutan, ange en delad hemlighet och klicka sedan på **OK**. Se till att du registrerar den här delade hemligheten och lagra posten på ett säkert sätt.
   ...
   ![Skapa en delad hemlighet för förtroende](./media/howto-mfa-nps-extension-rdg/image11.png)
...
1. I den **NPS (lokal)** konsolen, expandera **RADIUS-klienter och servrar**, och välj **fjärranslutna RADIUS-Server**.
   ...
   ![Network Policy Server-hanteringskonsolen som visar fjärranslutna RADIUS-Server](./media/howto-mfa-nps-extension-rdg/image12.png)
...
1. I den **gruppegenskaper för TS GATEWAY SERVER** dialogrutan Välj IP-adressen eller namnet på den NPS-server som du har konfigurerat för att lagra auktoriseringsprinciper för fjärrskrivbordsanslutning och klicka sedan på **redigera**.
   ...
   ![Välj IP-Adressen eller namnet på NPS-servern som tidigare har konfigurerat](./media/howto-mfa-nps-extension-rdg/image13.png)
...
1. I den **antal sekunder mellan begäranden när servern anses vara otillgänglig** fältet, ändrar du standardvärdet på 30 sekunder till ett värde som är lika med eller större än värdet du angav i föregående steg.
   ...
   ![Redigera Radius-Server timeout-inställningar på fliken för belastningsutjämning](./media/howto-mfa-nps-extension-rdg/image14.png)
...
1. På **inställningar** klickar du på fliken, under Vidarebefordra anslutningsbegäran **autentisering**. RADIUS-klienten har konfigurerats att vidarebefordra begäranden för autentisering.
   ...
   ![Konfigurera autentiseringsinställningar att ange servergruppen](./media/howto-mfa-nps-extension-rdg/image15.png)
...
1. I den **delad hemlighet** och **Bekräfta delad hemlighet** fält, anger du samma hemlighet som du använt tidigare.
   ...
   ![Konfigurera ett eget namn och IP-Adressen eller DNS-adress](./media/howto-mfa-nps-extension-rdg/image18.png)
...
1. Högerklicka på **anslutningar till andra klientåtkomstservrar**, och klicka på **duplicera princip**.
   ...
   ![Duplicera anslutningen till andra servrar åtkomstprincip](./media/howto-mfa-nps-extension-rdg/image19.png)
...
1. I den **kopia av anslutningar till andra klientåtkomstservrar** i dialogrutan **principnamn**, ange ett lämpligt namn, till exempel _RDG_CAP_. Kontrollera **principen är aktiverad**, och välj **bevilja åtkomst**. Valfritt: i **typ av nätverksåtkomstserver**väljer **fjärrskrivbordsgateway**, eller lämna den som **Ospecificerad**.
   ...
   ![Namn på principen, aktivera och bevilja åtkomst](./media/howto-mfa-nps-extension-rdg/image21.png)
...
1. Du kan också klicka på den **villkor** fliken och Lägg till villkor som måste uppfyllas för anslutningen ska ha befogenhet, till exempel medlemskap i en specifik Windows-grupp.
   ...
   ![Du kan också ange villkor för anslutningen](./media/howto-mfa-nps-extension-rdg/image23.png)
...
1. Kontrollera att den nya principen är överst i listan att principen är aktiverad, och att den ger åtkomst.
   ...
   ![Flytta din princip till överst i listan](./media/howto-mfa-nps-extension-rdg/image24.png)
...
När du har autentiserats med hjälp av den sekundära autentiseringsmetoden, är du inloggad på fjärrskrivbordsgateway som vanligt. Men eftersom du behöver använda en metod för sekundär autentisering med hjälp av en mobilapp på en betrodd enhet är inloggningsprocessen säkrare än det annars skulle vara.
...
![Visa händelser med hjälp av PowerShell](./media/howto-mfa-nps-extension-rdg/image28.png)
...
![Visa auktoriseringsprincip för klientanslutning med hjälp av PowerShell](./media/howto-mfa-nps-extension-rdg/image29.png)
...
![Exempel på händelser som lyckad inloggning](./media/howto-mfa-nps-extension-rdg/image30.png)

Du kan också visa säkerhetsloggen eller anpassad vy nätverkspolicy och åtkomsttjänster enligt nedan:

![Nätverksprinciper och Access Services Loggboken](./media/howto-mfa-nps-extension-rdg/image31.png)
...
![Visningsprogrammet AuthZ-händelseprogramloggar](./media/howto-mfa-nps-extension-rdg/image32.png)
...
![Exempel på en händelse för Misslyckad inloggning](./media/howto-mfa-nps-extension-rdg/image33.png)

Nedan visas en relaterade händelse från AzureMFA-loggar:

![Exemplet Azure MFA-loggen i Loggboken](./media/howto-mfa-nps-extension-rdg/image34.png)
...
![Exempel på Shareware app IAS-parsern](./media/howto-mfa-nps-extension-rdg/image35.png)
...
![Microsoft Message Analyzer som visar filtrerade trafik](./media/howto-mfa-nps-extension-rdg/image36.png)
