...
4. Om du vill identifiera användarnamn, lösenord och domänvariabler automatiskt anger du inloggningsadressen (till exempel) `https://localhost/contoso/auth/login.aspx`i dialogrutan Konfigurera formulärbaserad webbplats automatiskt och klickar på **OK**.
...
3. Klicka på **Lägg till**.
4. I dialogrutan Lägg till bas-URL anger du URL:en för <http://localhost/owa>webbplatsen där HTTP-autentisering utförs (till exempel) och anger ett programnamn (valfritt). Programnamnet visas i Azure Multi-Factor Authentication-rapporter och kan visas i autentiseringsmeddelanden i SMS- eller mobilappar.
...
1. Om du kör på IIS 6 klickar du på fliken **ISAPI.** Välj den webbplats som webbprogrammet körs under (t.ex. standardwebbplats) för att aktivera AZURE Multi-Factor Authentication ISAPI-filterplug-in för den platsen.
2. Om du kör på IIS 7 eller senare klickar du på fliken **Inbyggd modul.** Välj server, webbplatser eller program för att aktivera IIS-plugin-programmet på önskade nivåer.
...
4. Ange IP-adress, intervall för IP-adresser eller undernät som ska tillåtas. Om du anger ett undernät väljer du lämplig nätmask och klickar på **OK**.
