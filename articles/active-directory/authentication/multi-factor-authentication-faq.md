...
I det här vanliga avsnittet besvaras vanliga frågor om Azure Multi-Factor Authentication och Multi-Factor Authentication-tjänsten. Den är uppdelad i frågor om tjänsten i allmänhet, fakturerings modeller, användar upplevelser och fel sökning.

## <a name="general"></a>Allmänt

**F: Hur hanterar Azure Multi-Factor Authentication-servern användar data?**

Med Multi-Factor Authentication-Server lagras användar data endast på lokala servrar. Inga beständiga användardata lagras i molnet. När användaren utför tvåstegsverifiering skickar Multi-Factor Authentication-servern data till moln tjänsten Azure Multi-Factor Authentication för autentisering. Kommunikation mellan Multi-Factor Authentication-Server och Multi-Factor Authentication Cloud Service använder Secure Sockets Layer (SSL) eller Transport Layer Security (TLS) över port 443 utgående.
...
* **Enhets** -token (när du använder autentisering med mobilapp)
* **Autentiseringsläge**
* **Autentiserings resultat**
* **Namn på Multi-Factor Authentication-Server**
* **IP för Multi-Factor Authentication Server**
...
Microsoft garanterar inte konsekvent SMS eller Voice-baserad Multi-Factor Authentication-leverans med samma nummer. Med hänsyn till våra användare kan Microsoft lägga till eller ta bort korta koder när vi gör väg justeringar för att förbättra SMS-leveranser. Microsoft stöder inte korta koder för länder/regioner utöver USA och Kanada.

## <a name="billing"></a>Fakturering

De flesta fakturerings frågor kan besvaras genom att referera till [sidan med priser för Multi-Factor Authentication](https://azure.microsoft.com/pricing/details/multi-factor-authentication/) eller dokumentationen om [hur du får Azure Multi-Factor Authentication](concept-mfa-licensing.md).
...
**F: Debiteras fakturerings modellen per användare för alla aktiverade användare eller bara de som utförde tvåstegsverifiering?**
...
**F: Hur fungerar faktureringen av Multi-Factor Authentication?**
...
**F: Kan min organisation växla mellan användnings modeller per användare och per autentisering när som helst?**
...
**F: Vad ska jag säga till att mina användare gör om de inte får något svar på sin telefon?**
...
Du kan peka dina användare till [fel söknings guiden](../user-help/multi-factor-authentication-end-user-troubleshoot.md)för slutanvändare.
...
**F: Kan jag ändra hur lång tid mina användare måste ange verifierings koden från ett SMS innan systemet nått tids gränsen?**
...
Om du använder Azure Multi-Factor Authentication Server kan du importera tredjeparts-tidsbaserad (ed) eng ång slö sen ord (TOTP mobilapp) och sedan använda dem för tvåstegsverifiering.

Du kan använda ActiveIdentity-token som är OATH-TOTP mobilapp-token om du använder den hemliga nyckeln i en CSV-fil och importera till Azure Multi-Factor Authentication Server. Du kan använda OATH-token med Active Directory Federation Services (AD FS) (ADFS), formulärbaserad autentisering i IIS (Internet Information Server) och RADIUS (Remote Authentication Dial-In User Service) så länge klient systemet kan acceptera användarindata.
...
**F: Jag konfigurerade uppringarens ID i MFA Server, men mina användare får fortfarande Multi-Factor Authentication-anrop från en anonym anropare.**
...
**F: Vad ska användarna göra om de ser att det inte finns något fel meddelande om att autentiseringsbegäran inte gäller för ett aktiverat konto när de använder meddelanden i mobilappen?**
...
**F: Vad ska användarna göra om de ser ett 0x800434D4L fel meddelande vid inloggning till ett program som inte är ett webb läsar program?**
