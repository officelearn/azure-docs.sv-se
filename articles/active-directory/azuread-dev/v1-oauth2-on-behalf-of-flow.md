...
| requested_token_use |kunna | Anger hur begäran ska bearbetas. I flödets räkning måste värdet vara **on_behalf_of**. |
| omfång |kunna | En blankstegsavgränsad lista över omfång för Tokenbegäran. För OpenID Connect måste omfångs- **OpenID** anges.|

#### <a name="example"></a>Exempel

Följande HTTP POST begär en åtkomsttoken för https://graph.microsoft.com webb-API. `client_id` identifierar den tjänst som begär åtkomsttoken.
...
Följande HTTP POST begär en åtkomsttoken för https://graph.microsoft.com webb-API med ett certifikat. `client_id` identifierar den tjänst som begär åtkomsttoken.
