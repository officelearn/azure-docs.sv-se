...
| client_id |kunna |Det program-ID som tilldelats din app när du registrerade den med Azure AD. Du hittar det här i Azure Portal. Klicka på **Azure Active Directory**, klicka på **app-registreringar**, Välj programmet och leta upp program-ID: t på program sidan. |
| response_type |kunna |Måste innehålla `id_token` för OpenID Connect-inloggning. Det kan också innehålla andra response_types, till exempel `code` eller `token`. |
...
När du har verifierat `id_token`kan du starta en-session med användaren och använda anspråk i `id_token` för att hämta information om användaren i din app. Den här informationen kan användas för visning, poster, anpassning osv. Läs [AAD id_tokens](../develop/id-tokens.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json)om du vill ha mer information om `id_tokens` och anspråk.
