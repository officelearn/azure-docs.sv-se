...
Om du vill logga först in användaren i din app, kan du skicka en [OpenID Connect](active-directory-v2-protocols-oidc.md) auktoriseringsbegäran och få en `id_token` från v2.0-slutpunkten:
...
| omfång |obligatorisk |En blankstegsavgränsad lista med omfattningar. Det måste innehålla omfånget för OpenID Connect, `openid`, vilket innebär att behörigheten ”logga du in” i godkännande-UI. Alternativt kan du också vill ska ingå i `email` eller `profile` [scope](active-directory-v2-scopes.md) för att få åtkomst till ytterligare användardata. Du kan också omfatta andra scope i den här begäran för att begära tillstånd att olika resurser. |
...
| id_token |Id_token som appen har begärt. Du kan använda id_token för att verifiera användarens identitet och starta en session med användaren. Mer information om id_tokens och deras innehåll som ingår i den [v2.0-slutpunkten tokenreferens](active-directory-v2-tokens.md). |
...
Mer information om anspråk i en id_token finns i den [v2.0-slutpunkten tokenreferens](active-directory-v2-tokens.md).
...
| redirect_uri |rekommenderas |Redirect_uri för din app, där autentiseringssvar kan skickas och tas emot av din app. Det måste exakt matcha en av redirect_uris som du registrerade i portalen, men det måste vara url-kodas. |
| omfång |obligatorisk |En blankstegsavgränsad lista med omfattningar. För att hämta token, omfattar alla [scope](active-directory-v2-scopes.md) du behöver för resursen i närheten. |
...
Mer information om anspråk som finns i åtkomst-token finns i den [tokenreferens för v2.0-slutpunkt](active-directory-v2-tokens.md)
