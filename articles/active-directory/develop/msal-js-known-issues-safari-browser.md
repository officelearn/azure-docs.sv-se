...
Du måste hantera `acquireTokenSilent` felen med ett interaktivt inhämta token-anrop, som uppmanas användaren att logga in.
För att undvika upprepade inloggningar är en metod som du kan implementera för att hantera `acquireTokenSilent` felen och ge användaren ett alternativ för att inaktivera ITP-inställningen i Safari innan du fortsätter med det interaktiva anropet. När inställningen är inaktive rad bör efterföljande tysta token-förnyelsen lyckas.
