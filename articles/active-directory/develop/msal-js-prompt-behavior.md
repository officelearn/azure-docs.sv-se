...
MSAL. js-biblioteket (från och med v 0.2.4) skickar inte någon prompt-parameter under de interaktiva begär`loginRedirect`Anden `loginPopup`( `acquireTokenRedirect` , `acquireTokenPopup`, och) och kommer därmed inte att tvinga fram några uppfrågande beteenden. För tysta token-begäranden `acquireTokenSilent` med metoden skickar MSAL. js en prompt-parameter inställd `none`på.
...
**Ingen:** Det här värdet ser till att användaren inte ser någon interaktiv prompt. Vi rekommenderar att du inte skickar det här värdet till interaktiva metoder i MSAL. js eftersom det kan orsaka oväntade beteenden. Använd i stället `acquireTokenSilent` metoden för att få tysta samtal.

## <a name="next-steps"></a>Nästa steg

Läs mer om `prompt` parametern i det [implicita MSAL för OAuth 2,0](v2-oauth2-implicit-grant-flow.md) som biblioteket använder.
