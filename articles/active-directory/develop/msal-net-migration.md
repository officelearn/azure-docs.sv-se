...
I följande bild sammanfattas några skillnader mellan ADAL.NET och MSAL.NET ![sida vid sida-kod](./media/msal-compare-msaldotnet-and-adaldotnet/differences.png)
...
Om du vill använda MSAL.NET måste du lägga till [Microsoft. Identity. client](https://www.nuget.org/packages/Microsoft.Identity.Client) NuGet-paketet och använda `Microsoft.Identity.Client`-namnrymden

### <a name="scopes-not-resources"></a>Webbprogramsomfattande omfattningar

ADAL.NET hämtar token för *resurser*, men MSAL.net hämtar token för *omfattningar*. Ett antal MSAL.NET AcquireToken-åsidosättningar kräver en parameter med namnet omfång (`IEnumerable<string> scopes`). Den här parametern är en enkel lista med strängar som deklarerar önskade behörigheter och resurser som begärs. Välkända omfattningar är [Microsoft graphs omfång](/graph/permissions-reference).
...
MSAL.NET har fler explicita undantag. Till exempel när tyst autentisering Miss lyckas i ADAL är proceduren att fånga undantaget och leta efter `user_interaction_required` felkod:
...
- `AdalClaimChallengeException` är ett undantag (som härleds från `AdalServiceException`) som har utlösts av tjänsten om en resurs kräver fler anspråk från användaren (t. ex. två faktorer autentisering). Den `Claims` medlemmen innehåller vissa JSON-fragment med anspråken, som förväntas.
- I ADAL.NET kan det offentliga klient programmet som tar emot detta undantag måste anropa `AcquireTokenInteractive` åsidosätta en anspråks parameter. Den här åsidosättningen av `AcquireTokenInteractive` har inte ens nått cachen eftersom det inte är nödvändigt. Orsaken är att token i cacheminnet inte har rätt anspråk (annars skulle en `AdalClaimChallengeException` inte ha utlösts). Därför behöver du inte titta på cachen. Observera att `ClaimChallengeException` kan tas emot i en WebAPI att göra OBO, medan `AcquireTokenInteractive` måste anropas i ett offentligt klient program som anropar webb-API: et.
...
- Det finns en `.WithClaim(claims)`-metod som kan tillämpas på `AcquireTokenInteractive` Builder. 
...
Med ADAL.NET kan du utöka `TokenCache`-klassen för att implementera de önskade beständiga funktionerna på plattformar utan en säker lagring (.NET Framework och .NET Core) genom att använda `BeforeAccess`och `BeforeWrite` metoder. Mer information finns i [cachelagring av token i ADAL.net](https://github.com/AzureAD/azure-activedirectory-library-for-dotnet/wiki/Token-cache-serialization).
...
V 2.0-slutpunkten (används av MSAL) genererar dock versionen av den token som webb-API: n accepterar. En egenskap hos applikations manifestet för webb-API: et gör det möjligt för utvecklare att välja vilken version av token som godkänns. Se `accessTokenAcceptedVersion` i referens dokumentationen för [program manifestet](reference-app-manifest.md) .
...
Om det gäller ett flöde av autentiseringsuppgifter för klient är det också `/.default`att omfånget som ska skickas. Det här omfånget talar om för Azure AD: "alla de behörigheter på program nivå som administratören har samtyckt till i program registreringen.
...
Eftersom den här metoden är avsedd för scenarier som inte är typiska är de inte lättillgängliga med `IConfidentialClientApplication` utan att först omvandla den till `IByRefreshToken`.

Det här kodfragmentet visar en kod för migrering i ett konfidentiellt klient program. `GetCachedRefreshTokenForSignedInUser` Hämta uppdateringstoken som lagrades i vissa lagrings enheter av en tidigare version av programmet som använde för att utnyttja ADAL 2. x. `GetTokenCacheForSignedInUser` deserialiserar en cache för den inloggade användaren (som konfidentiella klient program bör ha en cache per användare).
