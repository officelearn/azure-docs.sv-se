...
ADAL.NET används från [Microsoft. IdentityModel. clients. ActiveDirectory](https://www.nuget.org/packages/Microsoft.IdentityModel.Clients.ActiveDirectory) NuGet-paketet. namn området som ska användas `Microsoft.IdentityModel.Clients.ActiveDirectory`är.
...
MSAL.NET har fler explicita undantag. Till exempel, när tyst autentisering Miss lyckas i ADAL, är proceduren att fånga undantags `user_interaction_required` felet och leta efter felkoden:
...
Med hjälp av MSAL.net fångar `MsalUiRequiredException` du enligt beskrivningen i [AcquireTokenSilent](https://github.com/AzureAD/microsoft-authentication-library-for-dotnet/wiki/AcquireTokenSilentAsync-using-a-cached-token).
...
- I ADAL.net är det offentliga klient programmet som tar emot det här undantaget nödvändigt att `AcquireTokenInteractive` anropa åsidosättningen med en anspråks parameter. Den här åsidosättningen av `AcquireTokenInteractive` försöker inte ens att nå cachen eftersom det inte är nödvändigt. Orsaken är att token i cacheminnet inte har rätt anspråk (annars `AdalClaimChallengeException` skulle a inte ha utlösts). Därför behöver du inte titta på cachen. Observera att `ClaimChallengeException` kan tas emot i en WebAPI att göra OBO, `AcquireTokenInteractive` medan måste anropas i ett offentligt klient program som anropar webb-API: et.
...
- `Claims` Visas`MsalServiceException`i.
- Det finns en `.WithClaim(claims)` metod som kan användas `AcquireTokenInteractive` för verktyget. 
...
Enhetskodflöde | [Enhets profil för enheter utan webbläsare](https://github.com/AzureAD/azure-activedirectory-library-for-dotnet/wiki/Device-profile-for-devices-without-web-browsers) | [Enhets kod flöde](msal-authentication-flows.md#device-code)
...
V 2.0-slutpunkten (används av MSAL) genererar dock versionen av den token som webb-API: n accepterar. En egenskap hos applikations manifestet för webb-API: et gör det möjligt för utvecklare att välja vilken version av token som godkänns. Se `accessTokenAcceptedVersion` i referens [](reference-app-manifest.md) dokumentationen för applikations manifestet.
...
Om du vill hämta token för specifika omfång av ett v 1.0-program (till exempel AAD-grafen, som https://graph.windows.net) du behöver skapa `scopes` genom att sammanfoga ett önskat resurs-ID med önskad OAuth2-behörighet för resursen.

Om du till exempel vill komma åt namnet på användaren a v 1.0 webb-API som app-ID URI `ResourceId`är, vill du använda:
...
Om du vill skriva det omfång som motsvarar Azure Resource Manager-API: t https://management.core.windows.net/) (måste du begära följande omfång (Observera de två snedstrecken) 
...
Detta beror på att Resource Manager-API förväntar sig ett snedstreck i mål`aud`grupps anspråket () och det finns ett snedstreck för att avgränsa API-namnet från omfånget.
...
I ADAL.NET v2. X visade uppdateringstoken att du kan utveckla lösningar kring användningen av dessa token genom att cachelagra dem och använda de `AcquireTokenByRefreshToken` metoder som tillhandahålls av ADAL 2. x. Några av dessa lösningar användes i scenarier som:
...
Detta är inte fallet med MSAL.NET, men vi rekommenderar inte längre att använda uppdateringstoken på det här sättet av säkerhets skäl. Detta skulle göra det svårt att migrera till MSAL 3. x eftersom API: et inte tillhandahåller ett sätt att skicka i tidigare hämtade uppdateringstoken. 

MSAL.NET har nu en API som gör att du kan migrera dina tidigare uppdateringstoken till`IConfidentialClientApplication` 
...
Eftersom den här metoden är avsedd för scenarier som inte är typiska går det inte att komma åt `IConfidentialClientApplication` den utan att först omvandla den till. `IByRefreshToken`
...
Du hittar mer information om omfattningarna i [omfattningar, behörigheter och medgivande i Microsoft Identity Platform-](v2-permissions-and-consent.md) slutpunkten
