...
Med `SingleAccountPublicClientApplication` klassen kan du skapa en MSAL-baserad app som endast tillåter att ett enda konto är inloggat i taget. `SingleAccountPublicClientApplication`skiljer sig `PublicClientApplication` från på följande sätt:
...
- `acquireTokenSilent`kräver ingen konto parameter.  Om du anger ett konto och det konto som du anger inte matchar det aktuella kontot som spåras av MSAL, genereras `MsalClientException` ett.
...
Följande pseudo-kod illustrerar användningen `SingleAccountPublicClientApplication`av.
...
Använd ett eller flera konton i ditt program genom att `acquireToken` anropa en eller flera gånger.  
...
Ta bort ett konto genom `removeAccount` att anropa med en konto identifierare.
