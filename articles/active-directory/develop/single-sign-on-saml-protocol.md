...
| --- | --- | --- |
| id | Obligatorisk | Azure AD använder det här attributet för att `InResponseTo` fylla attributet för det returnerade svaret. ID får inte börja med en siffra, så en gemensam strategi är att lägga en sträng som "ID" till sträng representationen av ett GUID. Till exempel `id6c1c178c166d486687be4aaf5e482730` är ett giltigt ID. |
...
Azure AD ignorerar `Conditions` också elementet i `AuthnRequest`.

### <a name="issuer"></a>Utfärdare

Elementet i en `AuthnRequest` måste exakt matcha ett av ServicePrincipalNames i moln tjänsten i Azure AD. `Issuer` Detta är vanligt vis inställt på **app-ID-URI: n** som anges vid program registrering.
...
* `urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified`: Det här värdet tillåter Azure Active Directory att välja anspråks format. Azure Active Directory utfärdar NameID som en identifierare.
...
Elementet, som innehåller en lista över identitets leverantörer, är valfritt i `AuthnRequest` element som skickas till Azure AD. `Scoping`

Inkludera `ProxyCount` inte `IDPListOption` attributet eller`RequesterID` elementet eftersom de inte stöds.
...
Azure AD ignorerar `Subject` `AuthnRequest` elementets element.
...
* `Destination`: När inloggningen är klar har detta angetts till `RedirectUri` tjänst leverantören (moln tjänsten).
* `InResponseTo`: Detta anges till `ID` attributet för det `AuthnRequest` element som initierade svaret.

### <a name="issuer"></a>Utfärdare

Azure AD ställer `Issuer` in elementet till `https://login.microsoftonline.com/<TenantIDGUID>/` där \<TenantIDGUID > är klient-ID: t för Azure AD-klienten.
...
Förutom `ID`, `IssueInstant` och ställer`Version` AzureADinföljandeelementi`Assertion` svarets element.
...
Attribut för elementet är alltid inställt på `urn:oasis:names:tc:SAML:2.0:cm:bearer`. `Method` `SubjectConfirmation`
...
Attributen `NotOnOrAfter` och anger det intervall under vilket kontrollen är giltig. `NotBefore`
...
* Värdet för `NotOnOrAfter` attributet är 70 minuter senare än värdet `NotBefore` för attributet.

#### <a name="audience"></a>Målgrupp

Innehåller en URI som identifierar en avsedd mål grupp. Azure AD anger värdet för det här elementet till värdet för `Issuer` elementet `AuthnRequest` i som initierade inloggningen. Om du vill `Audience` utvärdera värdet använder du värdet för det `App ID URI` som angavs under program registreringen.
...
Precis som `Audience` värdet måste värdet exakt matcha ett av tjänstens huvud namn som representerar moln tjänsten i Azure AD. `Issuer` Men `Issuer` om värdet för elementet inte är ett URI `Audience` -värde, är värdet i svaret `spn:`det `Issuer` värde som föregås av.
...
* **Namn anspråk** – värdet för `Name` attributet (`http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name`) är User Principal Name av `testuser@managedtenant.com`den autentiserade användaren, t. ex.
