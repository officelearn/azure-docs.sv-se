...
När du har verifierat den `id_token`, du kan starta en session med användaren och använda anspråk i den `id_token` att hämta information om användare i din app. Den här informationen kan användas för visning, poster, auktoriseringar osv. Mer information om typer av token och anspråk [stöds Token och anspråkstyper](v1-id-and-access-tokens.md).
