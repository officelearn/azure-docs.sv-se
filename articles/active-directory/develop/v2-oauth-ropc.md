...
> [!IMPORTANT]
>
> * Slut punkten för Microsoft Identity Platform stöder endast ROPC för Azure AD-klienter, inte personliga konton. Det innebär att du måste använda en klient-/regionsspecifika slut punkt (`https://login.microsoftonline.com/{TenantId_or_Name}`) eller slut punkten för @no__t 1.
...
> [![Try som kör denna begäran i Postman](./media/v2-oauth2-auth-code-flow/runInPostman.png)](https://app.getpostman.com/run-collection/f77994d794bab767596d)
...
| --- | --- | --- |
| `tenant` | Krävs | Den katalog klient som du vill logga in användaren i. Detta kan vara i ett GUID eller eget namn format. Den här parametern kan inte anges till `common` eller `consumers`, men kan anges till `organizations`. |
...
| `grant_type` | Krävs | Måste vara inställd på `password`. |
...
| `client_secret`| Krävs ibland | Om din app är en offentlig klient kan inte `client_secret` eller `client_assertion` tas med.  Om appen är en konfidentiell klient måste den tas med. | 
...
| `id_token` | JWT | Utfärdat om den ursprungliga `scope`-parametern innehöll området `openid`. |
| `refresh_token` | Ogenomskinlig sträng | Utfärdat om den ursprungliga `scope`-parametern ingår `offline_access`. |
...
| `invalid_request` | Begäran har inte konstruerats korrekt | Anslags typen stöds inte för autentiserings kontexterna `/common` eller `/consumers`.  Använd `/organizations` eller klient-ID i stället. |
