...
1. Klicka sedan på **Lägg** till för att lägga till en roll tilldelning.
    ...
    ![Lägg till en roll tilldelning](./media/entitlement-management-logs-and-reporting/workspace-set-role-assignment.png)
...
Om du har flera Log Analytics arbets ytor i den prenumerationen returnerar cmdleten [Get-AzOperationalInsightsWorkspace](/powershell/module/Az.OperationalInsights/Get-AzOperationalInsightsWorkspace) listan över arbets ytor. Sedan kan du hitta den som har Azure AD-loggarna. Det `CustomerId` fältet som returnerades av denna cmdlet är detsamma som värdet för "arbetsyte-ID" som visas i Azure Portal i översikten över Log Analytics-arbetsytan.
