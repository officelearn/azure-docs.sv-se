...
Fel under export till Azure AD indikerar att åtgärden \(lägga till, uppdatera, ta bort osv.\) försökte utföra Azure AD Connect \(Sync-motorn\) på Azure Active Directory misslyckades.
...
* När Azure AD Connect \(Synkroniseringsmotorn\) instruerar Azure Active Directory att lägga till eller uppdatera objekt, matchar Azure AD det inkommande objektet med attributet **sourceAnchor** till attributet **immutableId** för objekt i Azure AD. Den här matchningen kallas för en **hård matchning**.
...
Den vanligaste orsaken till att InvalidSoftMatch-felet är två objekt med olika SourceAnchor \(immutableId\) har samma värde för attributen ProxyAddresses och/eller UserPrincipalName, som används under den mjuka matchnings processen i Azure AD. För att åtgärda den ogiltiga mjuka matchningen
...
1. Admin skapar en ny e-postaktiverad säkerhets grupp i Office 365 för skatte avdelningen och tillhandahåller en e-postadress som tax@contoso.com. Den här gruppen har tilldelats attributvärdet ProxyAddresses för **SMTP: skatte\@contoso.com**
...
Den vanligaste orsaken till AttributeValueMustBeUnique-felet är två objekt med olika SourceAnchor \(immutableId\) har samma värde för attributen ProxyAddresses och/eller UserPrincipalName. För att åtgärda AttributeValueMustBeUnique-fel
...
#### <a name="related-articles"></a>Relaterade artiklar
-[duplicera eller ogiltiga attribut förhindra katalog synkronisering i Office 365](https://support.microsoft.com/kb/2647098)
...
2. Tillåt nästa synkronisering att försöka synkronisera. Den här tidssynkroniseringen kommer att lyckas och det kommer att uppdatera UserPrincipalName av Bob till bob@fabrikam.com som förväntat.
