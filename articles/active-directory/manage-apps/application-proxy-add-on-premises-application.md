...
Eftersom programmet som du lägger till är lokalt behövs en Windows-server som kör Windows Server 2012 R2 eller senare som du kan installera anslutningsprogrammet för programproxyn på. Anslutningsservern måste ansluta till programproxytjänsten i Azure och de lokala programmen som du har för avsikt att publicera.
...
Om din brandvägg eller proxyserver tillåter DNS-listor över tillåtna program kan du tillåta anslutningsprogram till msappproxy.net och servicebus.windows.net. Om inte måste du tillåta åtkomst till [Azure DataCenter IP-intervallen](https://www.microsoft.com/download/details.aspx?id=41653) som uppdateras varje vecka.
...
Om du tidigare har installerat ett anslutningsprogram måste du installera om för att hämta den senaste versionen.
...
Om ditt program använder WebSockets för att ansluta måste du se till att du har installerat det senaste anslutningsprogrammet.  WebSockets stöds på anslutningsprogram med version 1.5.612.0 eller senare.
...
5. Konfigurera ytterligare inställningar om det behövs. De flesta programmen bör behålla dessa inställningarna i standardtillstånden. 
    ...
    | **Använd endast HTTP-cookie** | Ställ in värdet på **Ja** för att programproxycookies ska inkluderas i HTTPOnly-flaggan i HTTP-svarsrubriken. Ställ in det på **Nej** om du använder fjärrskrivbordstjänster.|
    | **Använd säker cookie**| Ställ in värdet på **Ja** för att se till att cookies endast skickas via en säker kanal, så som en krypterad HTTPS-begäran.
    ...
    | **Översätt webbadresser i brödtext för program** | Behåll det här värdet som **Nej** såvida du inte har hårdkodade HTML-länkar till andra lokala program och inte använder anpassade domäner. Mer information finns i [Länka översättning med programproxy](application-proxy-configure-hard-coded-link-translation.md). |
...
Om du vill testa att ditt program har lagts till korrekt kan du lägga till ett användarkonto i programmet och försöka logga in. 
...
2. Du bör se startskärmen och ska kunna logga in med testkontot som du har konfigurerat.
...
I den här självstudien förberedde du din lokala miljö för arbete med programproxy och installerade och registrerade sedan anslutningsprogrammet för programproxy. Därefter lade du till ett program till Azure AD-klientorganisationen och kontrollerade att det fungerade genom att logga in i programmet med ett Azure AD-konto.
...
Nu är du redo att konfigurera programmet för enkel inloggning. Det finns flera metoder för enkel inloggning och hur du väljer den bästa beror på hur programmet autentiserar. Länken nedan hjälper dig att hitta den aktuella självstudien för enkel inloggning för ditt program.
