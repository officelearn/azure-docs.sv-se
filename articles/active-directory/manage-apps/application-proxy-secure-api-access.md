...
- Klistra in **omdirigerings** -URI `<add key="ida:RedirectUri" value="" />` för AppProxyNativeAppSample i fältet. Du kan hitta och kopiera det här värdet (en URI) från sidan AppProxyNativeAppSample- **autentisering** .
...
- Klistra in **URL: en** för SecretAPI Start `<add key="todo:TodoListBaseAddress" value="" />` sida i fältet. Du kan hitta och kopiera det här värdet (en URL) från sidan för SecretAPI- **anpassning** .
