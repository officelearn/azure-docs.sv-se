...
1. **Azure AD**: om `Restrict-Access-To-Tenants: <permitted tenant list>` finns, utfärdar bara Azure AD säkerhetstoken för tillåtna innehavare.

2. **Lokal Proxy Server-infrastruktur**: den här infrastrukturen är en proxy-enhet som kan Secure SOCKETS Layer (SSL). Du måste konfigurera proxyn så att den infogar rubriken som innehåller listan över tillåtna klienter i trafik som är avsedda för Azure AD.
...
- Klienter måste lita på certifikatkedjan som presenteras av proxyn för SSL-kommunikation. Om till exempel certifikat från en intern [offentlig nyckel infrastruktur (PKI)](/windows/desktop/seccertenroll/public-key-infrastructure) används, måste det interna utfärdande rot certifikat utfärdarens certifikat vara betrott.

- Den här funktionen ingår i Office 365-prenumerationer, men om du vill använda klient begränsningar för att kontrol lera åtkomsten till andra SaaS-appar krävs Azure AD Premium 1-licenser.
...
Klienter måste vara tvungen att använda proxyn för alla begäranden till login.microsoftonline.com login.microsoft.com och login.windows.net. Om till exempel PAC-filer används för att dirigera klienter till att använda proxyn, bör slutanvändare inte kunna redigera eller inaktivera PAC-filerna.
...
En exempelanvändare finns i Contoso-nätverket, men försöker komma åt ett delat SaaS-program som Outlook online Fabrikam-instansen. Om Fabrikam är en icke-tillåten klient för Contoso-instansen ser användaren ett nekat åtkomst meddelande, som säger att du försöker få åtkomst till en resurs som tillhör en organisation som inte har godkänts av IT-avdelningen.
...
Office 365-program måste uppfylla två villkor för att fullständigt stödja klient begränsningar:
