...
1. Hitta ditt klient-ID genom att gå till **Azure Active Directory** > **Egenskaper** > **katalog-ID**. I avsnittet cmdlets använder du detta ID när du behöver ange resourceId.
...
Använd följande cmdlet för att hämta alla inbyggda och anpassade Azure AD-roller i din Azure AD-organisation (klient). Det här viktiga steget ger mappningen mellan roll namnet och roleDefinitionId. RoleDefinitionId används i alla dessa cmdletar för att referera till en speciell roll.
...
Om du vill uppdatera roll inställningen måste du först definiera ett inställnings objekt enligt följande:
