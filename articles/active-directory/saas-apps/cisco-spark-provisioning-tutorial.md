...
Syftet med den här kursen är att ange vilka åtgärder som ska utföras i Cisco Spark och Azure Active Directory (Azure AD) för att konfigurera Azure AD att automatiskt etablera och avetablera användare Cisco Spark.
...
*   Ett användarkonto i Cisco Spark med administratörsbehörigheter


> [!NOTE]
> Azure AD-etablering integration förlitar sig på den [Cisco Spark Webservice](https://developer.webex.com/getting-started.html), som är tillgängliga för Cisco Spark team.

## <a name="adding-cisco-spark-from-the-gallery"></a>Att lägga till Cisco Spark från galleriet
Du måste lägga till Cisco Spark från Azure AD application gallery i listan över hanterade SaaS-program innan du konfigurerar Cisco Spark för automatisk användaretablering med Azure AD.

**Utför följande steg för att lägga till Cisco Spark från Azure AD application gallery:**
...
5. Välj i resultatpanelen **Cisco Spark**, och klicka sedan på den **Lägg till** för att lägga till Cisco Spark i listan med SaaS-program.
...
Innan du konfigurerar och aktiverar automatisk användaretablering, bör du bestämma vilka användare i Azure AD behöver åtkomst till Cisco Spark. När du valt, kan du tilldela dessa användare till Cisco Spark genom att följa anvisningarna här:
...
*   Vi rekommenderar att en enda Azure AD-användare har tilldelats till Cisco Spark testa automatisk konfiguration för användaretablering. Ytterligare användare kan tilldelas senare.

*   När du tilldelar en användare Cisco Spark, måste du välja någon giltig programspecifika roll (om tillgängligt) i dialogrutan tilldelning. Användare med den **standard åtkomst** roll är undantagna från etablering.

## <a name="configuring-automatic-user-provisioning-to-cisco-spark"></a>Konfigurera automatisk användaretablering Cisco Spark

Det här avsnittet hjälper dig igenom stegen för att konfigurera Azure AD etableras för att skapa, uppdatera och inaktivera användare i Cisco Spark baserat på användare i Azure AD.
...
2. Välj Cisco Spark från listan med SaaS-program.
...
5. Under den **administratörsautentiseringsuppgifter** avsnitt, ange den **klient URL**, och **hemlighet Token** för din Cisco Spark-konto.

    *   I den **klient URL** fältet, Fyll i Cisco Spark SCIM API-URL för din klient i form av `https://api.ciscospark.com/v1/scim/[Tenant ID]/`, där `[Tenant ID]` är en alfanumerisk sträng som beskrivs i steg 6.

    *   I den **hemlighet Token** fältet, fylla hemlighet-Token som beskrivs i steg 6.

1. Den **klient-ID** och **hemlighet Token** för Cisco-Spark konto kan hittas genom att logga in på [Cisco Spark developer plats](https://developer.webex.com/) med ditt administratörskonto. Efter att du loggat –
    ...
    * Gå till den [sidan få information om min egen](https://developer.webex.com/endpoint-people-me-get.html)
        ...
        * Skriv ordet ”ägar” följt av ett blanksteg och klistra in hemlighet-Token i fältet auktorisering ![Cisco Spark-autentiseringstoken](./media/cisco-spark-provisioning-tutorial/GetMyDetails.png)
        * Klicka på Kör
...
1. För att fylla i fälten som visas i steg 5, klickar du på **Testanslutningen** så Azure AD kan ansluta till Cisco Spark. Om anslutningen misslyckas, kontrollera din Cisco Spark-kontot har administratörsbehörighet och försök igen.
...
10. Under den **mappningar** väljer **synkronisera Azure Active Directory-användare Cisco Spark**.

    ![Cisco Spark etablering](./media/cisco-spark-provisioning-tutorial/UserMapping.png)

11. Granska användarattribut som ska synkroniseras från Azure AD med Cisco Spark i den **attributmappning** avsnitt. De attribut som valts som **matchande** egenskaper som används för att matcha användarkonton i Cisco Spark för uppdateringsåtgärder. Välj den **spara** för att genomföra ändringarna.
...
13. Om du vill aktivera Azure AD-tjänsten för Cisco Spark-etablering, ändra den **Status för etablering** till **på** i den **inställningar** avsnitt.

    ![Cisco Spark etablering](./media/cisco-spark-provisioning-tutorial/ProvisioningStatus.png)

14. Ange användare och/eller grupper som du vill att etablera till Cisco Spark genom att välja önskade värden i **omfång** i den **inställningar** avsnitt.
