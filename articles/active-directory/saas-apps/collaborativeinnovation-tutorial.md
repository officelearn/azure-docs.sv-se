...
1. Samarbetsfunktioner Innovation program som förväntar SAML-intyg i ett visst format. Konfigurera följande anspråk för det här programmet. Du kan hantera värdena för dessa attribut från den ”**användarattribut**” på sidan för integrering av program. Följande skärmbild visar ett exempel för detta.
...
1. Att konfigurera enkel inloggning på **samarbetsfunktioner Innovation** sida, som du behöver skicka de hämtade **XML-Metadata för** till [samarbetsfunktioner Innovation supportteamet](https://www.unilever.com/contact/). De ställer du in SAML SSO ansluta till korrekt inställda på båda sidorna.
