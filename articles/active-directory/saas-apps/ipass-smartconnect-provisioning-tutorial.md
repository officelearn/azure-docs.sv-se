...
5. Under avsnittet **admin credentials** , in`https://openmobile.ipass.com/moservices/scim/v1` i **klient-URL**. Ange Bearer-token som hämtades tidigare i **hemlig token**. Klicka på **Testa anslutning** för att se till att Azure AD kan ansluta till iPass SmartConnect. Om anslutningen Miss lyckas kontrollerar du att iPass SmartConnect-kontot har administratörs behörighet och försöker igen.
