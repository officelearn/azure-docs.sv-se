...
1. Du måste importera certifikatet för metadata till F5 (Kerberos) som kommer att användas senare i installations processen. Gå till **System > certifikat hantering > hantering av trafik certifikat > > lista över SSL-certifikat**. Klicka på **Importera** till höger hörn.

    ![F5-konfiguration (Kerberos)](./media/kerbf5-tutorial/configure01.png) 

1. Dessutom behöver du också ett **SSL-certifikat** för värd namnet`Kerbapp.superdemo.live`(), i det här exemplet användes certifikat med jokertecken.

    ![F5-konfiguration (Kerberos)](./media/kerbf5-tutorial/configure02.png) 
 
1. Gå till – **F5 Big-IP klicka på åtkomst > guidad konfiguration > Federation > SAML-** tjänstprovider.
...
1. Skapa en ny virtuell server, ange **mål adressen**. Välj det **jokertecken** (eller det **certifikat** som du laddade upp för programmet) som vi laddade upp tidigare och den **tillhör ande privata nyckeln**.
...
1. Under **Inställningar för enkel inloggning**väljer du **Kerberos** och sedan **Avancerade inställningar**. Begäran måste skapas i `user@domain.suffix`.

1. Ange`session.saml.last.attr.name. http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname`i **källa för användar namn** . Se bilaga för en fullständig lista över variabler och värden.
...
1. Kontakta [F5-support teamet (Kerberos)](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45) för **slut punkts kontroll egenskaper** dokumentations information.

    ![F5-konfiguration (Kerberos)](./media/kerbf5-tutorial/configure10.png) 

1. Kontakta [F5-klientens support team](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45) för **Egenskaper för sessionshantering** dokumentation information.
