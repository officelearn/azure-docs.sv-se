...
5. Under avsnittet **admin credentials** , in`https://admin.mindtickle.com/scim` i **klient-URL**. Ange värdet för **JWT-token** som hämtades tidigare i text rutan för hemligt token, ange värdet för **JWT-token** som gavs av MindTickle support team. Klicka på **Testa anslutning** för att se till att Azure AD kan ansluta till principer. Om anslutningen Miss lyckas kontrollerar du att MindTickle-kontot har administratörs behörighet och försöker igen.
