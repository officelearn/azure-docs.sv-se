...
1.  För att hämta den **hemliga** Miro-kontakta support teamet på support@miro.com. Det här värdet anges i fältet Hemlig token på fliken etablering i ditt Miro-program i Azure Portal.
...
5. Under avsnittet **admin credentials** , in`https://miro.com/api/v1/scim` i **klient-URL**. Mata in **scim-autentiseringstoken** som hämtades tidigare i **hemlig token**. Klicka på **Testa anslutning** för att se till att Azure AD kan ansluta till Miro. Om anslutningen Miss lyckas kontrollerar du att Miro-kontot har administratörs behörighet och försöker igen.
