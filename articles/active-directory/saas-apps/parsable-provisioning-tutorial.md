---
title: 'Självstudie: Konfigurera parsning för automatisk användar etablering med Azure Active Directory | Microsoft Docs'
description: Lär dig att automatiskt etablera och avetablera användar konton från Azure AD till att parsas.
...
I den här självstudien beskrivs de steg som du måste utföra i både parsable och Azure Active Directory (Azure AD) för att konfigurera automatisk användar etablering. När Azure AD har kon figurer ATS, etablerar och avetablerar Azure AD automatiskt användare och grupper som kan [tolkas](https://www.parsable.com/) med Azure AD Provisioning-tjänsten. Viktig information om vad den här tjänsten gör, hur den fungerar och vanliga frågor finns i [Automatisera användaretablering och avetablering för SaaS-program med Azure Active Directory](../manage-apps/user-provisioning.md). 
...
* En Parsbar klient (Team).
* Ett användar konto som går att parsa med administratörs behörighet.
...
3. Bestäm vilka data som ska [mappas mellan Azure AD och parsas](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes). 

## <a name="step-2-configure-parsable-to-support-provisioning-with-azure-ad"></a>Steg 2. Konfigurera tolkning som stöd för etablering med Azure AD

1. Kontakta den tolknings bara kund som är ansvarig för att delta i den här för hands versions funktionen.
2. De kommer att ytterligare hjälpa till att få fram ett support ärende för att erhålla nödvändig **Bearer-token** (hemlig token).
3. Kopiera och spara **Bearer-token**. Det här värdet anges i fältet **hemlig token** * på fliken etablering i det tolknings bara programmet i Azure Portal.

## <a name="step-3-add-parsable-from-the-azure-ad-application-gallery"></a>Steg 3. Lägg till parsning från Azure AD-programgalleriet

Lägg till parsning från Azure AD-programgalleriet för att börja hantera etablering till parsning. Om du tidigare har konfigurerat en parsning för SSO kan du använda samma program. Vi rekommenderar dock att du skapar en separat app när du testar integreringen i början. Lär dig mer om att lägga till ett program från galleriet [här](https://docs.microsoft.com/azure/active-directory/manage-apps/add-gallery-app). 
...
* När du tilldelar användare och grupper till att kunna parsas måste du välja en annan roll än **standard åtkomst**. Användare med rollen Standardåtkomst undantas från etableringen och markeras som icke-berättigade i etableringsloggarna. Om den enda rollen som är tillgänglig i programmet är standardrollen för åtkomst, kan du [uppdatera applikationsmanifest](https://docs.microsoft.com/azure/active-directory/develop/howto-add-app-roles-in-azure-ad-apps) och lägga till fler roller. 
...
## <a name="step-5-configure-automatic-user-provisioning-to-parsable"></a>Steg 5. Konfigurera automatisk användar etablering till parsning 

Det här avsnittet vägleder dig genom stegen för att konfigurera Azure AD Provisioning-tjänsten för att skapa, uppdatera och inaktivera användare och/eller grupper i TestApp baserat på användar-och/eller grupp tilldelningar i Azure AD.

### <a name="to-configure-automatic-user-provisioning-for-parsable-in-azure-ad"></a>Konfigurera automatisk användar etablering för parsning i Azure AD:

1. Logga in på [Azure-portalen](https://portal.azure.com). Välj **Företagsprogram** och sedan **Alla program**.

    ![Bladet Företagsprogram](common/enterprise-applications.png)

2. I listan program väljer du kan **parsas**.

    ![Den parsade länken i program listan](common/all-applications.png)
...
5. Under avsnittet **admin credentials** kan du skriva in den parsade klient-URL: en och den hemliga token. Klicka på **Testa anslutning** för att se till att Azure AD kan ansluta till parsning. Om anslutningen Miss lyckas kontrollerar du att det går att parsa kontot administratörs behörighet och försöker igen.
...
8. Under avsnittet **mappningar** väljer du **Synkronisera Azure Active Directory användare att parsa**.

9. Granska de användarattribut som synkroniseras från Azure AD till parsning i avsnittet **attribut-mappning** . Attributen som väljs som **matchande** egenskaper används för att matcha de användar konton som kan parsas för uppdaterings åtgärder. Om du väljer att ändra [matchande målattribut](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes)måste du se till att det parsade API: et stöder filtrering av användare baserat på attributet. Välj knappen **Spara** för att spara ändringarna.
