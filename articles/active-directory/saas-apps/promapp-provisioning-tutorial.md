...
5. Under avsnittet **admin credentials** , in`https://api.promapp.com/api/scim` i **klient-URL**. Mata in **scim-autentiseringstoken** som hämtades tidigare i **hemlig token**. Klicka på **Testa anslutning** för att se till att Azure AD kan ansluta till Promapp. Om anslutningen Miss lyckas kontrollerar du att Promapp-kontot har administratörs behörighet och försöker igen.
