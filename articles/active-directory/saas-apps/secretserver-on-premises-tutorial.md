...
2. **[Konfigurera Secret Server (on-PREMISES) SSO](#configure-secret-server-on-premises-sso)** – för att konfigurera enskilda Sign-On-inställningar på program sidan.
...
När du klickar på panelen Secret Server (On-Premises) på åtkomst panelen, bör du loggas in automatiskt på den Secret Server (On-Premises) som du ställer in SSO för. Mer information om åtkomstpanelen finns i [introduktionen till åtkomstpanelen](../user-help/my-apps-portal-end-user-access.md).
