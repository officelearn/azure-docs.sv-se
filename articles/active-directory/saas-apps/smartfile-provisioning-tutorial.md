...
5.  Under avsnittet **admin credentials** , in`https://<SmartFile sitename>.smartfile.com/ftp/scim` i **klient-URL**. Ett exempel skulle se ut som `https://demo1test.smartfile.com/ftp/scim`. Ange värdet för **Bearer-token** (ProductKey: ProductPassword) som du hämtade tidigare i **hemlig token**. Klicka på **Testa anslutning** för att se till att Azure AD kan ansluta till SmartFile. Om anslutningen Miss lyckas kontrollerar du att SmartFile-kontot har administratörs behörighet och försöker igen.
