...
5. Under avsnittet **admin credentials** , in`https://sigateway.com/scim/v2/sync/serviceproviderconfig` i **klient-URL**. Mata in **scim-autentiseringstoken** som hämtades tidigare i **hemlig token**. Klicka på **Testa anslutning** för att se till att Azure AD kan ansluta till Soloinsight-CloudGate SSO. Om anslutningen Miss lyckas kontrollerar du att ditt Soloinsight-CloudGate SSO-konto har administratörs behörighet och försöker igen.
