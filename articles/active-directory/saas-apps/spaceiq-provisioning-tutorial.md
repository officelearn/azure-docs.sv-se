...
5. Under avsnittet **admin credentials** , in`https://api.spaceiq.com/scim` i **klient-URL**. Mata in **scim-autentiseringstoken** som hämtades tidigare i **hemlig token**. Klicka på **Testa anslutning** för att se till att Azure AD kan ansluta till SpaceIQ. Om anslutningen Miss lyckas kontrollerar du att SpaceIQ-kontot har administratörs behörighet och försöker igen.
