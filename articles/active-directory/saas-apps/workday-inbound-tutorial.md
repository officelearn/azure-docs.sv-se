...
* Finns det användare som behöver etableras både Active Directory och Azure Active Directory (till exempel ”hybrid” användare)?
* Finns det användare som ska etableras på Azure Active Directory, men inte Active Directory (till exempel ”endast molnbaserad” användare)?
...
En allokering kontakt med en enda källsystemet API-gränssnitt och hjälper etableringsdata till ett enda mål-system. De flesta etablering kopplingar som har stöd för Azure AD är för ett enda käll- och system (t.ex, Azure AD att ServiceNow) och kan konfigureras genom att lägga till appen i fråga från galleriet Azure AD app (till exempel ServiceNow).
...
![AAD App-galleriet](./media/workday-inbound-tutorial/WD_Gallery.PNG)
...
1. Ange redigera säkerhetsgrupp i sökrutan och klicka sedan på **redigera säkerhetsgrupp**.
    ...
    ![Redigera säkerhetsgrupp](./media/workday-inbound-tutorial/IC750983.png "redigera säkerhetsgrupp")
1. Söka efter och välj den nya säkerhetsgruppen för integrering av namn.

    ![Redigera säkerhetsgrupp](./media/workday-inbound-tutorial/IC750984.png "redigera säkerhetsgrupp")
...
1. Ange **domän säkerhetsprinciper** i sökrutan och klicka på länken **domän säkerhetsprinciper för funktionsområde**.  
    ...
    ![Domän säkerhetsprinciper](./media/workday-inbound-tutorial/IC750986.png "säkerhetsprinciper för domänen")  
2. Sök efter system och välj den **System** funktionsområde.  Klicka på **OK**.  

    ![Domän säkerhetsprinciper](./media/workday-inbound-tutorial/IC750987.png "säkerhetsprinciper för domänen")  
3. I listan över säkerhetsprinciper för de funktionella området System, expanderar **säkerhetsadministration** och välj säkerhetsprincip för domän **externa Kontoetablering**.  
    ...
    ![Domän säkerhetsprinciper](./media/workday-inbound-tutorial/IC750988.png "säkerhetsprinciper för domänen")  
1. Klicka på **Redigera behörigheter**, och klicka sedan på den **Redigera behörigheter** dialogrutan sidan lägger till den nya säkerhetsgruppen i listan säkerhetsgrupper med **hämta** och **placera**  integrering behörigheter.

    ![Redigera behörighet](./media/workday-inbound-tutorial/IC750989.png "redigera behörighet")  
...
2. Börja aktiviteten aktivera väntande ändringar i säkerhet genom att ange en kommentar för granskningsändamål och klicka sedan på **OK**. 
    ...
    ![Aktivera väntande säkerhet](./media/workday-inbound-tutorial/IC750993.png "aktivera väntande säkerhet")  
1. Slutföra uppgiften på nästa skärm genom att markera kryssrutan **Bekräfta**, och klicka sedan på **OK**.

    ![Aktivera väntande säkerhet](./media/workday-inbound-tutorial/IC750994.png "aktivera väntande säkerhet")  
...
6. Om du vill spara dina mappningar klickar du på **spara** överst i avsnittet attribut mappar.
...
5. Välj **externa**, och välj Human_Resources WSDL-filen som du hämtade i steg 2.
    ...
    ![Arbetsdagar Studio](./media/workday-inbound-tutorial/WDstudio1.PNG)
...
8.  Klicka på små **konfigurera** länken nedan rutorna begäran och svar för att ange dina Workday-autentiseringsuppgifter. Kontrollera **autentisering**, och ange användarnamn och lösenord för kontot Workday-integrering system. Se till att formatera användarnamnet som name@tenant, och lämna den **WS-Security UsernameToken** alternativ som valts.
    ...
    ![Arbetsdagar Studio](./media/workday-inbound-tutorial/WDstudio2.PNG)
...
13. I kommando-fältet i Studio för Workday Välj **fil > öppen fil...**  och öppna XML-filen som du sparade. Då öppnas den i Workday Studio XML-redigeraren.

    ![Arbetsdagar Studio](./media/workday-inbound-tutorial/WDstudio3.PNG)

14. I trädet filen navigera genom **/ENV: Envelope > env: Body > wd:Get_Workers_Response > wd:Response_Data > wd: Worker** att hitta dina användardata. 
...
1. Ta bort den **/env:Envelope / ENV / wd:Get_Workers_Response / wd:Response_Data /** prefix från det kopierade uttrycket.

18. Om det sista objektet i det kopierade uttrycket är en nod (exempel ”: / wd: Birth_Date”), lägger **/text()** i slutet av uttrycket. Detta är inte nödvändigt om det sista objektet är ett attribut (exempel ”:/@wd: typ”).
...
5. Välj **redigera attributlistan för Workday**.
    ...
    ![Arbetsdagar Studio](./media/workday-inbound-tutorial/WDstudio_AAD1.PNG)
...
10. Välj **lägga till attributet**.
    ...
    ![Arbetsdagar Studio](./media/workday-inbound-tutorial/WDstudio_AAD2.PNG)

11. Välj **spara** ovan, och sedan **Ja** till dialogrutan. Stäng fönstret attribut mappar om det fortfarande är öppet.
