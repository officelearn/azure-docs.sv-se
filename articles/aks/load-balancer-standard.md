...
Du kan också använda **`load-balancer-outbound-ports`** parametrarna när du skapar ett kluster, men du måste även ange antingen **`load-balancer-managed-outbound-ip-count`** , **`load-balancer-outbound-ips`** eller **`load-balancer-outbound-ip-prefixes`** .  Ett exempel:
