...
När den här Pod distribueras, till exempel att `kubectl apply -f gpu-toleration.yaml`använda, kan Kubernetes schemalägga Pod på noderna med den använda bismaken. Med den här logiska isoleringen kan du kontrol lera åtkomsten till resurser i ett kluster.
...
En POD-specifikation lägger sedan `nodeSelector` till egenskapen för att definiera en Node-selektor som matchar etikett uppsättningen på en nod:
