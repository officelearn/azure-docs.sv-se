...
4. Ange inställningarna för API:et.
    ...
    |**Namn**|*tom-api*|Tillhandahåller ett unikt namn för API:et.|
    |**Webbtjänst-URL** (valfritt)|*https://httpbin.org*| Om du vill testa ett API, kanske du inte skriver in något. <br/>I det här [https://httpbin.org](https://httpbin.org)fallet anger vi . Detta är en offentlig testtjänst. <br/>Om du vill importera ett API som mappas till en serverdel automatiskt, se något av ämnena i avsnittet [relaterade ämnen](#related-topics).|
    ...
    |**URL-suffix**|*hbin (hbin)*| Suffixet är ett namn som identifierar det här specifika API:et i den här APIM-instansen. Det måste vara unikt i den här APIM-instansen.|
