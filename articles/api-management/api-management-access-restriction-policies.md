...
| api        | Lägg till en eller flera av de här elementen för att införa en gräns för anrops frekvensen för API: er i produkten. Hastighets begränsningar för produkt-och API-anrop tillämpas oberoende av varandra. Du kan referera till API: t antingen via `name` eller `id`. Om båda attributen anges används `id` och `name` kommer att ignoreras.                    | Nej       |
| operation  | Lägg till en eller flera av de här elementen för att införa en gräns för anrops frekvensen för åtgärder inom ett API. Hastighets begränsningar för produkt, API och åtgärd tillämpas oberoende av varandra. Åtgärden kan refereras antingen via `name` eller `id`. Om båda attributen anges används `id` och `name` kommer att ignoreras. | Nej       |

### <a name="attributes"></a>Attribut

| Namn           | Beskrivning                                                                                           | Krävs | Default |
| -------------- | ----------------------------------------------------------------------------------------------------- | -------- | ------- |
| namn           | Namnet på det API som hastighets begränsningen ska tillämpas för.                                                | Ja      | Ej tillämpligt     |
| samtal          | Maximalt antal anrop som tillåts under det tidsintervall som anges i `renewal-period`. | Ja      | Ej tillämpligt     |
...
| Increment-Condition | Det booleska uttryck som anger om begäran ska räknas mot kvoten (`true`).        | Nej       | Ej tillämpligt     |
| förnyelse-period      | Tids perioden i sekunder efter vilken kvoten återställs.                                              | Ja      | Ej tillämpligt     |
...
| adresspool                                   | Anger en enskild IP-adress som du vill filtrera efter.   | Minst ett `address`-eller `address-range`-element krävs. |
...
| ip-filter action="allow &#124; forbid"    | Anger om anrop ska tillåtas eller inte för de angivna IP-adresserna och intervallen. | Ja                                                | Ej tillämpligt     |
...
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| kvot     | Rot element.                                                                                                                                                                                                                                                                                | Ja      |
| api       | Lägg till en eller flera av de här elementen för att införa anrops kvot för API: er i produkten. Kvoten för produkt-och API-anrop tillämpas oberoende av varandra. Du kan referera till API: t antingen via `name` eller `id`. Om båda attributen anges används `id` och `name` kommer att ignoreras.                    | Nej       |
...
| Namn           | Beskrivning                                                                                               | Krävs                                                         | Default |
| -------------- | --------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ------- |
| namn           | Namnet på det API eller den åtgärd som kvoten gäller.                                             | Ja                                                              | Ej tillämpligt     |
| bredd      | Det maximala antalet kilobyte som tillåts under det tidsintervall som anges i `renewal-period`. | Antingen `calls`, `bandwidth`eller båda tillsammans måste anges. | Ej tillämpligt     |
| samtal          | Maximalt antal anrop som tillåts under det tidsintervall som anges i `renewal-period`.     | Antingen `calls`, `bandwidth`eller båda tillsammans måste anges. | Ej tillämpligt     |
| förnyelse-period | Tids perioden i sekunder efter vilken kvoten återställs.                                                  | Ja                                                              | Ej tillämpligt     |
...
| utfärdare-signerings nycklar | En lista med base64-kodade säkerhets nycklar som används för att validera signerade tokens. Om det finns flera säkerhets nycklar, så provas varje nyckel tills antingen alla är förbrukade (i vilket fall valideringen Miss lyckas) eller tills en lyckad (användbart för token-förnyelse). Nyckel element har ett valfritt `id`-attribut som används för att matcha mot `kid`-anspråk.               | Nej       |
| dekryptering – nycklar     | En lista med base64-kodade nycklar som används för att dekryptera tokens. Om det finns flera säkerhets nycklar, så provas varje nyckel tills alla nycklar är uttömda (i vilket fall valideringen Miss lyckas) eller tills en nyckel lyckas. Nyckel element har ett valfritt `id`-attribut som används för att matcha mot `kid`-anspråk.                                                 | Nej       |
...
| rubrik-namn                     | Namnet på det HTTP-huvud som innehåller token.                                                                                                                                                                                                                                                                                                                                                                                                         | Ett av `header-name`måste `query-parameter-name` eller `token-value` anges. | Ej tillämpligt                                                                               |
| fråga-parameter-Name            | Namnet på frågeparametern som innehåller token.                                                                                                                                                                                                                                                                                                                                                                                                     | Ett av `header-name`måste `query-parameter-name` eller `token-value` anges. | Ej tillämpligt                                                                               |
| token-värde                     | Uttrycket returnerar en sträng som innehåller JWT-token                                                                                                                                                                                                                                                                                                                                                                                                     | Ett av `header-name`måste `query-parameter-name` eller `token-value` anges. | Ej tillämpligt                                                                               |
| id                              | Med attributet `id` i `key`-elementet kan du ange den sträng som ska matchas mot `kid` anspråk i token (om det finns) för att ta reda på vilken nyckel som ska användas för verifiering av signaturen.                                                                                                                                                                                                                                           | Nej                                                                               | Ej tillämpligt                                                                               |
...
| Kräv schema                  | Namnet på token-schemat, t. ex. "Bearer". När det här attributet anges ser principen till att det angivna schemat finns i värdet för Authorization-huvudet.                                                                                                                                                                                                                                                                                    | Nej                                                                               | Ej tillämpligt                                                                               |
| Kräv-signerade-token           | Booleskt. Anger om en token måste signeras.                                                                                                                                                                                                                                                                                                                                                                                           | Nej                                                                               | true                                                                              |
| brytning                       | Sträng. Anger en avgränsare (t. ex. ",") som ska användas för att extrahera en uppsättning värden från ett multi-Value-anspråk.                                                                                                                                                                                                                                                                                                                                          | Nej                                                                               | Ej tillämpligt                                                                               |
...
| output-token-variable-name      | Sträng. Namnet på den Sammanhangs variabel som kommer att ta emot token-värde som ett objekt av typen [`Jwt`](api-management-policy-expressions.md) vid lyckad token-verifiering                                                                                                                                                                                                                                                                                     | Nej                                                                               | Ej tillämpligt                                                                               |
