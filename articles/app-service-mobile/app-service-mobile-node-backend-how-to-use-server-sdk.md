...
Du kan hitta exempel för varje användnings fall i [katalogen samples på GitHub].
...
### <a name="create-a-basic-nodejs-back-end-by-using-the-command-line"></a><a name="howto-cmdline-basicapp"></a>Skapa en grundläggande Node. js-Server del med hjälp av kommando raden
...
1. Kör `npm init` för att initiera paket strukturen:
   ...
   `npm init` Kommandot ber en uppsättning frågor att initiera projektet. Se exempel på utdata:
...
Det här programmet skapar en Mobile-optimerad webb-API med en enda`/tables/TodoItem`slut punkt () som ger oautentiserad åtkomst till ett underliggande SQL-datalager med hjälp av ett dynamiskt schema. Det är lämpligt att följa snabb starter för klient bibliotek:
...
### <a name="create-a-nodejs-back-end-by-using-visual-studio-2015"></a><a name="howto-vs2015-basicapp"></a>Skapa en Node. js-Server del med hjälp av Visual Studio 2015
...
1. Välj **Stäng**.
1. Öppna filen app. js för att lägga till stöd för Mobile Apps SDK. Lägg till följande kod på rad 6 längst ned `require` i biblioteks instruktionerna:
    ...
    Lägg till följande kod på ungefär rad `app.use` 27 efter andra instruktioner:
...
1. Kör programmet lokalt (API: et betjänas `http://localhost:3000`) eller publicera till Azure.

### <a name="create-a-nodejs-back-end-by-using-the-azure-portal"></a><a name="create-node-backend-portal"></a>Skapa en Node. js-Server del med hjälp av Azure Portal
...
4. Kör `git clone` kommandot med hjälp av URL: en för git-klonen. Ange ditt lösen ord vid behov, som i följande exempel:

        $ git clone https://username@todolist.scm.azurewebsites.net:443/todolist.git

5. Bläddra till den lokala katalogen (`/todolist` i föregående exempel) och Observera att projektfilerna har hämtats. Leta upp filen todoitem. json i `/tables` katalogen. Den här filen definierar behörigheter för tabellen. Hitta också filen todoitem. js i samma katalog. Den definierar åtgärds skripten för CRUD för tabellen.
...
### <a name="publish-your-nodejs-back-end-to-azure"></a><a name="howto-publish-to-azure"></a>Publicera Node. js-Server delen till Azure
...
* Så här [anger du Node-version]
* [Använda Node-moduler]

### <a name="enable-a-home-page-for-your-application"></a><a name="howto-enable-homepage"></a>Aktivera en start sida för ditt program
...
## <a name="table-operations"></a><a name="TableOperations"></a>Tabell åtgärder
...
Vi rekommenderar att du definierar varje tabell i en JavaScript-fil i `tables` katalogen och använder sedan `tables.import()` metoden för att importera tabellerna. Genom att utöka exemplet på Basic-app kan du justera filen app. js:
...
Tabeller använder ett dynamiskt schema som standard. Om du vill stänga av det dynamiska schemat globalt ställer `MS_DynamicSchema` du in appens inställning på falskt i Azure Portal.
...
Tabellen kan definieras i tabell definitionens JavaScript-fil (finns i `tables` katalogen) enligt följande:
...
Om du definierar tabeller statiskt måste du också anropa `tables.initialize()` metoden för att skapa databasschemat vid start. `tables.initialize()` Metoden returnerar ett [löfte] så att webb tjänsten inte hanterar begär Anden innan databasen initieras.

### <a name="use-sql-server-express-as-a-development-data-store-on-your-local-machine"></a><a name="howto-sqlexpress-setup"></a>Använd SQL Server Express som utvecklings data lager på din lokala dator
...
I Mobile Apps Node. js SDK används [MSSQL Node. js-paketet] för att upprätta och använda en anslutning till både SQL Server Express och SQL Database. Det här paketet kräver att du aktiverar TCP-anslutningar på SQL Server Express-instansen.
...
Node. js-programmet läser `SQLCONNSTR_MS_TableConnectionString` miljö variabeln för anslutnings strängen för den här databasen. Du kan ställa in den här variabeln i din miljö. Du kan till exempel använda PowerShell för att ställa in miljövariabeln:
...
### <a name="configure-your-project-for-local-development"></a><a name="howto-config-localdev"></a>Konfigurera ditt projekt för lokal utveckling
...
| **MS_DebugMode** |felsökning |Aktiverar eller inaktiverar fel söknings läge |SANT, FALSKT |
...
### <a name="use-sql-database-as-your-production-data-store"></a><a name="howto-use-sqlazure"></a>Använd SQL Database som produktions data lager
...
1. Logga in på [Azure-portalen].
1. Längst upp till vänster i fönstret väljer du knappen **+ ny** > **webb och mobilt** > **mobilapp**och anger sedan ett namn för Mobile Apps Server delen.
...
1. I den nya Mobile Apps Server delen väljer du **Inställningar** > **mobilappar** > **data** > **+ Lägg till**.
...
Det kan ta några minuter att skapa databasen. Använd **aviserings** området för att övervaka förloppet för distributionen. Förloppet inte förrän databasen har distribuerats. När databasen har distribuerats skapas en anslutnings sträng för SQL Database-instansen i Mobile Apps Server dels inställningar för backend-appen. Du kan se den här appens inställning i **Inställningar** > **anslutnings strängar**för**program inställningar** > .
...
### <a name="use-authentication-claims-with-your-tables"></a><a name="howto-tables-getidentity"></a>Använda autentiserings anspråk med dina tabeller
Du kan ställa in olika anspråk som begärs när autentisering har kon figurer ATS. Dessa anspråk är normalt inte tillgängliga via `context.user` objektet. Du kan dock hämta dem med hjälp av- `context.user.getIdentity()` metoden. `getIdentity()` Metoden returnerar ett löfte som matchar ett objekt. Objektet`facebook`anges av autentiseringsmetoden (, `google` `twitter` `microsoftaccount`,, eller `aad`).
...
Om du vill se vilka anspråk som är tillgängliga använder du en webbläsare för `/.auth/me` att Visa slut punkten för din webbplats.

### <a name="disable-access-to-specific-table-operations"></a><a name="howto-tables-disabled"></a>Inaktivera åtkomst till vissa tabell åtgärder
...
### <a name="adjust-the-query-that-is-used-with-table-operations"></a><a name="howto-tables-query"></a>Justera frågan som används med tabell åtgärder
...
Åtgärder som vanligt vis kör en fråga har en egenskap för fråga som du kan justera med `where` hjälp av en sats. Egenskapen Query är ett [QueryJS] -objekt som används för att konvertera en OData-fråga till något som kan utföras av data server delen. För enkla likhets fall (precis som föregående) kan du använda en karta. Du kan också lägga till vissa SQL-satser:

```javascript
context.query.where('myfield eq ?', 'value');
```

### <a name="configure-a-soft-delete-on-a-table"></a><a name="howto-tables-softdelete"></a>Konfigurera en mjuk borttagning för en tabell

En mjuk borttagning tar egentligen inte bort poster. I stället markeras de som borttagna i databasen genom att ange den borttagna kolumnen till sant. Mobile Apps SDK tar automatiskt bort rader som inte tagits bort från resultatet om inte SDK: n `IncludeDeleted()`för mobila klienter använder. Om du vill konfigurera en tabell för en mjuk borttagning anger `softDelete` du egenskapen i tabell definitions filen:
...
Vi rekommenderar att du explicit anropar `tables.initialize()` metoden för att skapa tabellen när tjänsten börjar köras.

### <a name="enable-swagger-support"></a><a name="Swagger"></a>Aktivera stöd för Swagger
...
Du vill förmodligen bara aktivera Swagger support i utvecklings versioner. Du kan göra detta med hjälp av `NODE_ENV` appens inställning:

```javascript
var mobile = azureMobileApps({ swagger: process.env.NODE_ENV !== 'production' });
```

`swagger` Slut punkten finns på http://*yoursite*. azurewebsites.net/Swagger. Du kan komma åt Swagger-ANVÄNDARGRÄNSSNITTET via `/swagger/ui` slut punkten. Om du väljer att kräva autentisering i hela programmet ger Swagger ett fel. För bästa resultat väljer du att tillåta oautentiserade begär anden i inställningarna för Azure App Service autentisering/auktorisering och kontrollerar sedan autentiseringen med hjälp av `table.access` egenskapen.
...
### <a name="send-push-notifications-to-an-authenticated-user-by-using-tags"></a><a name="push-user"></a>Skicka push-meddelanden till en autentiserad användare med hjälp av Taggar
...
1. Skapa en JavaScript-definition JavaScript-fil `api` i katalogen.
...
Vi tar ett exempel-API som returnerar Server datumet med hjälp av `Date.now()` metoden. Här är API/datum. js-filen:
...
Varje parameter är en av standard RESTful-verben: GET, POST, PATCH eller DELETE. Metoden är en standard [ExpressJS mellan] funktioner som skickar nödvändiga utdata.
...
Mobile Apps SDK implementerar autentisering på samma sätt för både `tables` slut punkten och anpassade API: er. Lägg till en `access` egenskap för att lägga till autentisering i det API som utvecklades i föregående avsnitt:
...
Samma token som används för `tables` slut punkten måste användas för anpassade API: er som kräver autentisering.

### <a name="handle-large-file-uploads"></a><a name="howto-customapi-auth"></a>Hantera stora fil överföringar
...
### <a name="execute-custom-sql-statements"></a><a name="howto-customapi-sql"></a>Köra anpassade SQL-uttryck
...
### <a name="debug-diagnose-and-troubleshoot-mobile-apps"></a><a name="howto-diagnostic-logs"></a>Felsöka, diagnostisera och Felsök Mobile Apps
