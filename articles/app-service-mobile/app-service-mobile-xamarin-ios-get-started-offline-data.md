...
---
# <a name="enable-offline-sync-for-your-xamarinios-mobile-app"></a>Aktivera synkronisering offline för din Xamarin. iOS-mobilapp
...
Aktivera nätverket igen.  När du kör programmet första gången anropas metoden `RefreshDataAsync`. Detta i tur anrop `SyncAsync` att synkronisera ditt lokala lager med Server dels databasen.
...
4. I appen klickar du på kryss rutan bredvid några objekt för att slutföra dem i det lokala arkivet.
   ...
   `CompleteItemAsync` anropar `SyncAsync` för att synkronisera varje slutfört objekt med Server delen för mobilappar. `SyncAsync` anropar både push och pull.
...
* `todoTable` medlem i `QSTodoService` är av `IMobileServiceSyncTable` typen i stället för `IMobileServiceTable`. IMobileServiceSyncTable dirigerar alla tabell åtgärder för att skapa, läsa, uppdatera och ta bort (CRUD) till den lokala lagrings databasen.
