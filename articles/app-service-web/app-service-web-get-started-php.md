<properties 
    pageTitle="Distribuera din första PHP-webbapp via Azure på fem minuter | Microsoft Azure" 
...
# Distribuera din första PHP-webbapp via Azure på fem minuter

I den här kursen lär du dig hur du distribuerar din första PHP-webbapp via [Azure App Service](../app-service/app-service-value-prop-what-is.md).
...
1. Ändra till en arbetskatalog i din kommandoradsterminal (`CD`) och klona exempelappen så här:
    ...
    ![Klona exempelkoden för din första webbapp i Azure](./media/app-service-web-get-started-languages/php-git-clone.png)
...
4. Distribuera din exempelkod till din Azure-app på samma sätt som du skickar annan kod med Git:
    ...
    Om du använde något av språkramverken ser resultatet annorlunda ut. Detta beror på att `git push` inte bara placerar kod i Azure, men även utlöser distributionsuppgifter i distributionsmotorn. Om du har några package.json i projektroten (databasen) kommer distributionsskriptet att återställa alla nödvändiga paket åt dig. Du kan också [aktivera tillägget Composer](web-sites-php-mysql-deploy-use-git.md#composer) om du vill att composer.json-filer i din PHP-app ska bearbetas automatiskt.
...
[Skapa, konfigurera och distribuera en Laravel-webbapp via Azure](app-service-web-php-get-started.md). Genom att genomgå den här självstudiekursen får du grundläggande kunskaper i hur du gör för att köra PHP-webbappar i Azure, till exempel:

- Skapa och konfigurera appar i Azure från PowerShell/Bash.
- Ange PHP-version.
- Använda en startfil som inte finns i rotkatalogen för programmet.
- Aktivera Commposer-automatisering.
