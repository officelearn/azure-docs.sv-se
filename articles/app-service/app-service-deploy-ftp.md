...
Den här artikeln visar hur du använder FTP eller FTPS för att distribuera din webbapp, mobilappserverdel eller API-app till [Azure App Service](http://go.microsoft.com/fwlink/?LinkId=529714).
...
För FTP-distribution med [Azure PowerShell](/cli/azure), se [ladda upp filer till en webbapp med FTP (PowerShell)](./scripts/app-service-powershell-deploy-ftp.md).
...
Kontrollera att du har angett rätt värdnamn och [autentiseringsuppgifter](#step-1--set-deployment-credentials). Kontrollera också att följande FTP-portar på din dator inte blockeras av en brandvägg:
...
Mer avancerade distributionsscenarier för, försök [distribuera till Azure med Git](app-service-deploy-local-git.md). Git-baserade distributionen till Azure gör det möjligt för versionskontroll, paketåterställning, MSBuild med mera.
