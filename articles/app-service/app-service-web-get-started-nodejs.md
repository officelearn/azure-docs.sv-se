...
- [Azure App Service-tillägget](vscode:extension/ms-azuretools.vscode-azureappservice) för Visual Studio Code.
...
1. I **Azure App Service** Explorer med vs Code väljer du ikonen blå UPPIL för att distribuera appen till Azure. (Du kan också anropa samma kommando från **kommando paletten** (**Ctrl**+**Shift**+**P**) genom att skriva "distribuera till webbapp" och välja **Azure App Service: distribuera till Web App**).
...
1. Om du riktar in dig på Windows följer du ytterligare prompter:
    1. Välj **skapa en ny resurs grupp**och ange sedan ett namn för resurs gruppen, t. ex. `AppServiceQS-rg`.
...
1. När du har svarat på alla prompter visar VS Code de Azure-resurser som skapas för din app i popup-fönstret för meddelanden.
    ...
    När du distribuerar till Linux väljer du **Ja** när du uppmanas att uppdatera konfigurationen för att köra `npm install` på mål-Linux-servern.
...
1. Om du distribuerar till Windows måste du först ställa in Node. js-versions numret för webbappen:
    ...
    1. Ange `10.15.2` som inställning svärdet.
