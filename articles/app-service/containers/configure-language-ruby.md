...
- **Aktivera betjäning av statiska filer** – om du vill betjäna statiska resurser från ruby-behållaren [anger du `RAILS_SERVE_STATIC_FILES` appens inställning](../configure-common.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json#configure-app-settings) till `true` . Ett exempel:
...
Om du vill använda ett eget `secret_key_base` värde i stället för att låta App Service generera ett åt dig, anger du `SECRET_KEY_BASE` [appens inställning](../configure-common.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json#configure-app-settings) med det värde som du vill använda. Ett exempel:
