...
Kör följande `az webapp up` kommando i mappen *python-dok-Hello-World* som innehåller exempel koden. Ersätt `<app-name>` med ett globalt unikt namn på appen (*giltiga tecken `a-z`är `0-9`, och `-` *). Ersätt `<location-name>` även med en Azure-region som till exempel **Central**, **asienöstra**, **westeurope**, **koreasödra**, **centrala**, **Kanada**och så vidare. (Du kan hämta en lista över tillåtna regioner för ditt Azure-konto genom att köra [`az account locations-list`](/cli/azure/appservice?view=azure-cli-latest.md#az-appservice-list-locations) kommandot.)
...
## <a name="run-the-sample-app-locally"></a>Kör exempel appen lokalt
...
Öppna *Application.py* i din favorit kod redigerare och ändra `return` instruktionen på den sista raden så att den matchar följande kod. `print` Instruktionen ingår här för att generera logga utdata som du arbetar med i nästa avsnitt. 
...
Distribuera om appen med följande `az webapp up` kommando med samma kommando som du använde för att distribuera appen första gången, och Ersätt `<app-name>` och `<location-name>` med samma namn som du använde tidigare. 
...
Först aktiverar du behållar loggning genom att köra följande kommando i en Terminal, `<app-name>` ersätta med namnet på din app och `<resource-group-name>` med namnet på den resurs grupp som visas i utdata från `az webapp up` kommandot som du använde (till exempel "appsvc_rg_Linux_centralus"):
