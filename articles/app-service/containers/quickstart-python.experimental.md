---
title: 'Snabbstart: Skapa Python-appen på Linux - Azure App Service'
...
I mappen *python-docs-hello-world* som innehåller exempelkoden kör `az webapp up` du följande kommando. Ersätt `<app-name>` med ett globalt unikt appnamn (*giltiga tecken är `a-z`, `0-9`och `-` *). Ersätt `<location-name>` också med en Azure-region som **centralus**, **eastasia**, **westeurope**, **koreasouth**, **brazilsouth**, **centralindia**och så vidare. (Du kan hämta en lista över tillåtna regioner [`az account locations-list`](/cli/azure/appservice?view=azure-cli-latest.md#az-appservice-list-locations) för ditt Azure-konto genom att köra kommandot.)
...
Det här kommandot kan ta några minuter färdig körning. Medan det körs visas information liknande den i följande exempel:
...
## <a name="run-the-sample-app-locally"></a>Köra exempelappen lokalt
...
![Köra ett exempel på Python-app lokalt](./media/quickstart-python/run-hello-world-sample-python-app-in-browser.png)
