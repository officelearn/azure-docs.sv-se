...
App Service-domäner (förhandsversion) är toppnivådomäner som hanteras direkt i Azure. De gör det enkelt att hantera anpassade domäner för [Azure Web Apps](app-service-web-overview.md). Den här självstudien visar hur du köper en App Service-domän och kopplar DNS-namn till Azure Web Apps.

Den här artikeln är för Azure App Service (Webbappar, API Apps, Mobile Apps, Logic Apps). Virtuell dator i Azure eller Azure Storage finns i [tilldela App Service-domän till Azure-VM eller Azure Storage](https://blogs.msdn.microsoft.com/appserviceteam/2017/07/31/assign-app-service-domain-to-azure-vm-or-azure-storage/). Cloud Services, se [konfigurera ett anpassat domännamn för en Azure cloud Services](../cloud-services/cloud-services-custom-domain-name-portal.md).
...
Använda anpassade domäner i Azure Web Apps, web appss [apptjänstplan](https://azure.microsoft.com/pricing/details/app-service/) måste vara en betald nivå (**delad**, **grundläggande**, **Standard**, eller  **Premium**). I det här steget ska se du till att webbappen är stöds prisnivå.
...
I den **Webbappar** klickar du på namnet på din webbapp, Välj **inställningar**, och välj sedan **anpassade domäner**
...
| Tilldela standard värdnamn | **www** och **@** | Välj önskade värdnamnet-bindningar, om så önskas. När domänen köp åtgärden har slutförts, kan din webbapp nås på den valda värdnamnen. Om webbappen är bakom [Azure Traffic Manager](https://azure.microsoft.com/services/traffic-manager/), ser du alternativet för att tilldela rotdomänen (@), eftersom Traffic Manager har inte stöd för A-poster. Du kan ändra värdnamnet tilldelningar efter köp domän har slutförts. |
...
Om du har tilldelat standard värdnamn till din webbapp kan se du även ett meddelande för lyckade för varje valda värddatornamnet. 
...
## <a name="assign-hostnames-to-web-app"></a>Tilldela värdnamn till webbapp

Om du väljer att inte tilldela en eller flera standard-värdnamn till din webbapp under inköpsprocessen, eller om du måste tilldela ett värdnamn som inte visas, kan du tilldela ett värdnamn på när som helst.
...
I den **Apptjänster** markerar du namnet på webbappen som du vill tilldela värdnamn, Välj **inställningar**, och välj sedan **anpassade domäner**.
...
> [!NOTE]
> Alla App Service-domäner i samma prenumeration visas i webbappens **anpassade domäner** sidan. Om din domän är i webbappens prenumeration, men du kan se den i webbappens **anpassade domäner** kan prova att öppna den **anpassade domäner** sidan eller uppdatera webbsidan. Kontrollera också meddelandeikonen överst i Azure portal för pågår eller skapa fel.
>
>
...
I den **Lägg till värddatornamn** kan tilldela andra värdnamnet till din webbapp, som du vill. När du är klar stänger den **Lägg till värddatornamn** sidan.
