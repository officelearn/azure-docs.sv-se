---
title: "Integrera din ILB Apptjänst-miljö med en Programgateway"
...
---
# <a name="integrate-your-ilb-app-service-environment-with-an-application-gateway"></a>Integrera din ILB Apptjänst-miljö med en Programgateway #

Den [Apptjänstmiljö för PowerApps](./intro.md) är en distribution av Azure App Service i undernätet för kundens virtuella Azure-nätverket. Den kan användas med en offentlig eller privat slutpunkt för åtkomst till appen. Distributionen av Apptjänst-miljön med en privat slutpunkt (det vill säga en intern belastningsutjämnare) kallas en ILB Apptjänst-miljö.  

Azure Application Gateway är en virtuell installation som tillhandahåller layer 7 belastningsutjämning, SSL-avlastning och web application firewall (Brandvägg) skydd. Det kan lyssna på en offentlig IP-adress och vidarebefordra trafik till programslutpunkten. 

Följande information beskriver hur du integrerar en konfigurerad Brandvägg Programgateway med en app i en ILB Apptjänst-miljö.  
...
* Offentliga DNS-namnet som används senare för att peka på Programgateway. 
...
4. I den **sammanfattning** , granskar du inställningarna och välj **OK**. Din Programgateway kan ta lite mer än 30 minuter att slutföra installationen.  

5. När din Programgateway avslutar du installationsprogrammet, gå till din gateway-portal för programmet. Välj **serverdelspool**. Lägg till ILB-adress för din ILB Apptjänst-miljö.
...
8. Gå till programgatewayen **översikt** avsnitt och kopiera den offentliga IP-adressen som använder din Programgateway. Ange IP-adressen som en A-post för din app-domännamnet eller Använd DNS-namn för den adressen i en CNAME-post. Det är lättare att välja den offentliga IP-adressen och kopiera den från Gränssnittet för den offentliga IP-adressen i stället för att kopiera den från länken i programgatewayen **översikt** avsnitt. 
