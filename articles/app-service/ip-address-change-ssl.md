...
1. I avsnittet SSL-bindningar väljer du värd namns posten. I redigeraren som öppnas väljer du **SNI SSL** på den nedrullningsbara menyn för **SSL-typ** och klickar på **Lägg till bindning**. När meddelandet åtgärden lyckades visas har den befintliga IP-adressen släppts.
...
Den här artikeln förklaras hur du förbereder en ändring av IP-adressen som initierades av Azure. Mer information om IP-adresser i Azure App Service finns i [SSL och SSL IP-adresser i Azure App Service](overview-inbound-outbound-ips.md).
