---
title: Säkerhetskopiera app – Azure App Service
description: Lär dig hur du skapar säkerhets kopior av dina appar i Azure App Service.
...
Följande databas lösningar stöds med funktionen säkerhets kopiering: 
   ...
   - [MySQL i-app](https://blogs.msdn.microsoft.com/appserviceteam/2017/03/06/announcing-general-availability-for-mysql-in-app)
...
* Funktionen säkerhets kopiering och återställning kräver att App Service plan finnas på **standard** -eller **Premium** -nivån. Mer information om hur du skalar App Service plan att använda en högre nivå finns i [skala upp en app i Azure](manage-scale-up.md).  
  **Premium** -nivån tillåter ett större antal dagliga säkerhets kopieringar än **standard** nivån.
...
1. I [Azure Portal](https://portal.azure.com)navigerar du till appens sida och väljer **säkerhets kopiering**. Sidan **säkerhets kopior** visas.
    ...
    ![Sidan säkerhets kopior][ChooseBackupsPage]
   ...
   > ![Välj lagrings konto](./media/web-sites-backup/01UpgradePlan1.png)
   > 
   > 

2. På sidan **säkerhets kopiering** klickar du på **Konfigurera**
![Klicka på Konfigurera](./media/web-sites-backup/ClickConfigure1.png)
3. På sidan **säkerhets kopierings konfiguration** klickar **du på lagring: Inte konfigurerad** för att konfigurera ett lagrings konto.
...
2. Schema alternativ för säkerhets kopiering visas, ange **schemalagd säkerhets kopiering** till **på**och konfigurera sedan schemat för säkerhets kopiering efter behov och klicka på **OK**.
   
    ![Aktivera automatiska säkerhets kopieringar][SetAutomatedBackupOn]
...
Ett enkelt sätt att komma åt dina filer är att använda kudu. Klicka på **Avancerade verktyg – > go-** inställning för din webbapp för att få åtkomst till kudu.

![Kudu med portalen][kudu-portal]
...
Ladda `_backup.filter` upp filen `D:\home\site\wwwroot\` till katalogen på platsen med [FTP](deploy-ftp.md) eller någon annan metod. Om du vill kan du skapa filen direkt med kudu `DebugConsole` och infoga innehållet där.
