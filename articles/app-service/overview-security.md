---
title: Översikt över säkerhet – Azure App Service | Microsoft Docs
...
Den här artikeln visar hur [Azure App Service](overview.md) hjälper dig att skydda din webbapp, mobilapp-serverdel, API-app och [funktionsapp](/azure/azure-functions/). Den visar även hur du kan öka skyddet din app med de inbyggda funktionerna för App Service.
...
- [VM-instanser och runtime programvara uppdateras regelbundet](overview-patch-os-runtime.md) för säkerhetsrisker i adressen som nyligen identifierats. 
...
App Service stöder både FTP och FTPS för att distribuera dina filer. Dock ska FTPS användas i stället för FTP, om det är möjligt. När en eller båda av dessa protokoll inte används, bör du [inaktivera dem](deploy-ftp.md#enforce-ftps).
...
App Service-autentisering och auktorisering stöd för flera autentiseringsproviders, inklusive Azure Active Directory, Microsoft-konton, Facebook, Google och Twitter. Mer information finns i [autentisering och auktorisering i Azure App Service](overview-authentication-authorization.md).
