---
title: "Konfigurera omdirigeringen Azure Application Gateway – PowerShell | Microsoft Docs"
description: "Den här sidan innehåller scenarier för att konfigurera omdirigering för Application Gateway med PowerShell"
...
Application Gateway stöder möjligheten att omdirigera trafik baserat på den angivna konfigurationen. Läs mer om omdirigering i allmänhet på [Översikt för omdirigering i Application Gateway](application-gateway-redirect-overview.md). Den här artikeln innehåller exempel på omdirigering från HTTP till HTTPS, sökväg-baserade omdirigeringar, omdirigeringar för flera platser och omdirigeringar till externa platser.

## <a name="http-to-https-redirect-on-an-existing-application-gateway"></a>HTTP till HTTPS-omdirigering på en befintlig application gateway

I följande exempel skapas en ny HTTP-lyssnare på port 80 för att omdirigera anrop till den befintliga lyssnaren på port 443.
...
När application gateway har uppdaterats kan du testa den nya lyssnaren på port 80.  I följande exempel visar hur `curl` testar omdirigeringen.
...
I följande exempel skapas en ny lyssnare och sökvägsregler baserat på en befintlig Application Gateway som endast omdirigerar trafik som motsvarar specifika sökvägar.
...
I följande exempel skapas en ny application gateway med 2 lyssnare för flera platser på port 80. Lyssnarna är för adatum.com och adatum.org. En omdirigeringsregel skapas för att dirigera trafik från adatum.org till adatum.com. Det krävs ytterligare konfiguration för att konfigurera CNAME-alias till den offentliga IP-adressen för application gateway. Mer information om hur du delegerar din domän till Azure DNS och skapar CNAME-poster för din domän finns i [Delegera en domän till Azure DNS](../dns/dns-delegate-domain-azure-dns.md).
...
## <a name="redirect-to-an-external-site"></a>Omdirigera till en extern webbplats

I följande exempel dirigeras trafik i application gateway till en extern webbplats (bing.com). När du använder `TargetUrl`-omkopplaren tillåts inte `IncludePath`- och `IncludeQuery`-omkopplare.
...
Besök [Konfigurera slutpunkt-till-slutpunkt-SSL med programgateway med hjälp av PowerShell](application-gateway-end-to-end-ssl-powershell.md)