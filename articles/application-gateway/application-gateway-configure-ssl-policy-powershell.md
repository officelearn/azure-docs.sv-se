...
description: Den här artikeln innehåller anvisningar för att konfigurera TLS-princip på Azure Application Gateway
...
När du konfigurerar en anpassad TLS-princip skickar du följande parametrar: PolicyType, MinProtocolVersion, CipherSuite och ApplicationGateway. Om du försöker skicka andra parametrar får du ett fel meddelande när du skapar eller uppdaterar Application Gateway. 

I följande exempel anges en anpassad TLS-princip på en Application Gateway. Den anger den lägsta protokoll versionen till `TLSv1_1` och aktiverar följande chiffersviter:

* TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384
* TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256

> [!IMPORTANT]
> TLS_RSA_WITH_AES_256_CBC_SHA256 måste väljas när du konfigurerar en anpassad TLS-princip. Application Gateway använder den här chiffersviter för Server dels hantering. Du kan använda detta i kombination med andra sviter, men den här måste också väljas. 
...
När du konfigurerar en fördefinierad TLS-princip skickar du följande parametrar: PolicyType, PolicyName och ApplicationGateway. Om du försöker skicka andra parametrar får du ett fel meddelande när du skapar eller uppdaterar Application Gateway.
...
## <a name="update-an-existing-application-gateway-with-a-pre-defined-tls-policy"></a>Uppdatera en befintlig Application Gateway med en fördefinierad TLS-princip

Om du vill ange en anpassad TLS-princip skickar du följande parametrar: **PolicyType** , **MinProtocolVersion** , **CipherSuite** och **ApplicationGateway**. Ange en fördefinierad TLS-princip genom att skicka följande parametrar: **PolicyType** , **PolicyName** och **ApplicationGateway**. Om du försöker skicka andra parametrar får du ett fel meddelande när du skapar eller uppdaterar Application Gateway.
