...
ms.locfileid: "67449537"
---
# <a name="create-an-application-gateway-using-the-azure-resource-manager-template"></a>Skapa en Programgateway med hjälp av Azure Resource Manager-mall
...
Den här artikeln beskriver hur du laddar ned och ändra en befintlig [Azure Resource Manager-mall](../azure-resource-manager/resource-group-authoring-templates.md) från GitHub och distribuerar mallen från GitHub, Azure PowerShell och Azure CLI.
...
1. Om du är bekant med Azure Resource Manager-mallar kan du gå vidare till steg 7.
2. Öppna filen som du sparade och titta på innehållet under **parametrar** i rad
3. Azure Resource Manager-mallens parametrar fungerar som platshållare för värden som kan anges under distributionen.
...
1. Spara filen. Du kan testa JSON-mall och parameter-mallen med hjälp av online JSON-verifieringsverktyg som [JSlint.com](https://www.jslint.com/).

## <a name="deploy-the-azure-resource-manager-template-using-azure-powershell"></a>Distribuera Azure Resource Manager-mallen med hjälp av Azure PowerShell

[!INCLUDE [updated-for-az](../../includes/updated-for-az.md)]

Om du aldrig har använt Azure PowerShell, se: [Hur du installerar och konfigurerar du Azure PowerShell](/powershell/azure/overview) och följ anvisningarna för att logga in på Azure och välj din prenumeration.
...
1. Kontrollera prenumerationerna för kontot.
    ...
    Du uppmanas att autentisera med dina autentiseringsuppgifter.
...
1. Om det behövs skapar du en resurs med det **New-AzureResourceGroup** cmdlet. I följande exempel skapar du en resursgrupp kallad appgatewayrg med platsen East USA.

    ```azurepowershell
    New-AzResourceGroup -Name AppgatewayRG -Location "West US"
    ```

1. Kör den **New AzResourceGroupDeployment** cmdlet för att distribuera det nya virtuella nätverket med hjälp av den föregående mall- och parameterfilerna filer du hämtade och ändrade.
...
## <a name="deploy-the-azure-resource-manager-template-using-the-azure-cli"></a>Distribuera Azure Resource Manager-mallen med hjälp av Azure CLI

Följ stegen nedan om du vill distribuera Azure Resource Manager-mallen som du hämtade med Azure CLI:
...
1. Kör den `az group deployment create` cmdlet för att distribuera det nya virtuella nätverket med hjälp av mall- och parameterfilerna filer du hämtade och ändrade i föregående steg. Listan som visas efter utdatan beskriver de parametrar som används.

    ```azurecli
    az group deployment create --resource-group appgatewayRG --name TestAppgatewayDeployment --template-file azuredeploy.json --parameters @azuredeploy-parameters.json
    ```

## <a name="deploy-the-azure-resource-manager-template-using-click-to-deploy"></a>Distribuera Azure Resource Manager-mallen med Klicka att distribuera
...
1. På sidan anpassade distribution **skapa**.
...
Om du vill konfigurera SSL-avlastning, se: [Konfigurera en Programgateway för SSL-avlastning](application-gateway-ssl.md).

Om du vill konfigurera en Programgateway för användning med en intern belastningsutjämnare, se: [Skapa en Programgateway med en intern belastningsutjämnare (ILB)](application-gateway-ilb.md).
