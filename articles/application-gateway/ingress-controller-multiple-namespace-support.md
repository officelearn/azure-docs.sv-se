...
Version 0,7 av AGIC kommer att fortsätta att uteslutande Observera `default` namn området, om detta inte uttryckligen ändras till ett eller flera olika namn rymder i Helm-konfigurationen (se avsnittet nedan).
...
1. ändra filen [Helm-config. yaml](#sample-helm-config-file) på något av följande sätt:
   ...
   - Lägg till flera namn områden avgränsade med kommatecken (`watchNamespace: default,secondNamespace`) – AGIC kommer att observera dessa namn områden exklusivt
2. tillämpa ändringar i Helm-mal len med: `helm install -f helm-config.yaml application-gateway-kubernetes-ingress/ingress-azure`
...
Anta till exempel följande dubbla ingress-resurser definierade namn områden `staging` och `production` för `www.contoso.com`:
```yaml
...
Observera att om du har skapat Application Gateway-resurser som har skapats med undantag för *lyssnare* och *regler*ingår namnet på namn området (`production`) som de har skapats för.
...
Om du till exempel lade till `staging` först, kommer AGIC att konfigurera Application Gateway för att dirigera trafik till mellanlagringsplatsen. I ett senare skede, som introducerar `production` ingress, kommer att göra att AGIC Application Gateway, vilket kommer att börja dirigera trafik till `production`-backend-poolen.
...
Som standard konfigurerar AGIC Application Gateway baserat på kommenterade ingångar i ett namn område. Om du vill begränsa det här beteendet kan du välja mellan följande alternativ:
  - begränsa namn områdena genom att uttryckligen definiera namn områden AGIC bör observera genom `watchNamespace` YAML-nyckeln i [Helm-config. yaml](#sample-helm-config-file)
