...
description: I den här artikeln får du lära dig hur du dirigerar webbtrafik baserat på URL:en till specifika skalbara pooler med hjälp av Azure CLI.
...
Använd `az network application-gateway create` för att skapa en programgateway med namnet *myAppGateway*. När du skapar en programgateway med hjälp av Azure CLI anger du konfigurationsinformation som kapacitet, sku och HTTP-inställningar. Programgatewayen är tilldelad *myAGSubnet* och *myAGPublicIPAddress*.
...
I den här artikeln skapar du tre skaluppsättningar för virtuella datorer som stöder de tre servergruppspoolerna som du har skapat. Skalningsuppsättningarna du skapar har namnen *myvmss1*, *myvmss2* och *myvmss3*. Varje skalningsuppsättning innehåller två virtuella datorinstanser där du installerar NGINX.
...
Hämta den offentliga IP-adressen för programgatewayen med az network public-ip show. Kopiera den offentliga IP-adressen och klistra in den i webbläsarens adressfält. Till exempel `http://40.121.222.19` `http://40.121.222.19:8080/images/test.htm`, `http://40.121.222.19:8080/video/test.htm`, eller .
