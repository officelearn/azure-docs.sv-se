...
4. Ge runbooken namnet "MyFirstRunbook-Graphic".
...
1. **Etikett** fältet är som standard namnet på cmdleten, men du kan ändra det till något mer användarvänligt. Ändra den till "skriva Hello World till utdata".
...
1. När Runbook-jobbet har slutförts visar test-pagn dess utdata. I det här fallet visas "Hello World".<br> ![Hello World](media/automation-first-runbook-graphical/runbook-test-results.png)
...
1. Ett jobb fönster öppnas för det Runbook-jobb som har skapats. Kontrol lera att **jobb statusen** är slutförd.
...
Du har testat och publicerat din Runbook, men hittills gör den inte något användbart för att hantera Azure-resurser. Innan du konfigurerar runbooken att autentisera måste du skapa en variabel som ska innehålla prenumerations-ID, konfigurera en aktivitet för att autentisera och sedan referera till variabeln. Genom att inkludera en referens till prenumerations kontexten kan du enkelt arbeta med flera prenumerationer.
...
1. Du behöver inte ange något mer i posten "Skriv Hello World till utdata". Klicka bara på ellipsen och välj **ta bort**.
...
1. På arbets ytan väljer du **Connect-AzAccount**. Skriv "logga in på Azure" i fältet **etikett** i fönstret konfigurations kontroll.
1. Klicka på **parametrar**så visas sidan konfiguration av aktivitets parameter.
1. Cmdleten **Connect-AzAccount** har flera parameter uppsättningar och du måste välja en innan du kan ange parameter värden. Klicka på **parameter uppsättning** och välj sedan parameter uppsättningen serviceprincipalcertificate-parameter uppsättningen.
...
1. Välj **set-AzContext** på arbets ytan. I fönstret konfigurations kontroll skriver du "ange prenumerations-ID" i fältet **etikett** .
1. Klicka på **parametrar** och sidan konfiguration av aktivitets parameter visas.
1. **Set-AzContext** -cmdleten har flera parameter uppsättningar och du måste välja en innan du tillhandahåller parameter värden. Klicka på **parameter uppsättning** och välj sedan SubscriptionId-parameter uppsättningen.
...
Nu måste du lägga till en **Start-AzVM** -aktivitet för att starta en virtuell dator. Du kan välja vilken virtuell dator som helst i din Azure-prenumeration och nu hårdkoda du namnet i cmdleten [Start-AzVM](https://docs.microsoft.com/powershell/module/az.compute/start-azvm?view=azps-3.5.0) .
...
1. Välj **Get-AzVM** och sedan **parameter uppsättning** för att Visa uppsättningarna för **Get-AzVM**. 
1. Välj GetVirtualMachineInResourceGroupNameParamSet-parameter uppsättningen. Fälten **ResourceGroupName** och **namn** har utrops tecken bredvid sig, vilket indikerar att de anger obligatoriska parametrar. Observera att båda fälten förväntar sig sträng värden.
...
1. Klistra in följande kodfragment i kod redigeraren.
...
1. För **InputObject**ändrar du **data källa** till **PowerShell-uttryck**och skriver uttrycket "$VMName har startats".
...
1. För **InputObject**ändrar du **data källa** till **PowerShell-uttryck**och skriver in uttrycket "$VMName kunde inte starta".
