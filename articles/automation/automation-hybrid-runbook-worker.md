...
Om du är en befintlig SMA-användare kan du flytta dina runbooks till Azure Automation för att användas med Hybrid Runbook Worker utan ändringar, förutsatt att de utföra sina egna autentisering av resurser som beskrivs i [köra runbooks på en Hybrid Runbook Worker](automation-hrw-run-runbooks.md).  Runbooks i SMA körs i kontexten för kontot på worker-server som kan tillhandahålla att autentiseringen för runbooks.
