...
> [!NOTE]
> Ändringsspårning och lager använder samma lösning. När en är aktive rad aktive ras även den andra.
...
### <a name="onboarding-limitations"></a><a name="resource-group-limit"></a>Onboarding-begränsningar
...
När flera datorer onboardas kan det finnas datorer som visas som `Cannot enable`. Det finns olika orsaker till att vissa datorer kanske inte är aktiverade. I följande avsnitt visas möjliga orsaker till `Cannot enable` tillstånd på en virtuell dator vid försök att publicera.
...
**Lösning:** Se [listan över klienter som stöds](automation-update-management.md#supported-client-types).
