...
1. Gränsen för minne.  Vi har dokumenterats gränser för hur mycket minne som allokerats till en Sandbox [Automation tjänstbegränsningarna](../azure-subscription-service-limits.md#automation-limits) så att ett jobb kan misslyckas den om du använder mer än 400 MB minne. 
...
**Felsökningstips:** någon av följande lösningar kommer att åtgärda problemet:  
...
* Du behöver uppdatera din Azure moduler genom att följa stegen [så här uppdaterar du Azure PowerShell-moduler i Azure Automation](automation-update-azure-modules.md).  
...
**Felsökningstips:**  
Något av följande tre sätt att åtgärda problemet:
...
* Kontrollera att modulen följer följande format:  
...
**Orsaken till felet:** när följande uttryck i **nod** nyckelord i DSC-konfigurationen utvärderas till $null sedan inga nodkonfigurationer produceras.    
...
**Felsökningstips:** följer du anvisningarna i den [DSC kända problem och begränsningar](https://msdn.microsoft.com/powershell/wmf/5.0/limitation_dsc) att åtgärda problemet.
...
Du kan granska supportalternativen nedan om du har följt felsökningsstegen ovan och det går inte att hitta svaret.
