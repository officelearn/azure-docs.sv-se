...
Windows-agenter måste konfigureras för att kommunicera med en Windows Server Update Services (WSUS)-server eller har tillgång till Microsoft Update. Också kan Windows-agenten inte hanteras samtidigt av System Center Configuration Manager. Den [Windows-agenten](../log-analytics/log-analytics-agent-windows.md) krävs. Den här agenten installeras automatiskt om du är en Azure-virtuella datorn.
...
> [!NOTE]
> Om Linux-system har konfigurerats för att kommunicera med en proxy- eller OMS-Gateway och onboarding använder den här lösningen, uppdatera den *proxy.conf* behörigheter för att ge gruppen omiuser läsbehörighet för filen genom att utföra följande kommandon:  
