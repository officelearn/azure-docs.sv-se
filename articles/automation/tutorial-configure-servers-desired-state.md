...
Anropa `Import-AzureRmAutomationDscConfiguration`-cmdlet: en för att överföra konfigurationen till ditt Automation-konto:
...
Anropa `Start-AzureRmAutomationDscCompilationJob`-cmdlet: en för att kompilera `TestConfig`-konfigurationen till en Node-konfiguration:
...
Detta tilldelar nodens konfiguration med namnet `TestConfig.WebServer` till den registrerade DSC-noden med namnet `DscVm`.
...
Azure Automation tillstånds konfiguration har stöd för användning av [partiella konfigurationer](/powershell/scripting/dsc/pull-server/partialconfigs).
...
Mer information om hur du registrerar en del konfiguration från pull-tjänsten finns i dokumentationen för [del konfigurationer](https://docs.microsoft.com/powershell/scripting/dsc/pull-server/partialconfigs#partial-configurations-in-pull-mode).

Mer information om hur team kan arbeta tillsammans med kollektivt hantera servrar med hjälp av konfiguration som kod finns i [förstå DSC-rollen i en CI/CD-pipeline](/powershell/scripting/dsc/overview/authoringadvanced).

## <a name="check-the-compliance-status-of-a-managed-node"></a>Kontrol lera status för efterlevnad för en hanterad nod

Du kan få rapporter om kompatibilitetsstatus för en hanterad nod genom att anropa `Get-AzureRmAutomationDscNodeReport`-cmdlet:
