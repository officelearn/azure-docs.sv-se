...
>[!IMPORTANT]
> Om den identitet som är tilldelad till konfigurations instansen inte längre har behörighet att packa upp instansens krypterings nyckel, eller om den hanterade nyckeln tas bort permanent, kommer den inte längre att kunna dekryptera känslig information som lagras i appen Konfigurations instans. Med hjälp av Azure Key Vault funktionen för [mjuk borttagning](../key-vault/key-vault-ovw-soft-delete.md) minimerar du risken för att oavsiktligt ta bort krypterings nyckeln.
...
2. Bevilja identiteten `GET`, `WRAP`och `UNWRAP` behörigheter i mål Key Vault åtkomst princip.
...
- [Skapa en .NET Framework-app med Azure App konfiguration](quickstart-dotnet-app.md)
...
1. Skapa en Azure Key Vault med hjälp av Azure CLI.  Observera att både `vault-name` och `resource-group-name` har angetts av användaren och måste vara unika.  Vi använder `contoso-vault` och `contoso-resource-group` i de här exemplen.
...
1. Skapa en Key Vault nyckel. Ange ett unikt `key-name` för den här nyckeln och ersätt namnen på de Key Vault (`contoso-vault`) som skapades i steg 1. Ange om du vill `RSA` eller `RSA-HSM` kryptering.
