...
1. Säkerhets objektets identitet autentiseras och en OAuth 2,0-token returneras.  Resurs namnet för att begära en token är `https://login.microsoftonline.com/{tenantID}` där `{tenantID}` matchar Azure Active Directory klient organisations-ID som tjänstens huvud namn tillhör.
