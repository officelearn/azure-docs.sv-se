...
FOS i det här avsnittet beskriver vanliga frågor om övervakning och fel sökning. Mer information om övervakning och fel sökning av Azure-cache för Redis-instanser finns i [så här övervakar du Azure cache för Redis](cache-how-to-monitor.md) och [hur du felsöker Azure cache för Redis](cache-how-to-troubleshoot.md).
...
* **Data flöde**: Premium-nivån erbjuder maximalt tillgängligt data flöde. Om cache-servern eller-klienten når bandbredds gränserna kan du få tids gränser på klient sidan. Mer information finns i följande tabell.
...
* **Konfigurera Redis**: På både standard-och Premium-nivån kan du konfigurera Redis för meddelanden om disk utrymme.
...
* **Dedikerad kärna för redis-server**: På Premium-nivån har alla cache-storlekar en särskild kärna för Redis. På Basic-/standard-nivåerna har C1-storleken och över en dedikerad kärna för Redis-servern.
...
I följande tabell visas de maximala bandbredds värden som observerats vid testning av olika storlekar på standard- `redis-benchmark.exe` och Premium-cachen med hjälp av en IaaS VM mot Azure-cachen för Redis-slutpunkten. För SSL-genomflöde används Redis-benchmark med stunnelserver för att ansluta till Azure-cachen för Redis-slutpunkten.
...
| Prisnivå | Size | Processorkärnor | Tillgänglig bandbredd | värde storlek 1 KB | värde storlek 1 KB |
| --- | --- | --- | --- | --- | --- |
...
Information om hur du använder Azure cache för Redis med PowerShell i Azure Government Cloud, Azure Kina 21Vianet-molnet och Microsoft Azure Tyskland finns i [så här ansluter du till andra moln – Azure cache för Redis PowerShell](cache-howto-manage-redis-cache-powershell.md#how-to-connect-to-other-clouds).
...
* **Timeout-värden**
  ...
  * `ConnectionMultiplexer.ClientName` Ange egenskapen till ett unikt namn för App-instansen.
  * Använd flera `ConnectionMultiplexer` instanser för anpassade arbets belastningar.
      ...
      * `ClientName` Ange egenskapen för varje multiplexor för att hjälpa med diagnostik.
...
> [!NOTE]
> Kommando rads verktygen för Redis fungerar inte med SSL-porten, men du kan använda ett verktyg `stunnel` för att på ett säkert sätt ansluta verktygen till SSL-porten genom att följa anvisningarna i anvisningarna i [använda kommando rads verktyget Redis med Azure cache för Redis](https://docs.microsoft.com/azure/azure-cache-for-redis/cache-how-to-redis-cli-tool) -artikel .
>
>
...
Redis-verktyg som `redis-cli` inte fungerar med SSL-porten, men du kan använda ett verktyg `stunnel` för att på ett säkert sätt ansluta verktygen till SSL-porten genom att följa anvisningarna i anvisningarna i [ASP.net Provider för Redis för hands version ](https://blogs.msdn.com/b/webdev/archive/2014/05/12/announcing-asp-net-session-state-provider-for-redis-preview-release.aspx)blogg inlägg.
...
* Börja med att `redis-benchmark.exe` använda för att få en känsla för eventuella data flöden innan du skriver dina egna perf-tester. Eftersom `redis-benchmark` inte stöder SSL måste du [Aktivera icke-SSL-porten via Azure Portal](cache-configure.md#access-ports) innan du kör testet. Exempel finns i [Hur kan jag mäta och testa prestanda för mitt cacheminne?](#how-can-i-benchmark-and-test-the-performance-of-my-cache)
...
* Testa pipeline GET-begäranden med en nytto last på 1 KB.
  OBS! Kör det UPPSÄTTNINGs test som visas ovan först för att fylla i cachen
...
* Du kan också ange minsta antal trådar genom att använda `<processModel>` [konfigurations inställningen *minIoThreads* eller *minWorkerThreads* ](https://msdn.microsoft.com/library/vstudio/7w2sway1(v=vs.100).aspx) under konfigurations elementet i, som `Machine.config`vanligt vis finns i `%SystemRoot%\Microsoft.NET\Framework\[versionNumber]\CONFIG\` . **Att ställa in antalet minsta trådar på det här sättet rekommenderas vanligt vis inte, eftersom det är en systemtäckande inställning.**
