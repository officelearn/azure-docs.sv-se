---
title: 'Självstudie: Attitydanalys på strömmad data med hjälp av Azure Databricks | Microsoft Docs'
description: Lär dig att använda Azure Databricks med Event Hubs och Cognitive Services API för att kunna köra attitydanalys på strömmande data i realtid.
...
I kursen får du lära dig att köra attitydanalys på en dataström i realtid med hjälp av Azure Databricks. Du konfigurerar datainmatningssystemet i realtid med Azure Event Hubs. Du kan använda meddelanden från Event Hubs i Azure Databricks med hjälp av Spark Event Hubs-anslutningsprogrammet. Slutligen kan du använda Microsoft Cognitive Service API:er för att köra attitydanalys på strömmad data. 
...
Om du vill få en dataström med tweets i realtid måste du skapa ett program i Twitter. Följ anvisningarna för att skapa ett Twitter-program och registrera de värden som du behöver för självstudien.
...
Om du vill köra anteckningsboken trycker du på **SKIFT + RETUR**. Du ser utdata som liknar följande kodfragment. Varje händelse i utdatan är en tweet i realtid som matas in i Event Hubs. 
...
Lägg till en ny kodcell och klistra in kodfragmentet nedan. Det här kodfragmentet krävs för att kunna parsa JSON-strängar.
...
Lägg till en ny kodcell och klistra in kodfragmentet nedan. Kodfragmentet definierar ett objekt med funktioner för att anropa API:n för textanalys som kör språkidentifiering och attitydanalys. Var noga med att ersätta platshållarna `<PROVIDE ACCESS KEY HERE>` och `<PROVIDE HOST HERE>` med de värden som du hämtade för Cognitive Services-kontot.
...
Klart! Med Azure Databricks har du strömmat realtidsdata till Azure Event Hubs, använt dataströmmar med Event Hubs-anslutningsprogrammet och sedan kört attitydanalys på strömmande data.
...
>[Machine Learning med Azure Databricks ](https://docs.azuredatabricks.net/spark/latest/mllib/decision-trees.html)
