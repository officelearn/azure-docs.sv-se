...
"Den här prenumerationen är inte registrerad för att använda namn området Microsoft. Databricks. Se https://aka.ms/rps-not-found så här registrerar du prenumerationer. (Kod: MissingSubscriptionRegistration) "
...
* Det här felet kan också inträffa om ditt e-postdomännamn tilldelas till flera kataloger i Azure AD. Undvik det här problemet genom att skapa en ny användare i katalogen som innehåller prenumerationen med din Databricks-arbetsyta.
    ...
    b. Lägg till en användare med `@<tenant_name>.onmicrosoft.com` ett e- `@<your_domain>` postmeddelande i stället för e-post. Du kan hitta det här alternativet i **anpassade domäner**under Azure AD i Azure Portal.
...
Azure-felkod: MissingSubscriptionRegistration Azure-fel meddelande: prenumerationen har inte registrerats för användning av namn området Microsoft. Compute. Se https://aka.ms/rps-not-found så här registrerar du prenumerationer. "
