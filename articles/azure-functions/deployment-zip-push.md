...
I följande kommando ersätter du `<zip_file_path>` plats hållaren med sökvägen till platsen för. zip-filen. Ersätt `<app_name>` också med det unika namnet på din Function-app och Ersätt `<resource_group>` med namnet på din resurs grupp.
...
Zip-distributionen integreras med den här funktionen, som du kan aktivera genom `WEBSITE_RUN_FROM_PACKAGE` att ställa in funktionen app på värdet `1`. Mer information finns i [köra funktioner från en distributions paket fil](run-functions-from-deployment-package.md).
