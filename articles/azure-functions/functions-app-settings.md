...
`true` means disable the default landing page that is shown for the root URL of a function app. Standardvärdet är `false`.
...
`true` means use Release mode when compiling .NET code; `false` means use Debug mode. Standardvärdet är `true`.
...
|-|-|-|
|AZURE_FUNCTION_PROXY_DISABLE_LOCAL_CALL|sant|Calls with a backend url pointing to a function in the local Function App will no longer be sent directly to the function, and will instead be directed back to the HTTP front end for the Function App|
|AZURE_FUNCTION_PROXY_DISABLE_LOCAL_CALL|false|Detta är standardvärdet. Calls with a  backend url pointing to a function in the local Function App will be forwarded directly to that Function|
...
|Nyckel|Värde|Beskrivning|
|-|-|-|
|AZURE_FUNCTION_PROXY_BACKEND_URL_DECODE_SLASHES|sant|Route parameters with encoded slashes will have them decoded. `example.com/api%2ftest` will become `example.com/api/test`|
