...
Komma åt BLOB- `context.bindings.<name from function.json>`data med hjälp av.
...
`@BlobInput` Attributet ger dig åtkomst till den blob som utlöste funktionen. Om du använder en byte mat ris med attributet, anges `dataType` till `binary`. Mer information finns i indata- [exemplet](#input---example) .
...
Få åtkomst till BLOB- `context.bindings.<name from function.json>`data med hjälp av.
...
`@BlobOutput` Attributet ger dig åtkomst till den blob som utlöste funktionen. Om du använder en byte mat ris med attributet, anges `dataType` till `binary`. Mer information finns i [utdata-exemplet](#output---example) .
