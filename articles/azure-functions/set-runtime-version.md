...
Azure Functions kan du rikta en specifik version av körningen med hjälp av den `FUNCTIONS_EXTENSION_VERSION` inställningen i en funktionsapp. Funktionsappen sparas på den angivna huvudversionen tills du uttryckligen väljer att flytta till en ny version.
...
> [!NOTE]
> Om du fäster till en specifik version av Azure Functions och försök sedan att publicera på Azure med Visual Studio, visas en dialogruta där du uppmanas att uppdatera till den senaste versionen eller Avbryt publiceringen. Undvik detta genom att lägga till den `<DisableFunctionExtensionVersionUpdate>true</DisableFunctionExtensionVersionUpdate>` -egenskapen i din `.csproj` fil.
...
> [!NOTE]
> Du kan inte ändra runtime-versionen för en funktionsapp som redan innehåller funktioner med Azure portal.
