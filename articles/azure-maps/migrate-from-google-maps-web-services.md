...
> [!TIP]
> POI search, POI kategori search och fuzzy Search-API: er kan användas i läget komplettera automatiskt genom att lägga till `&amp;typeahead=true` till URL: en för begäran. Detta talar om för servern att indatamängden är troligt vis delvis. API: n kommer att genomföra sökningen i förutsägande läge.
...
> [!TIP]
> Som standard returnerar Azure Maps Route API bara en sammanfattning. Det returnerar avståndet och tiderna och koordinaterna för vägens sökväg. Använd parametern `instructionsType` för att hämta inaktiverade instruktioner. Och Använd `routeRepresentation` parameter för att filtrera fram sammanfattningen och vägens sökväg.
...
Lägg till markörer med hjälp av parametern `markers` i URL: en. Parametern `markers` tar i ett format och en lista över platser som ska återges på kartan med det formatet enligt nedan:

```
&markers=markerStyles|markerLocation1|markerLocation2|...
```

Om du vill lägga till fler stilar använder du `markers` parametrar till URL: en med ett annat format och en uppsättning med platser.

Ange markör platser med formatet "latitud, longitud".

Lägg till markör format med `optionName:value` formatet, med flera format separerade med vertikalstreck (\|) som detta "optionName1: värde1\|optionName2: värde2". Observera att alternativ namn och värden åtskiljs med kolon (:). Använd följande namn på stil-alternativet för att formatera markörer i Google Maps:
...
Lägg till markörer till en statisk kart bild genom att ange parametern `pins` i URL: en. Som Google Maps anger du ett format och en lista över platser i parametern. Parametern `pins` kan anges flera gånger för att stödja markörer med olika format.
...
Lägg till PIN-format med `optionNameValue`-formatet. Separera flera format med vertikalstreck (\|). Till exempel: `iconType|optionName1Value1|optionName2Value2`. Alternativ namn och värden är inte separerade. Använd följande format alternativ namn till format markörer:

- `al` – anger markörens opacitet (alfa). Välj ett tal mellan 0 och 1.
- `an` – anger PIN-ankaret. Ange X-och y-pixelvärdena i formatet "X y".
- `co` – PIN-kodens färg. Ange en 24-bitars hexadecimal färg: `000000` att `FFFFFF`.
- `la` – anger etikettens ankare. Ange X-och y-pixelvärdena i formatet "X y".
- `lc` – etikettens färg. Ange en 24-bitars hexadecimal färg: `000000` att `FFFFFF`.
...
Lägg till linjer och polygon till en statisk kart bild med hjälp av parametern `path` i URL: en. Parametern `path` tar i ett format och en lista över platser som ska återges på kartan, enligt nedan:
...
Lägg till Sök vägs format med `optionName:value` format, separera flera format med pipe-tecknen (\|). Och separata alternativ namn och värden med kolon (:). Så här: `optionName1:value1|optionName2:value2`. Följande format alternativ namn kan användas för att formatera sökvägar i Google Maps:
...
Lägg till linjer och polygoner till en statisk kart bild genom att ange parametern `path` i URL: en. Som Google Maps anger du ett format och en lista över platser i den här parametern. Ange `path` parameter flera gånger för att återge flera cirklar, linjer och polygoner med olika format.
...
Lägg till Sök vägs format med formatet `optionNameValue`. Separera flera format efter pipes (\|) tecken, som det här `optionName1Value1|optionName2Value2`. Alternativ namn och värden är inte separerade. Använd följande format alternativ namn för att formatera sökvägar i Azure Maps:
