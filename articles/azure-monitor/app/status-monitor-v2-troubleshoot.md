...
Du kan använda kommandot `Get-Module -ListAvailable` för att avgöra vilka moduler som är installerade.

#### <a name="import-a-module-into-the-current-session"></a>Importera en modul till den aktuella sessionen
Om en modul inte har lästs in i en PowerShell-session kan du läsa in den manuellt med hjälp av kommandot `Import-Module <path to psd1>`.
...
Se [API-referensen](status-monitor-v2-api-get-status.md) för en detaljerad beskrivning av hur du använder den här cmdleten.
...
Använd kommandot `Get-ApplicationInsightsMonitoringStatus -InspectProcess` för att kontrol lera DLL-filerna.
...
1. I en kommando konsol med administratörs behörighet kör du kommandot `iisreset /stop` för att stänga av IIS och alla webbappar.
2. I PerfView väljer du **starta insamling**.
3. I en kommando konsol med administratörs behörighet kör du kommandot `iisreset /start` för att starta IIS.
