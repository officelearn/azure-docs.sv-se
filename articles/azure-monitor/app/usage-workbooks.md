...
Varje avsnitt har sina egna avancerade inställningar, som du kan komma åt via inställnings ikonen ![Application Insights arbets boks avsnitt](./media/usage-workbooks/005-settings.png) redigera kontroller som finns till höger om knappen **Lägg till parametrar** .
...
Det finns fyra olika typer av parametrar som stöds för närvarande:
  ...
  |         |          |
   | ---------------- |:-----|
   | **Information**    | användaren redigerar en text ruta och du kan också ange en fråga för att fylla i standardvärdet. |
   | **Listmeny** | Användaren kommer att välja från en uppsättning värden. |
