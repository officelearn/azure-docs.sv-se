...
- **Omdirigerings-URI (valfritt)** : Det här är en **Web** programtypen och det grundläggande URL-värdet bör vara `https://afd.hosting.portal.azure.net/monitoring/Content/iframe/infrainsights.app/web/base-libs/auth/auth.html`.
- När du har registrerat programmet, från den **översikt** väljer **autentisering** från det vänstra fönstret. På den **autentisering** sidan under **avancerade inställningar** implicit bevilja **åtkomsttoken** och **ID-token** och spara dina ändringar.
...
När du visar händelser kan du dessutom kan begränsa resultaten med hjälp av den **Filter** pill finns till höger om sökfältet. Beroende på vilka resurser som du har valt visas pill en pod, en namnrymd eller ett kluster att välja från.  
