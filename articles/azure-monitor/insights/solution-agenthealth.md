---
title: Hälsolösning för ombud i Azure Monitor | Microsoft-dokument
description: Den här artikeln är avsedd att hjälpa dig att förstå hur du använder den här lösningen för att övervaka hälsotillståndet för dina agenter som rapporterar direkt till Log Analytics eller System Center Operations Manager.
ms.subservice: ''
ms.topic: conceptual
author: bwren
ms.author: bwren
...
---
#  <a name="agent-health-solution-in-azure-monitor"></a>Hälsolösning för agent i Azure Monitor
Agent Health-lösningen i Azure hjälper dig att förstå, för alla agenter som rapporterar direkt till Log Analytics-arbetsytan i Azure Monitor eller en System Center Operations Manager-hanteringsgrupp som är ansluten till Azure Monitor, som inte svarar och lämna in operativa data.  Du kan också hålla reda på hur många agenter distribueras, var de är fördelade geografiskt och utföra andra frågor för att övervaka distributionen av agenter i Azure, andra miljöer i molnet eller lokalt.    

## <a name="prerequisites"></a>Krav
Innan du distribuerar den här lösningen bekräftar du att du för närvarande har stöd för [Windows-agenter som](../../log-analytics/log-analytics-windows-agent.md) rapporterar till Log Analytics-arbetsytan eller rapporterar till en [Hanteringsgrupp](../../azure-monitor/platform/om-agents.md) för Operations Manager som är integrerad med arbetsytan.
...
Lägg till agenthälsolösningen på arbetsytan Log Analytics med hjälp av processen som beskrivs i [Lägg till lösningar](solutions.md). Det krävs ingen ytterligare konfiguration.
...
| Distribution enligt hanteringsgrupp | En partition av de olika Operations Manager Management-grupperna i din miljö.|
| Geoplats för agenter | En partition mellan de olika länder/regioner där du har agenter och ett totalt antal antal agenter som har installerats i varje land/region.|
| Antalet installerade Gateways | Antalet servrar som har Log Analytics-gatewayen installerad och en lista över dessa servrar.|
...
Lösningen skapar en typ av post på log analytics-arbetsytan.  
...
| `IsGatewayInstalled` | Om Log Analytics-gatewayen är installerad är värdet *sant*, annars är värdet *falskt*.|
| `ComputerIP` | Datorns offentliga IP-adress. På virtuella Azure-datorer visar detta den offentliga IP-adressen om en sådan är tillgänglig. För virtuella datorer med privata IP-adresser visar detta Azure SNAT-adressen (inte den privata IP-adressen). |
...
| Heartbeat &#124; where iff(isnotnull(toint(IsGatewayInstalled)), IsGatewayInstalled == true, IsGatewayInstalled == "true") == true &#124; distinct Computer |Antal logga analytics-gateways installerade |
...
* Lär dig mer om [aviseringar i Azure Monitor](../platform/alerts-overview.md) för information om hur du genererar aviseringar från loggfrågor. 
