...
Med den här uppgraderingen lagras Azure Monitor for VMs prestanda data uppsättningar i samma `InsightsMetrics`-tabell som [Azure Monitor för behållare](container-insights-overview.md)och gör det enklare för dig att fråga de två data uppsättningarna. Du kan också lagra Mer Diverse data uppsättningar som vi inte kunde lagra i tabellen som vi tidigare använde.  Våra prestanda visningar kommer också att uppdateras för att använda den här nya tabellen.
...
Vi inser att vi ber befintliga kunder att uppgradera orsakar avbrott i sitt arbets flöde, vilket är anledningen till att vi har valt att göra detta nu i den offentliga för hands versionen i stället för senare när vi kommer till GA.
...
Om du väljer att aktivera prestanda räknarna manuellt i arbets ytan kan du se data i några av våra prestanda diagram som visas från Azure Monitor. När den nya lösningen lanseras uppdaterar vi våra prestanda diagram för att fråga data som lagras i tabellen `InsightsMetrics`. Om du vill se data från tabellen i dessa diagram måste du uppgradera till den nya versionen av Azure Monitor for VMs.
...
Om du väljer att inte uppgradera till **VMInsights** -lösningen kommer vi fortsätta att tillhandahålla äldre versioner av arbets böckerna för prestanda som refererar till data i tabellen `Perf`.  
...
Om du är en befintlig kund som använder hälso funktionen och vill använda den för en ny distribution kontaktar du oss på vminsights@microsoft.com för att begära instruktioner.
