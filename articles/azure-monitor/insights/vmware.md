...
1. Öppna ESXi-värd brand väggen för syslog. **ESXi värd konfiguration** > **program** > **säkerhets profil** > **brand vägg** och öppna **Egenskaper**.  
...
1. När Log Analytics-agenten för Linux har installerats går du till katalogen/etc/opt/Microsoft/omsagent/sysconf/omsagent.d och kopierar filen vmware_esxi. conf till katalogen/etc/opt/Microsoft/omsagent/conf/omsagent.d och ändrar ägare/grupp och filens behörigheter. Exempel:
...
1. Testa anslutningen mellan Linux-servern och ESXi-värden med hjälp av kommandot `nc` på ESXi-värden. Exempel:
...
| Systemet | Log Analytics agent för Linux | SCOM-agent | Azure Storage | SCOM krävs? | SCOM agent-data som skickats via hanterings grupp | samlings frekvens |
| --- | --- | --- | --- | --- | --- | --- |
| Linux |&#8226; |  |  |  |  |var 3: e minut |
...
När du klickar på ett ESXi-värdnamn visar du information från den ESXi-värden. Om du vill begränsa resultatet med händelse typen lägger du till `“ProcessName_s=EVENT TYPE”` i din Sök fråga. Du kan välja **processname** i Sök filtret. Detta begränsar informationen åt dig.
...
* Den virtuella datorn med Log Analytics agent har inte angetts korrekt. Testa detta genom att utföra följande steg:
  ...
  1. Du bör se port `1514/tcp` öppen. Om du inte gör det kontrollerar du att omsagent har installerats korrekt. Om du inte ser portinformation är syslog-porten inte öppen på den virtuella datorn.
