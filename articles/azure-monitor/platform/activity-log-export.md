...
**Hur länge aktivitets loggen ska behållas i ett lagrings konto.** En kvarhållning på noll dagar innebär att loggar bevaras för alltid. Annars kan värdet vara valfritt antal dagar mellan 1 och 365.
...
3. Använd `az monitor log-profiles create` för att skapa en ny logg profil:
   ...
   ```
    ...
    | platser |Ja |Blankstegsavgränsad lista över regioner för vilka du vill samla in aktivitets logg händelser. Du kan visa en lista över alla regioner för din prenumeration med `az account list-locations --query [].name`hjälp av. |
