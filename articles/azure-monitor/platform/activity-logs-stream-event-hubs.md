...
ms.locfileid: "57309697"
---
# <a name="stream-the-azure-activity-log-to-event-hubs"></a>Stream Azure-aktivitetsloggen till Event Hubs
Du kan strömma den [Azure-aktivitetsloggen](../../azure-monitor/platform/activity-logs-overview.md) i nära realtid för program genom att antingen:
...
1. Bläddra till den **aktivitetsloggen** avsnittet med hjälp av den **alla tjänster** sökning på vänster sida av portalen.
   ...
   ![Att välja aktivitetsloggen i listan över tjänster i portalen](./media/activity-logs-stream-event-hubs/activity-logs-portal-navigate-v2.png)
2. Välj den **exportera till Event Hub** längst upp i loggen.
   
   ![Exportera-knappen i portalen](./media/activity-logs-stream-event-hubs/activity-logs-portal-export-v2.png)
...
3. I avsnittet som visas, väljer **alla regioner**. Markera inte specifika regioner.
   ...
   ![Exportera avsnittet](./media/activity-logs-stream-event-hubs/export-audit.png)
...
1. Använd `Get-AzLogProfile` att identifiera om det finns en loggprofil.  Om det finns en loggprofil, leta upp den *namn* egenskapen.
2. Använd `Remove-AzLogProfile` att ta bort log-profilen med värdet från den *namn* egenskapen.
...
Schemat för aktivitetsloggen finns i [övervaka prenumeration med Azure-aktivitetsloggen](../../azure-monitor/platform/activity-logs-overview.md). Varje händelse är i en matris av JSON-blobar som kallas *poster*.
