...
ms.locfileid: "72932820"
---
# <a name="how-to-troubleshoot-issues-with-the-log-analytics-agent-for-linux"></a>Så här felsöker du problem med Log Analytics agent för Linux 
...
| 4 | Ogiltig proxy angavs för omsadmin-skriptet. Verifiera proxyn och se vår [dokumentation för att använda en HTTP-proxy](log-analytics-agent.md#network-firewall-requirements). |
...
| 34 | Skript för Metaconfiguration-generering saknas. Försök att registrera igen med `sudo sh /opt/microsoft/omsagent/bin/omsadmin.sh -w <Workspace ID> -s <Workspace Key>`. |
...
* Log Analytics agent för Linux User `omsagent` kan inte komma åt den anpassade loggen på grund av behörigheter eller inte hittas.  Följande fel kan visas:
