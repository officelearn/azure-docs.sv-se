...
När du är klar visas **Microsoft Monitoring Agent** i **Kontrollpanelen**. To confirm it is reporting to Log Analytics, review [Verify agent connectivity to Log Analytics](#verify-agent-connectivity-to-log-analytics). 
...
|MMA-specific options                   |Anteckningar         |
|---------------------------------------|--------------|
...
På datorn går du till **Kontrollpanelen** och letar upp objektet **Microsoft Monitoring Agent**.  Select it and on the **Azure Log Analytics** tab, the agent should display a message stating: **The Microsoft Monitoring Agent has successfully connected to the Microsoft Operations Management Suite service.**<br><br> ![MMA-anslutningsstatus till Log Analytics](media/agent-windows/log-analytics-mma-laworkspace-status.png)

You can also perform a simple log query in the Azure portal.  

1. Klicka på **Alla tjänster** i Azure-portalen. In the list of resources, type **Azure Monitor**. När du börjar skriva filtreras listan baserat på det du skriver. Select **Azure Monitor**.  
