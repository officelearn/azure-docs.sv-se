...
>
> När du har godkänt från listan vänta får du instruktioner för att installera PowerShell-modulen för för hands versionen.

## <a name="create-template-spec"></a>Skapa mall-specifikation

Mallens specifikation är en ny resurs typ med namnet **Microsoft. Resources/templateSpecs**. Du kan använda Azure PowerShell eller en ARM-mall för att skapa en mall. I alla alternativ behöver du en ARM-mall som är paketerad i mallen specifikation.

Med PowerShell skickas ARM-mallen som en parameter till kommandot. Med ARM-mall är ARM-mallen som ska paketeras i specifikationen för mallen inbäddad i definitionen av mallen definition.
...
1. När du använder en ARM-mall för att skapa en mall, bäddas mallen in i resurs definitionen. Kopiera följande mall och spara den lokalt som **azuredeploy.jspå**. Den här snabb starten förutsätter att du har sparat till en sökväg **c:\Templates\azuredeploy.js** men du kan använda valfri sökväg.
   ...
   > [!NOTE]
   > I den inbäddade mallen måste alla vänster hak paren tes föregås av en andra vänsterparentes. Använd `[[` i stället för `[` .
