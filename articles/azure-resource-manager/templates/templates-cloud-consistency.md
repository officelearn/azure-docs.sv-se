...
En introduktion till Azure Resource Manager-mallar finns i [malldistributionen](overview.md).
...
Du kan hämta samma lista med Azure PowerShell-cmdleten [Get-AzureRmVMImagePublisher](/powershell/module/az.compute/get-azvmimagepublisher) och ange den plats du vill med den `-Location` parametern. Ett exempel:
...
Du kan också köra Azure PowerShell [Get-AzureRmVmImagePublisher](/powershell/module/az.compute/get-azvmimagepublisher) cmdlet och Använd `-Location` att ange platsen för avbildningen av virtuella datorn. Ett exempel:
...
* [Metodtips för Azure Resource Manager-mallar](template-syntax.md)
