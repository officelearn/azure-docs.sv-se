...
description: Lär dig hur du flyttar din Azure SQL Database-eller Azure SQL-hanterade instans till en annan region.
...
I den här artikeln lär du dig ett allmänt arbets flöde för att flytta Azure SQL Database eller Azure SQL-hanterad instans till en ny region.

## <a name="overview"></a>Översikt

Det finns olika scenarier där du vill flytta befintliga Azure SQL Database eller hanterade instanser från en region till en annan. Du kan till exempel expandera din verksamhet till en ny region och vill optimera den för den nya kund basen. Eller så måste du flytta åtgärderna till en annan region för att kunna följa dessa orsaker. Eller Azure har lanserat en helt ny region som ger bättre närhet och förbättrar kund upplevelsen.  
...
## <a name="move-azure-sql-database"></a>Flytta Azure SQL Database
...
1. Skapa en [redundans grupp](failover-group-add-single-database-tutorial.md#2---create-the-failover-group) mellan serverns Server och målets Server.  
...
1. För granskning på server nivå, se till att:
    - Lagrings behållaren, Log Analytics eller händelsehubben med befintliga gransknings loggar flyttas till mål regionen.
    - Gransknings konfigurationen konfigureras på mål servern. Mer information finns i [granskning av SQL Database](../../azure-sql/database/auditing-overview.md).
1. Om instansen har en långsiktig bevarande princip (brv) förblir de befintliga säkerhets kopieringarna fortfarande kopplade till den aktuella servern. Eftersom mål servern är annorlunda kommer du att kunna komma åt äldre LTR-säkerhetskopieringar i käll regionen med käll servern, även om servern tas bort.
...
## <a name="move-azure-sql-managed-instance"></a>Flytta Azure SQL-hanterad instans

### <a name="verify-prerequisites"></a>Verifiera kraven

1. För varje SQL-hanterad SQL-instans skapar du en mål SQL-hanterad instans av samma storlek i mål regionen.  
1. Konfigurera nätverket för en SQL-hanterad instans. Mer information finns i [nätverks konfiguration](../managed-instance/how-to-content-reference-guide.md#network-configuration).
...
1. Om dina databaser är krypterade med TDE och använder din egen krypterings nyckel i Azure Key Vault, se till att Azure Key Vault med identiska krypterings nycklar finns i både käll-och mål regionerna. Mer information finns i [TDE med Kundhanterade nycklar i Azure Key Vault](transparent-data-encryption-byok-overview.md).
1. Om granskning har Aktiver ATS för SQL-hanterad instans kontrollerar du att:
    - Lagrings containern eller händelsehubben med befintliga loggar flyttas till mål regionen.
    - Granskning har kon figurer ATS på mål instansen. Mer information finns i [granskning med hanterad instans](../managed-instance/auditing-configure.md).
...
Skapa en redundans grupp mellan varje SQL-hanterad SQL-instans och motsvarande mål-SQL-hanterade instans.
...
1. Anslut till den mål-SQL-hanterade instans som använder den sekundära slut punkten `<fog-name>.secondary.database.windows.net` .
...
[Hantera](manage-data-after-migrating-to-database.md) din Azure SQL Database när den har migrerats.
