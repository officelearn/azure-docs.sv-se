---
title: Konfigurera VNN med Azure Load Balancer för HADR
description: Lär dig hur du konfigurerar en Azure Load Balancer för att dirigera trafik till det virtuella nätverks namnet (VNN) för din tillgänglighets grupp eller FCI (failover Cluster instance) med SQL Server på virtuella Azure-datorer för hög tillgänglighet och haveri beredskap (HADR).
...
Den här artikeln lär dig hur du konfigurerar en belastningsutjämnare med hjälp av Azure Load Balancer tjänsten. Belastningsutjämnaren dirigerar trafik till din [tillgänglighets grupp (AG)-lyssnare](availability-group-overview.md) eller [instanser av redundanskluster (skyddas)](failover-cluster-instance-overview.md) med SQL Server på virtuella Azure-datorer för hög tillgänglighet och haveri beredskap (hadr). 
...
- Den senaste versionen av [PowerShell](/powershell/azure/install-az-ps?view=azps-4.2.0)har installerats. 
...
Om du vill testa anslutningen loggar du in på en annan virtuell dator i samma virtuella nätverk. Öppna **SQL Server Management Studio** och anslut till SQL Server FCI-namnet eller till tillgänglighets gruppens lyssnare.
