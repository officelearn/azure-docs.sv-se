...
2. Konfigurera rätt IP-adress i huvudkortshantering (BMC) och tillämpa alla BIOS-inställningarna per din OEM-tillverkarens dokumentation.
3. Gäller den aktuella baslinjen för inbyggd programvara för alla komponenter med hjälp av verktygen som tillhandahålls av tillverkaren som körs på HLH.
4. Kör sedan Lägg till nod i Azure Stack-administratörsportalen.
5. Verifiera att lägga till noden åtgärden lyckas. Du gör detta genom att kontrollera den [ **Status** av Skalningsenheten](#monitor-add-node-operations). 

## <a name="add-the-node"></a>Lägga till noden 
Du kan använda administratörskonsolen eller PowerShell för att lägga till nya noder. Lägga till noden åtgärden först lägger till den nya noden i skala enhet som tillgängliga beräkningskapacitet och utökas lagringskapaciteten automatiskt. Kapaciteten expanderar automatiskt eftersom Azure Stack är ett hyperkonvergerat system där *compute* och *storage* skalas tillsammans.

### <a name="use-the-admin-console"></a>-Administratörskonsolen
1. Logga in på administrationsportalen för Azure Stack som en Azure Stack-operator.
2. Gå till **regionshantering** > **Skalningsenheter**, och välj sedan skalningsenheten som du vill utöka kapaciteten för.  
   ![Välj skalningsenhet](media/azure-stack-add-scale-node/select-node1.png)

   ![Visa skala enhet information](media/azure-stack-add-scale-node/select-node2.png)
 
3. Det krävs följande parametrar:  
   - **BMC IP-adress** för varje nod för skala-enhet som du lägger till. (Ett IP-adress per rad.) ![Lägg till nod](media/azure-stack-add-scale-node/add-node.png)

### <a name="use-powershell"></a>Använd PowerShell

Använd den **New AzsScaleUnitNodeObject** cmdlet för att lägga till en eller flera noder.  
...
## <a name="monitor-add-node-operations"></a>Övervaka åtgärderna Lägg till nod 
...
Granska [noden åtgärder](azure-stack-node-actions.md) 
