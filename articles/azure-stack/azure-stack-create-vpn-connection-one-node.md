...
Den här artikeln beskriver hur du skapar en plats-till-plats VPN-anslutning mellan två virtuella nätverk i två separata Azure Stack Development Kit (ASDK)-miljöer. När du konfigurerar anslutningarna får du lära dig hur VPN-gatewayer i Azure Stack fungerar.
...
* Två servrar och andra krav som uppfyller maskinvarukraven ASDK enligt beskrivningen i [snabbstarten: Utvärdera Azure Stack Development Kit](azure-stack-deploy-overview.md).
...
För att slutföra konfigurationen, måste du distribuera två ASDK miljöer.

> [!NOTE]
> För varje ASDK som du distribuerar, följer du de [distributionsanvisningarna](azure-stack-run-powershell-script.md). I den här artikeln ASDK miljöer kallas **POC1** och **POC2**.
...
I följande tabell sammanfattas nätverkskonfigurationen för båda ASDK-miljöer. Stegen som visas efter tabellen att lägga till den externa BGPNAT-adressen som är specifik för ditt nätverk.

### <a name="network-configuration-table"></a>Tabell för konfiguration av nätverk
...
> [!NOTE]
> Externa BGPNAT IP-adresser i exempelmiljön är 10.16.167.195 för POC1 och 10.16.169.131 för POC2. Använd följande procedur för att bestämma externa BGPNAT IP-adresser för ASDK-värdar och lägga till dem i tabellen ovan network configuration.
...
Nu kan du skapa POC1 nätverksresurser som du behöver för att skapa din gateway. I följande anvisningar beskrivs hur du skapar resurser med hjälp av Azure Stack-användarportalen. Du kan också använda PowerShell-kod för att skapa resurser.
...
4. I **namn**, ange **GW1**.
5. Välj den **virtuellt nätverk** för att välja ett virtuellt nätverk. Välj **VNET-01** i listan.
6. Välj den **offentliga IP-adressen** menyalternativ. När den **Välj offentlig IP-adress** öppnas, Välj **Skapa ny**.
...
Ett sätt att tänka detta mer allmänt är att den lokala gateway-nätverksresursen alltid anger en fjärrgateway i den andra änden av anslutningen. Du måste ange IP-adressen för det externa nätverkskortet på (NAT) virtuell dator av andra ASDK som offentliga IP-adressen för den lokala nätverksgatewayen för network address translation på grund av det sätt som har utformats för Azure Stack Development Kit. Du kan sedan skapa NAT-mappningar på NAT-VM för att se till att båda ändar är korrekt anslutna.
...
### <a name="sign-in-as-a-tenant-again"></a>Logga in igen som en klient

En tjänstadministratör kan logga in som en klient att testa de planer, erbjudanden och prenumerationer som deras klientorganisationer använder. Om du inte redan har en, [skapa ett klientkonto](azure-stack-add-new-user-aad.md) innan du loggar in.

### <a name="create-virtual-network-and-vm-subnet"></a>Skapa virtuellt nätverk och undernät för virtuella datorer
...
### <a name="create-gateway-subnet"></a>Skapa gateway-undernät
...
4. Namnet på undernätet ställs in på **GatewaySubnet** som standard. Gatewayundernät är speciella och måste ha det här specifika namnet för att fungera ordentligt.
5. I den **adressintervall** fältet, verifiera att adressen är **10.0.21.0/24**.
6. Välj **OK** att skapa gateway-undernätet.

### <a name="create-virtual-network-gateway"></a>Skapa en virtuell nätverksgateway
...
### <a name="create-local-network-gateway-resource"></a>Skapa resurs för lokal gateway

1. Välj i användarportalen POC2 **+ skapa en resurs**.
2. Gå till **Marketplace**, och välj sedan **nätverk**.
3. Välj i listan över resurser **lokal nätverksgateway**.
4. I **namn**, ange **POC1-GW**.
5. I **IP-adress**, ange den externa BGPNAT-adressen för POC1 som anges tidigare i tabellen network configuration.
6. I **adressutrymme**, från POC1, ange den **10.0.10.0/23** adressutrymmet för **VNET-01**.
7. Kontrollera att din **prenumeration**, **resursgrupp**, och **plats** är korrekta och välj sedan **skapa**.

## <a name="create-connection"></a>Skapa anslutning
...
9. I **delad nyckel (PSK)**, ange **12345**. Om du väljer ett annat värde, Kom ihåg att det måste matcha värdet för den delade nyckeln som du skapade på POC1. Välj **OK**.
10. Granska den **sammanfattning** bladet och välj sedan **OK**.

## <a name="create-a-virtual-machine"></a>Skapa en virtuell dator

Nu kan du skapa en virtuell dator i POC2 och placera den i VM-undernätet i det virtuella nätverket:
...
Eftersom ASDK är fristående och isolerad från nätverket där den fysiska värden har distribuerats i *externa* VIP-nätverket som gatewayerna är anslutna till inte är faktiskt extern. I stället är VIP-nätverket dold bakom en router som utför nätverksadresser.

Routern är en Windows Server-dator som kallas **AzS-bgpnat01**, som kör rollen Routing och Remote Access Services (RRAS) i ASDK-infrastruktur. Du måste konfigurera NAT på den virtuella datorn i AzS-bgpnat01 så att VPN-anslutningen för plats-till-plats att ansluta i båda ändar.

Om du vill konfigurera VPN-anslutningen måste du skapa en statisk karta väg för NAT som mappar det externa gränssnittet på den virtuella datorn BGPNAT till VIP för edge gatewaypool. En statisk väg för NAT-mappning måste anges för varje port i en VPN-anslutning.
...
> [!IMPORTANT]
> Du måste slutföra den här proceduren för båda ASDK-miljöer.
...
3. Kopiera och redigera följande PowerShell-skript. Om du vill konfigurera NAT i varje Azure Stack Development Kit kan du köra skriptet i en upphöjd Windows PowerShell ISE. I skriptet, Lägg till värden i den `External BGPNAT address` och `Internal IP address` platshållare:
...
Nu när plats-till-plats-anslutningen har upprättats bör du kontrollera att du kan hämta trafik som passerar genom den. Verifiera genom att logga in på en av de virtuella datorerna som du skapade i antingen ASDK-miljö. Sedan kan pinga den virtuella datorn som du skapade i den andra miljön.

För att säkerställa att du ska skicka trafik via plats-till-plats-anslutning, kontrollerar du att du pinga den Direct IP (DIP)-adressen för den virtuella datorn på fjärrundernätet, inte VIP. Du gör detta genom att hitta den DIP-adressen på den andra änden av anslutningen. Spara adressen för senare användning.
...
6. Öppna en förhöjd behörighet **Windows PowerShell** fönster.
7. Ange **ipconfig/all**.
8. En IPv4-adress visas som ligger inom **10.0.20.0/24**. I exempelmiljön är adressen är **10.0.20.4**, men din postadress vara annorlunda.
9. Om du vill skapa en brandväggsregel som tillåter den virtuella datorn svarar på ping, kör du följande PowerShell-kommando:
...
10. Pinga den virtuella datorn på POC1, genom tunneln från den virtuella datorn i POC2. Om du vill göra detta måste pinga du den DIP du antecknade från VM01. I exempelmiljön är **10.0.10.4**, men se till att pinga adressen som du antecknade i labbet. Du bör se ett resultat som ser ut som i följande exempel:

    ![Lyckad ping](media/azure-stack-create-vpn-connection-one-node-tp2/image19b.png)
11. Ett svar från fjärrdatorn virtuella anger testet lyckats. Du kan stänga fönstret för den virtuella dator. Du kan prova andra typer av dataöverföringar, till exempel en filkopia om du vill testa anslutningen.
...
3. I den **anslutning** fönster och statistik för **Data i** och **Data ut** visas. I följande skärmbild är det stora antalet tillskrivas ytterligare filöverföringen. Du bör se några nollvärden det.
