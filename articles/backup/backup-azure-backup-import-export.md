...
## <a name="prerequisites"></a>Krav
  ...
  > [!NOTE]
  > Följande nödvändiga komponenter och arbets flöde gäller endast offline-säkerhetskopiering av filer och mappar med hjälp av den [senaste mars-agenten](https://aka.ms/azurebackup_agent). Om du vill utföra säkerhets kopiering offline för arbets belastningar med System Center DPM eller Azure Backup Server, se [den här artikeln](backup-azure-backup-server-import-export-.md).
...
* Se till att endast den [senaste versionen av Azure Backup-agenten](https://aka.ms/azurebackup_agent) har installerats på Windows Server/Windows-klienten, enligt vad som är tillämpligt och att datorn har registrerats med Recovery Services-valvet.
...
* Registrera Microsoft. ImportExport-resurs leverantören med prenumerationen som innehåller det Azure Storage kontot. Så här registrerar du resurs leverantören:
    ...
    ![att registrera resurs leverantören](./media/backup-azure-backup-import-export/registerimportexport.png)
...
1. När du schemalägger en säkerhets kopiering på MARS-agenten visas följande skärm.
    ...
    * **ID för Azure-prenumeration**: ID för Azure-prenumerationen där Azure Storages kontot skapas.
...
2. Öppna en kommando tolk med förhöjd behörighet på Kopiera datorn med katalogen *AzureOfflineBackupDiskPrep* Utility som den aktuella katalogen och kör följande kommando:
    ...
    | --- | --- |
    | s: *sökväg till&lt;mellanlagrings plats*&gt; |Obligatoriskt invärden används för att ange sökvägen till den mellanlagringsplats som du angav i arbets flödet **Starta säkerhets kopiering offline** . |
...
2. Klicka på **alla tjänster** i huvud menyn och skriv import i dialog rutan alla tjänster. När du ser **import/export-jobb**klickar du på den.
    ![att ange leverans information](./media/backup-azure-backup-import-export/search-import-job.png)<br/>
