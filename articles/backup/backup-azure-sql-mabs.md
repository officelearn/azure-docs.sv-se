...
description: I den här artikeln lär du dig konfigurations stegen för säkerhets kopiering av SQL Server databaser med hjälp av Microsoft Azure Backup Server (MABS).
ms.reviewer: kasinh
author: dcurwin
manager: carmonm
ms.service: backup
...
8. Klicka på **Nästa**
    ...
    MABS visar det övergripande tillgängliga lagrings utrymmet och det potentiella disk utrymme som utnyttjas.

    ![Diskallokering](./media/backup-azure-backup-sql/pg-storage.png)

    Som standard skapar MABS en volym per data källa (SQL Server databas) som används för den första säkerhets kopian. Med den här metoden begränsar LDM (Logical Disk Manager) MABS-skydd till 300-data källor (SQL Server-databaser). Undvik den här begränsningen genom att välja **samplacera data i DPM-lagringspool**, alternativ. Om du använder det här alternativet använder MABS en enda volym för flera data källor, vilket gör att MABS kan skydda upp till 2000 SQL-databaser.

    Om alternativet **för volymer utökas automatiskt** är MARKERAT kan Mabs konto för den ökade säkerhets kopierings volymen eftersom produktions data växer. Om alternativet **för att utöka automatiskt** inte är valt, begränsar Mabs lagrings utrymmet som används för data källorna i skydds gruppen.
...
10. Välj när du vill att konsekvens kontrollen ska köras och klicka på **Nästa**.
    ...
    MABS kan utföra en konsekvens kontroll för att kontrol lera säkerhets kopierings platsens integritet. Den beräknar säkerhets kopian för säkerhets kopian på produktions servern (SQL Server datorn i det här scenariot) och säkerhetskopierade data för filen på MABS. Om det uppstår en konflikt förutsätts det att den säkerhetskopierade filen på MABS är skadad. MABS likriktare säkerhetskopierade data genom att skicka blocken som motsvarar matchnings fel för kontroll summan. Eftersom konsekvens kontrollen är en prestanda intensiv åtgärd har administratörer möjlighet att schemalägga konsekvens kontrollen eller köra den automatiskt.
