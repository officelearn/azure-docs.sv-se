...
| Ögonblicksbildsåtgärden misslyckades eftersom ingen nätverksanslutning på den virtuella datorn – se till att den virtuella datorn har nätverksåtkomst. Att det lyckas, antingen godkänd Azure datacenter IP-intervall eller konfigurera en proxyserver för nätverksåtkomst. Mer information finns i http://go.microsoft.com/fwlink/?LinkId=800034. Om du redan använder proxyserver, se till att proxyserverinställningarna är rätt konfigurerade | Det här felet returneras när du neka utgående internet-anslutning på den virtuella datorn. Internetanslutning krävs för tillägget för ögonblicksbild av virtuell dator att ta en ögonblicksbild av underliggande diskarna för den virtuella datorn. [Läs mer](backup-azure-troubleshoot-vm-backup-fails-snapshot-timeout.md#snapshot-operation-failed-due-to-no-network-connectivity-on-the-virtual-machine) om hur du löser ögonblicksbild fel som beror på blockerade nätverksåtkomst. |
| VM-agenten kan inte kommunicera med Azure Backup-tjänsten. – Se till att den virtuella datorn är ansluten till nätverket och VM-agenten är senaste och körs. Mer information finns i  http://go.microsoft.com/fwlink/?LinkId=800034 |Det här felet returneras om det uppstår ett problem med VM-agenten eller nätverksåtkomst till Azure-infrastrukturen är blockerad på något sätt. [Läs mer](backup-azure-troubleshoot-vm-backup-fails-snapshot-timeout.md#vm-agent-unable-to-communicate-with-azure-backup) ögonblicksbild problem om hur du felsöker virtuell dator.<br> Om den Virtuella datoragenten inte orsakar problem, och starta sedan om den virtuella datorn. Ett felaktigt tillstånd för virtuell dator kan ibland orsaka problem och starta om den virtuella datorn återställer den här ”felaktigt tillstånd”. |
| Virtuell dator är i felläge för etablering – starta om den virtuella datorn och se till att den virtuella datorn är i körningsläge eller nedstängt läge för säkerhetskopiering | Detta inträffar när en av datortillägg leder tillstånd för virtuell dator vara etableringsstatusen misslyckades. Gå till listan över tillägg och se om det finns ett tillägg för misslyckade, ta bort den och försök att starta om den virtuella datorn. Om alla tillägg är körs för närvarande kan du kontrollera om VM-agenttjänsten körs. Annars kan du starta om tjänsten VM agent. | 
| Vmsnapshot-tillägget misslyckades för managed disks – försök säkerhetskopiera igen. Om problem upprepas följer du anvisningarna i ”http://go.microsoft.com/fwlink/?LinkId=800034”. Om det fortfarande inte fungerar, kontakta Microsoft support | Det här felet när Azure Backup-tjänsten inte kan utlösa en ögonblicksbild. [Läs mer](backup-azure-troubleshoot-vm-backup-fails-snapshot-timeout.md#vmsnapshot-extension-operation-failed) ögonblicksbild problem om hur du felsöker VM. |
...
| Recovery services-tillägg-åtgärden misslyckades. – Kontrollera att senaste VM-agenten finns på den virtuella datorn och agent-tjänsten körs. Försök att säkerhetskopiera igen och kontakta Microsoft-supporten om det misslyckas. |Det här felet returneras när VM-agenten är inaktuell. Se ”uppdaterar VM-Agent” avsnittet nedan för att uppdatera VM-agenten. |
| Virtuell dator finns inte. – Kontrollera att den virtuella datorn finns eller välj en annan virtuell dator. |Detta händer när den primära virtuella datorn tas bort men säkerhetskopieringspolicyn fortsätter att söka efter en virtuell dator för att utföra säkerhetskopiering. Åtgärda det här felet: <ol><li> Återskapa den virtuella datorn med samma namn och samma resursgruppnamn [cloud service name]<br>(OR)<br></li><li>Sluta skydda den virtuella datorn utan att ta bort säkerhetskopierade data. [Mer information](http://go.microsoft.com/fwlink/?LinkId=808124)</li></ol> |
...
| Kopierar de virtuella hårddiskarna från säkerhetskopieringsvalvet tidsgränsen – försök igen om några minuter. Kontakta Microsoft-supporten om problemet kvarstår. | Detta inträffar om det finns ett tillfälligt fel på lagringssidan eller om backup-tjänsten inte får tillräckligt med IOPS från storage-konto som är värd för den virtuella datorn för att överföra data inom tidsgränsen till valvet. Kontrollera att du följt [bästa praxis](backup-azure-vms-introduction.md#best-practices) när du konfigurerar säkerhetskopiering. Försök att flytta virtuell dator till en annan lagring säkerhetskopieringen av kontot som inte har lästs in och försök igen.|
| Säkerhetskopieringen misslyckades med ett internt fel – försök igen om några minuter. Om problemet kvarstår kontaktar du Microsoft Support |Du kan få det här felet 2 skäl: <ol><li> Det finns ett övergående problem vid VM-lagring. Kontrollera [Azure-Status](https://azure.microsoft.com/status/) att se om det finns några pågående problem som rör compute, lagring eller nätverk i regionen. Försök att säkerhetskopiera igen när problemet är löst. <li>Den ursprungliga virtuella datorn har tagits bort och därför återställningspunkten kan inte utföras. Hela säkerhetskopierade data för en borttagen virtuell dator, men ta bort backup fel: ta bort skyddet från den virtuella datorn och välja alternativet för att behålla data. Den här åtgärden stoppar schemalagda säkerhetskopieringsjobbet och återkommande felmeddelanden. |
...
| Virtuell dator är inte i ett tillstånd som tillåter säkerhetskopieringar. |<ul><li>Kontrollera om VM är i ett tillfälligt tillstånd mellan körs och stänga ned. Om det är, väntar du tills tillstånd för virtuell dator att vara en av dem och utlösa säkerhetskopiera igen. <li> Om den virtuella datorn är en Linux VM och använder [Security Enhanced Linux] kernel-modul, måste du utesluta sökvägen för Linux-agenten (_/var/lib/waagent_) från security principen för att se till att säkerhetskopieringstillägget installeras.  |
| Azure virtuell dator hittades inte. |Detta händer när den primära virtuella datorn tas bort men säkerhetskopieringspolicyn fortsätter att söka efter en virtuell dator att utföra säkerhetskopiera. Åtgärda det här felet: <ol><li>Återskapa den virtuella datorn med samma namn och samma resursgruppnamn [cloud service name] <br>(OR) <li> Inaktivera skyddet för den här virtuella datorn så inte skapas säkerhetskopieringsjobben. </ol> |
...
| Ögonblicksbildsåtgärden misslyckades på grund av VSS-skrivarna är i felaktigt tillstånd |Du måste starta om VSS (Volume Shadow copy Service)-skrivare som är i felaktigt tillstånd. Om du vill göra detta, från en upphöjd kommandotolk, kör _vssadmin list writers_. Utdata innehåller alla VSS-skrivare och deras tillstånd. Starta om VSS-skrivaren genom att köra följande kommandon från en upphöjd kommandotolk för varje VSS-skrivaren vars tillstånd är inte ”[1] Stable”:<br> _net stop serviceName_ <br> _net start serviceName_|
| Ögonblicksbildsåtgärden misslyckades på grund av parsning av konfigurationen |Detta inträffar på grund av ändrade behörigheter för MachineKeys-katalogen: _%systemdrive%\programdata\microsoft\crypto\rsa\machinekeys_ <br>Kör kommandot nedan och kontrollera att behörigheterna för MachineKeys directory är standard-som:<br>_Icacls %systemdrive%\programdata\microsoft\crypto\rsa\machinekeys_ <br><br> Standardbehörigheterna är:<br>Everyone:(R,W) <br>BUILTIN\Administrators:(F)<br><br>Om behörigheter på MachineKeys directory annorlunda än standard, följer du stegen nedan för att rätt behörighet, ta bort certifikatet och utlösa säkerhetskopieringen.<ol><li>Åtgärda behörighet på MachineKeys directory.<br>Med hjälp av Explorer säkerhetsegenskaper och avancerade säkerhetsinställningar för katalogen återställa behörigheter till standardvärdena, ta bort alla extra (än standardvärdet) användarobjekt från katalogen och kontrollera att gruppen ”Alla” behörigheter hade särskild behörighet för:<br>-Lista mappar / läsa data <br>-Läsa attribut <br>-Läsa utökade attribut <br>– Skapa filer / skriva data <br>-Skapa mappar / lägga till data<br>-Skriva attribut<br>-Skriva utökade attribut<br>-Läsa behörigheter<br><br><li>Ta bort alla certifikat med fältet utfärdat till = ”Windows Azure Service Management för tillägg” eller ”Windows Azure CRP Certificate Generator”.<ul><li>[Öppna konsolen certifikat (lokal dator)](https://msdn.microsoft.com/library/ms788967(v=vs.110).aspx)<li>Ta bort alla certifikat (under Personal -> certifikat) med fältet utfärdat till = ”Windows Azure Service Management för tillägg” eller ”Windows Azure CRP Certificate Generator”.</ul><li>Utlösaren säkerhetskopiering av virtuella datorer. </ol>|
...
|Installation av ögonblicksbild misslyckades med felet tillägg – COM + kunde inte kommunicera med Microsoft Distributed Transaction Coordinator | Försök att starta windows-tjänst ”COM + System Application” (från en upphöjd kommandotolk - _net start COMSysApp_). <br>Om det misslyckas vid start, följer du stegen nedan:<ol><li> Kontrollera att inloggningskontot för tjänsten ”Distributed Transaction Coordinator” är ”Nätverkstjänst”. Om den inte ändra det till ”nätverkstjänst”, starta om den här tjänsten och försök sedan att starta tjänsten ”COM + System Application” ”.<li>Om det fortfarande inte att starta, avinstallera och installera tjänsten ”Distributed Transaction Coordinator” genom att följa stegen nedan:<br> -Stoppa MSDTC-tjänsten<br> – Öppna en kommandotolk (cmd) <br> -Köra kommandot ”msdtc-avinstallera” <br> -Köra kommandot ”msdtc-installera” <br> -Starta MSDTC-tjänsten<li>Starta windows-tjänsten COM + System Application och när den har startats, aktivera säkerhetskopiering från portalen.</ol> |
...
| Ögonblicksbildsåtgärden misslyckades på grund av att skapa en säker nätverkskommunikationskanal | <ol><Li> Öppna Registereditorn genom att köra regedit.exe i en förhöjd behörighet. <li> Identifiera alla versioner av. NetFramework finns i systemet. De finns under hierarkin för registernyckeln ”HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft” <li> För var och en. NetFramework finns i registernyckeln, lägga till följande nyckel: <br> "SchUseStrongCrypto"=dword:00000001 </ol>|
...
| Backup-valvet och målresurserna lagringskonto finns i två olika regioner – Kontrollera att lagringskontot som angetts i återställningsåtgärden finns i samma Azure-region som säkerhetskopieringsvalvet. |Ingen |
...
* Installera den senaste från lagringsplatsen för distribution. Vi **rekommenderar** agent installeras endast via distribution lagringsplats. Information om hur paketnamn, finns i [lagringsplats för Linux-agent](https://github.com/Azure/WALinuxAgent) 
* För klassiska virtuella datorer, [uppdatera VM-egenskapen](http://blogs.msdn.com/b/mast/archive/2014/04/08/install-the-vm-agent-on-an-existing-azure-vm.aspx) att indikera att agenten är installerad. Det här steget krävs inte för Resource Manager-datorer.

### <a name="updating-the-vm-agent"></a>Uppdatera VM-agenten
För Windows-datorer:

* Det är enkelt att uppdatera VM-agenten. Du installerar bara om [binärfilerna för VM-agenten](http://go.microsoft.com/fwlink/?LinkID=394789&clcid=0x409). Du måste dock se till att ingen säkerhetskopieringsåtgärd körs medan VM-agenten uppdateras.

För virtuella Linux-datorer:

* Följ anvisningarna på [uppdaterar Linux VM-agenten](../virtual-machines/linux/update-agent.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json).
Vi **rekommenderar** uppdaterar agenten endast via distribution lagringsplats. Vi rekommenderar inte ladda ned agenten kod från github direkt och uppdaterar den. Om den senaste versionen av agenten inte är tillgänglig för din distribution, kontaktar du distributionsplatsen stöd för instruktioner om hur du installerar senaste agenten. Du kan kontrollera senaste [Windows Azure Linux-agent](https://github.com/Azure/WALinuxAgent/releases) information i github-lagringsplats.

### <a name="validating-vm-agent-installation"></a>Verifiera installationen av VM-agenten
Hur du kontrollerar VM-agentversionen på virtuella Windows-datorer:
...
2. Virtuella datorer med Sql Server säkerhetskopiering har konfigurerats kan orsaka fördröjning för uppgift för ögonblicksbild <br>
   Som standard säkerhetskopieringsproblem VSS fullständig säkerhetskopiering av virtuella datorer på virtuella Windows-datorer. Detta kan orsaka fördröjning i ögonblicksbildskörning på virtuella datorer som kör Sql-servrar och om säkerhetskopiering av Sql Server har konfigurerats. Ange följande registernyckel om du upplever Säkerhetskopieringsfel på grund av problem med ögonblicksbild.
...
3. VM-status rapporteras felaktigt eftersom Virtuella datorn stängs av i RDP.  <br>
   Om du har stänger av den virtuella datorn i RDP, kontrollera tillbaka i portalen att VM-statusen avspeglas korrekt. Om inte, Stäng den virtuella datorn i portalen med alternativet ”avslutning' i VM-instrumentpanelen.
4. Om fler än fyra virtuella datorer delar samma molntjänst kan du konfigurera flera principer för säkerhetskopiering för att mellanlagra säkerhetskopieringstider så inga fler än fyra säkerhetskopior av virtuella datorer startas på samma gång. Försök att sprida säkerhetskopiering starttider timmen sig mellan principer.
5. Virtuella datorn körs på hög processor/minne.<br>
   Om den virtuella datorn körs på hög CPU usage(>90%) eller minne, uppgiften ögonblicksbilder är i kö, fördröjd och kommer så småningom hämtar tog för lång tid. Prova säkerhetskopiering på begäran i sådana situationer.
