...
| ------ | --- |
| Backup kunde inte utföra åtgärden eftersom den virtuella datorn (VM) finns inte längre: <br>Sluta skydda den virtuella datorn utan att ta bort säkerhetskopierade data. Mer information finns i [sluta skydda virtuella datorer](http://go.microsoft.com/fwlink/?LinkId=808124). |Det här felet inträffar när den primära virtuella datorn tas bort, men fortfarande verkar principen för säkerhetskopiering för en virtuell dator att säkerhetskopiera. Om du vill åtgärda det här felet, gör du följande: <ol><li> Återskapa den virtuella datorn med samma namn och samma resursgruppnamn **molntjänstnamnet**,<br>**eller**</li><li> Sluta skydda den virtuella datorn med eller utan att ta bort säkerhetskopierade data. Mer information finns i [sluta skydda virtuella datorer](https://go.microsoft.com/fwlink/?LinkId=808124).</li></ol> |
| Ögonblicksbildsåtgärden misslyckades eftersom det fanns ingen nätverksanslutning på den virtuella datorn: <br>Kontrollera att den virtuella datorn har nätverksåtkomst. För ögonblicksbilden ska lyckas, antingen godkänd Azure datacenter IP-intervall eller konfigurera en proxyserver för nätverksåtkomst. Mer information finns i [felsöka Azure Backup-fel: Problem med agenten eller tillägget](http://go.microsoft.com/fwlink/?LinkId=800034). <br><br>Om du redan använder en proxyserver kan du kontrollera att proxyserverinställningarna är rätt konfigurerade. | Det här felet inträffar när du neka utgående internet-anslutning på den virtuella datorn. Tillägget för ögonblicksbild av virtuell dator kräver Internetanslutning för att ta en ögonblicksbild av underliggande diskar. Se [ExtensionSnapshotFailedNoNetwork](backup-azure-troubleshoot-vm-backup-fails-snapshot-timeout.md#ExtensionSnapshotFailedNoNetwork-snapshot-operation-failed-due-to-no-network-connectivity-on-the-virtual-machine). |
| Azure VM-agenten (VM-Agent) inte kan kommunicera med Azure Backup-tjänsten: <br>Kontrollera att den virtuella datorn är ansluten till nätverket och VM-agenten är senast och körs. Mer information finns i [felsöka Azure Backup-fel: Problem med agenten eller tillägget](http://go.microsoft.com/fwlink/?LinkId=800034). |Felet kan inträffa om det uppstår ett problem med VM-agenten eller nätverksåtkomst till Azure-infrastrukturen är blockerad på något sätt. Läs mer om [felsökning VM ögonblicksbild problem](backup-azure-troubleshoot-vm-backup-fails-snapshot-timeout.md#UserErrorGuestAgentStatusUnavailable-vm-agent-unable-to-communicate-with-azure-backup). <br><br>Om den Virtuella Datoragenten inte orsakar problem kan du starta om den virtuella datorn. Ett felaktigt tillstånd för virtuell dator kan orsaka problem och starta om den virtuella datorn återställer tillståndet. |
...
| Den **VMSnapshot** tilläggsåtgärden misslyckades för hanterade diskar: <br>Försök att säkerhetskopiera igen. Om problemet kvarstår, följ instruktionerna i [felsöka Azure Backup-fel](http://go.microsoft.com/fwlink/?LinkId=800034). Kontakta Microsoft-supporten om problemet kvarstår. | Det här felet uppstår när Backup-tjänsten inte kan utlösa en ögonblicksbild. Läs mer om [felsökning VM ögonblicksbild problem](backup-azure-troubleshoot-vm-backup-fails-snapshot-timeout.md#ExtentionOperationFailed-vmsnapshot-extension-operation-failed). |
...
| Recovery services-tilläggsåtgärden misslyckades: <br>Kontrollera att den senaste VM-agenten finns på den virtuella datorn och VM-agenttjänsten körs. Försök att säkerhetskopiera igen. Om säkerhetskopieringen misslyckas, kontaktar du Microsoft Support. |Det här felet inträffar när VM-agenten är inaktuell. Referera till [felsöka Azure VM backup](#updating-the-VM-agent) att uppdatera VM-agenten. |
...
| Backup-tjänsten har inte behörighet att komma åt resurser i din prenumeration. |För att lösa det här felet kan återställa diskar med hjälp av stegen i [återställa säkerhetskopierade diskar](backup-azure-arm-restore-vms.md#restore-backed-up-disks). Därefter använder du PowerShell steg i [skapa en virtuell dator från återställda diskar](backup-azure-vms-automation.md#create-a-vm-from-restored-disks) skapa en fullständig virtuell dator från återställda diskar. |

## <a name="backup-or-restore-takes-time"></a>Säkerhetskopieringen eller återställningen tar tid
Om din säkerhetskopia tar mer än 12 timmar eller återställning tar mer än 6 timmar:
* Förstå [faktorer som bidrar till säkerhetskopieringstiden](backup-azure-vms-introduction.md#total-vm-backup-time) och [faktorer som bidrar till återställningstid](backup-azure-vms-introduction.md#total-restore-time).
...
- Listan över godkända Azure-datacenter IP-intervall:
