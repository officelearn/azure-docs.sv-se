
# [Dokumentation om Batch](index.yml)

# Översikt
## [Vad är Azure Batch?](batch-technical-overview.md)

# Snabbstarter
## [Köra ett Batch-jobb – CLI](quick-create-cli.md)
## [Köra ett Batch-jobb – Portal](quick-create-portal.md)
## [Köra ett Batch-jobb – .NET](quick-run-dotnet.md)
## [Köra ett Batch-jobb – Python](quick-run-python.md)


# Självstudier

## [Parallell filbearbetning – .NET](tutorial-parallel-dotnet.md)
## [Parallell filbearbetning – Python](tutorial-parallel-python.md)
## [Scenrendering med Arnold](tutorial-rendering-cli.md)
## [Parallell R-simulering](tutorial-r-doazureparallel.md)

# Exempel
## Kodexempel
### [Azure-kodexempel](https://azure.microsoft.com/resources/samples/?service=batch)
### [Lagringsplats för Batch-exempel](https://github.com/Azure/azure-batch-samples)
## [Azure CLI](cli-samples.md)

# Begrepp
## [Utvecklarfunktioner](batch-api-basics.md)
## [API:er och verktyg](batch-apis-tools.md)
## [Kvoter och begränsningar](batch-quota-limit.md)

# Instruktionsguider
## Hantera Batch-konton
### [Hantera Batch-konton med portalen](batch-account-create-portal.md)
### [Hantera Batch-konton med Batch Management .NET](batch-management-dotnet.md)

## Autentisera med Azure AD
### [Azure AD med Batch-tjänsten](batch-aad-auth.md)
### [Azure AD med Batch Management](batch-aad-auth-management.md)

## [Skapa programpaket](batch-application-packages.md)

## Skapa och hantera pooler
### [Automatisk skalning för beräkningsnoder](batch-automatic-scaling.md)
### [Välja en VM-storlek för beräkningsnoder](batch-pool-vm-sizes.md)
### [Konfigurera åtkomst till beräkningsnoder](pool-endpoint-configuration.md)
### [Skapa en pool med en anpassad bild](batch-custom-images.md)
### [Skapa en pool i ett virtuellt nätverk](batch-virtual-network.md)
### [Använda beräkningsintensiva virtuella datorer](batch-pool-compute-intensive-sizes.md)
### [Använda Linux-beräkningsnoder](batch-linux-nodes.md)
### [Använda lågprioriterade virtuella datorer](batch-low-pri-vms.md)

## Hantera jobb och uppgifter
### [Aktiviteter för jobbförberedelse och -slutförande](batch-job-prep-release.md)
### [Samtidiga nodaktiviteter](batch-parallel-node-tasks.md)
### [Räkna aktiviteter efter tillstånd](batch-get-task-counts.md)
### [Aktivitetsberoenden](batch-task-dependencies.md)
### [Användarkonton för att köra aktiviteter](batch-user-accounts.md)
### [Kör batchjobb utan kod](batch-cli-templates.md)


## [Bevara jobb- och uppgiftsutdata](batch-task-output.md)
### [Spara utdata med Batch-API](batch-task-output-files.md)
### [Spara utdata med File Conventions-bibliotek](batch-task-output-file-conventions.md)

## Köra arbetsbelastningar
### [MPI-jobb](batch-mpi.md)
### [Behållararbetsbelastningar](batch-docker-container-workloads.md)
### [Rendering](batch-rendering-service.md)
### [Databearbetning med Batch och Data Factory](../data-factory/v1/data-factory-data-processing-using-batch.md?toc=%2fazure%2fbatch%2ftoc.json)

## Övervaka resurser
### [Diagnostikloggning](batch-diagnostics.md)
### [Effektiv lista: frågor](batch-efficient-list-queries.md)

## Använda utvecklarverktyg
### [Använda .NET för Batch](batch-dotnet-get-started.md)
### [Använda Python för Batch](batch-python-tutorial.md)
### [Använda Node.js för Batch](batch-nodejs-get-started.md)
### [Använda PowerShell-cmdletar](batch-powershell-cmdlets-get-started.md)
### [Använda Azure CLI](batch-cli-get-started.md)

# Referens

## [Azure CLI](/cli/azure/batch)
## [Azure PowerShell](/powershell/module/azurerm.batch)
## [.NET](/dotnet/api/overview/azure/batch)
## [Java](/java/api/overview/azure/batch)
## [Node.js](/javascript/api/overview/azure/batch)
## [Python](/python/api/overview/azure/batch)
## REST
### [Batch-tjänst](/rest/api/batchservice)
### [Batch Management](/rest/api/batchmanagement/)
## [Batchanalys](batch-analytics.md)
### [Händelse för skapande av pool](batch-pool-create-event.md)
### [Starthändelse för borttagning av pool](batch-pool-delete-start-event.md)
### [Sluthändelse för borttagning av pool](batch-pool-delete-complete-event.md)
### [Starthändelse för storleksändring av pool](batch-pool-resize-start-event.md)
### [Sluthändelse för storleksändring av pool](batch-pool-resize-complete-event.md)
### [Starthändelse för uppgift](batch-task-start-event.md)
### [Sluthändelse för uppgift](batch-task-complete-event.md)
### [Händelse för misslyckad uppgift](batch-task-fail-event.md)
## [Miljövariabler för Batch-beräkningsnod](batch-compute-node-environment-variables.md)

# Resurser
## [Azure-översikt](https://azure.microsoft.com/roadmap/)
## [Batch-forum](https://social.msdn.microsoft.com/Forums/en-us/home?forum=azurebatch)
## [Batch Shipyard](https://github.com/Azure/batch-shipyard)
## [Blogg](https://blogs.technet.microsoft.com/windowshpc/)
## [HPC-lösningar i Azure](../virtual-machines/linux/high-performance-computing.md?toc=%2fazure%2fbatch%2ftoc.json)
## [Prissättning](https://azure.microsoft.com/pricing/details/batch/)
## [Priskalkylator](https://azure.microsoft.com/pricing/calculator/)
## [Resource Manager-mall](/azure/templates/microsoft.batch/batchaccounts)
## [Tjänstuppdateringar](https://azure.microsoft.com/updates/?product=batch&updatetype=&platform=)
## [Stack Overflow](http://stackoverflow.com/questions/tagged/azure-batch)
## [Videoklipp](https://azure.microsoft.com/documentation/videos/index/?services=batch)


