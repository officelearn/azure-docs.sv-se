...
1. Kör `cmdkey` kommando rads verktyget med en start uppgift i konfigurationen för poolen. Detta behåller autentiseringsuppgifterna på varje Windows-nod. Kommando raden start uppgift liknar:
...
När du har lagrat autentiseringsuppgifterna använder du åtgärds kommando raderna för att montera resursen och referera till resursen i Läs-eller Skriv åtgärder. Som ett grundläggande exempel använder aktivitetens kommando rad i följande kodfragment `dir` kommandot för att lista filer i fil resursen. Se till att köra varje jobb aktivitet med samma [användar identitet](batch-user-accounts.md) som du använde för att köra start uppgiften i poolen. 
