...
## <a name="split-a-single-reserved-instance-into-two-reserved-instances"></a>Dela en enda instans som är reserverade i två reserverade instanser
 När du köper fler än en instans kan du vill tilldela olika prenumerationer instanser inom ett reserverat instans. Som standard är alla instanser (antal anges vid köpet) har ett scope - antingen enda prenumeration eller delad. Till exempel du köpt 10 Standard D2 VMs och ange omfånget för prenumerationen A. Du kan nu vill ändra omfattningen för sju reserverade instanser till prenumeration A och återstående 3 till B. dela en reserverad instans-prenumeration kan du distribuera instanser för detaljerade omfångshantering. Du kan förenkla tilldelning till prenumerationer genom att välja delade scope. Men du kan allokera kvantiteter till specifika prenumerationer för kostnad hanterings- eller budget är.
...
2. Hämta information om reserverade instans:
    ...
    ```
3. Reserverade instansen i två delar och distribuera instanser:
...
Du kan delegera hanteringen av ett reserverat instans genom att lägga till personer roller på reserverade-instansen. Som standard personen som har köpt reserverade instansen och kontoadministratören har rollen som ägare på reserverade-instansen. 
...
1.  Logga in på [Azure-portalen](https://portal.azure.com).
2.  Välj **alla tjänster** > **Reservation** att lista reserverade instanser som du har åtkomst till.
