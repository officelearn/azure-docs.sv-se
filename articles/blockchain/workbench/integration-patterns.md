...
ms.locfileid: "54330475"
---
# <a name="smart-contract-integration-patterns"></a>Smarta kontrakt integration mönster

Smarta kontrakt representerar ofta arbetsflöden inom företagen som krävs för att integrera med externa system och enheter.
...
REST-API och meddelanden integration skickar transaktioner från externa system till smarta kontrakt som ingår i ett program med Azure Blockchain Workbench. Den skickar också händelsemeddelanden till externa system baserat på ändringar som äger rum inom ett program.
...
Det här avsnittet tittar vi på mönster som fokuserar på de aspekter av REST-API som skickar transaktioner till en distribuerad redovisning och mönster fråga data om transaktioner från Azure Blockchain Workbench *av kedjan* SQL-databas.
...
Azure Blockchain Workbench REST API skickar autentiserade begäranden att köra transaktioner på en distribuerad redovisning.

![Skicka transaktioner till en distribuerad redovisning](./media/integration-patterns/send-transactions-ledger.png)

Köra transaktioner sker med hjälp av processen beskrivs tidigare var:
...
-   REST API-paket på begäran som ett meddelande och skickar den till en Service Bus. Här är det hämtas, registrerat och skickas till lämplig distribuerat transaktionsregister.
...
Azure Blockchain Workbench REST API skickar autentiserade begäranden till Frågedetaljer som rör smarta kontrakt körning på en distribuerad redovisning.

![Fråga metadata](./media/integration-patterns/querying-metadata.png)

Frågor sker med hjälp av processen beskrivs tidigare var:
...
Meddelanden integration underlättar interaktion med system, tjänster och enheter där en interaktiv inloggning inte är möjligt eller önskvärt. Integrering av meddelanden som fokuserar på två typer av meddelanden: meddelanden begär transaktioner köras på en distribuerad redovisning och händelser som visas av att när transaktioner har ägt rum.
...
Det här avsnittet tittar vi på mönster som fokuserar på aspekter av meddelandebaserat API som skickar transaktioner till en distribuerad redovisning och mönster som motsvarar händelsemeddelanden som exponeras av den underliggande distribuerat transaktionsregister.
...
Ett exempel på det här scenariot är att när en transaktion utförs en konsument skulle bli aviserad om och kan vidta åtgärder, till exempel spela in informationen i en SQL-databas eller Common Data Service. Det här scenariot är samma mönster som arbetsstationen följer för att fylla i dess *av kedjan* SQL DB.
...
Det här scenariot sker med hjälp av processen beskrivs tidigare var:
...
Den här leverans sker med hjälp av processen beskrivs tidigare var:

-   En händelse inträffar på ett externt system som utlöser skapandet av ett meddelande för Azure Blockchain Workbench.
-   Det externa systemet har kod som skrivs till att skapa det här meddelandet i ett känt format och skickar dem direkt till Service Bus.
...
Detta sker med hjälp av processen beskrivs tidigare var:
...
Beroende på möjligheten att anpassa det externa systemet kan det kanske eller kanske inte att leverera meddelanden i ett standardformat som förväntar sig att Azure Blockchain Workbench. Baserat på externa system kan generera en av dessa meddelanden fastställa vilket av följande två returnera sökvägar är upptaget.

##### <a name="direct-delivery-of-an-azure-blockchain-workbench-in-the-expected-format"></a>Direkt leverans av en Azure Blockchain Workbench det förväntade formatet

![](./media/integration-patterns/direct-delivery.png)

I den här modellen kommunikationen till kontraktet och efterföljande tillståndsändring sker följande på att föregående process där -
...
-   Innehållet i meddelandet paketeras och skickas till en viss funktion i smarta kontrakt. Den här leveransen görs åt användaren som är associerade med det externa systemet.
...
I den här modellen där ett meddelande i ett standardformat inte kan skickas direkt, kommunikation till kontraktet och efterföljande tillståndsändring sker efter föregående bearbeta var:
...
6. Innehållet i meddelandet paketeras och skickas till en viss funktion i smarta kontrakt. Den här leveransen görs åt användaren som är associerade med det externa systemet.
...
På Azure-plattformen görs integrering med enheter normalt med IoT Hub. IoT Hub ger meddelanderoutning baserat på innehåll och gör det möjligt för typ av funktion som beskrivs ovan.

![IoT-meddelanden](./media/integration-patterns/iot.png)

Hur du visar ett mönster:
...
Många scenarier kan kräva behovet av att införliva attestable filer. Flera skäl är det olämpligt att skicka filer till en blockkedja. I stället är en vanlig metod att utföra en kryptografisk hash (till exempel SHA-256) mot en fil och dela den hashvärdet för en distribuerad redovisning. Utför hashen igen när som helst framtida ska returnera samma resultat. Om filen ändras även om bara en bildpunkt ändras i en bild, returnerar hashen ett annat värde.
...
2. En eller flera lämpliga smarta kontrakt för Azure Blockchain Workbench-programmet har funktioner som definierats för att acceptera händelser från din externa system eller enhet.
3. Programmets konfigurationsfil för dina smarta kontrakt innehåller rollen som systemet eller enheten har tilldelats.
4. Programmets konfigurationsfil för dina smarta kontrakt identifierar i vilka tillstånd som den här funktionen anropas av rollen definierad.
...
Integrera med externa system är ett krav för många scenarier. Det är önskvärt att kunna verifiera smarta kontrakt design tidigare eller parallellt till utvecklingen av kod för att integrera med externa system.

Användning av Azure Active Directory (Azure AD) kan avsevärt snabbare produktivitet för utvecklare och tid till värde. Mer specifikt kan code-integrering med ett externt system ta en icke-trivialt tidsperiod. Med hjälp av Azure AD och den automatisk genereringen av UX genom Azure Blockchain Workbench, kan du att utvecklare kan logga in på Blockchain Workbench som det externa systemet och Fyll i värden från det externa systemet via UX. Du kan snabbt utveckla och validera idéer i en proof of concept-miljö innan integration kod skrivs för externa system.
