...
- [Cachenyckel omskrivning](cdn-rules-engine-reference-features.md#cache-key-rewrite)
...
| Kontinent (begär) | % {geo_continent} | Anger den som begär kontinent via en förkortning. <br />Giltiga värden är: <br />AF: Afrika<br />AS: Asien<br />EU: Europa<br />NA: Nordamerika<br />OC: Oceanien<br />SA: Sydamerika<br /><br />**Inaktuella:** % {virt_dst_continent}. <ber />Den här variabeln är inaktuell och ersatts med % {geo_continent}. <br />Även om en regel som använder den här föråldrad variabeln fortsätter att fungera, bör du uppdatera den om du vill använda den nya variabeln.| Gäller inte |
...
| Port (begär) | % {virt_dst_port} | Anger den som begär tillfällig port. | 55885 |
...
| Parametern för frågesträngen hittades | % {is_amp} | Värdet för den här variabeln varierar beroende på om begäran innehåller minst en frågesträngsparameter.<br /><br />-Parametern hittades: &<br />-Inga parametrar: NULL | & |
| Parametern för Frågesträngsvärdet | % {arg_&lt;parametern&gt;} | Returnerar värdet för frågesträngparametern som identifieras av den &lt;parametern&gt; termen. | Exempel: <br />% {arg_language}<br /><br />Exemplet frågesträngparametern: <br />? språk = SV<br /><br />Exempel på värde: en |
| Frågesträngsvärdet | % {query_string} | Anger hela frågesträngsvärdet som definierats i fråge-URL. |key1 = val1 & key2 = val2 & key3 = val3 |
| Referentdomän | % {referring_domain} | Anger den domän som definierats i Referer rubriken. | www.google.com |
...
| Begär värden | % {värden} | Anger värden som definierats i fråge-URL. | www.mydomain.com |
...
| ------ | -------- | ---------- |
| %{&lt;HTTPVariable&gt;} | % {värden} | Använd den här syntaxen för att få hela värdet för den angivna &lt;HTTPVariable&gt;. |
...
| --------- | ----------- | --------|
| Undantagstecken % symbol | Symbolen procent kan undantas genom att använda ett omvänt snedstreck. <br />Exempelvärde till höger behandlas som ett exakt värde och inte som en HTTP-variabel.| \%{värden} |
...
| Överskridande tecken | Tecken som löper en variabel behandlas som exakta värden. <br />Exempelvärde till höger innehåller en avslutande klammerparentes som kommer att behandlas som ett literalvärde. | % {värden}} |
...
| --------- | ------ | --------| ----------- |
| Ange en rubrik till ett standardvärde när den uppfyller något av följande villkor: <br /><br />-Huvud saknas <br /><br />-Huvudets värde anges till NULL.| % {Variabel: = Value} | % {http_referer: = okänt} | Referer-rubrik kommer endast anges till *Ospecificerad* när den är saknas eller är inställd på NULL. Ingen åtgärd ska ske om den har ställts in. |
| Ange en rubrik till ett standardvärde när det saknas. | % {Variabel = Value} | % {http_referer = okänt} | Referer-rubrik kommer endast anges till *Ospecificerad* när det saknas. Ingen åtgärd ska ske om den har ställts in. |
| Ange rubriken till ett standardvärde när den inte uppfyller något av följande villkor: <br /><br />-Saknas<br /><br /> -Värdet NULL. | % {Variabel: värdet +} | % {http_referer: + Ospecificerad} | Referer-rubrik kommer endast anges till *Ospecificerad* när ett värde har tilldelats till den. Ingen åtgärd utförs om den saknas eller null. |
...
- Värdet som tilldelas förskjutning termen avgör det första tecknet delsträngens:
     ...
     - Noll: Det första tecknet delsträngens är det första tecknet i strängen.
...
- Delsträngens längd bestäms av den *längd* termen:
     ...
     - Positivt: Anger den understräng från det första tecknet till höger.
     - Negativt: Anger den understräng från det första tecknet till vänster.
...
| % {Variabeln ^ hitta} | Konvertera den första förekomsten av det angivna mönstret till versaler. |
