...
[Azure PowerShell](/powershell/azure/overview) är ett av de snabbaste och mest kraftfulla sätt att administrera dina Azure-tjänster. Använd den `Get-AzStorageBlob` cmdlet för att hämta en referens till bloben, ange den `.ICloudBlob.Properties.CacheControl` egenskapen. 
...
Ange en blob `Cache-Control` rubrik med hjälp av .NET-kod, Använd den [Azure Storage-klientbiblioteket för .NET](../storage/blobs/storage-dotnet-how-to-use-blobs.md) att ställa in den [CloudBlob.Properties.CacheControl](/dotnet/api/microsoft.azure.storage.blob.blobproperties.cachecontrol) egenskapen.
