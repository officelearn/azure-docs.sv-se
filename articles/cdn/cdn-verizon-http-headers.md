...
Om du vill förhindra att någon av dessa reserverade huvuden läggs till i Azure CDN (Content Delivery Network) POP-begäran till ursprungs servern, måste du skapa en regel med [funktionen för Special rubriker för proxy](cdn-verizon-premium-rules-engine-reference-features.md#proxy-special-headers) i regel motorn. I den här regeln utelämnar du rubriken som du vill ta bort från standard listan över rubriker i fältet rubriker. Om du har aktiverat [funktionen Felsök cache](cdn-verizon-premium-rules-engine-reference-features.md#debug-cache-response-headers)-svarshuvuden, se till att lägga till de nödvändiga `X-EC-Debug` rubrikerna. 

Om du t. ex. `Via` vill ta bort rubriken ska rubrik fältet i regeln innehålla följande lista över huvuden: *X-forwarded – for, x-forwardd-proto, x-Host, x-Midgress, x-Gateway-List, x-EG-Name, Host*. 
