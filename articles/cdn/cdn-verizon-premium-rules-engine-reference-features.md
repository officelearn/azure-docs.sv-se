---
title: Azure CDN från Verizon Premium regelmotor – funktioner | Microsoft Docs
...
Den här artikeln innehåller detaljerade beskrivningar av tillgängliga funktioner för Azure Content Delivery Network (CDN) [regelmotor](cdn-verizon-premium-rules-engine.md).
...
## <a name="azure-cdn-from-verizon-premium-rules-engine-features-reference"></a>Azure CDN från Verizon Premium regler modulreferens funktioner
...
**Syfte:** Anger om bandbredd begränsning parametrarna (till exempel ec_rate och ec_prebuf) är aktiva.
...
**Syfte:** Anger uppsättningen ytterligare HTTP-metoder som kan cachelagras i nätverket.
...
--|--
 ...
 Exkludera | Anger att endast de angivna parametrarna är exkluderad från cache-nyckeln. Alla andra parametrar för frågesträngen som ingår i cache-nyckeln.
...
- **Typ:** Exkludera
- **Parametrar:** sessioned användar-ID
...
**Standardbeteende:** En begäran cachenyckel bestäms av begärande-URI.
...
**Syfte:** Anger om ett svar kan innehålla [X-EG-Debug svarshuvuden](cdn-http-debug-headers.md), som innehåller information om cache-principen för den begärda tillgången.
...
**Syfte:** Anger tidsintervallet maxåldern för webbläsaren till POP cache omverifieringen. Med andra ord, hur lång tid som ska förflyta innan en webbläsare kan söka efter en ny version av en tillgång från en POP.
...
**Syfte:** Anger om begäranden kan omdirigeras till värdnamn som definierats i Location-huvudet som returneras av en kund ursprungsservern.
...
**Syfte:** Anger max-age-intervall för POP till ursprunget server cache omverifieringen. Med andra ord, hur lång tid som ska förflyta innan en POP kan kontrollera om en cachelagrad tillgång matchar den tillgång som lagras på den ursprungliga servern.
...
**Standardbeteende:** MP4 och F4V media som standard har stöd för HTTP-progressiv nedladdning.
...
**Syfte:** Anger om en HTTP-klient no-cache-begäranden ska vidarebefordras till den ursprungliga servern.
...
**Standardbeteende:** Standardinställningen är att respektera ovannämnda direktiv.
...
**Syfte:** Anger svaret som returneras till klienter när en förfrågan genererar statuskod 416 begärda intervallet inte Satisfiable.
...
**Syfte:** Kontrollerar hur länge de senaste normala förfallotiden en cachelagrad tillgång kan hanteras från en POP när POP inte kan verifiera den cachelagrade tillgången med den ursprungliga servern.
...
**Syfte:** Anger om en frågesträng kommer att lagras tillsammans med URL: en i åtkomstloggar.
...
**Syfte:** Anger om cachelagrat innehåll ska vara berättigade tidig Omverifiering innan dess TTL upphör att gälla.
...
**Standardbeteende:** Inaktivera. Omverifieringen kan endast ske när det cachelagrade innehållet TTL har gått ut.
...
**Syfte:** Definierar uppsättningen med [Verizon-specifika HTTP-begärans sidhuvud](cdn-verizon-http-headers.md) som vidarebefordras från en POP till en ursprungsservern.
...
**Syfte:** Anger hur en HTTP-klientens begäran för en tillgång på 0 byte-cache hanteras av POP: erna.
...
**Syfte:** Definierar uppsättningen med statuskoder som kan resultera i cachelagrat innehåll.
...
**Standardbeteende:** Cachelagring är bara aktiverat för svar som genererar en 200 OK statuskod.
...
- Den här funktionen fungerar beror enligt den valda enheten.
    ...
    - **Av:** Välj ”Off” för att kräva validering innan en begäran för inaktuella innehåll kan betjänas.
        - Ange hur lång tid eftersom det är och kommer att ignoreras.

**Standardbeteende:** Inaktivera. Omverifieringen måste ske innan det begärda innehållet kan hanteras.
...
**Syfte:** Anger om tokenbaserad autentisering kommer att tillämpas på en begäran.
...
**Syfte:** Anger typ av svar som ska returneras till en användare när en begäran nekas på grund av tokenbaserad autentisering.
...
302|Hitta|Den här statuskoden omdirigerar obehöriga användare till den URL som anges i Location-huvudet. Den här statuskoden är bransch standardmetoden för att utföra en omdirigering.
...
404|Det gick inte att hitta filen|Den här statusen indikerar att HTTP-klienten kunde kommunicera med servern, men det gick inte att hitta det begärda innehållet.
...
**Syfte:** Anger om URL-jämförelser som gjorts av tokenbaserad autentisering är skiftlägeskänsliga.
...
**Syfte:** Anger om frågesträngparametern för tokenbaserad autentisering ska ändras.
