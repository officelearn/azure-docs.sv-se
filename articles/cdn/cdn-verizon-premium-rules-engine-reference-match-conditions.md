...
Namn | Syfte
-----|--------
[Anordningar](#device) | Identifierar förfrågningar som gjorts från en mobil enhet baserat på dess egenskaper.
...
- Begärd URI använder typen av innehålls åtkomst punkt (till exempel/000001) som har definierats i detta matchnings villkor:
  - CDN-URL: Begärd URI måste innehålla den valda innehålls åtkomst punkten.
  - Edge CNAME-URL: Motsvarande Edge CNAME-konfiguration måste peka på den valda innehålls åtkomst punkten.
...
- Ange flera IP-adresser och/eller IP-adressblock genom att begränsa var och en av dem med ett enda blank steg. Exempel:
...
- **Matchningar**: Kräver att en begäran innehåller angiven cookie med ett värde som matchar minst ett av de värden som har definierats i matchnings villkoret.
...
- Cookie-värde:
  ...
  - Ett cookie-värde kan dra nytta [](cdn-verizon-premium-rules-engine-reference.md#wildcard-values)av jokertecken.
...
- **Matchningar**: Kräver en begäran om att innehålla angiven cookie med ett värde som matchar det angivna reguljära uttrycket.
- **Matchar inte**: Kräver att begäran uppfyller något av följande villkor:
  - Den innehåller inte den angivna cookien.
...
- Matchning av jokertecken för URL-sökväg: Ange [URL-sökvägen matchnings villkor](#url-path-wildcard) för jokertecken till den katalog som ska skyddas. 
...
- Lands matchning: Ange landets matchnings villkor till önskad uppsättning länder.
  - Tillåt Ange villkoret för land matchning till stämmer **inte överens** för att endast tillåta att de angivna länderna har åtkomst till innehåll som lagras på den plats som definieras av URL-sökvägen matchnings villkor för jokertecken.
  - Undantaget Ange villkoret för land matchning till matchningar för att blockera de angivna länderna från åtkomst till innehåll som lagras på den plats som definieras av matchnings villkoret för URL-sökvägen.

- Neka åtkomst (403) funktion: Aktivera [funktionen neka åtkomst (403)](cdn-verizon-premium-rules-engine-reference-features.md#deny-access-403) för att replikera funktionen för att tillåta eller blockera del av funktionen land filtrering.
...
Operativsystemsversion för enhet | %{wurfl_cap_device_os_version} | En sträng som anger versions numret för det operativ system som är installerat på enheten. | 1.0.1
Dubbel orientering | %{wurfl_cap_dual_orientation} | Ett booleskt värde som anger om enheten stöder dubbel orientering. | true
HTML-föredragen DTD | %{wurfl_cap_html_preferred_dtd} | En sträng som anger den mobila enhetens primära dokument typ definition (DTD) för HTML-innehåll. | inga<br/>xhtml_basic<br/>html5
Bild inlägg | %{wurfl_cap_image_inlining} | Ett booleskt värde som anger om enheten stöder base64-kodade avbildningar. | false
...
Modellnamn | %{wurfl_cap_model_name} | En sträng som anger enhetens modell namn. | s3
...
Utgivningsdatum | %{wurfl_cap_release_date} | En sträng som anger året och månaden då enheten lades till i WURFL-databasen.<br/><br/>Format: `yyyy_mm` | 2013_december
...
- **Matchningar**: Kräver att det refererande värd namnet matchar de angivna värdena. 
...
- Matchnings villkoret [](cdn-verizon-premium-rules-engine-reference.md#wildcard-values)har stöd för jokertecken.
...
- En Sök vägs specifikation för URL kan dra [](cdn-verizon-premium-rules-engine-reference.md#wildcard-values)nytta av jokertecken.
...
- **Matchningar**: URL: en för begäran måste innehålla ett fil namns tillägg som exakt matchar det angivna mönstret.

   Om du till exempel anger "htm", matchas "htm" till gångar, men inte "HTML"-till gångar.  

- **Matchar inte**: Kräver att URL-begäran innehåller ett fil namns tillägg som inte matchar det angivna mönstret.
...
- Ett fil namns värde kan dra nytta [](cdn-verizon-premium-rules-engine-reference.md#wildcard-values)av jokertecken. Varje fil namns mönster kan till exempel bestå av en eller flera asterisker (*), där varje asterisk matchar en sekvens med ett eller flera tecken.
...
- Värdet som anges för en URL-sökväg kan dra nytta [](cdn-verizon-premium-rules-engine-reference.md#wildcard-values)av jokertecken. Varje URL Path-mönster kan innehålla en eller flera asterisker (*), där varje asterisk kan matcha en sekvens med ett eller flera tecken.
...
- **Matchningar**: Kräver att en begäran innehåller den angivna parametern med ett värde som matchar minst ett av de värden som har definierats i matchnings villkoret.
- **Matchar inte**: Kräver att begäran uppfyller något av följande villkor:
...
Detta matchnings villkor är ett enkelt sätt att ange parameter namn/värde-kombinationer. Om du vill ha mer flexibilitet om du matchar en frågesträngparametern kan du använda [URL](#url-query-wildcard) -frågans jokertecken matchnings villkor.
...
Email | Johan\* | Det här mönstret matchas när frågesträngen för en begärd URL innehåller en e-postparameter som börjar med "Johan".
...
- **Matchningar**: Kräver att begäran innehåller en URL-frågesträng som matchar det angivna wildcard-värdet.
- **Matchar inte**: Kräver att begäran innehåller en URL-frågesträng som inte matchar det angivna wildcard-värdet.
...
- Ange flera värden genom att begränsa var och en med ett enda blank steg.
   ...
   Exempel: *Parameter1 = värdea* *Värdeb* *Parameter1 = ValueC & parameter2 = värde*
