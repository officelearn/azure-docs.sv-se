...
|enableNativeCodeExecution|boolean|Valfritt. Standardvärdet är `true`; inbyggd kod körning och fullständigt förtroende är aktiverat som standard. Ange det här attributet `false` till om du vill inaktivera intern kod körning för webb rollen och använda Azure partiellt förtroende i stället.|  
...
|protokollhanterare|sträng|Krävs. Transport protokollet för den externa slut punkten. `HTTP`För en webb roll är möjliga värden, `HTTPS` `UDP`, eller. `TCP`|  
