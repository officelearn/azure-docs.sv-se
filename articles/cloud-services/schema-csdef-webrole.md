...
|enableNativeCodeExecution|Booleskt värde|Valfri. Standardvärdet är `true`; intern körning och fullständigt förtroende är aktiverade som standard. Ange det här attributet `false` inaktivera körning av intern kod för webbrollen och i stället använda Azure partiellt förtroende.|  
...
|Protokollet|Sträng|Krävs. Transportprotokollet för den externa slutpunkten. För en webbroll möjliga värden är `HTTP`, `HTTPS`, `UDP`, eller `TCP`.|  
