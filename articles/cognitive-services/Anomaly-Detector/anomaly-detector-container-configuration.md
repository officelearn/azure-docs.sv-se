...
description: Körnings miljön för avvikelse detektorns API-behållare konfigureras med hjälp av kommando argumenten `docker run`. Den här behållaren har flera inställningar som krävs, tillsammans med några valfria inställningar.
...
Inställningen `Billing` anger slut punkts-URI för _avvikelse identifierings_ resursen på Azure som används för att mäta fakturerings information för behållaren. Du måste ange ett värde för den här konfigurations inställningen och värdet måste vara en giltig slut punkts-URI för en _avvikelse detektor_ resurs på Azure.

Du hittar den här inställningen på följande plats:

* Azure Portal: **avvikelse i avvikelse detektor** , med etiketten `Endpoint`
...
* **Rad fortsättnings bokstav**: Docker-kommandona i följande avsnitt använder omvänt snedstreck `\`, som ett fortsättnings streck för ett bash-gränssnitt. Ersätt eller ta bort detta baserat på värd operativ systemets krav. Linje fortsättnings tecknet för Windows är till exempel ett cirkumflex, `^`. Ersätt omvänt snedstreck med cirkumflex. 
...
|-------------|-------|---|
| **{API_KEY}** | Slut punkts nyckeln för `Anomaly Detector` resursen på sidan för Azure `Anomaly Detector`-nycklar. | `xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx` |
| **{ENDPOINT_URI}** | Värdet för fakturerings slut punkten är tillgängligt på översikts sidan för Azure-`Anomaly Detector`.| Se [samla in obligatoriska parametrar](anomaly-detector-container-howto.md#gathering-required-parameters) för explicita exempel. |
