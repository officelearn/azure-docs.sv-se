---
title: Hämta Autosuggest resultat med hjälp av Bing Autosuggest API (Azure kognitiva Services) | Microsoft Docs
description: Visar hur du använder för Bing Autosuggest.
services: cognitive-services
author: v-jaswel
manager: kamrani
ms.assetid: 2575A80C-FC74-4631-AE5D-8101CF2591D3
ms.service: cognitive-services
ms.component: bing-autosuggest
ms.topic: article
ms.date: 09/28/2017
ms.author: v-jaswel
ms.openlocfilehash: f66af61ed06f4c6a7fe36cd44aa7781b71bb18ea
ms.sourcegitcommit: 95d9a6acf29405a533db943b1688612980374272
ms.translationtype: MT
ms.contentlocale: sv-SE
ms.lasthandoff: 06/23/2018
ms.locfileid: "35354585"
---
# <a name="build-a-web-page-autosuggest-client"></a>Skapa en webbsida automatiska förslag-klient

I den här kursen ska vi skapa en webbsida som användarna kan frågor till Bing Autosuggest API.

I den här självstudiekursen lär du dig att:

> [!div class="checklist"]
> - Göra en enkel fråga för Bing Autosuggest API
> - Visa frågeresultat

## <a name="prerequisites"></a>Förutsättningar

Om du vill följa kursen behöver du en prenumeration för för Bing Autosuggest API. Om du inte har någon [registrera dig för en kostnadsfri utvärderingsversion](https://azure.microsoft.com/try/cognitive-services/?api=autosuggest-api).

## <a name="create-a-new-web-page"></a>Skapa en ny webbsida

Öppna en textredigerare. Skapa en ny fil med namnet, till exempel autosuggest.html.

## <a name="html-header"></a>HTML-huvud

Lägg till informationen i HTML-huvudet och börja avsnittet skriptet på följande sätt.

```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Bing Autosuggest</title>

<style type="text/css">
    html, body, div, p, h1, h2 {font-family: Verdana, "Lucida Sans", sans-serif;}

    html, body, div, p  {font-weight: normal;}
    h1, h2 {font-weight: bold;}
    sup {font-weight: normal;}

    html, body, div, p  {font-size: 12px;}
    h1 {font-size: 20px;}
    h2 {font-size: 16px;}
    h1, h2 {clear: left;}

    img#logo {float: right;
</style>

<script type="text/javascript">
```

## <a name="getsubscriptionkey-function"></a>getSubscriptionKey funktion

Funktionen getSubscriptionKey returnerar Bing Autosuggest API-nyckeln. Den hämtar från lokal lagring (det vill säga en cookie) eller ber användaren om det behövs.

Börja funktionen getSubscriptionKey och deklarera cookie-namn på följande sätt.

```html
getSubscriptionKey = function() {

    var COOKIE = "bing-autosuggest-api-key";   // name used to store API key in key/value storage
```

Hjälpfunktion findCookie returnerar värdet för den angivna cookien. Om det gick inte att hitta cookien, returneras en tom sträng.

```html
    function findCookie(name) {
        var cookies = document.cookie.split(";");
        for (var i = 0; i < cookies.length; i++) {
            var keyvalue = cookies[i].split("=");
            if (keyvalue[0].trim() === name) {
                return keyvalue[1];
            }
        }
        return "";
        }
```

Hjälpfunktion getSubscriptionKeyCookie uppmanas användaren att ange värdet för Bing Autosuggest API-nyckeln och returnerar värdet för nyckeln.

```html
    function getSubscriptionKeyCookie() {
        var key = findCookie(COOKIE);
        while (key.length !== 32) {
            key = prompt("Enter Bing Autosuggest API subscription key:", "").trim();
            var expiry = new Date();
            expiry.setFullYear(expiry.getFullYear() + 2);
            document.cookie = COOKIE + "=" + key.trim() + "; expires=" + expiry.toUTCString();
        }
        return key;
    }
```

Hjälpfunktion getSubscriptionKeyLocalStorage försöker först hämta Bing Autosuggest API-nyckeln genom att leta upp rätt cookien. Om cookien inte hittas, uppmanas användaren att ange värdet för nyckeln. Den returnerar sedan värdet för nyckeln.

```html
    function getSubscriptionKeyLocalStorage() {
        var key = localStorage.getItem(COOKIE) || "";
        while (key.length !== 32)
            key = prompt("Enter Bing Autosuggest API subscription key:", "").trim();
        localStorage.setItem(COOKIE, key)
        return key;
    }
```

Hjälpfunktion getSubscriptionKey tar en parameter **ogiltigförklara**. Om **ogiltigförklara** är **SANT**, getSubscriptionKey tar bort cookien som innehåller Bing Autosuggest API-nyckeln. Om **ogiltigförklara** är **FALSKT**, getSubscriptionKey returnerar värdet för Bing Autosuggest API-nyckeln.

```html
    function getSubscriptionKey(invalidate) {
        if (invalidate) {
            try {
                localStorage.removeItem(COOKIE);
            } catch (e) {
                document.cookie = COOKIE + "=";
            }
        } else {
            try {
                return getSubscriptionKeyLocalStorage();
            } catch (e) {
                return getSubscriptionKeyCookie();
            }
        }
    }
```

Returnera hjälpfunktion getSubscriptionKey resultatet av funktionen yttre getSubscriptionKey. Stäng definitionen av funktionen yttre getSubscriptionKey.

```html
    return getSubscriptionKey;

}();
```

## <a name="helper-functions"></a>Hjälpfunktioner

Pre hjälpfunktion returnerar den angivna texten förformaterad med den [före](https://www.w3schools.com/tags/tag_pre.asp) HTML-tagg.

```html
function pre(text) {
    return "<pre>" + text.replace(/&/g, "&amp;").replace(/</g, "&lt;") + "</pre>"
}
```

Funktionen renderSearchResults visar angivna resultat från Bing Autosuggest API, med JSON snyggt utskrift.

```html
function renderSearchResults(results) {
    document.getElementById("results").innerHTML = pre(JSON.stringify(results, null, 2));
}
```

Funktionen renderErrorMessage visar angivna felmeddelande och felkoden.

```html
function renderErrorMessage(message, code) {
    if (code)
        document.getElementById("results").innerHTML = "<pre>Status " + code + ": " + message + "</pre>";
    else
        document.getElementById("results").innerHTML = "<pre>" + message + "</pre>";
}
```

## <a name="bingautosuggest-function"></a>bingAutosuggest funktion

Funktionen bingAutosuggest anropas för varje gång som användaren anger text i ett HTML-fält.
Det tar två parametrar: innehållet i HTML-formulär fält och Bing Autosuggest API-nyckeln.

```html
function bingAutosuggest(query, key) {
```

Ange Bing Autosuggest API-slutpunkt och deklarerar ett XMLHttpRequest-objekt som ska användas för att skicka begäranden till slutpunkten.

```html
    var endpoint = "https://api.cognitive.microsoft.com/bing/v7.0/Suggestions";

    var request = new XMLHttpRequest();

    try {
        request.open("GET", endpoint + "?q=" + encodeURIComponent(query));
    }
    catch (e) {
        renderErrorMessage("Bad request");
        return false;
    }
```

Ange den **Ocp-Apim-prenumeration-nyckeln** sidhuvud till värdet för Bing Autosuggest API-nyckeln.

```html
    request.setRequestHeader("Ocp-Apim-Subscription-Key", key);
```

Hantera svaret från slutpunkten. Om statusen är 200 (OK), visas resultaten. Annars visas information om felet.

```html
    request.addEventListener("load", function() {
        if (this.status === 200) {
            renderSearchResults(JSON.parse(this.responseText));
        }
        else {
            if (this.status === 401) getSubscriptionKey(true);
            renderErrorMessage(this.statusText, this.status);
        }
    });
```

Hantera även möjligt felhändelser från XMLHttpRequest-objektet.

```html
    request.addEventListener("error", function() {
        renderErrorMessage("Network error");
    });

    request.addEventListener("abort", function() {
        renderErrorMessage("Request aborted");
    });
```

Skicka begäran. Stäng bingAutosuggest-funktionen i **skriptet** -taggen och **head** tagg.

```html
    request.send();
    return false;
}
// --></script>

</head>
```

## <a name="html-body"></a>HTML-brödtext

När sidan läses in, kontrollera att vi har Bing Autosuggest API-nyckeln fråga användaren för det om det behövs.

```html
<body onload="document.forms.bing.query.focus(); getSubscriptionKey();">
```

Visa Bing-logotypen.

```html
<img id="logo" align=base src="data:image/png;base64,">
```

Skapa ett HTML-formulär med ett textfält. Hantera den **oninput** händelse och anropa funktionen bingAutosuggest skicka innehållet i textfältet och Bing Autosuggest API-nyckeln.

```html
<form name="bing" oninput="return bingAutosuggest(this.query.value, getSubscriptionKey())">
    <h2>Autosuggest</h2>
    <input type="text" name="query" size="80" placeholder="Autosuggest" autocomplete=off>
</form>
```

Lägg till HTML **div** tagg som vi använder för att visa resultatet. JavaScript som vi definierade tidigare refererar till den här **div** tagg.

```html
<h2>Results</h2>
<div id="results">
<p>None yet.</p>

</div>

</body>
</html>
```

Spara filen.

## <a name="display-results"></a>Visa resultaten

Öppna webbsidan i webbläsaren. Ange din prenumeration Bing Autosuggest API-nyckel i Kommandotolken. Ange en fråga (till exempel ”seglar”) i den **automatiska förslag** textruta. När du skriver uppdateras sidan automatiskt om du vill visa resultatet automatiska förslag.

```json
{
  "_type": "Suggestions",
  "queryContext": {
    "originalQuery": "sail"
  },
  "suggestionGroups": [
    {
      "name": "Web",
      "searchSuggestions": [
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=vheQSvKZylM3dlX_B9bQ8-hQEsEJo8zDD2y7H1nsBjE&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailor%2bbrinkley%2bcook%26FORM%3dUSBAPI&p=DevEx,5003.1",
          "displayText": "sailor brinkley cook",
          "query": "sailor brinkley cook",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=EStLqAfxGCa44Ur3jEMXBv-Qp-lXUSFJbkBfnUdKKDg&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailor%2bbrinkley%26FORM%3dUSBAPI&p=DevEx,5004.1",
          "displayText": "sailor brinkley",
          "query": "sailor brinkley",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=gvtP9TS9NwhajSapY2Se6y1eCbP2fq_GiP2n-cxi6OY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailrite%26FORM%3dUSBAPI&p=DevEx,5005.1",
          "displayText": "sailrite",
          "query": "sailrite",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=c0QOA_j6swCZJy9FxqOwke2KslJE7ZRmMooGClAuCpY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailboats%2bfor%2bsale%26FORM%3dUSBAPI&p=DevEx,5006.1",
          "displayText": "sailboats for sale",
          "query": "sailboats for sale",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=mnMdREUH20SepmHQH1zlh9Hy_w7jpOlZFm3KG2R_BoA&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailing%2banarchy%26FORM%3dUSBAPI&p=DevEx,5007.1",
          "displayText": "sailing anarchy",
          "query": "sailing anarchy",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=0udadFl0gCTKCp0QmzQTXS3_y08iO8FpwsoKPHPS6kw&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailboatdata%26FORM%3dUSBAPI&p=DevEx,5008.1",
          "displayText": "sailboatdata",
          "query": "sailboatdata",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=BTS0G6AakxntIl9rmbDXtk1n6rQpsZZ99aQ7ClE7dTY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsail%2bsand%2bpoint%26FORM%3dUSBAPI&p=DevEx,5009.1",
          "displayText": "sail sand point",
          "query": "sail sand point",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=quBMwmKlGwqC5wAU0K7n416plhWcR8zQCi7r-Fw9Y0w&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailflow%26FORM%3dUSBAPI&p=DevEx,5010.1",
          "displayText": "sailflow",
          "query": "sailflow",
          "searchKind": "WebSearch"
        }
      ]
    }
  ]
}
```

## <a name="next-steps"></a>Nästa steg

> [!div class="nextstepaction"]
> [Använda och visa krav](../UseAndDisplayRequirements.md)