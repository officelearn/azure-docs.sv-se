---
title: Hämta stavningskontroll kontrollera resultatet med hjälp av Bing stavningskontroll kontrollera API (Microsoft kognitiva tjänster i Azure) | Microsoft Docs
description: Visar hur du använder Bing stavningskontroll.
services: cognitive-services
author: v-jaswel
manager: kamrani
ms.assetid: 2575A80C-FC74-4631-AE5D-8101CF2591D3
ms.service: cognitive-services
ms.component: bing-spell-check
ms.topic: article
ms.date: 09/28/2017
ms.author: v-jaswel
ms.openlocfilehash: 4e4cdbb8a3d6ab01888d8f273083155c33eb06c1
ms.sourcegitcommit: 95d9a6acf29405a533db943b1688612980374272
ms.translationtype: MT
ms.contentlocale: sv-SE
ms.lasthandoff: 06/23/2018
ms.locfileid: "35352365"
---
# <a name="build-a-web-page-spell-check-client"></a>Skapa en webbsida stavningskontroll klient

I den här kursen ska vi skapa en webbsida som användarna kan fråga Bing stavningskontroll kontrollera API.

I den här självstudiekursen lär du dig att:

> [!div class="checklist"]
> - Skapa en enkel fråga till Bing stavningskontroll kontrollera API
> - Visa frågeresultat

## <a name="prerequisites"></a>Förutsättningar

Om du vill följa kursen behöver du en prenumeration för för Bing stavningskontroll kontrollera API. Om du inte har någon [registrera dig för en kostnadsfri utvärderingsversion](https://azure.microsoft.com/try/cognitive-services/?api=spellcheck-api).

## <a name="create-a-new-web-page"></a>Skapa en ny webbsida

Öppna en textredigerare. Skapa en ny fil med namnet, till exempel spellcheck.html.

## <a name="add-html-header"></a>Lägg till HTML-huvud

Lägg till informationen i HTML-huvudet och börja avsnittet skriptet på följande sätt.

```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8"> 
    <title>Bing Spell Check</title>

<style type="text/css">
    html, body, div, p, h1, h2 {font-family: Verdana, "Lucida Sans", sans-serif;}

    html, body, div, p  {font-weight: normal;}
    h1, h2 {font-weight: bold;}
    sup {font-weight: normal;}

    html, body, div, p  {font-size: 12px;}
    h1 {font-size: 20px;}
    h2 {font-size: 16px;}
    h1, h2 {clear: left;}

    img#logo {float: right;
</style>

<script type="text/javascript">
```

## <a name="getsubscriptionkey-function"></a>getSubscriptionKey funktion

Funktionen getSubscriptionKey returnerar Bing stavningskontroll kontrollera API-nyckeln. Den hämtar från lokal lagring (det vill säga en cookie) eller ber användaren om det behövs.

Börja funktionen getSubscriptionKey och deklarera cookie-namn på följande sätt.

```html
getSubscriptionKey = function() {

    var COOKIE = "bing-spell-check-api-key";   // name used to store API key in key/value storage
```

Hjälpfunktion findCookie returnerar värdet för den angivna cookien. Om det gick inte att hitta cookien, returneras en tom sträng.

```html
    function findCookie(name) {
        var cookies = document.cookie.split(";");
        for (var i = 0; i < cookies.length; i++) {
            var keyvalue = cookies[i].split("=");
            if (keyvalue[0].trim() === name) {
                return keyvalue[1];
            }
        }
        return "";
        }
```

Hjälpfunktion getSubscriptionKeyCookie uppmanas användaren att ange värdet för Bing stavningskontroll kontrollera API-nyckeln och returnerar värdet för nyckeln.

```html
    function getSubscriptionKeyCookie() {
        var key = findCookie(COOKIE);
        while (key.length !== 32) {
            key = prompt("Enter Bing Spell Check API subscription key:", "").trim();
            var expiry = new Date();
            expiry.setFullYear(expiry.getFullYear() + 2);
            document.cookie = COOKIE + "=" + key.trim() + "; expires=" + expiry.toUTCString();
        }
        return key;
    }
```

Hjälpfunktion getSubscriptionKeyLocalStorage försöker först hämta Bing stavningskontroll kontrollera API-nyckeln genom att leta upp rätt cookien. Om cookien inte hittas, uppmanas användaren att ange värdet för nyckeln. Den returnerar sedan värdet för nyckeln.

```html
    function getSubscriptionKeyLocalStorage() {
        var key = localStorage.getItem(COOKIE) || "";
        while (key.length !== 32)
            key = prompt("Enter Bing Spell Check API subscription key:", "").trim();
        localStorage.setItem(COOKIE, key)
        return key;
    }
```

Hjälpfunktion getSubscriptionKey tar en parameter **ogiltigförklara**. Om **ogiltigförklara** är **SANT**, getSubscriptionKey tar bort cookien som innehåller Bing stavningskontroll kontrollera API-nyckeln. Om **ogiltigförklara** är **FALSKT**, getSubscriptionKey returnerar värdet för Bing stavningskontroll kontrollera API-nyckeln.

```html
    function getSubscriptionKey(invalidate) {
        if (invalidate) {
            try {
                localStorage.removeItem(COOKIE);
            } catch (e) {
                document.cookie = COOKIE + "=";
            }
        } else {
            try {
                return getSubscriptionKeyLocalStorage();
            } catch (e) {
                return getSubscriptionKeyCookie();
            }
        }
    }
```

Returnera hjälpfunktion getSubscriptionKey resultatet av funktionen yttre getSubscriptionKey. Stäng definitionen av funktionen yttre getSubscriptionKey.

```html
    return getSubscriptionKey;

}();
```

## <a name="helper-functions"></a>Hjälpfunktioner

Pre hjälpfunktion returnerar den angivna texten förformaterad med den [före](https://www.w3schools.com/tags/tag_pre.asp) HTML-tagg.

```html
function pre(text) {
    return "<pre>" + text.replace(/&/g, "&amp;").replace(/</g, "&lt;") + "</pre>"
}
```

Funktionen renderSearchResults visar angivna resultat från Bing stavningskontroll kontrollera API, med JSON snyggt utskrift.

```html
function renderSearchResults(results) {
    document.getElementById("results").innerHTML = pre(JSON.stringify(results, null, 2));
}
```

Funktionen renderErrorMessage visar angivna felmeddelande och felkoden.

```html
function renderErrorMessage(message, code) {
    if (code)
        document.getElementById("results").innerHTML = "<pre>Status " + code + ": " + message + "</pre>";
    else
        document.getElementById("results").innerHTML = "<pre>" + message + "</pre>";
}
```

## <a name="bingspellcheck-function"></a>bingSpellCheck funktion

Funktionen bingSpellCheck anropas för varje gång som användaren anger text i ett HTML-fält.
Det tar två parametrar: innehållet i HTML-formulär fält och Bing stavningskontroll kontrollera API-nyckeln.

```html
function bingSpellCheck(query, key) {
```

Ange Bing stavningskontroll kontrollera API-slutpunkt och deklarerar ett XMLHttpRequest-objekt som ska användas för att skicka begäranden till slutpunkten.

```html
    var endpoint = "https://api.cognitive.microsoft.com/bing/v7.0/spellcheck/";

    var request = new XMLHttpRequest();

    try {
        request.open("GET", endpoint + "?mode=proof&mkt=en-US&text=" + encodeURIComponent(query));
    }
    catch (e) {
        renderErrorMessage("Bad request");
        return false;
    }
```

Ange den **Ocp-Apim-prenumeration-nyckeln** sidhuvud till värdet för Bing stavningskontroll kontrollera API-nyckeln.

```html
    request.setRequestHeader("Ocp-Apim-Subscription-Key", key);
```

Hantera svaret från slutpunkten. Om statusen är 200 (OK), visas resultaten. Annars visas information om felet.

```html
    request.addEventListener("load", function() {
        if (this.status === 200) {
            renderSearchResults(JSON.parse(this.responseText));
        }
        else {
            if (this.status === 401) getSubscriptionKey(true);
            renderErrorMessage(this.statusText, this.status);
        }
    });
```

Hantera även möjligt felhändelser från XMLHttpRequest-objektet.

```html
    request.addEventListener("error", function() {
        renderErrorMessage("Network error");
    });

    request.addEventListener("abort", function() {
        renderErrorMessage("Request aborted");
    });
```

Skicka begäran. Stäng bingSpellCheck-funktionen i **skriptet** -taggen och **head** tagg.

```html
    request.send();
    return false;
}
// --></script>

</head>
```

## <a name="html-body"></a>HTML-brödtext

När sidan läses in, kontrollera att vi har Bing stavningskontroll kontrollera API-nyckeln fråga användaren för det om det behövs.

```html
<body onload="document.forms.bing.query.focus(); getSubscriptionKey();">
```

Visa Bing-logotypen.

```html
<img id="logo" align=base src="data:image/png;base64,">
```

Skapa ett HTML-formulär med ett textfält. Hantera den **onsubmit** händelse och anropa funktionen bingSpellCheck skicka innehållet i textfältet och Bing stavningskontroll kontrollera API-nyckeln.

```html
<form name="bing" onsubmit="return bingSpellCheck(this.query.value, getSubscriptionKey())">
    <h2>Spell Check</h2>
    <input type="text" name="query" size="80" placeholder="Spell Check" autocomplete=off>
</form>
```

Lägg till HTML **div** tagg som vi använder för att visa resultatet. JavaScript som vi definierade tidigare refererar till den här **div** tagg.

```html
<h2>Results</h2>
<div id="results">
<p>None yet.</p>

</div>

</body>
</html>
```

Spara filen.

## <a name="display-results"></a>Visa resultaten

Öppna webbsidan i webbläsaren. Ange din prenumeration Bing stavningskontroll kontrollera API-nyckel i Kommandotolken. Ange en fråga (till exempel ”ylld, wlrd”!) i den **stavningskontroll** textrutan och tryck på **RETUR**. Webbsidan visar resultatet av frågan.

```json
{
  "_type": "SpellCheck",
  "flaggedTokens": [
    {
      "offset": 0,
      "token": "Hollo",
      "type": "UnknownToken",
      "suggestions": [
        {
          "suggestion": "Hello",
          "score": 0.856629936217145
        },
        {
          "suggestion": "Hollow",
          "score": 0.816717853225633
        }
      ]
    },
    {
      "offset": 7,
      "token": "wlrd",
      "type": "UnknownToken",
      "suggestions": [
        {
          "suggestion": "world",
          "score": 0.856629936217145
        }
      ]
    }
  ]
}
```

## <a name="next-steps"></a>Nästa steg

> [!div class="nextstepaction"]
> [Använda och visa krav](../UseAndDisplayRequirements.md)
