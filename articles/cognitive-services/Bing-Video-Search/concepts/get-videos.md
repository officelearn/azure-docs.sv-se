...
Om Bing kan expandera frågan för att begränsa den ursprungliga sökningen innehåller objektet [Videos](https://docs.microsoft.com/rest/api/cognitiveservices/bing-video-api-v7-reference#videos) fältet `queryExpansions`. Om frågan till exempel var *Cleaning Gutters* (rensa stuprännor) kan de expanderade frågorna vara: Gutter Cleaning **Tools** (verktyg), Cleaning Gutters **From the Ground** (från marken), Gutter Cleaning **Machine** (maskin) och **Easy** (enkel) Gutter Cleaning.
