...
Om du vill utesluta vissa svar från svaret, till exempel bilder, lägga ett `-`-Character till svars typen. Exempel:

```
&responseFilter=-images,-videos
```

Följande visar hur du använder `responseFilter` för att begära bilder, videor och nyheter från seglingen dinghies. När du kodar frågesträngen ändras kommatecken till% 2C.  
...
Om du vill få Sök Resultat från en speciell domän inkluderar du parametern `site:` fråga i frågesträngen.  
...
* `Month` – returnera webb sidor som identifierats under de senaste 30 dagarna

Du kan också ange den här parametern till ett anpassat datum intervall i formatet `YYYY-MM-DD..YYYY-MM-DD`. 
...
Om du lägger till den `responseFilter` frågeparametern i den föregående frågan och anger den till webb sidor och nyheter, innehåller svaret endast webb sidor eftersom nyheterna inte rangordnas.
...
Om du anger `promote` till nyheter, innehåller svaret inte nyhets svaret eftersom det inte är ett Rankat svar&mdash;du bara kan befordra bara rankade svar.
...
Du får bara använda `promote` om du anger parametern `answerCount` fråga.
