...
Den här exempel appen illustrerar hur du använder Custom Vision för att träna en enhet med en kamera för att identifiera visuella tillstånd. Du kan köra det här identifierings scenariot på en IoT-enhet genom att använda en ONNX-modell som exporter ATS från Custom Vision-tjänsten.
...
1. Integrera ditt Custom Vision-projekt:
    1. I _CustomVision\CustomVisionServiceWrapper.cs_ -skriptet uppdaterar du `ApiKey`-variabeln med din utbildnings nyckel.
    1. Uppdatera sedan `Endpoint`-variabeln med slut punkts-URL: en som är kopplad till din nyckel.
    1. Uppdatera variabeln `targetCVSProjectGuid` med motsvarande ID för det Custom Vision projekt som du vill använda. 
...
När appen går in i läget för att **fånga in inlärnings avbildningar** , kommer den att fånga om två avbildningar varje sekund tills den har nått mål antalet bilder. Som standard är detta 30 bilder, men du kan ange den här parametern genom att skicka det önskade antalet som ett argument till metoden `EnterLearningMode` IoT Hub. 
...
Du kan när som helst upprepa steget med att ladda upp inlärnings avbildningar genom att klicka på knappen överst till höger eller anropa metoden `EnterLearningMode` igen.

Om du kör appen på en enhet och behöver hämta IP-adressen igen (för att upprätta en fjärr anslutning via [Windows IoT fjärrklienten](https://www.microsoft.com/p/windows-iot-remote-client/9nblggh5mnxz#activetab=pivot:overviewtab)) kan du till exempel anropa metoden `GetIpAddress` via IoT Hub.
