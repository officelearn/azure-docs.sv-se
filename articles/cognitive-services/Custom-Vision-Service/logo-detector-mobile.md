---
title: 'Självstudiekurs: Använd anpassad logotypdetektor för att känna igen Azure-tjänster – Anpassad vision'
titleSuffix: Azure Cognitive Services
description: I den här självstudien går du igenom en exempelapp som använder Anpassad syn som en del av ett scenario för identifiering av logotyper. Lär dig hur Custom Vision ska användas med andra komponenter för att leverera ett program för slutpunkt till slutpunkt.
...
---
# <a name="tutorial-recognize-azure-service-logos-in-camera-pictures"></a>Självstudiekurs: Identifiera Azure-tjänstlogotyper i kamerabilder

I den här självstudien ska du utforska en exempelapp som använder Anpassad syn som en del av ett större scenario. Appen AI Visual Provision, en Xamarin.Forms-app för mobila plattformar, analyserar kamerabilder av Azure-tjänstens logotyper och distribuerar sedan de faktiska tjänsterna till användarens Azure-konto. Här lär du dig hur det använder Custom Vision tillsammans med andra komponenter för att leverera ett användbart slutpunkt till slutpunkt-program. Du kan köra hela appscenariot själv, eller så kan du bara slutföra den anpassade visiondelen av installationen och utforska hur appen använder den.
...
I den vänstra rutan ställer du in reglaget **Taggar** på **Tagged** (Taggad) för att visa bilderna. Välj sedan den gröna knappen överst på sidan för att träna modellen. Algoritmen kommer att träna att känna igen samma taggar i nya bilder. Det testar också modellen på några av dina befintliga bilder för att generera noggrannhetspoäng.

![Webbplatsen Custom Vision finns på fliken Träningsbilder. I den här skärmbilden beskrivs tågknappen](media/azure-logo-tutorial/train-model.png)

## <a name="get-the-prediction-url"></a>Hämta URL:en för förutsägelse

När din modell har tränats är du redo att integrera den med din app. Du måste hämta slutpunkts-URL:en (adressen till din modell, som appen kommer att fråga efter) och förutsägelsenyckeln (för att ge appen åtkomst till förutsägelsebegäranden). På fliken **Prestanda** väljer du knappen **Prediction URL** (Förutsägelse-URL) längst upp på sidan.
...
Kopiera slutpunkts-URL:en och värdet **Förutsägelse-Key** till lämpliga fält i filen *Source\VisualProvision\AppSettings.cs:*
...
![Tjänsten Datorseende i Azure-portalen, med snabbstartsmenyn markerad. En länk för nycklar samt slutpunkts-URL för API är markerade](media/azure-logo-tutorial/comvis-keys.png)
...
1. På nästa skärm väljer du din Azure-prenumeration i den nedrullningsbara menyn. (Den här menyn ska innehålla alla prenumerationer som ditt huvudnamn har åtkomst till.) Välj knappen **Fortsätt.** I det här läget kan appen uppmana dig att ge åtkomst till enhetens kamera och fotolagring. Bevilja åtkomstbehörigheterna.
