---
title: Använd den en Custom Vision Service REST-API – Azure Cognitive Services | Microsoft Docs
description: Använda REST API för att skapa, träna, testa och exportera en anpassad vision-modell.
...
ms.locfileid: "40250275"
---
# <a name="tutorial-use-the-custom-vision-rest-api"></a>Självstudier: Använd Custom Vision REST-API

Lär dig hur du använder REST-API för anpassad visuellt innehåll för att skapa, träna, testa och exportera en modell.

Informationen i det här dokumentet visar hur du använder en REST-klient för att arbeta med REST API för att träna Custom Vision service. Exemplen visar hur du använder en API med hjälp av den `curl` utility från ett bash-miljön och `Invoke-WebRequest` från Windows PowerShell.
...
* En grundläggande kunskaper med REST Representational State Transfer (). Det här dokumentet går inte in på detaljnivå på saker som HTTP-verb, JSON eller andra saker som ofta används med REST.

* Antingen en bash (Bourne igen Shell) med den [curl](https://curl.haxx.se) verktyg eller Windows PowerShell 3.0 (eller senare).

* Custom Vision-konto. Mer information finns i den [skapa en klassificerare](getting-started-build-a-classifier.md) dokumentet.

## <a name="get-keys"></a>Hämta nycklar

När du använder REST API, måste du autentisera med hjälp av en nyckel. När du utför hantering eller utbildning åtgärder kan du använda den __utbildning nyckeln__. När du använder modellen för att göra förutsägelser kan du använda den __förutsägelse nyckeln__.

När du gör en begäran skickas nyckeln som en rubrik för begäran.

Om du vill hämta nycklar för ditt konto, gå till den [Custom Vision-webbsida](https://customvision.ai) och välj den __kugghjulsikonen__ i det övre högra hörnet. I den __konton__ och kopiera värdena från den __utbildning nyckeln__ och __förutsägelse nyckeln__ fält.

![Bild av nycklar UI](./media/rest-api-tutorial/training-prediction-keys.png)

> [!IMPORTANT]
> Eftersom nycklarna som används för att autentisera varje begäran, förutsätter exemplen i det här dokumentet att viktiga värden finns i miljövariabler. Använd följande kommandon för att lagra nycklar till miljövariabler innan du använder andra kodfragment i det här dokumentet:
>
> ```bash
...
I följande exempel skapar ett nytt projekt med namnet `myproject` i din Custom Vision service-instans. Den här tjänsten som standard den `General` domän:
...
Svaret på begäran som liknar följande JSON-dokument:
...
> [!TIP]
> Den `id` posten i svaret är ID för det nya projektet. Det här används i andra exemplen senare i det här dokumentet.

Mer information om den här begäran finns i [CreateProject](https://southcentralus.dev.cognitive.microsoft.com/docs/services/d0e77c63c39c4259a298830c15188310/operations/5a59953940d86a0f3c7a8290).

### <a name="specific-domains"></a>Vissa domäner

Om du vill skapa ett projekt för en specifik domän kan du ange den __domän-ID__ som en valfri parameter. I följande exempel visas hur du hämtar en lista över tillgängliga domäner:
...
Mer information om den här begäran finns i [GetDomains](https://southcentralus.dev.cognitive.microsoft.com/docs/services/d0e77c63c39c4259a298830c15188310/operations/5a59953940d86a0f3c7a827d).

Följande exempel visar hur du skapar ett nytt projekt som använder den __landmärken__ domän:
...
Att tagga bilder, måste du använda en tagg-ID. I följande exempel visar hur du skapar en ny tagg med namnet `cat` och få en tagg-ID. Ersätt `{projectId}` med ID: T för ditt projekt. Använd den `name=` parametern för att ange namnet på taggen:
...
Spara den `id` eftersom den används när tagga bilder.

Mer information om den här begäran finns i [CreateTag](https://southcentralus.dev.cognitive.microsoft.com/docs/services/d0e77c63c39c4259a298830c15188310/operations/5a59953940d86a0f3c7a829d).

## <a name="add-images"></a>Lägga till bilder

Följande exempel visar att lägga till en fil från URL: en. Ersätt `{projectId}` med ID: T för ditt projekt. Ersätt `{tagId}` med ID: T för tag för avbildningen:
...
Mer information om den här begäran finns i [CreateImagesFromUrls](https://southcentralus.dev.cognitive.microsoft.com/docs/services/d0e77c63c39c4259a298830c15188310/operations/5a59953940d86a0f3c7a8287).

## <a name="train-the-model"></a>Träna modellen

Följande exempel visar hur du tränar modellen. Ersätt `{projectId}` med ID: T för ditt projekt:
...
Spara den `id` värde, som används för att testa och exportera modellen.
...
Följande exempel visar hur du utför ett test av modellen. Ersätt `{projectId}` med ID: T för ditt projekt. Ersätt `{iterationId}` med ID: T som returneras från träna modellen. Ersätt `https://linktotestimage` med sökvägen till testavbildningen.
...
Exportera en modell är en tvåstegsprocess. Du måste först ange modell-format och begär sedan URL-Adressen för den exporterade modellen.

### <a name="request-a-model-export"></a>Begär en modellexport

Följande exempel visar hur du exporterar en `coreml` modell. Ersätt `{projectId}` med ID: T för ditt projekt. Ersätt `{iterationId}` med ID: T som returneras från träna modellen.
...
### <a name="download-the-exported-model"></a>Ladda ned exporterade modellen

Följande exempel visar hur du hämtar URL: en för den exporterade modellen. Ersätt `{projectId}` med ID: T för ditt projekt. Ersätt `{iterationId}` med ID: T som returneras från träna modellen.
