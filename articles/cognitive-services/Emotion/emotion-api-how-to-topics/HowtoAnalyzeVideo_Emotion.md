...
Den här koden startar varje analys i en separat uppgift som kan köras i bakgrunden medan vi fortsätter insamlingen av nya bildrutor. Detta förhindrar blockering huvudtråden medan du väntar på att ett API-anrop ska returneras. Vi har dock tappat bort några av garantierna som den enkla versionen tillhandahöll – flera API-anrop kan ske parallellt och resultaten kan returneras i fel ordning. Det kan också leda till att flera trådar går in i funktionen ConsumeResult() samtidigt, vilket kan vara farligt om funktionen inte är trådsäker. Slutligen håller den här enkla koden inte reda på de uppgifter som skapas, så undantag försvinner tyst. Därför är den sista komponenten som vi ska lägga till en ”konsumenttråd” som spårar analysuppgifterna, skapar undantag, avslutar långvariga uppgifter och ser till att resultaten förbrukas i rätt ordning, ett i taget.
