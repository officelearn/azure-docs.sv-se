...
description: I den här snabb starten ska du skapa ett Python-skript som använder Ansikts-API för att identifiera och Rama in en fjärrbild.
...
I den här snabb starten skapar du ett Python-skript som använder Azure-Ansikts-API, via python SDK, för att identifiera mänskliga ansikten i en fjärran sluten bild. Appen visar en utvald bild och ritar en ram runt varje identifierat ansikte.
...
Utdata representerar en lista över identifierade ansikten. Varje objekt i listan är en **dict**-instans där `faceId` är ett unikt ID för det identifierade ansiktet och `faceRectangle` beskriver positionen för det identifierade ansiktet. 
...
Längs ned i skriptet lägger du sedan till följande kod. Den här koden skapar en enkel funktion för att parsa Rectangle-koordinaterna och använder Pillow för att rita rektanglar på den ursprungliga bilden. Sedan visar den bilden i ditt standardprogram för bildvisning.
