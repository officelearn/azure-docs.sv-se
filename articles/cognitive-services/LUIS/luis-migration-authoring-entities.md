...
Överväg en v2-sammansatt för att ändra `order` en pizza som använder:
...
Överväg en hierarkisk entitet i v2 för att ändra `order`en pizza:
...
|--|--|
|Överordnad komponent enhet med namnet`Order`|Överordnad dator – inlärd entitet med namnet`Order`|
|Underordnad-hierarkisk entitet med ursprunglig och slutgiltig pizza-topping|* Lägg till roll `Order` för varje topping.|
