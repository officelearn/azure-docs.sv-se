...
ms.locfileid: "73828529"
---
# <a name="tutorial-extract-structured-data-with-machine-learned-entities-in-language-understanding-luis"></a>Självstudie: extrahera strukturerade data med kundlärt entiteter i Language Understanding (LUIS)
...
I praktiska termer kommer du att skapa en enhets medveten entitet som representerar en order för en pizza-app. Ordningen bör ha alla delar som krävs för att fullfil ordern. För att kunna börja kommer entiteten att innehålla all beskrivande text och specifikt dra ut storlek och kvantitet. 

En uttryck för `deliver one large cheese pizza` bör extrahera hela uttryck som order och sedan extrahera `1` och `large`. 

Det finns ytterligare diskompositioner som du kan göra, till exempel toppings eller crust. Efter den här självstudien bör du känna till att lägga till dessa del komponenter till din befintliga `Order` entitet.
...
1. I avsnittet **Hantera** går du till fliken **versioner** , väljer version, väljer **klon** för att klona versionen och namnger den `mach-learn`. Välj sedan **klar** för att slutföra klonings processen. Eftersom versionsnamnet används i webbadressen får namnet inte innehålla några tecken som är ogiltiga i webbadresser.

    > [!TIP] 
    > Kloning är en bra metod innan du ändrar appen. När du är färdig med en version exporterar du en version, som en. JSON-eller. lu-fil, och kontrollerar sedan i käll kontrollen.

1. Välj **build** och **avsikt** för att se de viktigaste Bygg stenarna för en Luis-app, avsikter.
...
1. I exempel listan yttranden väljer du följande uttryck. 
    ...
    > [!NOTE]
    > En entitet är inte alltid hela uttryck. I det här fallet anger `pickup` hur ordningen ska tas emot, så den ska vara en del av den märkta entiteten för ordern. 

1. I rutan **Välj typ av enhet** väljer du **Lägg till struktur** och sedan **Nästa**. Det krävs en struktur för att få del komponenter, till exempel storlek och kvantitet.
...
1. Märk `Order` entiteten i återstående yttranden tillsammans med entiteten storlek. Hakparenteserna i texten visar etiketten `Order` entiteten och `Size` entiteten i.
    ...
    > [!CAUTION]
    > Hur behandlar du underförstådda data, till exempel bokstavs `a` som inbegriper en enda pizza? Eller om det saknas `pickup` och `delivery` för att indikera var pizza förväntas? Eller om det saknas en storlek för att ange din standard storlek eller liten eller stor? Överväg att hantera underförstådd data hantering som en del av dina affärs regler i klient programmet. 
...
1. Efter utbildningen lägger du till ett nytt exempel-uttryck för att förstå hur bra LUIS förstår den enhet som har lärts ur enheten. 
    ...
    Den streckade länken visar förutsägelsen. 

1. I ändra förutsägelsen till en etikettad entitet väljer du raden och väljer **Bekräfta enhets förutsägelser**.
    ...
    I det här läget fungerar enheten som sparats av datorn eftersom den kan hitta entiteten i ett nytt exempel uttryck. När du lägger till exempel yttranden, om entiteten inte är korrekt förväntad, kan du namnge entiteten och del komponenterna. Om entiteten förutsägs korrekt kontrollerar du att bekräfta förutsägelserna. 

## <a name="add-prebuilt-number-to-app-to-help-extract-data"></a>Lägg till ett fördefinierat nummer till appen för att hjälpa till att extrahera data

Beställnings informationen bör också innehålla hur många av ett objekt i ordningen, till exempel hur många pizzor. Om du vill extrahera dessa data måste en ny dator som har lärts läggas till i `Order` och komponenten måste ha en begränsning av ett fördefinierat nummer. Genom att begränsa entiteten till ett fördefinierat nummer kommer entiteten att hitta och extrahera siffror om texten är en siffra, `2`eller text `two`.

Börja med att lägga till det fördefinierade numret i appen. 
...
## <a name="label-example-utterance-with-subcomponent-for-quantity-to-teach-luis-about-the-entity"></a>Label-exempel uttryck med del komponent för kvantitet att lära LUIS om entiteten
