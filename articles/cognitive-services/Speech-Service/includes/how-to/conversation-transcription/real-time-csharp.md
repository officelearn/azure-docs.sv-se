...
I följande exempel visas hur du skapar en röst-signatur med [hjälp av REST API](https://aka.ms/cts/signaturegenservice) i C#. Observera att du måste ersätta verklig information för din `subscriptionKey` , `region` och sökvägen till en exempel `.wav` fil.
...
Om funktionen körs `GetVoiceSignatureString()` returneras en röst signatur sträng i rätt format. Kör funktionen två gånger så att du kan använda två strängar som inmatade för variablerna `voiceSignatureStringUser1` och `voiceSignatureStringUser2` nedan.
...
* Skapar en `AudioStreamReader` från exempel `.wav` filen som ska skrivas av.
* Skapar en `Conversation` med `CreateConversationAsync()` .
* Skapar en `ConversationTranscriber` med hjälp av konstruktorn och prenumererar på de nödvändiga händelserna
* Lägger till deltagare i konversationen. Strängarna `voiceSignatureStringUser1` och `voiceSignatureStringUser2` ska visas som utdata från stegen ovan från funktionen `GetVoiceSignatureString()` .
* Ansluter till konversationen och påbörjar avskriften.

Anropa funktionen `TranscribeConversationsAsync()` för att starta konversations avskrift.
