...
Många ingredienser går du till en bra anpassade röst. Den här guiden fungerar som en introduktion till processen, med särskilt fokus på problem som du kan råka ut.
...
* En tom kolumn där du skriver i tid-kod för varje uttryck: den tid då hittar du det i inspelade ljudfilen.
...
Som tur är kan är det enkelt att undvika dessa problem helt och hållet. Det finns många källor med text som du kan använda utan behörighet eller licens.
...
|Allmän egendom|Fungerar för vilka copyright har uttryckligen anges eller som har varit dedikerad till den offentliga domänen.|
|Permissively licensierad fungerar|Fungerar distribueras under en licens till exempel Creative Commons eller licensen GNU kostnadsfri dokumentation. Wikipedia använder GFDL. Vissa licenser, men kan införa begränsningar för prestanda för licensierade innehåll som kan påverka skapandet av en anpassad voice-modell, så Läs noggrant.|

## <a name="recording-your-script"></a>Spela in skriptet

Helst bör du spela in din skriptet i en professionell inspelning studio som specialiserar sig på voiceover arbete. De har en inspelning monter, rätt utrustning för jobbet och rätt personer att driva den. Det lönar sig inte för att spela in ändå.
...
Skapa en referens spela in tidigt i sessionen för en typisk uttryck och spela upp den till röst personal regelbundet för att hjälpa dem att bevara deras prestanda. Teknikern kan använda den som referens för nivåer och konsekventa ljud. Detta är särskilt viktigt när återupptar inspelning efter ett avbrott eller en annan dag.

Träna din personal för att dra en djupgående hårt och pausa under en kort stund innan varje uttryck. Registrera ett par sekunder tystnad mellan yttranden. Hastighetsstyrningen ska vara konsekvent; en metronome som spelas upp via den personal hörlurar kan vara till hjälp om de har problem. Ord bör vara uttalas på samma sätt varje gång som de visas.
...
> [!TIP] 
> När du registrerar ett stort antal yttranden kan att förlora en enda uttryck inte påverka resulterande röst på någon märkbar sätt. Därför kan det vara mer lämpligt att helt enkelt tänka yttranden som har problem, exkluderar dem från din datauppsättning och se hur din anpassade röst har visat sig. Du kan alltid gå tillbaka till studio och registrera missade exemplen senare.

De flesta studior har en stor ”tid kod” display som indikerar den aktuella tiden i inspelningen. Anteckna tid på ditt skript för varje uttryck. Be teknikern om de kan markera varje uttryck i den inspelningen metadata eller låt dig ledas samt.
...
Studio levererar förmodligen en eller flera ljudfiler som innehåller många yttranden. Varje uttryck måste vara i en egen fil för att ladda upp inspelningarna anpassad Voice-portalen. Spela in-tekniker har placerats en markör i filen (eller tillhandahålls en separat stack-lista) som anger där varje uttryck startar. Dessa metadata kan användas för att extrahera talade.

Du behöver gå igenom den fullständiga inspelningen och göra en WAV-fil för varje uttryck. Använd dina anteckningar för att hitta exakt yttranden som du vill ha och sedan använda ett ljud redigera verktyget som [Avid Pro verktyg](http://www.avid.com/en/pro-tools), [Adobe Audition](https://www.adobe.com/products/audition.html), eller den kostnadsfria [Audacity](https://www.audacityteam.org/) att kopiera var och en till en ny fil.
...
Lyssna på varje fil noggrant. I det här skedet kan du redigera ut små oönskade ljud som du har missat under registreringen, så länge de inte överlappar några faktiska tal. Om du inte kan åtgärda en fil, ta bort den från datauppsättningen helt, vilket gör noterat att du har gjort det.
