...
De anpassade tal behållarna använder [volym monteringar](https://docs.docker.com/storage/volumes/) för att bevara anpassade modeller. Du kan ange en volym montering genom att lägga `-v` till alternativet `--volume`(eller) i kommandot [Docker körning](https://docs.docker.com/engine/reference/commandline/run/) .

Anpassade modeller hämtas första gången en ny modell matas in som en del av den anpassade tal containerns Docker Kör-kommando. Sekventiella körningar av `ModelId` samma för en anpassad tal behållare kommer att använda den tidigare nedladdade modellen. Om volym montering inte anges kan inte anpassade modeller vara bestående.
...
| ----------- | ----- | ----------------- |
| **{API_KEY}** | `Speech` Resursens slut punkts nyckel på sidan med `Speech` Azure-nycklar.   | `xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`                                                                                  |
| **{ENDPOINT_URI}** | Värdet för fakturerings slut punkten är tillgängligt på sidan `Speech` Azure-översikt. | Se [samla in obligatoriska parametrar](speech-container-howto.md#gathering-required-parameters) för explicita exempel. |
