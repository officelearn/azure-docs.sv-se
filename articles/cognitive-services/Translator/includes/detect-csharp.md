...
Öppna projektet och öppna `detect-sample.csproj`sedan. Kontrol lera att `LangVersion` är inställt på 7,1 eller senare. Om det inte finns en egenskaps grupp för språk versionen lägger du till följande rader:
...
I- `Program` klassen skapar du en funktion som `DetectTextRequest()`heter. Den här klassen kapslar in den kod som används för att anropa Detect-resursen och skriver ut resultatet till konsolen.
