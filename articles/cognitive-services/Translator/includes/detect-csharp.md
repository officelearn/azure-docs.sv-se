...
Den här snabb starten kräver C# 7,1 eller senare. Det finns några sätt att ändra C#-versionen för projektet. I den här hand boken visar vi hur du justerar `detect-sample.csproj` filen. För alla tillgängliga alternativ, till exempel att ändra språk i Visual Studio, se [väljer du språk versionen C#](https://docs.microsoft.com/dotnet/csharp/language-reference/configure-language-version).

Öppna projektet och öppna sedan `detect-sample.csproj` . Kontrol lera att `LangVersion` är inställt på 7,1 eller senare. Om det inte finns en egenskaps grupp för språk versionen lägger du till följande rader:
...
I `Program` -klassen skapar du en funktion som heter `DetectTextRequest()` . Den här klassen kapslar in den kod som används för att anropa Detect-resursen och skriver ut resultatet till konsolen.
...
## <a name="serialize-the-detect-request"></a>Serialisera identifierings förfrågan
