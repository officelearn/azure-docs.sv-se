...
Lägg `Main`märke till att du är `subscriptionKey`deklarerad, `host`och `route`. Dessutom uppmanas du att ange användaren för indata med `Console.Readline()` och tilldela värdet till. `textToTranslate`
