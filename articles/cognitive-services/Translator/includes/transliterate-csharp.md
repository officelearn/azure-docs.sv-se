...
Denna snabbstart kräver C# 7.1 eller senare. Det finns några sätt att ändra C#-versionen för ditt projekt. I den `transliterate-sample.csproj` här guiden visar vi hur du justerar filen. Alla tillgängliga alternativ, till exempel ändra språket i Visual Studio, finns i [Välj C#-språkversion](https://docs.microsoft.com/dotnet/csharp/language-reference/configure-language-version).

Öppna projektet och `transliterate-sample.csproj`öppna sedan . Se till `LangVersion` att den är inställd på 7.1 eller senare. Om det inte finns någon egenskapsgrupp för språkversionen lägger du till följande rader:
...
`Program` Skapa en asynkron funktion som `TransliterateTextRequest()`kallas . Den här funktionen `subscriptionKey`tar `endpoint` `route`fyra `inputText`argument: , , och .
...
## <a name="serialize-the-translation-request"></a>Serialisera översättningsbegäran

Sedan behöver vi skapa och serialisera det JSON-objekt som innehåller den text du vill översätta. Tänk på att du kan skicka mer `body`än ett objekt i .
...
Det sista steget är att anropa `TransliterateTextRequest()` i funktionen `Main`. I det här exemplet transliterar vi från japanska till latinska skript. Leta `static void Main(string[] args)` upp och ersätt den med den här koden:
...
Du kommer att `Main`märka att i `subscriptionKey`, `endpoint` `route`du deklarerar , , `textToTransliterate`och skriptet för att translitterera .
