...
Här följer huvudena som din begäran bör ange. `Content-Type` Rubrikerna `Ocp-Apim-Subscription-Key` och är de enda huvuden som krävs, men du bör även `User-Agent`inkludera `X-MSEdge-ClientID`, `X-MSEdge-ClientIP`, och `X-Search-Location`.
...
Du kan också ange `enableEntityData` attributet i rubriken till `true` för detaljerad information om huvud enheten i den avbildning som du överför, inklusive länkar till webb-och behörighets information. Det här fältet `false` är som standard.
...
Om avbildningen innehåller en erkänd entitet, till exempel en kulturellt välkänd/populär person, plats eller sak, kan en av taggarna innehålla en inblick i enheten. Fälten `mainEntity` och `data` är bara tillgängliga om `enableEntityData` attributet i `Content-Type` rubriken är inställt på. `true`
