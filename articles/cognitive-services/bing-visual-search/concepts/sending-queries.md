...
Objektet `filters` innehåller ett platsfilter (se fältet `site`) som du kan använda för att begränsa liknande bilder och liknande produktresultat till en specifik domän. Om bilden till exempel är en Surface Book kan du ange `site` `www.microsoft.com`.
...
Här följer frågeparametrarna som din begäran bör ange. Du bör minst inkludera parametern `mkt` fråga:
...
Varje begäran måste innehålla `Content-Type`s huvudet. Huvudet måste vara inställt på: `multipart/form-data; boundary=\<boundary string\>`, där \<avgränsnings sträng\> är en unik, ogenomskinlig sträng som identifierar gränserna för formulär data. Till exempel `boundary=boundary_1234-abcd`.
...
Om du överför en lokal avbildning visar följande fragment de formulär data som du måste inkludera i POSTens brödtext. Formulär data måste innehålla `Content-Disposition` rubrik. Parametern `name` måste anges till ”image” och parametern `filename` kan anges till valfri sträng. `Content-Type`s huvudet kan ställas in på valfri MIME-typ som används ofta. Innehållet i formuläret är de binära data i bilden. Den maximala bildstorlek som du kan ladda upp är 1 MB. Det som är störst av bredden eller höjden ska vara 1 500 bildpunkter eller mindre.
...
Om det finns insikter för avbildningen innehåller svaret en eller flera `tags` som innehåller insikterna. Fältet `image` innehåller insikt-token för indata-avbildningen:
...
Om bilden innehåller text som tjänsten känner igen innehåller en av taggarna en TextResults-insikt (åtgärd). Insiktens `displayName` innehåller den tolkade texten:
