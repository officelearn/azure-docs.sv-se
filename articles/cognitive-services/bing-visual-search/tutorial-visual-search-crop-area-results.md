...
Den här bilden beskärs genom att `ImageInfo` skapa ett objekt från beskärnings ytan och läsa `ImageInfo` in objektet i `VisualSearchRequest`en. `ImageInfo` Objektet innehåller även bildens URL:
...
## <a name="get-urls-for-pagesincluding-actiontype-images"></a>Hämta URL: `PagesIncluding` `ActionType` er för bilder
