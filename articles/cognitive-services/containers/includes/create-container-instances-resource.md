...
2. On the **Basics** tab, enter the following details:
    ...
    |Resursgrupp|Select the available resource group or create a new one such as `cognitive-services`.|
    |Containerns namn|Enter a name such as `cognitive-container-instance`. The name must be in lower caps.|
    |Plats|Select a region for deployment.|
