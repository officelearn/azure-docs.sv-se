---
layout: HubPage
title: Dokumentation om Azure Cognitive Services
description: Lär dig hur du skapar intelligenta algoritmer med dina appar, webbplatser och robotar så att de kan se, höra, tala, förstå och tolka användarnas behov genom naturlig kommunikation. Självstudier, snabbstarter, API-referenser och exempel visar hur du använder API:erna.
services: cognitive-services
author: nitinme
manager: nitinme
ms.service: cognitive-services
ms.author: nitinme
ms.date: 08/21/2019
ms.topic: hub-page
ms.openlocfilehash: 13bd9d8c9b5c8337e616b315187242633fce8308
ms.sourcegitcommit: 47b00a15ef112c8b513046c668a33e20fd3b3119
ms.translationtype: HT
ms.contentlocale: sv-SE
ms.lasthandoff: 08/22/2019
ms.locfileid: "69971418"
---
<div id="main" class="v2">
<h1>Dokumentation om Azure Cognitive Services</h1>
        <p style="font-size: 1.12rem;margin-bottom: 1rem;">Lär dig hur du skapar intelligenta algoritmer med dina appar, webbplatser och robotar så att de kan se, höra, tala och förstå användarnas behov genom naturlig kommunikation. Självstudier, snabbstarter, API-referenser och exempel med anvisningar.</p>
        <p> Varje tjänst har sitt eget <a href="/azure/cognitive-services/language-support">språkstöd</a> och <a href="/azure/cognitive-services/cognitive-services-and-machine-learning#service-requirements-for-the-data-model">datamodellkrav</a>, från en fullständigt tränad modell till en fullständigt anpassningsbar modell. 


<h2>Komma igång</h2>
<ul class="cardsY panelContent singlePanelContent" style="display:flex!important;">
        <li>
            <a href="/azure/cognitive-services/welcome/">
                <div class="cardSize">
                    <div class="cardPadding">
                        <div class="card">
                            <div class="cardImageOuter">
                                <div class="cardImage">
                                    <img src="media/index/i_overview.svg" alt="Learn about Azure Cognitive Services and how to use the different APIs to build intelligent apps." />
                                </div>
                            </div>
                            <div class="cardText">
                                <h3>Vad är Cognitive Services?</h3>
                                <p>Läs mer om Azure Cognitive Services och hur du använder olika API: er för att skapa smarta appar.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </a>
        </li>
        <li>
            <a href="/azure/cognitive-services/cognitive-services-and-machine-learning/">
                <div class="cardSize">
                    <div class="cardPadding">
                        <div class="card">
                            <div class="cardImageOuter">
                                <div class="cardImage">
                                    <img src="media/index/i_guidelines.svg" alt="Learn where Azure Cognitive Services fits in with other Azure offerings for machine learning." />
                                </div>
                            </div>
                            <div class="cardText">
                                <h3>Cognitive Services och Machine Learning</h3>
                                <p>Lär dig hur Azure Cognitive Services passar ihop med andra Azure-erbjudanden för maskininlärning.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </a>
        </li>
        <li>
            <a href="/azure/cognitive-services/cognitive-services-apis-create-account/">
                <div class="cardSize">
                    <div class="cardPadding">
                        <div class="card">
                            <div class="cardImageOuter">
                                <div class="cardImage">
                                    <img src="media/index/account-management.svg" alt="Learn how to create an Azure Cognitive Services resource." />
                                </div>
                            </div>
                            <div class="cardText">
                                <h3>Skapa en -resurs för Cognitive Services</h3>
                                <p>Lär dig hur du skapar en Azure Cognitive Services-resurs.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </a>
        </li>
        <li>
            <a href="/azure/cognitive-services/language-support/">
                <div class="cardSize">
                    <div class="cardPadding">
                        <div class="card">
                            <div class="cardImageOuter">
                                <div class="cardImage">
                                    <img src="media/index/mic-abc.svg" alt="Learn about the human languages supported by Cognitive Services APIs." />
                                </div>
                            </div>
                            <div class="cardText">
                                <h3>Språkstöd i Cognitive Services</h3>
                                <p>Läs mer om de mänskliga språk som stöds av Cognitive Services API:er.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </a>
        </li>
        <li>
            <a href="/azure/cognitive-services/cognitive-services-container-support/">
                <div class="cardSize">
                    <div class="cardPadding">
                        <div class="card">
                            <div class="cardImageOuter">
                                <div class="cardImage">
                                    <img src="media/index/virtual-containers.svg" alt="Learn how to use Azure Cognitive Services in Containers." />
                                </div>
                            </div>
                            <div class="cardText">
                                <h3>Använda Cognitive Services i Containers</h3>
                                <p>Lär dig hur du använder Azure Cognitive Services i Containers.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </a>
        </li>
        <li>
            <a href="/azure/cognitive-services/authentication/">
                <div class="cardSize">
                    <div class="cardPadding">
                        <div class="card">
                            <div class="cardImageOuter">
                                <div class="cardImage">
                                    <img src="media/index/account-management.svg" alt="Learn how to authenticate API requests to Azure Cognitive Services." />
                                </div>
                            </div>
                            <div class="cardText">
                                <h3>Autentisera en förfrågan</h3>
                                <p>Lär dig hur du autentiserar API-begäran till Azure Cognitive Services.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </a>
        </li>
        <li>
            <a href="https://docs.microsoft.com/learn/browse/?products=azure-cognitive-services">
                <div class="cardSize">
                    <div class="cardPadding">
                        <div class="card">
                            <div class="cardImageOuter">
                                <div class="cardImage">
                                    <img src="media/index/grad-hat.svg" alt="Up your game with a module or learning path tailored to today's developer and technology masterminds and designed to prepare you for industry-recognized Microsoft certifications." />
                                </div>
                            </div>
                            <div class="cardText">
                                <h3>Microsoft Learn</h3>
                                <p>Ge dig in i leken med en modul eller en utbildningsväg som är anpassad efter dagens begåvade utvecklare och tekniker och utformad för att förbereda dig för branschledande Microsoft-certifieringar.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </a>
        </li>
</ul>



<h2>API:er för Cognitive Services</h2>
<ul class="cardsF panelContent singlePanelContent" style="display:flex!important;">
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3>Visuellt innehåll</h3>
                        <h4>Avkänning, identifiering, textning, indexering och moderering för bilder, videor och innehåll med digitala pennanteckningar.</h4><br>
                        <p><a href="/azure/cognitive-services/computer-vision/">Visuellt innehåll</a></p>
                        <p><a href="/azure/cognitive-services/Custom-Vision-Service/">Custom Vision Service</a></p>
                        <p><a href="/azure/cognitive-services/face/">Ansikts-API</a></p>
                        <p><a href="https://go.microsoft.com/fwlink/?linkid=2088514"><span>Formigenkänning&nbsp;<sup style="font-size:70%">FÖRHANDSVERSION</sup></span></a></p>
                        <p><a href="https://go.microsoft.com/fwlink/?linkid=2088323"><span>Handskriftsigenkänning&nbsp;<sup style="font-size:70%">FÖRHANDSVERSION</sup></span></a></p>
                        <p><a href="/azure/cognitive-services/video-indexer/video-indexer-overview">Video Indexer</a></p>
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3>Tal</h3>
                        <h4>Konvertera tal till text och text till tal som låter naturligt. Översätt från ett språk till ett annat och aktivera talarverifiering och -igenkänning.</h4><br>
                        <p><a href="/azure/cognitive-services/speech-service">Speech Services</a></p>
                        <p><a href="/azure/cognitive-services/speaker-recognition/home"><span>Talarigenkänning&nbsp;<sup style="font-size:70%">FÖRHANDSVERSION</sup></span></a></p>
                        <p><a href="/azure/cognitive-services/speech/home"><span>API för Bing-taligenkänning&nbsp;<sup style="font-size:70%">TAS UR BRUK</sup></span></a></p>
                        <p><a href="/azure/cognitive-services/translator-speech/"><span>Translator Speech&nbsp;<sup style="font-size:70%">TAS UR BRUK</sup></span></a></p>
                     </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3>Språk</h3>
                        <h4>Tillåt att dina appar behandlar naturligt språk med fördefinierade skript, utvärderar känslor och lär sig att känna igen vad användare vill.</h4><br>
                        <p><a href="/azure/cognitive-services/immersive-reader"><span>Avancerad läsare&nbsp;<sup style="font-size:70%">FÖRHANDSVERSION</sup></span></a></p>                        <p><a href="/azure/cognitive-services/luis/">Språkförståelse (LUIS)</a></p>
                        <p><a href="/azure/cognitive-services/qnamaker/index">QnA Maker</a></p>
                        <p><a href="/azure/cognitive-services/text-analytics/">Textanalys</a></p>
                        <p><a href="/azure/cognitive-services/translator/">Talöversättning</a></p>
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3>Search</h3>
                        <h4>Lägg till API:er för Bing-sökning i dina appar och utnyttja möjligheten att söka i miljontals webbsidor, bilder, videor och nyheter med ett enda API-anrop.</h4><br>
                        <p><a href="/azure/cognitive-services/bing-news-search/">Nyhetssökning i Bing</a></p>
                        <p><a href="/azure/cognitive-services/Bing-Video-Search/">Videosökning i Bing</a></p>
                        <p><a href="/azure/cognitive-services/bing-web-search/">Webbsökning i Bing</a></p>
                        <p><a href="/azure/cognitive-services/Bing-Autosuggest">Automatiska förslag i Bing</a></p>
                        <p><a href="/azure/cognitive-services/bing-custom-search">Anpassad sökning i Bing</a></p>
                        <p><a href="/azure/cognitive-services/bing-entities-search/">Entitetssökning i Bing</a></p>
                        <p><a href="/azure/cognitive-services/bing-image-search">Bildsökning i Bing</a></p>
                        <p><a href="/azure/cognitive-services/bing-visual-search">Visuell sökning i Bing</a></p>
                        <p><a href="/azure/cognitive-services/bing-spell-check/">Stavningskontroll i Bing</a></p>
                        <p><a href="/azure/cognitive-services/bing-local-business-search"><span>Sökning efter lokala företag i Bing&nbsp;<sup style="font-size:70%">FÖRHANDSVERSION</sup></span></a></p>
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3>Besluts-API</h3>
                        <h4>Skapa appar som ger rekommendationer för välinformerade och effektiva beslut.</h4><br>
                        <p><a href="/azure/cognitive-services/anomaly-detector/"><span>Avvikelseidentifiering&nbsp;<sup style="font-size:70%">FÖRHANDSVERSION</sup></span></a></p>
                        <p><a href="/azure/cognitive-services/content-moderator/">Content Moderator</a></p>
                        <p><a href="https://go.microsoft.com/fwlink/?linkid=2088324"><span>Personanpassning&nbsp;<sup style="font-size:70%">FÖRHANDSVERSION</sup></span></a></p>
                    </div>
                </div>
            </div>
        </div>
    </li>
</ul>
</div>

<h2>Snabbstarter på 5 minuter</h2>
<ul class="cardsF panelContent singlePanelContent cols cols3" style="display:flex!important;">
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/i_quick-start.svg" alt="">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Analysera avbildning med hjälp av visuellt innehåll</h3>
                        <p><a href="/azure/cognitive-services/Computer-vision/quickstarts-sdk/csharp-analyze-sdk">Använda C#</a></p>
                        <p><a href="/azure/cognitive-services/bing-visual-search/quickstarts/java">Använda Java</a></p>
                        <p><a href="/azure/cognitive-services/Computer-vision/Quickstarts/node-analyze">Använda Node.js</a></p>
                        <p><a href="/azure/cognitive-services/Computer-vision/quickstarts-sdk/python-sdk">Använda Python</a></p>
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/i_quick-start.svg" alt="">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Identifiera tal med Speech Service</h3>
                        <p><a href="/azure/cognitive-services/speech-service/quickstart-csharp-windows/">Använda C#</a></p>
                        <p><a href="/azure/cognitive-services/speech-service/quickstart-js-browser">Använda JavaScript</a></p>
                        <p><a href="/azure/cognitive-services/speech-service/quickstart-python">Använda Python</a></p>
                        <p><a href="/azure/cognitive-services/speech-service/quickstart-java-jre">Använda Java</a></p>
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/i_quick-start.svg" alt="">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Hämta avsikt med LUIS</h3>
                        <p><a href="/azure/cognitive-services/luis/luis-get-started-cs-get-intent">Använda C#</a></p>
                        <p><a href="/azure/cognitive-services/luis/luis-get-started-java-get-intent">Använda Java</a></p>
                        <p><a href="/azure/cognitive-services/luis/luis-get-started-node-get-intent">Använda Node.js</a></p>
                        <p><a href="/azure/cognitive-services/luis/luis-get-started-python-get-intent">Använda Python</a></p>
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/i_quick-start.svg" alt="">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Identifiera språk, analysera attityd med textanalys</h3>
                        <p><a href="/azure/cognitive-services/Text-Analytics/quickstarts/csharp">Använda C#</a></p>
                        <p><a href="/azure/cognitive-services/Text-Analytics/quickstarts/java">Använda Java</a></p>
                        <p><a href="/azure/cognitive-services/Text-Analytics/quickstarts/nodejs">Använda Node.js</a></p>
                        <p><a href="/azure/cognitive-services/Text-Analytics/quickstarts/python">Använda Python</a></p>
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/i_quick-start.svg" alt="">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Få relevanta webbfrågor med webbsökning i Bing</h3>
                        <p><a href="/azure/cognitive-services/bing-web-search/quickstarts/csharp">Använda C#</a></p>
                        <p><a href="/azure/cognitive-services/bing-web-search/quickstarts/java">Använda Java</a></p>
                        <p><a href="/azure/cognitive-services/bing-web-search/quickstarts/nodejs">Använda Node.js</a></p>
                        <p><a href="/azure/cognitive-services/bing-web-search/web-sdk-python-quickstart">Använda Python</a></p>
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/i_quick-start.svg" alt="">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Få relevanta bildfrågor med bildsökning i Bing</h3>
                        <p><a href="/azure/cognitive-services/bing-image-search/quickstarts/csharp">Använda C#</a></p>
                        <p><a href="/azure/cognitive-services/bing-image-search/quickstarts/java">Använda Java</a></p>
                        <p><a href="/azure/cognitive-services/bing-image-search/quickstarts/nodejs">Använda Node.js</a></p>
                        <p><a href="/azure/cognitive-services/bing-image-search/image-sdk-python-quickstart">Använda Python</a></p>
                    </div>
                </div>
            </div>
        </div>
    </li>
</ul>

<h2>Steg för steg-självstudier</h2>
<ul class="cardsM panelContent singlePanelContent cols cols3" style="display:flex!important;">
    <li>
        <a class="card" href="/azure/cognitive-services/Computer-vision/tutorials/csharptutorial" data-linktype="external"><img class="cardImage" alt="" src="media/index/i_tasks.svg" data-linktype="external">
            <div class="cardText">
                <h3>Skapa en app med API för visuellt innehåll</h3>
            </div>
        </a>
    </li>
    <li>
        <a class="card" href="/azure/cognitive-services/speech-service/how-to-recognize-intents-from-speech-csharp" data-linktype="external"><img class="cardImage" alt="" src="media/index/i_tasks.svg" data-linktype="external">
            <div class="cardText" >
                <h3>Identifiera avsikter med Speech SDK och LUIS</h3>
            </div>
        </a>
    </li>
    <li>
        <a class="card" href="/azure/cognitive-services/luis/luis-quickstart-intents-only" data-linktype="external"><img class="cardImage" alt="" src="media/index/i_tasks.svg" data-linktype="external">
            <div class="cardText">
                <h3>Skapa en LUIS-app</h3>
            </div>
        </a>
    </li>
    <li>
        <a class="card" href="/azure/cognitive-services/Text-Analytics/tutorials/tutorial-power-bi-key-phrases" data-linktype="external"><img class="cardImage" alt="" src="media/index/i_tasks.svg" data-linktype="external">
            <div class="cardText">
                <h3>Använd textanalys med Power BI</h3>
            </div>
        </a>
    </li>
    <li>
        <a class="card" href="/azure/cognitive-services/bing-web-search/tutorial-bing-web-search-single-page-app" data-linktype="external"><img class="cardImage" alt="" src="media/index/i_tasks.svg" data-linktype="external">
            <div class="cardText">
                <h3>Skapa en app för webbsökning i Bing</h3>
            </div>
        </a>
    </li>
    <li>
        <a class="card" href="/azure/cognitive-services/bing-image-search/tutorial-bing-image-search-single-page-app" data-linktype="external"><img class="cardImage" alt="" src="media/index/i_tasks.svg" data-linktype="external">
            <div class="cardText">
                <h3>Skapa en app för bildsökning i Bing</h3>
            </div>
        </a>
    </li>
</ul>

<h2>Ytterligare resurser</h2>
<ul class="cardsF panelContent singlePanelContent" style="display:flex!important;">
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3></h3>
                        <p><a href="https://docs.microsoft.com/cli/azure/cognitiveservices?view=azure-cli-latest">Azure CLI</a></p>
                        <p><a href="https://docs.microsoft.com/powershell/module/az.cognitiveservices/?view=azps-2.5.0#cognitive_services">PowerShell</a></p>
                     </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3></h3>
                        <p><a href="https://docs.microsoft.com/rest/api/cognitiveservices/">REST API:er</a></p>
                     </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3></h3>
                        <p><a href="https://docs.microsoft.com/dotnet/api/overview/azure/cognitiveservices/client?view=azure-dotnet">.Net</a></p>
                        <p><a href="https://godoc.org/github.com/Azure/azure-sdk-for-go/services/cognitiveservices">Go</a></p>
                        <p><a href="https://docs.microsoft.com/java/api/overview/azure/cognitiveservices/client?view=azure-java-stable">Java</a></p>
                        <p><a href="https://docs.microsoft.com/javascript/api/overview/azure/cognitive-services?view=azure-node-latest">Node.js</a></p>
                        <p><a href="https://docs.microsoft.com/python/api/overview/azure/cognitive-services?view=azure-python">Python</a></p>
                     </div>
                </div>
            </div>
        </div>
    </li>
</ul>
</div>