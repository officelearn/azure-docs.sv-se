...
* [**Inbyggda program**](#built-ins): dessa inbyggda utlösare och åtgärder är "interna" för att Azure Logic Apps och hjälpa dig att skapa Logi Kap par som körs på anpassade scheman, kommunicerar med andra slut punkter, ta emot och svara på förfrågningar och anropa Azure Functions, Azure API Apps (webb Appar), dina egna API: er som hanteras och publicerats med Azure API Management och kapslade Logic-appar som kan ta emot förfrågningar. Du kan också använda inbyggda åtgärder som hjälper dig att organisera och kontrol lera din Logic app-arbetsflöde och även arbeta med data.
...
|   |   |   |   | 
|---|---|---|---| 
| [![API ikon @ no__t-2**schema**][recurrence-doc] | – Kör din Logic app enligt ett angivet schema, från grundläggande till komplex upprepning, med **upprepnings** utlösaren. <p>– Pausa din Logic app under en angiven varaktighet med **fördröjnings** åtgärden. <p>– Pausa din Logi Kap par fram till angivet datum och tid med **fördröjningen innan** åtgärden. | [![API Icon @ no__t-2**http**][http-doc] | Kommunicera med valfri slut punkt via HTTP med både utlösare och åtgärder för HTTP, HTTP + Swagger och HTTP + webhook. | 
| [![API ikon @ no__t-2-**begäran**][http-request-doc] | – Gör ditt Logic app-anrop från andra appar eller tjänster, Utlös på Event Grid resurs händelser eller Utlös på svar till Azure Security Center aviseringar med utlösaren för **begäran** . <p>– Skicka svar till en app eller tjänst med **svars** åtgärden. | [![API Icon @ no__t-2**batch**][batch-doc] | – Bearbeta meddelanden i batchar med utlösaren **batch-meddelanden** . <p>– Anropa Logi Kap par som har befintliga batch-utlösare med åtgärden **skicka meddelanden till batch** . | 
| [![API Icon @ no__t-2**Azure Functions**][azure-functions-doc] | Anropa Azure Functions som kör anpassade kod avsnitt (C# eller Node. js) från dina Logic Apps. | [![API Icon @ no__t-2**Azure API Management**][azure-api-management-doc] | Anropa utlösare och åtgärder som definierats av dina egna API: er som du hanterar och publicerar med Azure API Management. | 
| [![API Icon @ no__t-2**Azure App Services**][azure-app-services-doc] | Anropa Azure API Apps eller Web Apps som finns på Azure App Service. Utlösare och åtgärder som definieras av dessa appar visas som alla andra utlösare och åtgärder för första klass när Swagger ingår. | [![API ikon @ no__t-2**Azure @ no__t-4Logic-appar**][nested-logic-app-doc] | Anropa andra Logi Kap par som börjar med en begär ande utlösare. | 
||||| 
...
|   |   |   |   | 
|---|---|---|---| 
| [![Built-ikonen @ no__t-2**condition**][condition-doc] | Utvärdera ett villkor och kör olika åtgärder baserat på om villkoret är sant eller falskt. | [![Built-ikonen @ no__t-2**för varje**][for-each-doc] | Utför samma åtgärder på varje objekt i en matris. | 
| [![Built-ikonen @ no__t **-2**][scope-doc] | Gruppera åtgärder i *omfattningar*, som får sin egen status efter att åtgärderna i omfånget har körts. | [![Built @ no__t-2**switch**][switch-doc] | Gruppera åtgärder i *fall*, som tilldelas unika värden förutom standard fallet. Kör bara det fall vars tilldelade värde matchar resultatet från ett uttryck, ett objekt eller en token. Kör standard fallet om inga matchningar finns. | 
| [![Built @ no__t-2**Terminate**][terminate-doc] | Stoppa ett aktivt Logic app-arbetsflöde som körs. | [![Built-ikonen @ no__t-2**tills**][until-doc] | Upprepa åtgärder tills det angivna villkoret är sant eller så har status ändrats. | 
||||| 
...
|   |   | 
|---|---| 
| [![Built-ikonen @ no__t-2**data åtgärder**][data-operations-doc] | Utför åtgärder med data: <p>- **Skriv**: skapa en enda utmatning från flera indata med olika typer. <br>- **Skapa CSV-tabell**: skapa en kommaavgränsad tabell (CSV) från en matris med JSON-objekt. <br>- **skapa HTML-tabell**: skapa en HTML-tabell från en matris med JSON-objekt. <br>- -**filter mat ris**: skapa en matris från objekt i en annan matris som uppfyller dina kriterier. <br>- **Join**: skapa en sträng från alla objekt i en matris och avgränsa dessa objekt med angiven avgränsare. <br>- **parsa JSON**: skapa användarvänliga token från egenskaper och deras värden i JSON-innehåll så att du kan använda dessa egenskaper i ditt arbets flöde. <br>- **Välj**: skapa en matris med JSON-objekt genom att transformera objekt eller värden i en annan matris och mappa dessa objekt till angivna egenskaper. | 
| ![Built @ no__t-1<br/>**Datum tid** | Utföra åtgärder med tidsstämplar: <p>- **Lägg till i tid**: Lägg till det angivna antalet enheter i en tidsstämpel. <br>- **konverterings tids zon**: konvertera en tidsstämpel från käll tids zonen till mål tids zonen. <br>- **aktuell tid**: returnera den aktuella tidsstämpeln som en sträng. <br>- **få framtida tid**: returnera den aktuella tidsstämpeln och de angivna tidsenheterna. <br>- **Hämta tidigare tid**: returnera den aktuella tidsstämpeln minus de angivna tidsenheterna. <br>- **Subtrahera från tid**: subtrahera ett antal tidsenheter från en tidsstämpel. |
| [![Built @ no__t-2**variabler**][variables-doc] | Utföra åtgärder med variabler: <p>-  tillägg**till mat ris variabel**: infoga ett värde som det sista objektet i en matris som lagras av en variabel. <br>- **Lägg till sträng variabel**: infoga ett värde som det sista tecken i en sträng som lagras av en variabel. <br>**variabeln**- : minska en variabel med ett konstant värde. <br>- **Increment-variabel**: öka en variabel med ett konstant värde. <br>- **initiera variabel**: skapa en variabel och deklarera dess datatyp och initialt värde. <br>- **ange variabel**: tilldela ett annat värde till en befintlig variabel. |
|  |  | 
...
|   |   |   |   | 
|---|---|---|---| 
| [![API Icon @ no__t-2**Azure Service Bus**][azure-service-bus-doc] | Hantera asynkrona meddelanden, sessioner och ämnesprenumerationer med det anslutningsprogram som oftast används i Logic Apps. | [![API Icon @ no__t-2**SQL Server**][sql-server-doc] | Anslut till din SQL Server lokalt eller en Azure SQL Database i molnet så att du kan hantera poster, köra lagrade procedurer eller köra frågor. | 
| [![API Icon @ no__t-2**Office 365 @ no__t-4Outlook**][office-365-outlook-doc] | Anslut till ditt Office 365-e-postkonto så att du kan skapa och hantera e-post, uppgifter, Kalender händelser och möten, kontakter, förfrågningar och mycket annat. | [![API Icon @ no__t-2**Azure Blob @ no__t-4Storage**][azure-blob-storage-doc] | Anslut till ditt lagrings konto så att du kan skapa och hantera BLOB-innehåll. | 
| [![API Icon @ no__t-2**SFTP**][sftp-doc] | Anslut till SFTP-servrar som du kan komma åt från Internet så att du kan arbeta med dina filer och mappar. | [![API Icon @ no__t-2**SharePoint @ no__t-4Online**][sharepoint-online-doc] | Anslut till SharePoint Online så att du kan hantera filer, bilagor, mappar med mera. | 
| [![API Icon @ no__t-2**Dynamics 365 @ no__t-4CRM online**][dynamics-365-doc] | Anslut till ditt Dynamics 365-konto så att du kan skapa och hantera poster, artiklar med mera. | [![API Icon @ no__t-2**FTP**][ftp-doc] | Anslut till FTP-servrar som du kan komma åt från Internet så att du kan arbeta med dina filer och mappar. | 
| [![API Icon @ no__t-2**Salesforce**][salesforce-doc] | Anslut till ditt Salesforce-konto så att du kan skapa och hantera objekt, till exempel poster, jobb, objekt med mera. | [![API Icon @ no__t-2**Twitter**][twitter-doc] | Anslut till ditt Twitter-konto så att du kan hantera tweets, följare, din tids linje och mer. Spara din tweets i SQL, Excel eller SharePoint. | 
| [![API Icon @ no__t-2**Azure Event Hubs**][azure-event-hubs-doc] | Använda och publicera händelser via en Event Hub. Du kan till exempel hämta utdata från din Logic app med Event Hubs och sedan skicka utdata till en leverantör av real tids analys. | [![API ikon @ no__t-2**Azure Event**</br>-**rutnät**][azure-event-grid-doc] | Övervaka händelser som publicerats av en Event Grid, till exempel när Azure-resurser eller resurser från tredje part ändras. | 
|||||
...
|   |   |   |   |   | 
|---|---|---|---|---| 
| ![API Icon @ no__t-1<br/>**Inköpsoffertförfrågan**</br> **Server** | [![API ikon @ no__t-2**File @ no__t-4 system**][file-system-doc] | [![API Icon @ no__t-2**IBM DB2**][ibm-db2-doc] | [![API Icon @ no__t-2**IBM**</br> **Informix**][ibm-informix-doc] | ![API Icon @ no__t-1<br/>**MySQL** | 
| [![API Icon @ no__t-2**Oracle DB**][oracle-db-doc] | ![API Icon @ no__t-1<br/>**PostgreSQL** | [![API Icon @ no__t-2**SharePoint @ no__t-4 Server**][sharepoint-server-doc] | [![API Icon @ no__t-2**SQL @ no__t-4 Server**][sql-server-doc] | ![API Icon @ no__t-1<br/>**Teradata** | 
|||||
...
|   |   |   |   | 
|---|---|---|---| 
| [![API Icon @ no__t-2**AS2 @ no__t-4-avkodning**][as2-doc] | [![API ikon @ no__t-2**AS2 @ no__t-4 encoding**][as2-doc] | [![API Icon @ no__t-2**EDIFACT @ no__t-4-avkodning**][edifact-decode-doc] | [![API ikon @ no__t-2**EDIFACT @ no__t-4 encoding**][edifact-encode-doc] | 
| [![API ikon @ no__t-2**Flat File @ no__t-4-avkodning**][flat-file-decode-doc] | [![API ikon @ no__t-2**Flat File @ no__t-4 encoding**][flat-file-encode-doc] | [![API Icon @ no__t-2**integration @ no__t-4account**][integration-account-doc] | [![API Icon @ no__t-2**flytande**</br>**Transforms**][json-liquid-transform-doc] | 
| [![API Icon @ no__t-2**X12 @ no__t-4-avkodning**][x12-decode-doc] | [![API ikon @ no__t-2**X12 @ no__t-4 encoding**][x12-encode-doc] | [![API Icon @ no__t-2**XML**</br>**Transforms**][xml-transform-doc] | [![API Icon @ no__t-2**XML <br/>validation**][xml-validate-doc] |  
||||| 
...
[microsoft-translator-doc]: ./connectors-create-api-microsofttranslator.md "Anslut till Microsoft Translator. Översätt text, identifiera språk och mer" 
