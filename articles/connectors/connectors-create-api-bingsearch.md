---
title: Ansluta till Bing Search - Azure Logic Apps | Microsoft Docs
...
2. I sökrutan anger du ”Bing search” som filter. Välj utlösaren som du vill använda från listan över utlösare. 
   ...
   Det här exemplet används den här utlösaren: **Bing Search - på nya nyhetsartikel**

   ![Hitta Bing Search-utlösare](./media/connectors-create-api-bing-search/add-trigger.png)

3. Om du uppmanas anslutningsinformation [skapa anslutningen Bing Search nu](#create-connection). Eller om anslutningen redan finns, ange nödvändig information för utlösaren.

   Ange villkor för att returnera matchande nyhetsartiklar från Bing Search i det här exemplet.

   | Egenskap  | Krävs | Värde | Beskrivning | 
   |----------|----------|-------|-------------| 
   | Sökfråga | Ja | <*sökord*> | Ange sökord som du vill använda. |
   | Marknad | Ja | <*Nationella inställningar*> | Sök nationella inställningar. Standardvärdet är ”en-US”, men du kan välja ett annat värde. | 
   | Säker sökning | Ja | <*Sök på servernivå*> | Filternivå för att undanta vuxet innehåll. Standardvärdet är ”måttlig”, men du väljer en annan nivå. | 
   | Antal | Nej | <*antal resultat*> | Returnera det angivna antalet resultat. Standardvärdet är 20, men du kan ange ett annat värde. Det faktiska antalet returnerade resultaten kan vara mindre än det angivna värdet. | 
   | Offset | Nej | <*Hoppa över-värde*> | Antalet resultat som hoppas över innan det returneras resultat | 
   ||||| 
...
4. Om du uppmanas anslutningsinformation [skapa anslutningen Bing Search nu](#create-connection). Eller om anslutningen redan finns, ange informationen som krävs för åtgärden. 
   ...
   | Egenskap  | Krävs | Värde | Beskrivning | 
   |----------|----------|-------|-------------| 
   | Sökfråga | Ja | <*Sök-uttryck*> | Ange ett uttryck för att fråga utlösaren resultaten. Du kan välja från fälten i listan med dynamiskt innehåll eller skapa ett uttryck med Uttrycksverktyget. |
   | Marknad | Ja | <*Nationella inställningar*> | Sök nationella inställningar. Standardvärdet är ”en-US”, men du kan välja ett annat värde. | 
   | Säker sökning | Ja | <*Sök på servernivå*> | Filternivå för att undanta vuxet innehåll. Standardvärdet är ”måttlig”, men du väljer en annan nivå. | 
   | Antal | Nej | <*antal resultat*> | Returnera det angivna antalet resultat. Standardvärdet är 20, men du kan ange ett annat värde. Det faktiska antalet returnerade resultaten kan vara mindre än det angivna värdet. | 
   | Offset | Nej | <*Hoppa över-värde*> | Antalet resultat som hoppas över innan det returneras resultat | 
...
1. När du uppmanas att ange anslutningsinformationen Tillhandahåll följande information:
   ...
   | API-version | Ja | <*API-versionen*> | Bing Search API-versionen är som standard den aktuella versionen. Du kan välja en tidigare version vid behov. | 
   | API-nyckel | Ja | <*API-nyckel*> | Bing Search API-nyckeln som du fick tidigare. Om du inte har en nyckel kan få din [nu API-nyckel](https://azure.microsoft.com/try/cognitive-services/?api=bing-news-search-api). |  
   |||||  
...
Teknisk information, till exempel utlösare och åtgärder gränser, enligt beskrivningen av kopplingens Swagger-fil, finns i den [anslutningsappens-referenssida](/connectors/bingsearch/). 
