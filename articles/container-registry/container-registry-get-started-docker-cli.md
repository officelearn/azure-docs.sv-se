---
title: Push Docker-avbildning till privat Azure Container Registry
...
* **Docker CLI** – du måste också ha Docker installerat lokalt. Docker innehåller paket som enkelt kan konfigurera Docker på alla [MacOS][docker-mac]-, [Windows][docker-windows]-och [Linux][docker-linux] -system.
...
Kör följande [Docker](https://docs.docker.com/engine/reference/run/) -körnings kommando för att starta en lokal instans av nginx-behållaren interaktivt (`-it`) på Port 8080. `--rm` Argumentet anger att behållaren ska tas bort när du stoppar den.
...
Om du vill stoppa och ta bort behållaren `Control`trycker du på + `C`.
...
## <a name="push-the-image-to-your-registry"></a>Push-överför avbildningen till registret
