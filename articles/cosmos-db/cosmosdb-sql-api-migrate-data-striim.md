...
1. I fältet **sökmall** skriver du "Cosmos" och väljer **mål: Azure Cosmos DB** och välj sedan **Oracle CDC för att Azure Cosmos DB**.
...
1. Ange sedan käll konfigurationen för din käll-Oracle-instans. Ange ett värde för **källans namn**. Käll namnet är bara en namngivnings konvention för Striims-programmet, du kan använda något som **src_onPremOracle**. Ange värden för resten av käll parametrarna **URL**, **användar namn**, **lösen ord**och välj **LogMiner** som läsare för att läsa data från Oracle. Välj **Nästa** för att fortsätta.

   ![Konfigurera käll parametrar](./media/cosmosdb-sql-api-migrate-data-striim/configure-source-parameters.png)
...
1. Striims kommer att kontrol lera din miljö och kontrol lera att den kan ansluta till din käll-Oracle-instans, har rätt behörigheter och att CDC har kon figurer ATS korrekt. När alla värden har verifierats väljer du **Nästa**.
   ...
   ![Verifiera käll parametrar](./media/cosmosdb-sql-api-migrate-data-striim/validate-source-parameters.png)

1. Välj de tabeller från Oracle-databasen som du vill migrera. Låt oss till exempel välja tabellen Orders och välj **Nästa**. 

   ![Välj käll tabeller](./media/cosmosdb-sql-api-migrate-data-striim/select-source-tables.png)
...
1. Nu ska vi konfigurera målet:
   ...
   * **ServiceEndpoint** – URI för ditt Azure Cosmos-konto finns i avsnittet **nycklar** i Azure Portal. 
   ...
   ![Konfigurera mål parametrar](./media/cosmosdb-sql-api-migrate-data-striim/configure-target-parameters.png)
...
1. I fönstret distribution kan du ange om du vill köra vissa delar av programmet på specifika delar av distributions miljön. Eftersom vi kör i en enkel distributions topologi via Azure använder vi standard alternativet.
   ...
   ![Använd standard alternativet](./media/cosmosdb-sql-api-migrate-data-striim/deploy-using-default-option.png)
