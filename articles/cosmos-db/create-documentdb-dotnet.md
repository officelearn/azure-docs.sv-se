...
Den här snabbstarten demonstrerar hur man skapar ett Azure Cosmos DB-konto, en dokumentdatabas och en samling med hjälp av Azure-portalen. Därefter skapar och distribuerar du en webbapp med en att göra-lista som bygger på [DocumentDB .NET-API:n](../documentdb/documentdb-sdk-dotnet.md)som visas i följande skärmbild. 
...
## <a name="add-a-collection"></a>Lägg till en samling
...
1. I datautforskaren visas den nya databasen i panelen samlingar. Expandera **Objekt**-databasen, expandera **ToDoList**-samlingen, klicka på **Dokument** och klicka sedan på **Nya dokument**. 

   ![Skapa nya dokument i datautforskaren i Azure-portalen](./media/create-documentdb-dotnet/azure-cosmosdb-data-explorer-new-document.png)
...
2. Kör följande kommando för att klona exempel-lagringsplatsen. 
...
Vi gör en snabb genomgång av vad som händer i appen. Öppna filen DocumentDBRepository.cs så ser du att de här kodraderna skapar Azure DB Cosmos-resurserna. 
...
Gå nu tillbaka till Azure-portalen för att få informationen om din anslutningssträng och kopiera den till appen.

1. I [Azure-portalen](http://portal.azure.com/), i ditt Azure Cosmos DB-konto i det vänstra navigeringsfönstret, klickar du på **nycklar** och därefter på **läs-skriv nycklar**. Du använder kopiera-knapparna till höger på skärmen för att kopiera URI:n och primärnyckeln i web.config-filen i nästa steg.

    ![Visa och kopiera åtkomsttangenten i Azure-portalen, nyckel-bladet](./media/create-documentdb-dotnet/keys.png)
...
Du kan nu gå tillbaka till datautforskaren och se fråga, ändra och arbeta med dessa nya data. 
