...
- Möjligheten att växla över från ett datacenter till en annan
...
|Active directory-integrering (RBAC)| Du kan också ange eller begränsa åtkomst till Cosmos-konto, databas, behållare och erbjudanden (dataflöde) med hjälp av åtkomstkontroll (IAM) i Azure-portalen. IAM ger rollbaserad åtkomstkontroll och integreras med Active Directory. Du kan använda inbyggda roller eller anpassade roller för enskilda användare och grupper. Se [Active Directory-integrering](role-based-access-control.md) artikeln för mer information.|
...
|Lokal replikering|Även i ett datacenter, Azure Cosmos DB automatiskt replikerar data för hög tillgänglighet, vilket ger dig valet av [konsekvensnivåer](consistency-levels.md). Replikeringen garanterar en 99,99% [tillgänglighets-SLA](https://azure.microsoft.com/support/legal/sla/cosmos-db) för alla och alla konton i flera regioner med restriktiva konsekvens och 99,999% läsningstillgänglighet för alla databaskonton för flera regioner konton.|
|Automatiserad onlinesäkerhetskopieringar|Azure Cosmos DB-databaser säkerhetskopieras regelbundet och lagras i ett geo-redundant lager. <br><br>Läs mer i [automatisk online säkerhetskopiering och återställning med Azure Cosmos DB](online-backup-and-restore.md).|
...
|Skydda och isolera känsliga data|Alla data i regioner enligt vad som är nytt? Nu är krypterade i vila.<br><br>Personliga data och andra känsliga data kan isoleras specifika behållaren och Skriv- eller läsbehörighet kan begränsas till specifika användare.|
...
|HTTPS/SSL/TLS-kryptering|Alla klient-till-tjänst Azure Cosmos DB-interaktioner är SSL/TLS 1.2-kompatibla. Alla intra datacenter och mellan datacenter replikeringar är dessutom SSL/TLS 1.2 tillämpas.|
...
|Säkerhet och data protection-certifieringar| Uppdaterad lista över certifieringar finns i totala [webbplatsen Azure efterföljande](https://www.microsoft.com/en-us/trustcenter/compliance/complianceofferings) samt senast [Azure efterlevnad dokumentet](https://gallery.technet.microsoft.com/Overview-of-Azure-c1be3942) med alla certifieringar (Sök efter Cosmos). En mer fokuserad finns Kolla in den 25 April 2018 inlägget [Azure #CosmosDB: Säker, privata och kompatibla som innehåller SOC 1/2 typ 2, HITRUST, PCI DSS Level 1, ISO 27001, HIPAA, FedRAMP High och många andra.
...
Mer information om huvudnycklar och resurstokens finns [skydda åtkomst till Azure Cosmos DB data](secure-access-to-data.md).

Läs mer om granskningsloggning [Diagnostisk loggning för Azure Cosmos DB](logging.md).

Läs mer om Microsoft-certifieringar [Azure Trust Center](https://azure.microsoft.com/support/trust-center/).