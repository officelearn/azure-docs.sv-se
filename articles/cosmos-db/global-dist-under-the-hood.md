...
En fysisk partition är materialiserad som en fristående och dynamiskt belastningsutjämnad grupp av repliker som sprids över flera feldomäner, kallas en replikuppsättning. Den här uppsättningen implementerar sammantaget protokollet replikerade tillstånd datorn för att göra data inom fysisk partition med hög tillgänglighet, hållbar och konsekvent. Medlemskap i replikuppsättningen N är dynamisk – det håller växlar mellan NMin och nmax baserat på felen, administrativa åtgärder och tiden för misslyckad repliker för att återskapa eller återställa. Baserat på medlemskap ändras kan replikeringen protokollet också Omkonfigurerar storleken på läsa och skriva beslutsförhet. Om du vill distribuera ett enhetligt sätt det dataflöde som har tilldelats en viss fysisk partition, ska vi använda två idéer: först kostnaden för bearbetning av skrivbegäranden på ledaren är högre än kostnaden för tillämpning av uppdateringarna på i det följande. På motsvarande sätt ledare budgeterade mer systemresurser än följare. För det andra, så långt möjligt Läs kvorum för en viss konsekvensnivå uteslutande består av följande replikerna. Vi undvika att kontakta ansvariga för att betjäna läsningar såvida inte. Vi använder ett antal idéer från research på relationen mellan [belastning och kapacitet](https://www.cs.utexas.edu/~lorenzo/corsi/cs395t/04S/notes/naor98load.pdf) i kvorum-datorer för fem konsekvens modeller som stöder Cosmos DB.  
