...
## <a name="high-throughput"></a>Högt dataflöde
...
![Serverdel](./media/relational-or-nosql/backend-scaled.png)
...
Om du hanterar data vars strukturer ständigt förändras i hög grad, särskilt om transaktioner kan komma från externa källor där det är svårt att genomdriva överensstämmelse i databasen, kanske du vill överväga en mer schema-oberoende metod använda en hanterad NoSQL Database-tjänst som Azure Cosmos DB.
...
En metod för metod tips i en NoSQL-dokument databas är att avnormalisera kategori namn och taggnamn direkt i ett "produkt dokument". För att se till att kategorier, taggar och produkter är synkroniserade har design alternativen för att under lätta detta ytterligare underhåll, eftersom data dupliceras över flera poster i produkten, i stället för en enkel uppdatering i en "en-till-många" relation och en koppling för att hämta data. 
