...
* **Objekt storlek**: När storleken på ett objekt ökar ökar också antalet ru: er som förbrukas för att läsa eller skriva objektet.
...
* **Antal objekt egenskaper**: Förutsatt att standard indexeringen är på alla egenskaper ökar antalet förbrukade ru: er för att skriva ett objekt när antalet objekt egenskaper ökar.

* **Indexerade egenskaper**: En index princip för varje behållare avgör vilka egenskaper som indexeras som standard. Begränsa antalet indexerade egenskaper för att minska RU-förbrukningen för Skriv åtgärder.

* **Data konsekvens**: De starka och begränsade inaktuella inaktuella konsekvens nivåerna förbrukar ungefär två gånger mer ru: er medan Läs åtgärder utförs jämfört med andra avslappnade konsekvens nivåer.

* **Fråga mönster**: En frågas komplexitet påverkar hur många ru: er som används för en åtgärd. Faktorer som påverkar kostnaden för Query-åtgärder är: 
...
* **Skript användning**: Precis som med frågor använder lagrade procedurer och utlösare ru: er baserat på komplexiteten för de åtgärder som utförs. När du utvecklar ditt program bör du kontrol lera [rubriken begär ande avgift](optimize-cost-queries.md#evaluate-request-unit-charge-for-a-query) för att bättre förstå hur mycket ru-kapacitet varje åtgärd förbrukar.
