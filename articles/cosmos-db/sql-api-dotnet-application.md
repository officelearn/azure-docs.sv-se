---
title: 'ASP.NET Core MVC självstudier för Azure Cosmos DB: Utveckling av webbapp'
description: ASP.NET Core MVC-självstudie för att skapa ett MVC-webbprogram med Azure Cosmos DB. Du lagrar JSON och komma åt data från en att göra-app på Azure App Service – ASP NET Core MVC självstudier steg för steg.
author: SnehaGunda
...
![Skärmbild av MVC-webbprogram som skapats av den här självstudien – ASP NET Core MVC självstudier steg för steg för att göra-lista](./media/sql-api-dotnet-application/asp-net-mvc-tutorial-image01.png)

Om du inte har tid att slutföra den här självstudien kan du hämta det fullständiga exempelprojektet från [GitHub][GitHub].
...
> [!TIP]
> Den här självstudien förutsätter att du har tidigare erfarenhet av ASP.NET Core MVC och Azure App Service. Om du har använt ASP.NET Core eller [verktygen som krävs](#prerequisites), rekommenderar vi att du hämta det fullständiga exempelprojektet från [GitHub][GitHub]lägger du till de nödvändiga NuGet-paketen och kör den. När du skapar projektet kan du läsa den här artikeln för att få information om koden i projektets sammanhang.
...
Alla skärmdumpar i den här artikeln har tagits med hjälp av Microsoft Visual Studio Community 2019. Om ditt system är konfigurerat med en annan version kan det hända att skärmbilder och alternativ inte ser riktigt likadana ut, men om ovanstående krav är uppfyllda ska lösningen fungera.
...
2. I den **nytt projekt** och Använd den **Sök efter mallar** textrutan att söka efter ”Web” och välj sedan **ASP.NET Core-Webbapp**. 

   ![Skapa ny ASP.NET Core-webbprogramprojektet](./media/sql-api-dotnet-application/asp-net-mvc-tutorial-new-project-dialog.png)

3. I rutan **Namn** skriver du namnet på projektet. I den här självstudiekursen används namnet ”todo”. Om du väljer att använda något annat än det här namnet, oavsett var den här självstudien pratar om namnområdet todo justera sedan de angivna kodexemplen om du vill använda det namn du gav ditt program. 

4. Välj **Bläddra** att navigera till mappen där du vill skapa projektet. Välj **Skapa**. 

5. Den **skapa ett nytt webbprogram för ASP.NET Core** dialogrutan visas. Välj i listan över mallar **webbprogram (Model-View-Controller)** .

6. Välj **skapa** och låt Visual Studio autogenererar runt tom ASP.NET Core MVC-mallen. 
...
Nu när vi har i ASP.NET Core MVC framework koden som vi behöver för lösningen och Lägg till NuGet-paket som krävs för att ansluta till Azure Cosmos DB.
...
1. Azure Cosmos DB .NET SDK paketeras och distribueras som ett NuGet-paket. Hämta NuGet-paketet i Visual Studio med hjälp av NuGet-pakethanteraren i Visual Studio genom att högerklicka på projektet i **Solution Explorer** och sedan välja **Hantera NuGet-paket**.
   ...
   ![Skärmbild av snabbemenyalternativen för webbprogramprojektet i Solution Explorer, med hantera NuGet-paket markerat.](./media/sql-api-dotnet-application/asp-net-mvc-tutorial-manage-nuget.png)
   
2. Dialogrutan **Hantera NuGet-paket** öppnas. I rutan för att **Bläddra** för NuGet skriver du **Microsoft.Azure.Cosmos**. Från resultatet installerar den **Microsoft.Azure.Cosmos** paketet. Den hämtar och installerar Azure Cosmos DB-paketet och dess beroenden. Välj **jag accepterar** i den **godkännande av licensen** fönster för att slutföra installationen.
...
3. När paketet har installerats, ska Visual Studio-projektet innehålla biblioteksreferens till Microsoft.Azure.Cosmos.
  
## <a name="set-up-the-mvc-application"></a>Steg 4: Ställ in ASP.NET Core MVC-program
...
1. Namnge den nya klassen **Item.cs** och välj **Lägg till**. 

1. Ersätt sedan koden i ”Item.cs” klassen med följande kod:

   [!code-csharp[Main](~/samples-cosmosdb-dotnet-core-web-app/src/Models/Item.cs)]
   
   De data som lagras i Azure Cosmos DB skickas via anslutningen och lagras som JSON. Du kan använda för att styra hur objekten serialiseras/deserialiseras av JSON.NET genom den **JsonProperty** attributet som visas i den **objekt** klass som du skapade. Inte bara styra formatet för egenskapsnamnet att försätta datorn i JSON, du kan också byta namn på dina .NET-egenskaper som du gjorde med den **slutförd** egenskapen. 
...
1. Välj **MVC-kontrollant – tom** och välj **Lägg till**.
   ...
   ![Skärmbild av dialogrutan Lägg till Kodskelett med MVC-kontrollant – tom alternativet markerat](./media/sql-api-dotnet-application/asp-net-mvc-tutorial-controller-add-scaffold.png)

1. Namnge din nya styrenhet **ItemController**, och Ersätt Koden i filen med följande kod:

   [!code-csharp[Main](~/samples-cosmosdb-dotnet-core-web-app/src/Controllers/ItemController.cs)]

   Attributet **ValidateAntiForgeryToken** används här för att skydda programmet mot attacker med förfalskning av begäran mellan webbplatser. Det finns mer att göra än att bara lägga till det här attributet. Även dina vyer bör fungera med den här antiförfalskningstoken. Mer information om ämnet och exempel på korrekt implementering finns i [det gick inte att begära förfalskning][Preventing Cross-Site Request Forgery] . The source code provided on [GitHub][GitHub] har detta fullständigt implementerat.
...
1. I **Solution Explorer** expanderar du mappen **Vyer** och högerklickar på den tomma **objektmapp** som Visual Studio skapade när du lade till **ItemController** tidigare. Klicka på **Lägg till** och sedan på **Vy**.
   ...
   ![Skärmbild av Solution Explorer som visar den objektmapp som Visual Studio skapade med kommandona Lägg till vy markerat](./media/sql-api-dotnet-application/asp-net-mvc-tutorial-add-view.png)
...
2. I dialogrutan **Lägg till vy** uppdaterar du följande värden:
   ...
   * Skriv ***~/Views/Shared/_Layout.cshtml*** i rutan på layoutsidan.
   ...
   ![Skärmbild som visar dialogrutan Lägg till vy](./media/sql-api-dotnet-application/asp-net-mvc-tutorial-add-view-dialog.png)
...
Det första du gör här är att lägga till en klass som innehåller logiken för att ansluta till och använda Azure Cosmos DB. I den här självstudiekursen kapslar vi in den här logiken i en klass med namnet `CosmosDBService` och ett gränssnitt kallas `ICosmosDBService`. Den här tjänsten utför CRUD och feed läsåtgärder, till exempel lista ofullständiga objekt, skapa, redigera och ta bort objekt. 
...
1. Lägg till följande kod till den **CosmosDBService** klassen och Ersätt Koden i filen med följande kod:

   [!code-csharp[Main](~/samples-cosmosdb-dotnet-core-web-app/src/Services/CosmosDbService.cs)]

1. Upprepa steg 2 – 3, men den här tiden för en klass med namnet **ICosmosDBService**, och Lägg till följande kod:

   [!code-csharp[Main](~/samples-cosmosdb-dotnet-core-web-app/src/Services/ICosmosDbService.cs)]
 
1. Den tidigare koden tar emot en `CosmosClient` som en del av konstruktorn. Följande ASP.NET Core pipeline, vi behöver att gå till projektets **Startup.cs** och initiera klienten baserat på konfigurationen som en Singleton-instans att matas in via [Beroendeinmatning](https://docs.microsoft.com/aspnet/core/fundamentals/dependency-injection). I den **ConfigureServices** hanteraren, definierar vi:

    ```csharp
    services.AddSingleton<ICosmosDbService>(InitializeCosmosClientInstanceAsync(Configuration.GetSection("CosmosDb")).GetAwaiter().GetResult());
    ```

1. I samma fil definierar vi vår hjälpmetoden **InitializeCosmosClientInstanceAsync**, som läser konfigurationen och initiera klienten.
...
1. Konfigurationen har definierats i projektets **appsettings.json** fil. Öppna den och lägga till ett avsnitt som heter **CosmosDb**:
...
Nu om du kör programmet, ASP.NET Core pipeline instantierar **CosmosDbService** och underhålla en enda instans som Singleton; när **ItemController** används för att bearbeta begäranden på klientsidan, den ska ta emot den instansen och att kunna använda den för att utföra CRUD-åtgärder.

Om du bygger och kör det här projektet nu bör du nu se något som ser ut så här:

![Skärmbild av webbprogram för att göra lista skapades av självstudien databas](./media/sql-api-dotnet-application/build-and-run-the-project-now.png)
...
1. Tryck på F5 i Visual Studio för att bygga appen i felsökningsläge. Den ska bygga appen och starta en webbläsare med den tomma rutnätssidan vi såg tidigare:
   ...
   ![Skärmbild av webbprogram för att göra lista skapas i den här självstudiekursen](./media/sql-api-dotnet-application/asp-net-mvc-tutorial-create-an-item-a.png)
...
3. Klicka på **Skapa**, så omdirigeras du tillbaka till vyn **Index** och objektet visas i listan. Du kan lägga till några fler objekt i att göra-listan.
    ...
    ![Skärmbild av vyn Index](./media/sql-api-dotnet-application/asp-net-mvc-tutorial-create-an-item.png)
  
4. Klicka på **Redigera** bredvid ett **objekt** på listan så kommer du till vyn **Redigera**, där du kan uppdatera alla egenskaper för objektet, inklusive flaggan **Slutförd**. Om du markerar den **Slutför** flagga och på **spara**, **objekt** visas som slutförd i listan.
   
   ![Skärmbild av indexvyn med rutan slutförd ikryssad](./media/sql-api-dotnet-application/asp-net-mvc-tutorial-completed-item.png)

5. När som helst kan du kontrollera status för data i Azure Cosmos DB-tjänsten med [Cosmos Explorer](https://cosmos.azure.com) eller Azure Cosmos DB-emulatorns Data Explorer.
...
2. I den **publicera** dialogrutan **Apptjänst**och välj sedan **Skapa ny** att skapa en App Service-profil eller välj **Välj befintligt**att använda en befintlig profil.
...
Inom några sekunder, Visual Studio publicerar ditt webbprogram och startar en webbläsare där du kan se ditt projekt som körs i Azure!

## <a name="next-steps"></a>Nästa steg
I den här självstudien har du lärt dig hur du skapar en ASP.NET Core MVC-webbprogram som kan komma åt data som lagras i Azure Cosmos DB. Nu kan du fortsätta till nästa artikel:

* [Lär dig om att partitionera dina data i Azure Cosmos DB](./partitioning-overview.md)
* [Lär dig mer om hur du gör mer avancerade frågor i Azure Cosmos DB](./how-to-sql-query.md)
* [Läs om hur du modellera dina data i ett mer avancerat scenario](./how-to-model-partition-example.md)
