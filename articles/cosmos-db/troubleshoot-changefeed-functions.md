...
* Kom ihåg att den unika identifieraren för ett dokument är `id` och dess partitionsnyckel (det kan finnas två dokument med samma `id` men olika partitionsnyckel) om du identifierar dokument precis som `id`.
...
Ett enkelt sätt att komma runt den här situationen är att använda en `LeaseCollectionPrefix/leaseCollectionPrefix` till din funktion med ett nytt/annat värde eller, även testa med en ny container container.
