...
- Ersätt ``{database_id}`` och ``{container_id}`` med namnet på databasen och behållaren i ditt Cosmos-konto. Om ``--database`` argumenten ``--container`` och inte har angetts körs frågan på [standard databasen och-behållaren](#set-default-database-for-queries).
...
- Ersätt ``{database_id}`` och ``{container_id}`` med namnet på databasen och behållaren i ditt Azure Cosmos-konto. Om ``--database`` argumenten ``--container`` och inte har angetts körs frågan på [standard databasen och-behållaren](#set-default-database-for-queries).
...
Du kan använda den inbyggda [nteract data Explorer](https://blog.nteract.io/designing-the-nteract-data-explorer-f4476d53f897) för att filtrera och visualisera en DataFrame. Om du vill aktivera den här funktionen ställer ``pd.options.display.html.table_schema`` du ``True`` in ``pd.options.display.max_rows`` alternativet till och det önskade värdet (du ``pd.options.display.max_rows`` kan ``None`` ställa in för att visa alla resultat).
...
```
> [!IMPORTANT]
> ``COSMOS_ENDPOINT`` Variablerna ``COSMOS_KEY`` och kan bara användas för SQL API. För andra API: er hittar du slut punkten och nyckeln i bladet **anslutnings strängar** eller **nycklar** i ditt Azure Cosmos-konto.  
