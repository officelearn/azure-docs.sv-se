...
>- **Som källa**, i Data explorer -> åtkomst, ge minst **Läs + kör** behörighet att visa och kopiera filerna i mappen/undermappar, eller **Läs** tillstånd att kopiera en fil, och väljer att Lägg till som **behörigheten och en standardbehörighetsinlägg**. Inga krav på kontot på åtkomstkontroll (IAM).
>- **Som mottagare**, i Data explorer -> åtkomst, ge minst **skriva + köra** behörighet att skapa underordnade objekt i mappen och välja att lägga till som **behörigheten och en standardbehörighetsinlägg**. Om du använder Azure IR för att kopiera (både källa och mottagare finns i molnet), i åtkomstkontroll (IAM), beviljar minst **läsare** roll för att identifiera Data Lake Store-region med Data Factory. Om du vill undvika den här IAM-rollen uttryckligen [skapa en Azure IR](create-azure-integration-runtime.md#create-azure-ir) med platsen för ditt Data Lake Store och associera i Data Lake Store-länkade tjänst som i följande exempel.
