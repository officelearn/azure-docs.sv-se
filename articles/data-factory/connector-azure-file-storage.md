...
Den här artikeln beskrivs hur du använder Kopieringsaktivitet i Azure Data Factory för att kopiera data från och till Azure File Storage (Azure Files). Den bygger på den [översikt över Kopieringsaktivitet](copy-activity-overview.md) artikel som ger en allmän översikt över Kopieringsaktivitet.

## <a name="supported-capabilities"></a>Funktioner som stöds

Du kan kopiera data från Azure Blob Storage till alla mottagarens datalager eller kopiera data från alla dataarkiv till Azure File Storage. En lista över datalager som stöds som källor/mottagare av Kopieringsaktivitet finns i den [datalager som stöds](copy-activity-overview.md#supported-data-stores-and-formats) tabell.
...
|:--- |:--- |:--- |
| typ | Type-egenskapen måste anges till: **Filserver**. | Ja |
| värd | Anger Azure File Storage-slutpunkt som `"host": "\\\\<storage name>.file.core.windows.net\\<file service name>"`. | Ja |
| användar-ID | Anger att användaren kommer åt Azure File Storage som `"userid": "AZURE\\<storage name>"`. | Ja |
...
En fullständig lista över avsnitt och egenskaper som är tillgängliga för att definiera datauppsättningar finns i artikeln datauppsättningar. Det här avsnittet innehåller en lista över egenskaper som stöds av Azure File Storage-datauppsättningen.

För att kopiera data från/till Azure File Storage, ange typegenskapen på datauppsättningen till **filresursen**. Följande egenskaper stöds:

| Egenskap  | Beskrivning | Krävs |
|:--- |:--- |:--- |
| typ | Type-egenskapen för datauppsättningen måste anges till: **Filresurs** |Ja |
| folderPath | Sökvägen till mappen. Jokerteckenfilter stöds inte. |Ja |
...
För att kopiera data från Azure File Storage, ange typ av datakälla i kopieringsaktiviteten till **FileSystemSource**. Följande egenskaper stöds i kopieringsaktiviteten **källa** avsnittet:
...
För att kopiera data till Azure File Storage, ange Mottagartyp i kopieringsaktiviteten till **FileSystemSink**. Följande egenskaper stöds i den **mottagare** avsnittet:
