...
description: Varje aktivitet för definierar en upprepande Kontrollflöde i din pipeline. Den används för att iterera över en samling och köra angivna aktiviteter.
...
Om **isSequential** är inställd på false, aktiviteten itererar parallellt med högst 20 samtidiga iterationer. Den här inställningen bör användas med försiktighet. Den här metoden är bra om samtidiga iterationer skriver till samma mapp men olika filer. Om samtidiga iterationer skriver samtidigt på exakt samma fil, gör den här metoden troligen ett fel. 

## <a name="iteration-expression-language"></a>Iteration Uttrycksspråk
Ange en matris ska vara hävdade över för egenskapen i ForEach-aktiviteten **objekt**”. Använd `@item()` och iterera över en enda uppräkning i ForEach-aktiviteten. Till exempel om **objekt** är en matris: [1, 2, 3], `@item()` returnerar 1 i den första upprepningen 2 i andra iteration och 3 i den tredje iterationen.

## <a name="iterating-over-a-single-activity"></a>Iterera över en enskild aktivitet
**Scenario:** kopiera från samma käll-filen i Azure Blob till flera filer i Azure Blob.
...
### <a name="blob-dataset-definition"></a>BLOB-datauppsättningsdefinitionen
...
Det är möjligt att iterera över flera aktiviteter (till exempel: kopiera och web aktiviteter) i en ForEach-aktiviteten. I det här scenariot rekommenderar vi att du abstrakt ut flera aktiviteter i en separat pipeline. Du kan sedan använda den [ExecutePipeline aktiviteten](control-flow-execute-pipeline-activity.md) i pipeline med ForEach-aktiviteten anropa separat pipelinen med flera aktiviteter. 
...
**Scenario:** Iterate över en InnerPipeline inom en ForEach-aktiviteten med köra Pipeline-aktivitet. Inre pipeline kopieras med schemadefinitioner som innehåller parametrar.
...
#### <a name="source-dataset-definition"></a>Datauppsättningsdefinitionen för källa
...
#### <a name="sink-dataset-definition"></a>Sink datauppsättningsdefinitionen
...
```
## <a name="aggregating-metric-output"></a>Sammanställa mått utdata
Uttryck för att samla in utdata från alla iterationer av en ForEach är `@activity('NameofInnerActivity')`. Till exempel om en ForEach-aktivitet som hävdade över ”MyCopyActivity” syntaxen är: `@activity('MyCopyActivity')`. Resultatet är en matris med varje objekt som ger information om en specifik iteration.

> [!NOTE]
> Om du vill ha mer information om en specifik iteration syntaxen är: `@activity('NameofInnerActivity')[0]` för den senaste upprepningen. Använda numret inom parentes för att få åtkomst till specifika upprepning av matrisen. Du vill använda för att komma åt en specifik egenskap för en specifik iteration: `@activity('NameofInnerActivity')[0].output` eller `@activity('NameofInnerActivity')[0].pipelineName`.

**Matrisen utdata information om alla iterationer:**
```json
[    
    {      
