...
| Mellanlagrad kopiering           | Om mellanlagrad kopiering har kon figurer ATS men inte användbart för ditt käll-Sink-par, rekommenderar vi att du tar bort det. |
...
| Gireringsblankett        | Tiden i slutet av föregående steg och IR-överföring av alla data från källan till Sink. Del steg under "överföring" körs parallellt.<br><br>- **tid till första byte: tiden som** förflutit mellan föregående stegs slut och den tid då IR tar emot den första byten från käll data lagret. Gäller icke-filbaserade källor.<br>källa för - s **listan:** hur lång tid det tog att räkna upp källfiler eller datapartitioner. Den senare gäller när du konfigurerar partitionsalternativ för databas källor, t. ex. När du kopierar data från databaser som Oracle/SAP HANA/Teradata/Netezza/etc.<br/>-**läsning från Källa:** den tid som krävs för att hämta data från käll data lager.<br/>- **att skriva till Sink:** hur lång tid det tog att skriva data till data lagret för mottagare. |
...
- **"Överförings registrerings källa" har haft lång arbets tid**: det är långsamt att räkna upp källfiler eller data partitioner för käll databasen.
  ...
  - När du kopierar data från filbaserad källa, om du använder **Wildcard-filter** på mappsökväg eller fil namn (`wildcardFolderPath` eller `wildcardFileName`), eller om du använder **fil filtret senast ändrad** (`modifiedDatetimeStart` eller`modifiedDatetimeEnd`), så kommer Observera att det här filtret resulterar i att kopierings aktiviteten visar alla filer under den angivna mappen på klient sidan och tillämpar sedan filtret. Sådan fil uppräkning kan bli Flask hals, särskilt när bara små mängder filer uppfyllde filter regeln.
...
- **"Överförings läsning från källa" har länge arbets tid**: 
  ...
  - Kontrol lera din kopia av käll-och mottagar mönster: 
    ...
    - Om ditt kopierings mönster stöder större än 4 data integrerings enheter (DIUs) – Läs mer i [det här avsnittet](copy-activity-performance.md#data-integration-units) om information, vanligt vis kan du prova att öka DIUs för att få bättre prestanda. 
