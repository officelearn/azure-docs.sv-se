...
1. Lägg till följande kod som **skapar och aktiverar en pipeline** till **Main**-metoden. Den här pipelinen har en **CopyActivity** som använder **BlobSource** som källa och **BlobSink** som mottagare.
...
3. Lägg till följande kod för att hämta körningsinformation för en datasektor till **Main**-metoden.
           ...
           Console.ReadKey();
4. Lägg till följande helper-metod som används av **Main**-metoden i **Program**-klassen.  
...
5. Expandera projektet i Solution Explorer (**DataFactoryAPITestApp**), högerklicka på **Referenser** och klicka på **Lägg till referens**. Markera kryssrutan för ”**System.Configuration**”-sammansättningen och klicka på **OK**. 
6. Skapa konsolprogrammet. Klicka på **Skapa** på menyn och klicka sedan på **Build Solution** (Skapa lösning). 
7. Kontrollera att det finns minst en fil i **adftutorial**-behållaren i Azure-blobblagringen. Om inte skapar du **Emp.txt**-filen i Anteckningar med följande innehåll och laddar upp den till adftutorial-behållaren.
   
       John, Doe
       Jane, Doe
8. Kör exemplet genom att klicka på **Felsök** -> **Börja felsöka** på menyn. När du ser **Getting run details of a data slice** (Hämta körningsdata för en datorsektor) väntar du några minuter och trycker sedan på **Retur**. 
9. Använd Azure-portalen och kontrollera att datafabriken **APITutorialFactory** har skapats med följande artefakter: 
