...
Samlar in alla värden i uttrycket i den aggregerade gruppen i en matris. Strukturer kan samlas in och omvandlas till alternativa strukturer under den här processen. Antalet objekt motsvarar antalet rader i gruppen och kan innehålla NULL-värden. Antalet insamlade objekt ska vara litet* ``collect(salesPerson)``
...
Returnerar true om något element i den angivna matrisen utvärderas som sant i angivet predikat. Contains förväntar sig en referens till ett element i predikatet-funktionen som #item* ``contains([1, 2, 3, 4], #item == 3) -> true``  
...
Vänster fyller strängen med den angivna utfyllnaden tills den har en viss längd. Om strängen är lika med eller större än längden, trimmas den till längden* ``lpad('dumbo', 10, '-') -> '-----dumbo'``  
* ``lpad('dumbo', 4, '-') -> 'dumb'``  
*' ' lpad (' Dumbo ', 8, ' <> ')-> ' <><dumbo'``  
___
### <code>LTRIM</code>
...
Subtraherar tal. Subtrahera från ett datum antal dagar. Subtrahera varaktighet från en tidsstämpel. Subtrahera två tidsstämplar för att få en skillnad i millisekunder. Samma som-operatorn* ``minus(20, 10) -> 10``  
* ``20 - 10 -> 10``  
...
Returnerar aktuellt partitions-ID för den angivna raden* ``partitionId()``  
___
### <code>power</code>
...
Ackumulerar element i en matris. Minska förväntar sig en referens till ett ackumulator värde och ett element i den första uttrycks funktionen som #acc och #item och det förväntar sig att det resulterande värdet är #result som ska användas i den andra uttrycks funktionen* ``toString(reduce(['1', '2', '3', '4'], '0', #acc + #item, #result)) -> '01234'``  
___
...
Höger pad strängen med den angivna utfyllnaden tills den har en viss längd. Om strängen är lika med eller större än längden, trimmas den till längden* ``rpad('dumbo', 10, '-') -> 'dumbo-----'``  
* ``rpad('dumbo', 4, '-') -> 'dumb'``  
* ``rpad('dumbo', 8, '<>') -> 'dumbo<><'``  
___
...
Returnerar det genererade UUID-värde* ``uuid()``  
___
...
##Mängd funktioner följande funktioner är bara tillgängliga i sammansättningar av typen aggregera, pivot, unpivot och Window___
### <code>avg</code>
...
Beräknar rangordningen för ett värde i en grupp med värden som anges i ett fönsters order by-sats. Resultatet är ett plus antalet rader före eller lika med den aktuella raden i ordningen för partitionen. Värdena kommer inte att skapa luckor i sekvensen. Kompakt rang fungerar även när data inte sorteras och söker efter ändrings värden* ``denseRank()``  
___
### <code>lag</code>
...
Beräknar rangordningen för ett värde i en grupp med värden som anges i ett fönsters order by-sats. Resultatet är ett plus antalet rader före eller lika med den aktuella raden i ordningen för partitionen. Värdena kommer att skapa luckor i sekvensen. Rang fungerar även när data inte sorteras och söker efter ändrings värden* ``rank()``  
___
### <code>rowNumber</code>
