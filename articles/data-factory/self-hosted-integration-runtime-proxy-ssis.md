...
Den här artikeln beskriver hur du kör SQL Server Integration Services-paket (SSIS) på Azure-SSIS Integration Runtime (IR) i Azure Data Factory (ADF) med egen värd-IR som kon figurer ATS som proxy.  Med den här funktionen kan du komma åt data lokalt utan att [ansluta till Azure-SSIS IR till ett virtuellt nätverk](https://docs.microsoft.com/azure/data-factory/join-azure-ssis-integration-runtime-virtual-network).  Detta är användbart när företags nätverket har en alltför komplex konfiguration/restriktiv princip som du kan använda för att mata in Azure-SSIS IR.

Med den här funktionen delas ditt paket med en data flödes aktivitet med en lokal data källa till två mellanlagrings uppgifter: den första som körs på din egen värd-IR flyttar först data från den lokala data källan till ett mellanlagringsområde i Azure-Blob Storage, medan den andra som körs på din Azure-SSIS IR flyttar sedan data från mellanlagringsområdet till det avsedda data målet.

Den här funktionen ger även andra förmåner/funktioner som gör att du kan tillhandahålla din egen värd-IR i regioner som ännu inte stöds av Azure-SSIS IR, vilket ger den offentliga statiska IP-adressen för din egen värd-IR i brand väggen för dina data källor osv.

## <a name="prepare-self-hosted-ir"></a>Förbered IR med egen värd
För att kunna använda den här funktionen måste du först skapa en ADF och etablera din Azure-SSIS IR under den, om du inte redan har gjort det, genom att följa artikeln så här etablerar du [en Azure-SSIS IR-](https://docs.microsoft.com/azure/data-factory/tutorial-deploy-ssis-packages-azure) artikel.

Du måste sedan etablera din egen värd-IR under samma ADF där din Azure-SSIS IR etableras genom att följa den här artikeln så här [skapar du en IR](https://docs.microsoft.com/azure/data-factory/create-self-hosted-integration-runtime) -artikel med egen värd.
...
När du har för berett den lokala IR-och Azure Blob Storage länkade tjänsten för mellanlagring kan du nu konfigurera din nya/befintliga Azure-SSIS IR med egen värd-IR som en proxy på ADF-portalen/-appen.  Om din befintliga Azure-SSIS IR körs stoppar du den innan du gör detta och startar sedan om den efteråt.

På sidan **Avancerade inställningar** kontrollerar du kryss rutan **konfigurera egen värd integration runtime som en proxy för din Azure-SSIS integration runtime** , väljer den lokala IR-och Azure-Blob Storage länkade tjänsten för mellanlagring och anger en BLOB container namn för mellanlagringsplatsen om du vill.
...
De första mellanlagrings aktiviteterna som körs på din egen värd-IR faktureras separat på samma sätt som alla data förflyttnings aktiviteter som körs på lokal IR-överföring faktureras som anges i pris artikeln för [ADF](https://azure.microsoft.com/pricing/details/data-factory/data-pipeline/) -datapipeline.

De andra mellanlagrings aktiviteter som körs på din Azure-SSIS IR debiteras inte separat, men din Azure-SSIS IR-överföring faktureras enligt vad som anges i artikeln [Azure-SSIS IR-prissättning](https://azure.microsoft.com/pricing/details/data-factory/ssis/) .
