...
Ställ in `allowPolyBase` egenskapen på **True** enligt följande exempel för Azure Data Factory att använda PolyBase för att kopiera data till Azure SQL Data Warehouse. När du anger allowPolyBase till true kan du ange PolyBase-egenskaper med hjälp av `polyBaseSettings` egenskaps gruppen. i avsnittet [SqlDWSink](#sqldwsink) finns mer information om egenskaper som du kan använda med polyBaseSettings.
...
> [!NOTE]
> När du kopierar data från ett lokalt data lager till Azure SQL Data Warehouse med PolyBase och mellanlagring, krävs JRE (Java Runtime Environment) på din Gateway-dator som används för att transformera dina källdata till rätt format, om Data Management Gateway versionen är under 2,4. Föreslå att du uppgraderar din gateway till senaste för att undvika ett sådant beroende.
>
...
Följande avsnitt innehåller ytterligare metod tips för de som nämns i [metod tips för Azure SQL Data Warehouse](../../synapse-analytics/sql-data-warehouse/sql-data-warehouse-best-practices.md).
...
Om du använder kopiera guiden för att kopiera data från SQL Server eller Azure SQL Database till Azure SQL Data Warehouse och tabellen som motsvarar käll tabellen inte finns i mål lagret, kan Data Factory automatiskt skapa tabellen i data lagret med hjälp av käll tabellens schema.
