...
Build-utdata för ett U-SQL Database-projekt är ett U-SQL-databas distributions paket med namnet `.usqldbpack`med suffixet. Paketet är en. zip-fil som innehåller alla DDL-instruktioner i ett enda U-SQL-skript i **DDL** -mappen och alla DLL-filer och ytterligare filer för sammansättningar i Temp-mappen. `.usqldbpack`
...
- *Projekt version*: Konfigurera de refererade databas miljöerna innan du skapar U-SQL-skripten. 
- *Lokalt kör mot (ett lokalt projekt) konto*: Refererade databas miljöer distribueras till (ett lokalt projekt)-konto innan U-SQL-skript körs. [Lär dig mer om lokala körningar och skillnaden mellan (lokala datorer) och (ett lokalt projekt)-konto här](data-lake-analytics-data-lake-tools-local-run.md).
