...
Den här artikeln gäller för följande program varu version – **Azure Stack Edge 2008**. 
...
- **Lagrings klasser** – i den tidigare versionen kunde du bara konfigurera lagring via SMB-eller NFS-resurser för tillstånds känsliga program som distribuerats på Kubernetes-klustret som körs på Azure Stack Edge-enheten. I den här versionen lades lagrings klasser till som gör det möjligt att dynamiskt etablera lagringen. Mer information finns i [Kubernetes Storage Management på Azure Stack Edge GPU-enhet](azure-stack-edge-gpu-kubernetes-storage.md#dynamicprovisioning). 
- **Kubernetes-instrumentpanel med mått Server** – i den här versionen läggs en Kubernetes-instrumentpanel till med ett mått Server tillägg. Du kan använda instrument panelen för att få en översikt över de program som körs på din Azure Stack Edge-enhet, Visa status för Kubernetes kluster resurser och se eventuella fel som har inträffat på enheten. Mått servern sammanställer processor-och minnes användningen mellan Kubernetes resurser på enheten. Mer information finns i [använda Kubernetes-instrumentpanelen för att övervaka Azure Stack Edge-GPU-enheten](azure-stack-edge-gpu-monitor-kubernetes-dashboard.md).
- **Azure-båge för Azure Stack Edge** -från den här versionen kan du distribuera program arbets belastningar på din Azure Stack gräns enhet via Azure Arc. Azure Arc är ett hybrid hanterings verktyg som gör att du kan distribuera program i Kubernetes-kluster. Mer information finns i [distribuera arbets belastningar via Azure Arc på din Azure Stack Edge-enhet](azure-stack-edge-gpu-deploy-arc-kubernetes-cluster.md).  
