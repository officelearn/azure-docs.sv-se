...
> [!IMPORTANT]
> Den här prenumerationen är inte ansluten eller relaterad till din Azure-prenumeration och finns lokalt på din enhet.

Den här prenumerationen kommer att användas för att distribuera de virtuella datorerna.
...
> [!NOTE]
> Ange `BlobType` till sida för att skapa en hanterad disk från VHD. Ställ in `BlobType` att blockera vid skrivning till skiktade lagrings konton med hjälp av AzCopy.

Du kan hämta disk avbildningarna från Marketplace. Detaljerade anvisningar finns i [Hämta den virtuella disk avbildningen från Azure Marketplace](azure-stack-edge-j-series-create-virtual-machine-image.md).

Ett exempel på utdata som använder AzCopy 7,3 visas nedan. Mer information om det här kommandot finns i [Ladda upp VHD-fil till lagrings kontot med AZCopy](../devtest-labs/devtest-lab-upload-vhd-using-azcopy.md).
...
Mer information finns i Dv2- [serien på generell användning VM-storlekar](../virtual-machines/dv2-dsv2-series.md#dv2-series).
...
Utför följande steg för att kontrol lera om miljövariabeln för AzCopy har ställts in korrekt:

1. Kör "AzCopy-kuvert"
2. Hitta `AZCOPY_DEFAULT_SERVICE_API_VERSION` parameter. Detta ska ha det värde som du angav i föregående steg.
