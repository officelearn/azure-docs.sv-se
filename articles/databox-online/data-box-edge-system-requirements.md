...
- De övervaknings kommandon som är tillgängliga via PowerShell-gränssnittet på enheten, till exempel:
    ...
    - `dkrdbe stats`för att hämta en Live-ström med container (s) användnings statistik för resurser. Kommandot stöder CPU, minnes användning, minnes gräns och nätverkets IO-mått.
    - `dkrdbe system df`för att få information om mängden disk utrymme som används. 
    - `dkrdbe image prune`för att rensa oanvända avbildningar och frigöra utrymme.
