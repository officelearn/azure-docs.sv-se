...
Baserat på de angivna resurs grupperna skapar Data Box Heavy en resurs för varje associerad resurs grupp per nod. Till exempel, om `mydbmdrg1` och `mydbmdrg2` skapades när ordern placerades, skapas följande resurser:
...
3. När NFS-klienten har installerats använder du följande kommando för att montera NFS-resursen på enheten:
    ...
    I följande exempel visas hur du ansluter via NFS till en Data Box-enhet eller Data Box Heavy resurs. Data Box-enhet-eller Data Box Heavys enhetens IP-adress är `169.254.250.200`är resurs `mydbmdrg1_MDisk` monterad på ubuntuVM, monterings punkt `/home/databoxubuntuhost/databox`.
