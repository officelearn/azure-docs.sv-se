...
3. När NFS-klienten har installerats använder du följande kommando för att montera NFS-resursen på Data Box-enheten:
    ...
    I följande exempel visas hur du ansluter via NFS till en Data Box Heavy-resurs. Data Box Heavy IP-adressen är `10.161.23.130`är resurs `Mystoracct_Blob` monterad på ubuntuVM, monterings punkt `/home/databoxheavyubuntuhost/databoxheavy`.
...
- Om du matar in Skift läges känslig katalog och fil namn från en NFS-resurs till NFS på Data Box Heavy: 
    ...
    Om du t. ex. kopierar `SampleFile.txt` och `Samplefile.Txt`bevaras ärendet i namnet när den kopieras till enheten, men den andra filen skriver över det första som dessa betraktas som en fil.
