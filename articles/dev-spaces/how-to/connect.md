...
---
# <a name="connect-your-development-computer-to-an-aks-cluster-preview"></a>Ansluta din utvecklingsdator till ett AKS-kluster (förhandsversion)
...
## <a name="connect-your-development-computer"></a>Anslut din utvecklings dator
...
I det här exemplet väljer `Azure Dev Spaces: Redirect an existing Kubernetes service to my machine [Preview]` du och väljer tjänsten *Bikes* .
...
Alternativet *Ersätt* ersätter nuvarande POD eller tjänst i AKS-klustret och dirigerar om all trafik för tjänsten till din utvecklings dator. Det här alternativet kan vara störande för andra tjänster i ditt AKS-kluster som interagerar med tjänsten som du omdirigerar kanske inte förrän du startar programmet på din utvecklings dator. Med alternativet *klona* kan du välja ett befintligt underordnat dev Space eller skapa ett nytt underordnat dev utrymme för att dirigera om trafik för en POD eller tjänst till din utvecklings dator. Med det här alternativet kan du arbeta i isolering och inte störa andra tjänster eftersom endast trafik till det underordnade dev-utrymmet kommer att omdirigeras till din utvecklings dator. *Klonings* alternativet kräver att ditt AKS-kluster har aktiverat Azure dev Spaces.
...
När du har valt appens TCP-port upprättar Azure dev-utrymmen en anslutning till AKS-klustret. Azure dev Spaces infogar en agent i AKS-klustret för att dirigera om trafik mellan AKS-klustret och din utvecklings dator. Det kan ta några minuter att upprätta anslutningen. Azure dev Spaces kommer också att begära administratörs åtkomst för att ändra *hosts* -filen på din utvecklings dator.

> [!IMPORTANT]
> När Azure dev Spaces upprättar en anslutning till ditt AKS-kluster, fungerar inte de andra tjänsterna i ditt AKS-kluster förrän du startar tjänsten i utvecklings datorn om du väljer *Ersätt* anslutnings läge. Du kan välja *klonings* anslutnings läge i stället för att skapa ett underordnat dev utrymme för omdirigeringen och undvika eventuella avbrott i det överordnade utrymmet. Om tjänsten har ett beroende som inte är tillgängligt i utvecklings datorn kan du behöva ändra ditt program eller ange [ytterligare konfiguration](#additional-configuration)
...
### <a name="configure-your-application-on-your-development-computer"></a>Konfigurera ditt program på din utvecklings dator
...
### <a name="start-your-application-on-your-development-computer"></a>Starta ditt program på din utvecklings dator
...
Ta bort Bryt punkten genom att placera markören på rad 233 `server.js` i och trycka på *F9*.
...
Loggning av utdata skrivs till fönstret för att *ansluta till dev Spaces* när du har anslutit din utvecklings dator till ditt AKS-kluster.

![Resultat](../media/how-to-connect/connect-output.png)

Klicka på status fältet i Azure dev Spaces och välj *Visa diagnostikinformation*. Det här kommandot skriver ut aktuella miljövariabler och DNS-hela i loggnings resultatet.
