...
### <a name="error-release-azds-identifier-spacename-servicename-failed-services-servicename-already-exists-or-pull-access-denied-for-servicename-repository-does-not-exist-or-may-require-docker-login"></a>Fel "release azds-\<Identifier\>-\<spacenameServiceNamemisslyckades\>:Services 'ServiceName\<-\<\> \>finns redan eller så har åtkomst nekats för \<ServiceName\>, databasen finns inte eller så kan det krävas Docker inloggning "
...
Åtgärda problemet genom att uppdatera `launch.json` filen `.vscode` under katalogen i projektmappen. Ändra den `configurations->cwd` direktiv så att den pekar till samma katalog som den `WORKDIR` definieras i projektets `Dockerfile`. Du kan också behöva uppdatera den `configurations->program` direktiv samt.
...
1. `azds.yaml` Ändra filen för att ange Bygg kontexten till lösnings nivån.
2. Ändra filerna `Dockerfile.develop` `.csproj`och för att referera till projektfilerna, till exempel korrekt i förhållande till den nya bygg kontexten. `Dockerfile`
3. Lägg till `.dockerignore` en i samma katalog `.sln` som filen.
4. `.dockerignore` Uppdatera med ytterligare poster efter behov.
