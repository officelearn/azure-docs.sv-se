---
title: 'Snabb start: skapa en CI/CD-pipeline för Ruby on-räler med Azure DevOps starter'
description: Med Azure DevOps Starter är det enkelt att komma igång med Azure. Du kan lansera en Ruby-webbapp på en Azure-tjänst med några enkla steg.
ms.prod: devops
...
Konfigurera kontinuerlig integrering (CI) och kontinuerlig leverans (CD) för din Ruby on räler-app med hjälp av Azure DevOps starter. DevOps starter fören klar den inledande konfigurationen av en pipeline för build och version av Azure DevOps.
...
Azure DevOps starter skapar en CI/CD-pipeline i Azure databaser. Du kan skapa en ny Azure DevOps-organisation eller använda en befintlig organisation. DevOps starter skapar också Azure-resurser i den Azure-prenumeration du väljer.
...
1. **Web App on Linux** är distributionsmålet som är standard.  Du kan alternativt välja **Web App for Containers**. Det programramverk som du valde tidigare avgör vilken typ av distributionsmål som finns tillgängliga för Azure-tjänsten här. 
...
1. Välj din Azure-prenumeration och plats, ange ett namn för appen och välj sedan **Klar**.  
    Efter några minuter visas DevOps starter-instrumentpanelen i Azure Portal. En exempelapp konfigureras i en lagringsplats i din Azure DevOps-organisation, en version körs och appen distribueras till Azure. 
...
Azure DevOps starter skapar en git-lagrings platsen i Azure pipelines eller GitHub. Visa lagringsplatsen och gör kodändringar i appen genom att utföra följande:
...
1. I webbläsaren går du till DevOps starter-instrumentpanelen. En version bör pågå. De ändringar du utfört skapas och distribueras automatiskt via en CI/CD-pipeline.

## <a name="examine-the-azure-pipelines-cicd-pipeline"></a>Granska Azure Pipelines-CI/CD-pipelinen

Azure DevOps starter konfigurerar automatiskt en fullständigt CI/CD-pipeline i Azure DevOps-organisationen. Utforska och anpassa pipelinen efter behov. Bekanta dig med Azure DevOps-bygg- och versionspipelines genom att göra följande:
