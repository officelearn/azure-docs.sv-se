---
title: Anpassade roller för SQL Server till SQL Database hanterade instanser online-migreringar | Microsoft Docs
description: Lär dig att använda de anpassade rollerna för SQL Server till SQL Database hanterade instanser online-migreringar.
...
Azure Database Migration Service använder ett APP-ID för att interagera med Azure-tjänster. APP-ID: t kräver antingen rollen deltagare på prenumerations nivån (som många företags säkerhets avdelningar inte tillåter) eller skapar anpassade roller som ger de behörigheter som krävs för Azure Database migrations service. Eftersom det finns en gräns på 2 000 anpassade roller i Azure Active Directory kanske du vill kombinera alla behörigheter som krävs specifikt av APP-ID: t i en eller två anpassade roller och sedan tilldela APP-ID: t den anpassade rollen för specifika objekt eller resurs grupper (vs. prioritetsordning). Om antalet anpassade roller inte är ett problem kan du dela upp de anpassade rollerna efter resurs typ för att skapa tre anpassade roller totalt enligt beskrivningen nedan.
