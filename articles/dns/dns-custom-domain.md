...
Azure IoT har inte alla anpassningar som behövs på själva tjänsten. Om du vill använda en anpassad domän med en IoT-hubb endast en CNAME-post som pekar på resurserna som krävs.

Gå till **Sakernas Internet** > **IoT-hubb** och välj din IoT-hubb. På den **översikt** bladet Observera FQDN för IoT-hubben.

![IoT Hub-bladet](./media/dns-custom-domain/iot.png)

Sedan navigerar du till DNS-zon och på **+ postuppsättningen**. Ange följande information på den **lägga till postuppsättning** bladet och klicka på **OK** skapa den.


|Egenskap  |Värde  |Beskrivning  |
|---------|---------|---------|
|Namn     | myiothub        | Det här värdet tillsammans med domännamnet är FQDN för IoT-hubben.        |
|Typ     | CNAME        | Använd en CNAME-post använder ett alias.
|TTL-VÄRDE     | 1        | 1 används för 1 timme        |
|TTL-enhet     | Timmar        | Timmar används som tidmätning         |
|Alias     | adatumIOT.azure devices.net        | DNS-namnet som du skapar alias för, i det här exemplet är det värdnamnet adatumIOT.azure devices.net som tillhandahålls av IoT-hubben.

När du har skapat posten testa namnmatchning med CNAME-post med`nslookup`
