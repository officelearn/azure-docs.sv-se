...
## <a name="getting-help"></a>Skaffa hjälp

Alla Azure CLI-kommandon som är relaterade till Azure DNS börjar med `az network dns`. Hjälp är tillgängligt för varje kommando med alternativet `--help` (kort form `-h`).  Exempel:
...
I följande exempel visas hur du skapar en DNS-zon med två [Azure Resource Manager Taggar](dns-zones-records.md#tags), *Project = demo* och *Kuvert = test*med hjälp av parametern `--tags` (kort form `-t`):
...
Använd `az network dns zone show` om du vill hämta en DNS-zon. Om du vill ha hjälp, så gå till `az network dns zone show --help`.
