...
När du har anslutit loggar du in med det `root` användar namn och lösen ord som du angav när du startade noden för första gången.  
...
Till exempel söker kommandot `ifconfig | grep -B5 inet` efter portar med Internet adresser och ger fem kontexter för att Visa port identifieraren.
...
* **Administratörs lösen ord** – ange lösen ordet för den administrativa standard `admin`användaren.
