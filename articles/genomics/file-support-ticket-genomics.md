...
Välj `New support request`sedan. 

![Ny supportbegäran](./media/file-support-ticket/new-support-request.png "Ny supportbegäran") 

Fyll i `Basics`, inklusive `Issue type` och din. `Subscription` Välj det som är `Issue type` mest relevant för ditt ärende. som exempel i bilden nedan `Technical` har marker ATS. Se till att välja `Microsoft Genomics` som relevant tjänst.  `Microsoft Genomics`visas under `Intelligence and Analytics` kategorin.   När du `Microsoft Genomics`har valt uppmanas du också att ange din `Support plan` typ.
...
Sist lägger du till din kontakt information och `Create` väljer längst ned på skärmen för att skicka in din supportbegäran.
