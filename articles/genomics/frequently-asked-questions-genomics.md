...
Du kan komprimera utdata-VCF-eller gvcf med ett valfritt argument för komprimering av utdata. Detta motsvarar `-bgzip` att köra följt av `-tabix` i VCF-eller gvcf-utdata för att skapa `.gz` (bgzip output) och `.tbi` (tabix output) filer. `bgzip`komprimerar VCF-eller gvcf-filen och `tabix` skapar ett index för den komprimerade filen. Argumentet är ett booleskt värde som är inställt `false` på som standard för VCF-utdata och `true` som standard för gcvf-utdata. Om du vill använda på kommando raden anger `-bz` du `--bgzip-output` eller `true` som (kör bgzip och tabix) `false`eller. Om du vill använda det här argumentet i filen config. txt `bgzip_output: true` lägger `bgzip_output: false` du till eller i filen.
...
Du kan få en fullständig lista över tillgängliga kommandon och argument genom att `msgen help`köra. Om inga ytterligare argument anges visas en lista över tillgängliga hjälp avsnitt, en för var och en `submit`av, `list` `cancel`, och. `status` Om du vill ha hjälp med ett speciellt kommando `msgen help command`skriver du; `msgen help submit` visar till exempel alla sändnings alternativ.
...
De vanligaste kommandona är argument för `msgen` klienten: 
 ...
 |`status`             |Returnerar statusen för det arbets flöde som anges `--workflow-id`av. Se även `msgen help status`. |
 |`cancel`             |Skickar en begäran om att avbryta bearbetningen av arbets flödet `--workflow-id`som anges av. Se även `msgen help cancel`. |
