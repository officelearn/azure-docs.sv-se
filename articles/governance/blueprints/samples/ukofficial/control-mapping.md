---
title: Exempel - Storbritannien officiella skissen - kontroll mappning
description: Kontrollen mappning av Storbritannien officiella skissen exemplet.
services: blueprints
author: DCtheGeek
ms.author: dacoulte
ms.date: 06/13/2019
...
ms.locfileid: "67276982"
---
# <a name="control-mapping-of-the-uk-official-blueprint-sample"></a>Mappning av kontroll i Storbritannien officiella skissen-exempel

I följande artikel beskriver hur Storbritannien officiella skissen exemplet mappas till Storbritannien officiella kontroller.
Läs mer om kontrollerna, [Storbritannien officiella](https://www.gov.uk/government/publications/government-security-classifications).

Följande mappningar avser den **Storbritannien officiella** kontroller. Använd navigeringen till höger för att gå direkt till en viss kontroll-mappning. Många av de mappade kontrollerna implementeras med en [Azure Policy](../../../policy/overview.md) initiativ. Om du vill granska fullständig initiativet öppna **princip** i Azure-portalen och välj den **definitioner** sidan. Hitta och välj sedan den  **[förhandsversion] gransknings-och Storbritannien officiella styr och distribuera specifika VM-tillägg för att stödja granskningskrav** inbyggd princip initiativ.
...
Den här skissen kan du tillämpa principen på användningen av cryptograph kontroller genom att tilldela 11 [Azure Policy](../../../policy/overview.md) definitioner som tillämpa specifika cryptograph kontroller och granska användning av svaga kryptografiska inställningar.
...
Den här skissen hjälper dig att skydda system informationstillgångar genom att tilldela en [Azure Policy](../../../policy/overview.md) definition som övervakar obegränsad storage-konton. Den här skissen tilldelar också en Azure Policy definition som övervakar godkända aktivitet.
...
Azure implementerar rollbaserad åtkomstkontroll (RBAC) till hjälper dig att hantera vem som har åtkomst till resurser i Azure. Med Azure-portalen kan granska du vem som har åtkomst till Azure-resurser och deras behörigheter. Den här skissen hjälper dig att begränsa och kontrollera rättigheter för privilegierad åtkomst genom att tilldela sex [Azure Policy](../../../policy/overview.md) definitioner för att granska externa konton med ägare och/eller Läs/Skriv-behörigheter och konton med ägare kan läsa och/eller skriva behörigheter som inte har aktiverat multifaktorautentisering.
...
- Granska externa konton med ägarbehörigheter för en prenumeration
- Granska externa konton med skrivbehörigheter för en prenumeration
- Granska externa konton med läsbehörigheter för en prenumeration

Den här skissen tilldelar två principdefinitioner i Azure att granska användning av Azure Active Directory-autentisering för SQL-servrar och Service Fabric. Autentisering möjliggör med Azure Active Directory förenklad behörighetshantering och centraliserad Identitetshantering för databasanvändare och andra Microsoft-tjänster.
...
Den här skissen tilldelar också fem principdefinitioner i Azure att granska konton som ska prioriteras för granskning, inklusive avskriven konton och externa konton. När de behövs kan konton blockeras från att logga in (eller tas bort), som direkt tar bort åtkomsträttigheter till Azure-resurser. Denna skiss tilldelar två Azure principdefinitioner audit avskriven-konto som du bör överväga att tas bort.
...
Den här skissen tilldelar också fyra principdefinitioner i Azure att granska konton som ska prioriteras för granskning, inklusive avskriven konton och externa konton med förhöjd behörighet. När de behövs kan konton blockeras från att logga in (eller tas bort), som direkt tar bort åtkomsträttigheter till Azure-resurser. Denna skiss tilldelar två Azure principdefinitioner audit avskriven-konto som du bör överväga att tas bort.
...
Den här skissen hjälper dig att säkerställa händelser loggas genom att tilldela 6 [Azure Policy](../../../policy/overview.md) definitioner som gransknings-och inställningar på Azure-resurser. En tilldelad princip granskar även om virtuella datorer inte skickar loggar till en angiven log analytics-arbetsyta.
