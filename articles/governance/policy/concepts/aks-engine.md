...
Om du vill visa Azure Policy tilläggs loggar använder du `kubectl` :
...
Gatekeeper-pod, _Gatekeeper-Controller Manager-0_, finns vanligt vis i `gatekeeper-system` `kube-system` namn området eller, men kan finnas i en annan namnrymd beroende på hur den distribueras.
