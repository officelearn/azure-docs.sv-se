...
Gatekeeper-pod, _Gatekeeper-Controller Manager-0_, finns vanligt vis i `gatekeeper-system`-eller `kube-system` namn området, men kan finnas i en annan namnrymd beroende på hur den har distribuerats.

Om du vill visa Gatekeeper-loggarna använder `kubectl`:
