...
Ändra lösen ordet i Azure Portal (på ditt lokala system) och vänta i 30 minuter på att synkroniseringen ska bli uppfångad.
...
Principen för villkorlig åtkomst eller MFA används för användaren. Eftersom interaktiv autentisering inte stöds än, behöver användaren eller klustret undantas från MFA/villkorlig åtkomst. Om du väljer att undanta klustret (IP-baserad undantags princip) kontrollerar du att AD `ServiceEndpoints` är aktiverat för det virtuella nätverket.
...
* Om du har en domänansluten [Windows-dator](../../active-directory-domain-services/manage-domain.md)kan du bläddra genom att använda standard verktygen i Windows AD. Detta kräver ett fungerande konto i domänen.
...
* Från Head-noden kan du söka genom att använda SAMBA-kommandon. Detta kräver en giltig Kerberos-session (lyckad kinit). NET Ads search (userPrincipalName = Bob *) "
    ...
    Sök-/Browse-resultaten ska visa `sAMAccountName` attributet. Du kan också titta på andra attribut som `pwdLastSet` `userPrincipalName` , `badPasswordTime`osv. för att se om dessa egenskaper matchar det du förväntar dig.
