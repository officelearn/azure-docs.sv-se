...
Om du vill aktivera utförlig loggning måste du starta om `sshd` med alternativet `-d`. Precis som `/usr/sbin/sshd -d` kan du också köra `sshd` på en anpassad port (till exempel 2222) så att du inte behöver stoppa huvud SSH-daemonen. Du kan också använda `-v` alternativet med SSH-klienten för att få fler loggar (klient sidan visar felen).
