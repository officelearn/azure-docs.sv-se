...
I den här artikeln kommer du lära dig hur du ansluter till ett HDInsight-kluster med hjälp av fjärrskrivbord och sedan kör Apache Hive-frågor med hjälp av Hive-kommandoradsgränssnittet (CLI).
...
>
> För HDInsight 3.4 eller större finns i [använda Apache Hive med HDInsight och Beeline](apache-hadoop-use-hive-beeline.md) information om hur du kör Hive-frågor direkt i klustret från en kommandorad.
...
4. Använd följande instruktioner för att skapa en ny ”intern” tabell med namnet **felvillkoren**:
   ...
   * **SKAPA TABELL OM DET INTE FINNS**: Skapar en tabell om den inte redan finns. Eftersom den **externa** nyckelord används inte, det här är en intern tabell som lagras i datalagret Hive och hanteras helt av Hive.

     > [!NOTE]  
     > Till skillnad från **externa** tabeller, släppa en intern tabell även tar bort underliggande data.
     >
     >
   * **LAGRAS SOM ORC**: Lagrar data i optimerad raden (ORC) kolumnformat. Det här är ett mycket optimerade och effektiv format för att lagra Hive-data.
