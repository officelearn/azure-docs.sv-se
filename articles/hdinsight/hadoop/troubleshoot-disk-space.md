...
1. Navigera till **Hive** > **configs** > **avancerade** > **Advanced Hive-log4j**. Granska följande inställningar:

    * `hive.root.logger=DEBUG,RFA`. Detta är standardvärdet, ändra [loggnings nivån](https://logging.apache.org/log4j/2.x/log4j-api/apidocs/org/apache/logging/log4j/Level.html) till `INFO` för att skriva ut färre loggar poster.
...
* Apache Ambari
    ...
    1. Navigera till **Hive** - > **config** > **Advanced** > **Resource Manager**. Se till att **Aktivera logg agg regering** är markerat. Om det här alternativet är inaktiverat behåller Name-noderna loggarna lokalt och inte samman dem i fjärrarkivet vid slut för ande av program.
...
* `/mnt/resource` kan vara fyllda med överblivna filer (som i fallet med Resource Manager omstart). Om det behövs kan du rensa `/mnt/resource/hadoop/yarn/log` och `/mnt/resource/hadoop/yarn/local`manuellt.
