...
Aviseringen Resource Health Center (RHC) `The HDInsight cluster is unable to access the key for BYOK encryption at rest`visas för Bring Your Own Key-kluster (BYOK) där klusternoderna har förlorat åtkomst till kunder Key Vault (KV). Liknande aviseringar kan också visas på Apache Ambari UI.
...
Mer information hittar du på sidan [Azure Key Vault tillgänglighet och redundans](../../key-vault/key-vault-disaster-recovery-guidance.md) och Azure-status https://status.azure.com/
