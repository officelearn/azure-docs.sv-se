...
Använd [Premium-Block Blob Storage konto](https://azure.microsoft.com/blog/azure-premium-block-blob-storage-is-now-generally-available/) som Fjärrlagring för att få en betydande förbättring av Läs åtgärder. Det här alternativet är endast möjligt om funktionen Skriv över-loggar är aktive rad.
...
* Har du använt `EXPLAIN` instruktionen för att förstå frågan planerar genereringen av "läsningar".
...
I HDInsight-HBase lagras HFiles på Fjärrlagring, vilket innebär att när det finns ett cache-minne, kommer kostnaden för läsningarna att vara högre än lokal system, som har data som backas upp av Local HDFS tack vare den nätverks fördröjning som ingår. I de flesta fall är intelligent användning av HBase-cache (block-cache och Bucket cache) utformad för att kringgå det här problemet. Det kommer dock att finnas tillfälliga fall där det kan vara problem med kunden. Att använda Premium Block Blob-kontot har hjälpt detta något. Men med WASB-blobben (Windows Azure Storage driv rutin) som `fs.azure.read.request.size` förlitar sig på vissa egenskaper, till exempel för att hämta data i block baserat på vad den bestämmer sig för att vara läsnings läge (sekventiell vs slumpmässig) kan vi fortsätta att se instanser av högre latens med läsningar. Vi har hittat de empiriska experiment som ställer in block storleken för läsnings begär Anden () till 512 KB och matchar block storleken för HBase-`fs.azure.read.request.size`tabellerna så att de blir det bästa resultatet i praktiken.

HDInsight HBase, för de flesta kluster med stora storleks noder `bucketcache` , tillhandahåller som en fil på lokal SSD som är kopplad till den virtuella `regionservers`datorn, vilket kör. Vid tillfällen kan användningen av heap-cachen i stället ge en viss förbättring. Detta har begränsningen att använda tillgängligt minne och kan vara mindre storlek än filbaserad cache, så det är inte alltid uppenbart att det är det bästa valet.

Några av de andra speciella parametrarna som vi har justerat som verkar ha hjälpt till att variera i grader enligt nedan:

1. Öka `memstore` storleken från standard 128 MB till 256 MB – den här inställningen rekommenderas vanligt vis för tungt Skriv scenario.
...
1. Undvik att `memstore` blockera tömning på grund av en lagrings gräns. `Hbase.hstore.blockingStoreFiles`kan höjas till 100 för att tillhandahålla den här bufferten.
...
1. Andra Phoenix-speciella konfigurationer:
    ...
    1. `Phoenix.rpc.index.handler.count`kan ställas in på 50 om vi har stora eller många indexs ökningar.
...
1. RPC-tidsgräns – HBase RPC-tidsgräns, timeout för klient skanner, och Phoenix-frågans tids gräns kan ökas till 3 minuter. Det är viktigt att Observera att `hbase.client.scanner.caching` parametern har angetts till ett värde som matchar vid Server slut och klient slut. Annars leder den här inställningen till fel relaterade `OutOfOrderScannerException` till vid klientens slut. Den här inställningen ska vara inställd på ett lågt värde för stora genomsökningar. Vi anger värdet 100.

## <a name="other-considerations"></a>Annat att tänka på

Några andra parametrar som ska beaktas vid justering:

1. `Hbase.rs.cacheblocksonwrite`– den här inställningen är inställd på sant som standard på HDI.
