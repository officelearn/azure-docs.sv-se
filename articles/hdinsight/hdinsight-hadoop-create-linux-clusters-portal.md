...
* **en Azure-prenumeration**. See [How to get Azure Free trial for testing Hadoop in HDInsight](https://azure.microsoft.com/documentation/videos/get-azure-free-trial-for-testing-hadoop-in-hdinsight/).
...
1. Select **1 Basics**. Then enter the following information:
    ...
    |Prenumeration|From the drop-down list, select the Azure subscription that's used for the cluster.|
    |Klustertyp|Select the type of cluster you want to create. Examples are Hadoop and Apache Spark. The **Operating system** will be **Linux**. Next, select a cluster type version. Use the default version if you don't know what to choose. Mer information finns i [HDInsight-klusterversioner](hdinsight-component-versioning.md).|
    |Användarnamn för klusterinloggning|Provide the username, default is **admin**.|
    |Lösenord för klusterinloggning|Provide the password.|
    |Secure Shell (SSH)-användarnamn|Default is **sshuser**. If you want the same SSH password as the admin password you specified earlier, select the **Use cluster login password for SSH** check box. If not, provide either a **PASSWORD** or **PUBLIC KEY** to authenticate the SSH user. A public key is the approach we recommend. Choose **Select** at the bottom to save the credentials configuration.  For more information, see [Connect to HDInsight (Apache Hadoop) by using SSH](hdinsight-hadoop-linux-use-ssh-unix.md).|
    |Resursgrupp|Ange om du vill skapa en ny resursgrupp eller använda en befintlig.|
    |Plats|Specify a datacenter where the cluster is created.|
...
1. From **3 Storage**, for **Storage Account Settings**, specify whether you want Azure Storage or Azure Data Lake Storage as your default storage. For more information, see the following table.
    ...
    | Primary Storage type | Beskrivning |
    |------------------|-------------|
...
1. From **7 Summary**, verify the information you entered earlier. Välj sedan **Skapa**.
    ...
    > [!NOTE]  
    > Det tar lite tid att skapa klustret, normalt cirka 20 minuter. Monitor **Notifications** to check on the provisioning process.
...
1. After the creation process finishes, select **Go to Resource** from the **Deployment succeeded** notification. The cluster window provides the following information.
    ...
    |Egenskap | Beskrivning |
    |---|---|
    |Översikt|Provides all the essential information about the cluster. Examples are the name, the resource group it belongs to, the location, the operating system, and the URL for the cluster dashboard.|
    ...
    |Ta bort|Deletes the HDInsight cluster.|
