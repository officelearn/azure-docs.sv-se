...
Azure-portalen är ett webbaserat verktyg för tjänster och resurser som finns i Microsoft Azure-molnet. I den här artikeln lär du dig skapa Linux-baserat HDInsight-kluster med hjälp av portalen.
...
Azure-portalen visar de flesta egenskaper för klustret. Med Azure Resource Manager-mall kan dölja du många detaljer. Mer information finns i [skapa Linux-baserade Hadoop-kluster i HDInsight med hjälp av Azure Resource Manager-mallar](hdinsight-hadoop-create-linux-clusters-arm-templates.md).
...
3. I den **HDInsight** bladet, klickar du på **anpassad (storlek, inställningar, appar)**, klickar du på **grunderna**, och ange följande information.
    ...
    * Från den **prenumeration** listrutan, Välj den Azure-prenumeration som används för klustret.
    ...
    * Klicka på **kluster typen**, och välj sedan typ av kluster (Hadoop, Spark, etc.) som du vill skapa. För **operativsystemet**, klickar du på **Linux** och välj sedan en version. Använda standardversionen om du inte vet vad du ska välja. Mer information finns i [HDInsight-klusterversioner](hdinsight-component-versioning.md).
        ...
        För Hadoop, Spark och interaktiva frågor klustertyper kan du också välja för att installera den **Enterprise säkerhetspaketet**. Enterprise-säkerhetspaketet kan säkerhetsfunktioner, till exempel Azure Active Directory-integrering och Apache Ranger för kluster. Mer information finns i [Enterprise säkerhetspaketet i Azure HDInsight](./domain-joined/apache-domain-joined-introduction.md).

        ![Aktivera Enterprise säkerhetspaketet](./media/hdinsight-hadoop-create-linux-cluster-portal/hdinsight-enable-enterprise-security-package.png "aktivera Enterprise säkerhetspaketet")
    ...
    * Ange ett datacenter **plats** när klustret skapas.
...
6. Klicka på **klusterstorleken** att visa information om de noder som används för det här klustret. Ange antalet arbetarnoder som du behöver för klustret. Uppskattade kostnaden för att köra klustret visas också.
   
    ![Noden prisnivåer](./media/hdinsight-hadoop-create-linux-cluster-portal/hdinsight-create-cluster-nodes.png "ange antalet noder i klustret")
...
7. Klicka på **avancerade inställningar** att konfigurera andra valfria inställningar, till exempel med hjälp av **skriptåtgärder** att anpassa ett kluster för att installera anpassade komponenter eller ansluta till en **virtuellt nätverk**. Titta på tabellen nedan för mer information.
    ...
    ![Noden prisnivåer](./media/hdinsight-hadoop-create-linux-cluster-portal/hdinsight-create-cluster-advanced.png "ange antalet noder i klustret")
...
8. För **sammanfattning**, kontrollerar du informationen som du angav tidigare och klicka sedan på **skapa**.
    ...
    ![Noden prisnivåer](./media/hdinsight-hadoop-create-linux-cluster-portal/hdinsight-create-cluster-summary.png "ange antalet noder i klustret")
    ...
    > 
    > 
12. När processen är klar klickar du på ikonen för klustret från startsidan. Fönstret kluster innehåller följande information.
    
    ![Klustret gränssnittet](./media/hdinsight-hadoop-create-linux-cluster-portal/hdinsight-create-cluster-completed.png "egenskaper för kluster")
