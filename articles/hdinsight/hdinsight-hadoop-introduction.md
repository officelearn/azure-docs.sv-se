<properties
    pageTitle="Vad är Hadoop i molnet? Introduktion till HDInsight | Microsoft Azure"
    description="Vad är Hadoop i molnet och hur hanteras det i HDInsight? En introduktion till komponenterna i Hadoop och analys av stordata."
...
# Vad är Hadoop i molnet? En introduktion till Hadoop-komponenterna i HDInsight för analys av stordata.
...
## Vad är Hadoop i molnet i HDInsight?

Azure HDInsight distribuerar och etablerar hanterade Apache Hadoop-kluster i molnet genom att tillhandahålla ett ramverk för programvara som utformats för att bearbeta, analysera och rapportera om stordata med hög tillförlitlighet och tillgänglighet. HDInsight använder Hadoop-distributionen för **Hortonworks Data Platform (HDP)** . Med Hadoop avses ofta hela Hadoop-ekosystemet av komponenter, som omfattar Apache HBase, Apache Spark och Apache Storm samt annan teknik som ryms under samlingsnamnet Hadoop. I [Översikt över Hadoop-ekosystemet i HDInsight](#overview) nedan finner du ytterligare information.


## Vad är stordata?
Med stordata avses data som samlas in i ständigt växande volymer, med allt högre hastighet och i ett växande antal ostrukturerade format och varierande semantiska sammanhang.

Stordata kan avse vilken stor samling digital information som helst, från texten i ett Twitter-flöde eller sensorinformation från industriell utrustning till information om hur kunder surfar och gör inköp i en onlinekatalog. Stordata kan vara historiska data (dvs. lagrade) eller realtidsdata (vilket innebär att de strömmas direkt från källan).
...
Azure HDInsight distribuerar och etablerar Hadoop-kluster i molnet på **Linux**. I tabellen nedan finns mer information.

Kategori | Hadoop på Linux
---------| -------------------
**Kluster-OS** | Långsiktigt stöd för Ubuntu 12.04 (LTS)
**Typ av kluster** | Hadoop, Spark, HBase, Storm
**Distribution** | Azure-portalen, Azure CLI, Azure PowerShell
**Klusteranvändargränssnitt** | Ambari
**Fjärråtkomst** | Secure Shell (SSH), REST API, ODBC, JDBC
...
> [AZURE.NOTE] Information om specifika komponenter och olika versioner finns i [Vad är nytt i de Hadoop-kluster-versioner som tillhandahålls av HDInsight?][component-versioning]
...
* Avancerade Hadoop-komponenter. Närmare information finns i [Vad är nytt i de Hadoop-klusterversioner som tillhandahålls av HDInsight?][component-versioning].

* Tillförlitliga kluster med hög tillgänglighet.  En andra huvudnod har lagts till i de Hadoop-kluster som distribueras med HDInsight för att öka tillgängligheten till tjänsten. Standardimplementeringar av Hadoop-kluster har normalt bara en huvudnod. HDInsight eliminerar denna enskilda felpunkt genom att lägga till en andra huvudnod. Övergången till en ny HA-klusterkonfiguration ändrar inte priset för klustret såvida inte kunderna skapar kluster med en extra stor huvudnod i stället för den stora standardnoden.

    Mer information finns i [Tillgänglighet och tillförlitlighet för Hadoop-kluster i HDInsight](hdinsight-high-availability-linux.md).
...
* Integrering med andra Azure-tjänster, inklusive [Web apps](../documentation/services/app-service/web/) och [SQL Database](../documentation/services/sql-database/).

* Ytterligare VM-storlekar. HDInsight-kluster är tillgängliga på VM av olika typer och storlekar. HDInsight-kluster kan nu använda A2- till A7-storlekar som byggts för allmänna ändamål: noder i D-serien med SSD-enheter och 60 procent snabbare processorer samt A8- och A9-storlekar som har InfiniBand-stöd för snabba nätverksfunktioner. Kunder med Apache HBase på Azure HDInsight kan utnyttja de större minneskonfigurationerna i D-serien för att öka prestandan. Kunder med Apache Storm på Azure HDInsight kan också dra nytta av ytterligare minne för inläsning av större referensdatamängder samt snabbare processorer som ger bättre genomströmning.
...
* Låg ingångskostnad. Prova en [kostnadsfri utvärderingsversion](/pricing/free-trial/) eller läs i [prisinformationen för HDInsight](/pricing/details/hdinsight/).
...
* [HDInsight-dokumentation](https://azure.microsoft.com/documentation/services/hdinsight/): Dokumentationssidan för Azure HDInsight med länkar till artiklar, videor och andra resurser.

* [Komma igång med HDInsight på Linux](hdinsight-hadoop-linux-tutorial-get-started.md): En snabbkurs i hur du etablerar HDInsight Hadoop-kluster på Linux-etablering och kör Hive-exempelfrågor.

* [Komma igång med Linux-baserade Storm på HDInsight](hdinsight-apache-storm-tutorial-get-started-linux.md): En snabbkurs i hur du etablerar ett Storm på HDInsight-kluster och kör exempel på Storm-topologier.

* [Etablera HDInsight på Linux](hdinsight-hadoop-provision-linux-clusters.md): Lär dig hur du etablerar ett HDInsight Hadoop-kluster på Linux via Azure-portalen, Azure CLI eller Azure PowerShell.

* [Arbeta med HDInsight på Linux](hdinsight-hadoop-linux-information.md): Få några snabba tips på hur du arbetar med Hadoop Linux-kluster som etablerats på Azure.

* [Hantera HDInsight-kluster med Ambari](hdinsight-hadoop-manage-ambari.md): Lär dig hur du övervakar och hanterar ditt Linux-baserade Hadoop på HDInsight-kluster med Ambari Web eller Ambari REST API.
...
### SQL Database på Azure

* [Azure SQL Database](/documentation/services/sql-database/): Dokumentation, självstudier och videor för SQL Database.

* [SQL Database i Azure-portalen](../sql-database/sql-database-manage-portal.md): Ett enkelt och användarvänligt databashanteringsverktyg för hantering av SQL Database i molnet.

* [Adventure Works för SQL Database](http://msftdbprodsamples.codeplex.com/releases/view/37304): Hämtningssida för en exempeldatabas för SQL Database.

### Microsoft Business Intelligence (för HDInsight på Windows)
...
* [Ansluta Excel till Hadoop med Power Query](hdinsight-connect-excel-power-query.md): Information om hur du ansluter Excel till det Azure Storage-konto som lagrar data som är associerade med ditt HDInsight-kluster med hjälp av Microsoft Power Query för Excel.

* [Ansluta Excel till Hadoop med ODBC-drivrutinen för Microsoft Hive](hdinsight-connect-excel-hive-ODBC-driver.md): Information om hur du importerar data från HDInsight med hjälp av ODBC-drivrutinen för Microsoft Hive.
...
* <a target="_blank" href="http://msdn.microsoft.com/library/hh231701.aspx">Mer information om SQL Server Analysis Services</a>.

* <a target="_blank" href="http://msdn.microsoft.com/library/ms159106.aspx">Information om SQLServer Reporting Services</a>.


### Prova Hadoop-lösningarna för stordataanalys (för HDInsight på Windows)

Tillämpa stordataanalys på din organisations data och få värdefulla insikter om verksamheten. Här följer några exempel:

* [Analysera HVAC-sensordata](hdinsight-hive-analyze-sensor-data.md): Lär dig att analysera sensordata med Hive med HDInsight (Hadoop) och visualisera data i Microsoft Excel. I det här exemplet använder du Hive för att bearbeta historiska data som producerats av HVAC-system för att se vilka system som inte kan bibehålla en inställd temperatur på ett tillförlitligt sätt.

* [Använda Hive med HDInsight för att analysera webbplatsloggar](hdinsight-hive-analyze-website-log.md): Lär dig använda HiveQL i HDInsight för att analysera webbplatsloggar och få inblick i antalet besök från externa webbplatser under en dag och en sammanfattning av de webbplatsfel som användarna råkat ut för.

* [Analysera sensordata i realtid med Storm och HBase i HDInsight (Hadoop)](hdinsight-storm-sensor-data-analysis.md): Lär dig hur du skapar en lösning som använder Storm-kluster i HDInsight för att bearbeta sensordata från Azure Event Hubs och därefter visa bearbetade sensordata nästan i realtid på en webbaserad instrumentpanel.
