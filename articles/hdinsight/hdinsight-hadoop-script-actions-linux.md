...
## <a name="bestPracticeScripting"></a>Metod tips för skript utveckling
...
### <a name="bPS1"></a>Rikta den Apache Hadoop versionen
...
Använd `lsb_release`för att kontrol lera operativ system versionen. Följande skript visar till exempel hur du refererar till en speciell tar-fil beroende på operativ system version:
...
### <a name="bps10"></a>Rikta in dig på operativ systemets version
...
### <a name="bPS2"></a>Tillhandahålla stabila länkar till skript resurser
...
Till exempel lagras de exempel som tillhandahålls av Microsoft i [https://hdiconfigactions.blob.core.windows.net/](https://hdiconfigactions.blob.core.windows.net/) lagrings konto. Den här platsen är en offentlig, skrivskyddad behållare som hanteras av HDInsight-teamet.

### <a name="bPS4"></a>Använd förkompilerade resurser
...
### <a name="bPS3"></a>Kontrol lera att skriptet för kluster anpassning är idempotenta
...
### <a name="bPS5"></a>Säkerställ hög tillgänglighet för kluster arkitekturen
...
### <a name="bPS6"></a>Konfigurera anpassade komponenter för användning av Azure Blob Storage
...
I det här exemplet använder kommandot `hdfs` överordnat standard kluster lagringen. För vissa åtgärder kan du behöva ange URI: n. Till exempel `adl:///example/jars` för Azure Data Lake Storage Gen1 `abfs:///example/jars` för Data Lake Storage Gen2 eller `wasb:///example/jars` för Azure Storage.

### <a name="bPS7"></a>Skriv information till STDOUT och STDERR
...
Mer information om hur du visar information som loggas av skript åtgärder finns i [Anpassa HDInsight-kluster med skript åtgärd](hdinsight-hadoop-customize-cluster-linux.md#troubleshooting)

### <a name="bps8"></a>Spara filer som ASCII med LF-linje slut
...
### <a name="bps9"></a>Använd logik för återförsök för att återställa efter tillfälliga fel
...
## <a name="helpermethods"></a>Hjälp metoder för anpassade skript
...
## <a name="commonusage"></a>Vanliga användnings mönster
...
Parametrar som skickas till skriptet kallas för *positions parametrar*och tilldelas `$1` för den första parametern `$2` för den andra osv. `$0` innehåller namnet på själva skriptet.
...
Där VARIABLENAME är namnet på variabeln. Använd `$VARIABLENAME`för att få åtkomst till variabeln. Om du till exempel vill tilldela ett värde som tillhandahålls av en positions parameter som en miljö variabel med namnet PASSWORD, använder du följande instruktion:

    PASSWORD=$1

Efterföljande åtkomst till informationen kan sedan använda `$PASSWORD`.

Miljövariabler som anges i skriptet finns bara inom skriptets omfattning. I vissa fall kan du behöva lägga till systemomfattande miljövariabler som behålls när skriptet har avslut ATS. Lägg till variabeln `/etc/environment`för att lägga till miljövariabler i hela systemet. Följande uttryck lägger till exempel till `HADOOP_CONF_DIR`:
...
> [!NOTE]  
> URI-formatet som används för att referera till skriptet varierar beroende på vilken tjänst som används. För lagrings konton som är associerade med HDInsight-klustret använder du `wasb://` eller `wasbs://`. Använd `http://` eller `https://`för offentligt läsbara URI: er. För Data Lake Storage använder du `adl://`.

## <a name="deployScript"></a>Check lista för att distribuera en skript åtgärd
...
## <a name="runScriptAction"></a>Så här kör du en skript åtgärd
