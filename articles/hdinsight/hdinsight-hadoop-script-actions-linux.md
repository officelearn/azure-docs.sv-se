...
Använd `lsb_release`om du vill kontrol lera operativ system versionen. Följande skript visar till exempel hur du refererar till en speciell tar-fil beroende på operativ system version:
...
En annan viktig skillnad mellan HDInsight 3,4 och 3,5 är `JAVA_HOME` att nu pekar på Java 8. Följande kod visar hur du avgör om skriptet körs på Ubuntu 14 eller 16:
...
Du kan hitta det fullständiga skriptet som innehåller de här kodfragmenten https://hdiconfigactions.blob.core.windows.net/linuxhueconfigactionv02/install-hue-uber-v02.shpå.
...
I det här exemplet använder `hdfs` kommandot Parent standard kluster lagringen. För vissa åtgärder kan du behöva ange URI: n. Till exempel `adl:///example/jars` för Azure Data Lake Storage Gen1 `abfs:///example/jars` för data Lake Storage Gen2 eller `wasb:///example/jars` för Azure Storage.
...
Som standard `echo` skickar STRÄNGEN till stdout. För att dirigera den till STDERR, `>&2` Lägg `echo`till före. Ett exempel:
...
| `untar_file TARFILE DESTDIR` |Extraherar en tar-fil `-xf`(med) till mål katalogen. |
...
Miljövariabler som anges i skriptet finns bara inom skriptets omfattning. I vissa fall kan du behöva lägga till systemomfattande miljövariabler som behålls när skriptet har avslut ATS. Lägg till variabeln i `/etc/environment`om du vill lägga till miljövariabler i hela systemet. Följande uttryck lägger till exempel till `HADOOP_CONF_DIR`:
...
> [!NOTE]  
> URI-formatet som används för att referera till skriptet varierar beroende på vilken tjänst som används. För lagrings konton som är associerade med HDInsight- `wasb://` klustret `wasbs://`använder du eller. För offentligt läsbara URI: `http://` er `https://`, använder eller. För Data Lake Storage använder `adl://`du.
