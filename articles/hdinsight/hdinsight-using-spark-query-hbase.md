...
* [URI-schemat](hdinsight-hadoop-linux-information.md#URI-and-scheme) för klustrets primära lagring. Det här schemat är wasb://för Azure Blob Storage, abfs://för Azure Data Lake Storage Gen2 eller adl://för Azure Data Lake Storage Gen1. Om säker överföring har Aktiver ATS för Blob Storage blir URI: n `wasbs://`.  Se även [säker överföring](../storage/common/storage-require-secure-transfer.md).
...
4. Kör `spark-shell` som refererar till Spark HBase-anslutaren genom dess maven-koordinater i alternativet `packages`.
...
1. Använd kommandot `ssh` för att ansluta till ditt HBase-kluster. Redigera kommandot nedan genom att ersätta `HBASECLUSTER` med namnet på ditt HBase-kluster och ange sedan kommandot:
...
1. I ditt öppna Spark-gränssnitt anger du följande `import`-uttryck:
...
1. Ange kommandot nedan för att definiera en katalog för den kontakt tabell som du skapade i HBase:
    ...
    Koden gör följande:  
     ...
     a. Definiera ett katalog schema för HBase-tabellen med namnet `Contacts`.  
     ...
     c. Rowkey måste också definieras i detalj som en namngiven kolumn (`rowkey`), som har en viss kolumn serie `cf` av `rowkey`.  

1. Ange kommandot nedan för att definiera en metod som ger en DataFrame runt din `Contacts`-tabell i HBase:
