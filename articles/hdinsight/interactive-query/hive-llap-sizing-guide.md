...
För D14v2 Worker-arbetsnoder är det rekommenderade värdet för LLAP-kön _ * 80 * *.   
...
Om du behöver justera antalet körningar rekommenderar vi att du funderar 4 GB minne per utförar enligt _hive. Tez. container. size * och se till att det totala minne som krävs för alla körningar inte överskrider det totala tillgängliga minnet för LLAP daemon-behållare.  
...
**_Hive. LLAP. io. trådpool. size_*_: det här värdet anger Poolens storlek för körningar. Eftersom körningar har åtgärd ATS som de har angetts kommer det att vara samma som antalet körningar per LLAP daemon. För D14 v2 är det rekommenderade värdet _* 12** .
