...
I `ambari-agent`s loggarna visas följande när problemet uppstår.
```
...
1. Gå till sammanfattning av Hive-komponent på Ambari-portalen och klicka på fliken `Configs`.

2. Gå till avsnittet `Advanced hive-log4j` i avancerade inställningar.
...
```
5. Ange `hive.root.logger` för `INFO,RFA` enligt följande. Standardinställningen är DEBUG, vilket gör att loggar blir mycket stora.
