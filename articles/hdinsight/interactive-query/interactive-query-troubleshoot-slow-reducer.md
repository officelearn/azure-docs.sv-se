...
ms.locfileid: "68968441"
---
# <a name="scenario-reducer-is-slow-in-azure-hdinsight"></a>Scenario: Minskning är långsam i Azure HDInsight
...
När du kör en fråga, `insert into table1 partition(a,b) select a,b,c from table2` till exempel en frågeplan, startar en mängd olika Minskare, men data från varje partition går till en enda minsknings punkt. Detta gör att frågan blir så låg som den tid det tar för den största partitionens minskning.
...
Om partitionerna i inmatnings tabellen är mindre (t. ex. mindre än 10), och så är antalet utgående partitioner, och variabeln är `true`inställd på, så gör det att data globalt sorteras och skrivs med hjälp av en enda minskning per partition. Även om antalet tillgängliga minsknings bara enheter är större, kan det hända att ett fåtal dämpare slutar på grund av data skevningen och att maximal parallellitet inte kan uppnås. När det ändras `false`till kan mer än en minskning hantera en enda partition och flera mindre filer skrivs ut, vilket resulterar i snabbare infogning. Detta kan påverka ytterligare frågor på grund av förekomsten av mindre filer.

Värdet `true` är meningsfullt när antalet partitioner är större och data inte är sneda. I sådana fall skrivs resultatet av kart fasen ut så att varje partition hanteras av en enda minskning som resulterar i bättre efterföljande frågeresultat.
