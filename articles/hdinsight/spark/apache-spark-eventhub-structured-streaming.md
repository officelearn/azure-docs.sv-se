...
8. Om du tittar på slutpunkten EventHub-kompatibel i följande format del som läser `iothub-xxxxxxxxxx` EventHub-kompatibel Namespace-namn och kan användas för `eventhubs.namespace`. Fältet `SharedAccessKeyName` kan användas för `eventhubs.policyname`, och `SharedAccessKey` för `eventhubs.policykey`: 

        Endpoint=sb://iothub-xxxxxxxxxx.servicebus.windows.net/;SharedAccessKeyName=xxxxx;SharedAccessKey=xxxxxxxxxx 

9. Klistra in den ändrade fragment i väntar scala > Kommandotolken och tryck på RETUR. Du bör se utdata som liknar följande:
...
10. Därefter du börjar skriva en Spark strukturerade strömning frågan att ange källa. Klistra in följande i Spark-gränssnittet och tryck på RETUR.
...
11. Du bör se utdata som liknar följande:

        inputStream: org.apache.spark.sql.DataFrame = [body: binary, offset: bigint ... 5 more fields]

12. Sedan redigera frågan så att den skrivs utdata till konsolen. Gör detta genom att klistra in följande i Spark-gränssnittet och trycka på RETUR.

        val streamingQuery1 = inputStream.writeStream.
        outputMode("append").
        format("console").start().awaitTermination()

13. Du bör se vissa grupper som börjar med utdata som liknar följande
...
14. Detta följs av resultatet med bearbetning av varje microbatch av händelser. 
...
15. När nya händelser anländer från händelsen producenten, bearbetas de av den här frågan strukturerade strömning.
16. Ta bort ditt HDInsight-kluster när du är klar med det här exemplet.
