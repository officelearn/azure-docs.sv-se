...
* Du föredrar att använda garn, eftersom det separeras `spark-submit` per batch.
...
* Föredra `ReduceByKey` med den fasta minnes gränsen till `GroupByKey`, som tillhandahåller agg regeringar, fönster och andra funktioner, men den har en minnes gräns som inte är kopplad till Ann.
...
En `Broadcast` koppling passar bäst för mindre data mängder eller där en sida av kopplingen är mycket mindre än den andra sidan. Den här typen av anslutning sänder ut en sida till alla körningar och kräver därför mer minne för sändningar i allmänhet.
...
Om du använder Bucket tabeller har du en tredje kopplings typ, den `Merge` kopplingen. En korrekt fördelad och försorterad data uppsättning hoppar över den dyra sorterings fasen från en `SortMerge` anslutning.
...
* Faktorer för att öka utförar storlek:
    ...
    5. Valfritt: öka användning och samtidighet med oversubscribing CPU.
...
Nyckeln till Spark 2. x-frågans prestanda är Tungsten-motorn, som är beroende av kodgenerering i hela fasen. I vissa fall kan generering av kod i hela fasen inaktive ras. Om du till exempel använder en icke-föränderligt typ (`string`) i agg regerings uttrycket visas `SortAggregate` i stället för `HashAggregate`. För bättre prestanda kan du till exempel prova följande och sedan återaktivera kodgenerering:
