...
`livy.server.session.timeout` svärdet styr hur lång tid Apache livy ska vänta tills en session har slutförts. När sessionens längd når `session.timeout`-värdet, stoppas livy-sessionen och programmet automatiskt.

## <a name="resolution"></a>Upplösning

För tids krävande jobb ökar du värdet för `livy.server.session.timeout` med hjälp av Ambari-ANVÄNDARGRÄNSSNITTET. Du kan komma åt livy-konfigurationen från Ambari-ANVÄNDARGRÄNSSNITTET med URL: en `https://<yourclustername>.azurehdinsight.net/#/main/services/LIVY/configs`.
