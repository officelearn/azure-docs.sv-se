...
description: Lär dig hur du skapar Storm- C#topologier i. Skapa en topologi för ord räkning i Visual Studio med hjälp av Hadoop-verktygen för Visual Studio.
...
> [!NOTE]  
> Medan stegen i det här dokumentet är beroende av en Windows-utvecklings miljö med Visual Studio, kan det kompilerade projektet skickas till antingen ett Linux-eller Windows-baserat HDInsight-kluster. Endast Linux-baserade kluster som skapats efter 28 oktober 2016 stöder SCP.NET-topologier.
...
> [!IMPORTANT]  
> C#-topologier på Linux-baserade kluster måste använda .NET 4.5 och använda Mono för att köra på HDInsight-klustret. Information om möjliga inkompatibiliteter finns i [mono-kompatibilitet](https://www.mono-project.com/docs/about-mono/compatibility/).
...
3. Ange `PATH`-miljövariabeln för att inkludera katalogen *% JAVA_HOME% \ bin* .
...
Så här skapar C# du ett Topology-projekt i Visual Studio:
...
1. Öppna *Spout.cs*. Huvud komponenterna för en kanalen är:
   ...
   * `Fail` (endast transaktionell topologi): hanterar tupler som inte kan bearbeta andra komponenter i topologin. Genom att implementera en `Fail` metod kan du skapa en ny tupel så att den kan bearbetas igen.
...
3. Öppna *splitter.cs*. Den har bara en metod som standard: `Execute`. Metoden `Execute` anropas när bulten tar emot en tupel för bearbetning. Här kan du läsa och bearbeta inkommande tupler och generera utgående tupler.
...
2. I dialog rutan **sändning Topology** , under List rutan **Storm-kluster** väljer du storm på HDInsight-kluster och väljer sedan **Skicka**. Du kan kontrol lera om överföringen lyckas genom att visa fönstret **utdata** .
...
3. I avsnittet **topologi Sammanfattning** väljer du **Kill** för att stoppa topologin.
...
* **Ack**: varje bult i topologin kan anropa `this.ctx.Ack(tuple)` för att bekräfta att en tupel har bearbetats. När alla bultar har godkänt tuppeln anropas `Ack`-metoden för kanalen. Med metoden `Ack` kan kanalen ta bort data som cachelagrats för uppspelning.

* **Misslyckades**: varje bult kan anropa `this.ctx.Fail(tuple)` för att indikera att bearbetningen har misslyckats för en tupel. Det går inte att sprida till `Fail`-metoden för kanalen, där tuppeln kan spelas upp med hjälp av cachelagrade metadata.

* **Sekvens-ID**: när du avger en tupel kan du ange ett unikt sekvens-ID. Det här värdet identifierar tuppeln för uppspelnings bearbetning (`Ack` och `Fail`). Till exempel använder kanalen i storm- **exempelprojektet** följande metod anrop vid sändning av data:

  `this.ctx.Emit(Constants.DEFAULT_STREAM_ID, new Values(sentence), lastSeqId);`

  Den här koden ger en tupel som innehåller en mening till standard data strömmen, med det sekvens-ID-värde som finns i `lastSeqId`. I det här exemplet ökas `lastSeqId` för varje tupel.
...
* kanalen och **Java-bult**: definieras i `HybridTopology_csharpSpout_javaBolt`-klassen. **C#**
...
Om du vill byta topologi som används när projektet skickas, flytta `[Active(true)]`-instruktionen till den topologi du vill använda, innan du skickar den till klustret.
...
* Använd `JavaComponentConstructor` för att skapa en instans av Java-klassen för en kanalen eller en bult.
...
> [!NOTE]  
> Du måste fortfarande använda `CustomizedInteropJSONSerializer` för att serialisera data som skapats av kanalen.
...
Om du använder en `Get` metod för att returnera en instans av-komponenten måste du se till att den passerar både parametrarna `Context` och `Dictionary<string, Object>` till konstruktorn. Följande exempel är en grundläggande `Get` metod som skickar dessa värden korrekt:
...
Mer information finns i avsnittet [använda ConfigurationManager](#configurationmanager) i det här dokumentet.
...
1. Använd följande kod som innehållet i klassen `LocalTest`:
    ...
    Ta en stund att läsa igenom kod kommentarerna. I den här koden används `LocalContext` för att köra komponenterna i utvecklings miljön. Den behåller data strömmen mellan komponenter till textfiler på den lokala enheten.
...
1. Spara ändringarna och välj **F5** eller välj **Felsök** > **Starta fel sökning** för att starta projektet. Ett konsol fönster bör visas och logg status visas som test förlopp. När `Tests finished` visas väljer du en tangent för att stänga fönstret.
...
1. Använd **Utforskaren** för att hitta den katalog som innehåller ditt projekt. (Till exempel: *C:\\användare\\\<your_user_name >\\source\\databaser\\WordCount\\WORDCOUNT*.) I den här katalogen öppnar du sedan *bin*och väljer sedan *Felsök*. Du bör se de textfiler som skapades när testerna kördes: *menings. txt*, *Counter. txt*och *delare. txt*. Öppna varje textfil och granska data.
   ...
   > [!NOTE]  
   > Sträng data sparas som en matris med decimal värden i de här filerna. Till exempel representerar `[[97,103,111]]` i **delare. txt** -filen ordet *sedan*.
...
Ersätt *sshuser* med SSH-användarkontot för klustret. Ersätt *kluster* namn med namnet på HDInsight-klustret. Mer information om hur du använder `scp` och `ssh` med HDInsight finns i [använda SSH med HDInsight](../hdinsight-hadoop-linux-use-ssh-unix.md).
...
Om *HDInsight-scpwebapi. out* -loggfilen innehåller en `FileNotFoundException`kan undantaget orsakas av följande:

* JDK finns inte i sökvägen till utvecklings miljön. Kontrol lera att JDK har installerats i utvecklings miljön och att `%JAVA_HOME%/bin` finns i sökvägen.
