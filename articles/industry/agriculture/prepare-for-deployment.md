...
- Prenumeration: deltagare till ägare
- Resurs grupp: ägare
...
2. I list menyn **Välj domän** väljer du alternativet **land**.
3. I list rutan **Välj användning** väljer du alternativet **utbildning**.
4. Välj ditt land på list menyn **Välj land** .
5. Välj **Registrera** för att slutföra registrerings processen.
...
**Fall 1**: installations programmet förutsätter att du har behörighet att skapa en Azure Active Directory-programregistrering i den önskade prenumerationen. Registrera dig genom att logga in på portalen, gå till **Azure Active directory** > **app Registration** > **ny registrering**.
...
1. Hämta registrerings [skriptet](https://aka.ms/FarmBeatsAADScript).
...
6. I Cloud Shell anger du följande kommandon för att köra skriptet.
...
2. Välj **Sök/Marketplace** eller **Skapa resurser**. Skriv **FarmBeats** om du vill visa information om erbjudandet. Hämta de guider som är tillgängliga via aka.ms-hyperlänkar på den här sidan innan du fortsätter till nästa steg.
3. Välj **skapa** och ange följande information:

   - Ange prenumerations namn.
   - Ange ett befintligt resurs grupp namn (endast tom resurs grupp) eller skapa en ny resurs grupp för att distribuera Azure-FarmBeats. Anteckna den här resurs gruppen och ange den i filen indata. json i ett senare skede.
   - Ange den region som du vill installera Azure-FarmBeats i. FarmBeats kan för närvarande installeras i följande regioner: centrala USA, Västeuropa, östra USA 2, norra Europa, västra USA, Sydostasien, östra USA, östra Australien, västra USA 2.
4. Välj **OK**, som omdirigerar dig till användningsvillkor sidan. Granska standard villkoren för Marketplace eller Välj hyperlänken för att granska användnings villkoren.
5. Välj **Stäng**, sedan kryss rutan "Jag accepterar" och välj sedan **skapa**.
6. Nu har du signerat licens avtalet (EULA) för Azure FarmBeats på Marketplace. Följ nästa steg i den här guiden om du vill fortsätta med distributionen.

### <a name="prepare-inputjson-file"></a>Förbered indata. JSON-fil

Den här filen är indatafilen till Azure Cloud Shell och parametrar vars värden anges i den här filen innan uppladdningen uppmanas inte under distributionen, vilket innebär att du sparar en stund.  
...
|"datahubResourceGroup"  | Resurs grupps namn för data Hubbs resurser.|
|"datahubLocation" | Den plats där du vill lagra data Hubbs resurserna.|
|"acceleratorWebsiteName"  |Unikt URL-prefix för att namnge din data hubb
Swagger webbplats. Standardvärdet är resurs grupps namnet för data hubb. Tryck på RETUR för att fortsätta med standardvärdet.|
|"acceleratorResourceGroup"  | Resurs grupps namn för data Hubbs resurser. |
|"acceleratorLocation"  | Plats för lagring av data nav resurser
"acceleratorWebsiteName"  | Unikt URL-prefix som namnger Accelerator-webbplatsen. Standardvärdet är Accelerator. Tryck på RETUR för att fortsätta med standardvärdet.|
|''sentinelUsername'' | användar namn för att logga in på: https://scihub.copernicus.eu/dhus/#/self-registration.|
|"sentinelPassword"  | Lösen ord för att logga in på: https://scihub.copernicus.eu/dhus/#/self-registration.|
|"farmbeatsAppId"  | Värden som ska delas av team Azure-FarmBeats.  |
|"farmbeatsPassword"  | Värden som ska delas av team Azure-FarmBeats.|
|"notificationEmailAddress"  | E-postadress för att ta emot meddelanden för aviseringar som du konfigurerar i data hubben.|
|"uppda" aadAppClientId ""  |[**Valfritt**] Parameter som ska ingå i indatamängden. JSON endast om Azure AD-appen redan finns.  -Sant/falskt. Falskt för en första installation och sant för uppgraderings scenario.|
...
Som en del av Marketplace-arbetsflödet har du skapat en resurs grupp och signerat slut användar licens avtalet, som kan granskas en gång igen som en del av den faktiska distributionen. Distributionen görs via Azure Cloud Shell (webbläsarbaserad kommando rad) med hjälp av bash-miljön.  
...
2. Starta **Cloud Shell** från det övre navigerings fönstret i **Azure** -portalen.

   ![Taktslag i projekt grupp](./media/prepare-for-deployment/navigation-bar-1.png)

3. Välj en prenumeration för att skapa ett lagrings konto och Microsoft Azure fil resurs.
4. Välj **Skapa lagring**.
5. Välj den nedrullningsbara miljön från vänster sida i Shell-fönstret (bash).

   ![Taktslag i projekt grupp](./media/prepare-for-deployment/bash-1-1.png)

### <a name="deployment-scenario-1"></a>Distributions scenario 1

Installations programmet skapar Azure AD (du har Läs behörighet för AD-klienten).  

1. Ladda ned den [inmatade JSON](https://aka.ms/PPInputJsonTemplate) -mallen. Inkludera Azure Application-klient-ID och lösen ord i indata-JSON-filen och spara den.
2. Öppna den nedladdade filen i ett anteckningar och fyll i filen genom att ange värdena.
    ...
    > [!NOTE]
    > Du kan hoppa över det här steget så kommer indata att uppmanas vid körning.

3. Spara filen och anteckna sökvägen (på den lokala datorn).  
4. Gå till Azure Cloud Shell och när autentiseringen är klar väljer du överför (se ikonen markerad i bilden nedan) och laddar upp indata. JSON-filen för att Cloud Shell lagring. Du behöver inte skicka Azure AD-parametern i JSON eftersom installations programmet kommer att skapa och konfigurera Azure AD-appens registrering.

   ![Taktslag i projekt grupp](./media/prepare-for-deployment/bash-2-1.png)

5. Skriv eller klistra in kommandot "distribution" i Cloud Shell. Se till att ändra sökvägen till indatamängden. JSON-fil och tryck på RETUR.  

    ```
    wget -N -O farmbeats-installer.sh https://aka.ms/FB_1.2.0 && bash farmbeats-installer.sh> /home/dummyuser/input.json

    ```
   Skriptet laddar ned alla beroenden automatiskt och skapar distributören. Sedan uppmanas du att godkänna licens avtalet för slutanvändare (EULA) för Azure FarmBeats.
   ...
   Sedan uppmanas du att ange en åtkomsttoken för distributionen. Kopiera koden som genereras och logga in på https://microsoft.com/devicelogin med dina Azure-autentiseringsuppgifter.

   > [!NOTE]
   > Koden upphör att gälla efter 60 minuter. När den går ut kan du starta om genom att skriva distributions kommandot igen.

6. Ange lösen ord för Sentinel-kontot i nästa prompt.
7. Installations programmet validerar nu och börjar distribuera, vilket kan ta ungefär 20 minuter.
8. När distributionen har slutförts visas följande utgående länkar:
    ...
    - Loggfil för **distributions logg filen**som skapades under distributionen. Den kan användas för fel sökning.

    - Ange "Y" om du accepterar och fortsätter till nästa steg.
    - Ange "N" om du inte accepterar villkoren och distributionen kommer att avslutas.

   Sedan uppmanas du att ange en åtkomsttoken för distributionen. Kopiera koden som genereras och logga in för att https://microsoft.com/devicelogin med dina Azure-autentiseringsuppgifter.

   > [!NOTE]
    > Anteckna dessa och behåll distributions logg filens sökväg som är praktisk för framtida användning.


### <a name="deployment-scenario-2"></a>Distributions scenario 2

Befintlig Azure Active Directory app-registrering används för att distribuera.

1. Ladda ned [indata-JSON](https://aka.ms/PPInputJsonTemplate) inkludera Azure Application-klient-ID och lösen ord i indata. JSON, spara det.
...
4. Skriv eller klistra in *distributions kommandot* i Cloud Shell. Se till att ändra sökvägen till indatamängden. JSON-fil och tryck på RETUR.
...
5. Skriptet laddar ned alla beroenden automatiskt och skapar distributören.
6. Du uppmanas att läsa och godkänna licens avtalet för slutanvändare (EULA) för Azure FarmBeats.

   - Ange "Y" om du accepterar och fortsätter till nästa steg.
   - Ange "N" om du inte accepterar villkoren och distributionen kommer att avslutas.

7. Du uppmanas att ange en åtkomsttoken för distributionen. Kopiera koden som genereras och logga in för att https://microsoft.com/devicelogin med dina Azure-autentiseringsuppgifter.
8. Nu kommer installations programmet att validera och börja skapa resurserna, vilket kan ta ungefär 20 minuter. Låt sessionen vara aktiv på Cloud Shell under den här tiden.
9. När distributionen har slutförts visas följande utgående länkar:
   ...
   - **Distributions logg fil**: sparar loggar under distributionen och kan användas för fel sökning.
...
3. Välj en prenumeration för att skapa ett lagrings konto och en Azure Files fil resurs.
4. Välj **Skapa lagring**.
5. Välj miljön i list rutan från vänster om Shell (bash).
6. Gör ändringar i indata. JSON-filen om det behövs och överför till Azure Cloud Shell. Du kan till exempel uppdatera din e-postadress för den avisering som du vill ta emot.
7. Anteckna sökvägen till indata-JSON-filen (på den lokala datorn).
8. Gå till Azure Cloud Shell. När du har autentiserats väljer du knappen överför (se markerad ikon i bilden nedan) och laddar upp indata. JSON-filen för att Cloud Shell lagring.

   ![Taktslag i projekt grupp](./media/prepare-for-deployment/bash-2-1.png)

9. Skriv eller klistra in **distributions kommandot** i Cloud Shell. Se till att ändra sökvägen till indatamängden. JSON-fil och tryck på RETUR.
...
10. Installations programmet efterfrågar automatiskt nödvändiga indata vid körning:

    - Ange en åtkomsttoken för distribution. Kopiera koden som genereras och logga in för att https://microsoft.com/devicelogin med dina Azure-autentiseringsuppgifter.
...
11. Installations programmet validerar nu och börjar skapa resurserna, vilket kan ta ungefär 20 minuter.
12. När distributionen har slutförts visas följande utgående länkar:
    ...
    > [!NOTE]
    > Anteckna länkarna ovan och behåll sökvägen till distributions logg filen som är praktisk för framtida bruk.
...
4. Välj resursgruppens namn. Skriv namnet igen för att dubbelklicka och klicka på ta bort för att ta bort resurs gruppen och alla dess underliggande resurser.
