...
För att förbereda din lokala utvecklings miljö använder du git för att klona [webui](https://github.com/Azure/pcs-remote-monitoring-webui) -lagringsplatsen för fjärrövervakning till din lokala dator:
...
1. I samma fil lägger du till den nya sidan `pagesConfig` i matrisen. Ange adressen för vägen, referera till SVG-ikonen och de översättningar som lagts till tidigare och `component` Ställ in på sidans behållare: `to`
...
Föregående kommando kör användar gränssnittet lokalt på [http://localhost:3000/dashboard](http://localhost:3000/dashboard).
...
1. Använd Azure Portal eller [AZ CLI](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest) för att aktivera SSH-åtkomst till den virtuella dator som är värd för mikrotjänsterna i din lösning. Exempel:
...
1. Använd Azure Portal eller [AZ CLI](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest) för att hitta namnet och den offentliga IP-adressen för den virtuella datorn. Exempel:
...
1. I din lokala kopia av [webui](https://github.com/Azure/pcs-remote-monitoring-webui) -lagringsplatsen för fjärrövervakning redigerar du **. kuvert** -filen för att lägga till URL: en för din distribuerade lösning:
...
Mer information om lösnings acceleratorn för fjärrövervakning finns i [arkitektur](iot-accelerators-remote-monitoring-sample-walkthrough.md)för fjärrövervakning.
