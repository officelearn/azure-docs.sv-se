...
1. När versionen har slutförts kör du ditt program i samma kommando tolk. Ersätt `<scopeid>` och `<devicekey>` med de värden du antecknade tidigare:

    ```cmd
    .\Release\devkit_device.exe mxchip-001 <scopeid> <devicekey>
    ```

1. Enhets programmet börjar skicka data till IoT Hub. Ibland visas felet `Error registering device for DPS` första gången du kör föregående kommando. Om du ser det här felet kan du försöka köra kommandot igen.
