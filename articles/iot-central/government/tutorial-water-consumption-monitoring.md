...
1. Hitta typen av telemetri för `Temperature`.
...
1. Välj **hög pH-avisering**, som är en av de förkonfigurerade reglerna i programmet.
    ...
    `High flow alert`s regeln har kon figurer ATS för att kontrol lera mot villkors `Acidity (pH)` `greater than` `Max flow threshold`. Högsta flödes tröskel är en moln egenskap som definierats i mallen **Smart ventil** enhet. Värdet för `Max flow threshold` anges per enhets instans.
...
1. Du kan också ändra program avbildningar genom att välja **Administration** > **program inställningar**. Välj en bild som ska laddas upp som program avbildning genom att välja knappen **Välj bild** .
