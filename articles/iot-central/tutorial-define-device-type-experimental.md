...
Du behöver ett Azure IoT Central-program för att kunna genomföra den här självstudien. Om du har genomfört snabbstarten för att [skapa ett Azure IoT Central-program](quick-deploy-iot-central-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json) kan du återanvända det program som du skapade i den snabbstarten. I annat fall utför du följande steg för att skapa ett tomt Azure IoT Central-program:
...
2. Ange den e-postadress och det lösenord som du använder för att få åtkomst till din Azure-prenumeration:
    ...
    ![Ange ditt organisationskonto](./media/tutorial-define-device-type-experimental/sign-in.png)

3. Börja skapa ett nytt Azure IoT Central-program genom att klicka på **Nytt program**:

    ![Sidan Application Manager (Programhanterare) i Azure IoT Central](./media/tutorial-define-device-type-experimental/iotcentralhome.png)
...
4. Skapa ett nytt Azure IoT Central-program:
    ...
    * Välj **Utvärdering**. Du behöver inte en Azure-prenumeration för att skapa ett utvärderingsprogram.
       ...
       Mer information om kataloger och prenumerationer finns i [snabbstarten om att skapa ett program](quick-deploy-iot-central-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json).
    ...
    * Klicka på **Skapa**.
    ...
    ![Sidan Skapa program i Azure IoT Central](./media/tutorial-define-device-type-experimental/iotcentralcreate.png)

    Mer information finns i [snabbstarten om att skapa ett program](quick-deploy-iot-central-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json).
...
Om du vill lägga till en ny enhetsmall i programmet går du till sidan **Enhetsmallar**. Gör detta genom att klicka på **Enhetsmallar** i den vänstra navigeringsmenyn.

![Sidan Enhetsmallar](./media/tutorial-define-device-type-experimental/devicetemplates.png)
...
1. På sidan **Enhetsmallar** klickar du på **+ Nytt**:

    ![Sidan Enhetsmallar, Skapa enhetsmall](./media/tutorial-define-device-type-experimental/newtemplate.png)

3. På sidan **Skapa enhetsmall** anger du **Ansluten luftkonditioneringsenhet** som namn på enheten. Klicka sedan på **Skapa**. Du kan även ladda upp en bild av enheten som är synlig för operatörer i Device Explorer:

    ![Anpassad enhet](./media/tutorial-define-device-type-experimental/createcustomdevice.png)
...
4. I enhetsmallen **Ansluten luftkonditioneringsenhet** kontrollerar du att du är på fliken **Mått**, där du definierar telemetrin. Varje enhetsmall som du definierar har separata flikar där du kan:
    ...
    * Anpassa enhetens _instrumentpanel_ för dina operatörer.
    ...
    ![Luftkonditioneringsmått](./media/tutorial-define-device-type-experimental/airconmeasurements.png)

    > [!NOTE]
    > Om du vill ändra namn på enhetsmallen klickar du på mallnamnet längst upp på sidan.

5. Om du vill lägga till temperaturens telemetrimått klickar du på **+ Nytt mått**. Välj sedan **Telemetri** som måttenhet:

    ![Mått för ansluten luftkonditioneringsenhet](./media/tutorial-define-device-type-experimental/airconmeasurementsnew.png)

6. Varje typ av telemetri som du definierar för en enhetsmall innehåller [konfigurationsalternativ](howto-set-up-template-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json) som:
    ...
    * Simuleringsparametrar.
    ...
    Du kan även välja en färg för telemetrivisningen. Om du vill spara telemetridefinitionen klickar du på **Spara**:

    ![Konfigurera temperatursimulering](./media/tutorial-define-device-type-experimental/temperaturesimulation.png)
...
7. Efter en kort stund visar fliken **Mått** ett diagram över temperaturtelemetrin från den simulerade anslutna luftkonditioneringsenheten. Använd kontrollerna för att hantera synlighet eller sammansättning eller redigera telemetridefinitionen:
    ...
    ![Visa temperatursimulering](./media/tutorial-define-device-type-experimental/viewsimulation.png)

8. Du kan även anpassa diagrammet med kontrollerna **Rad**, **Staplad** och **Redigera tidsintervall**:

    ![Anpassa diagrammet](./media/tutorial-define-device-type-experimental/customizechart.png)
...
1. Lägg till händelsemåttet **Fel på fläktmotor** genom att klicka på **+ Nytt mått**. Välj sedan **Händelse** som måttyp:

    ![Mått för ansluten luftkonditioneringsenhet](./media/tutorial-define-device-type-experimental/eventnew.png)

2. Varje typ av händelse som du definierar för en enhetsmall innehåller [konfigurationsalternativ](howto-set-up-template-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json) som:
   ...
   * Allvarlighetsgrad.
    ...
    Spara händelsedefinitionen genom att klicka på **Spara**:

    ![Konfigurera händelsemätning](./media/tutorial-define-device-type-experimental/eventconfiguration.png)
...
3. Efter en kort stund visar fliken **Mått** ett diagram över de händelser som genereras slumpmässigt från den simulerade anslutna luftkonditioneringsenheten. Använd kontrollerna för att hantera synlighet eller redigera händelsedefinitionen:
    ...
    ![Visa händelsesimulering](./media/tutorial-define-device-type-experimental/eventview.png)

1. Klicka på händelsen i diagrammet om du vill se mer information om händelsen:

    ![Visa händelseinformation](./media/tutorial-define-device-type-experimental/eventviewdetail.png)
...
1. Lägg till tillståndsmåttet **Fläktläge** genom att klicka på **+ Nytt mått**. Välj sedan **Tillstånd** som måttyp:

    ![Tillståndsmått för ansluten luftkonditioneringsenhet](./media/tutorial-define-device-type-experimental/statenew.png)

2. Varje typ av tillstånd som du definierar för en enhetsmall innehåller [konfigurationsalternativ](howto-set-up-template-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json) som exempelvis:
   ...
   * Färg för varje värde.
    ...
    Spara definitionen för tillståndsmåttet genom att klicka på **Spara**:

    ![Konfigurera tillståndsmått](./media/tutorial-define-device-type-experimental/stateconfiguration.png)
...
3. Efter en kort stund visar fliken **Mått** ett diagram över de tillstånd som genereras slumpmässigt från den simulerade anslutna luftkonditioneringsenheten. Använd kontrollerna för att hantera synlighet eller redigera tillståndsdefinitionen:
    ...
    ![Visa tillståndssimulering](./media/tutorial-define-device-type-experimental/stateview.png)

4. Om för många datapunkter skickas från enheten på kort tid, visas tillståndsmåttet med ett annat utseende. Klicka på diagrammet för att se alla datapunkter inom tidsperioden i kronologisk ordning. Du kan även begränsa tidsintervallet för att se måtten i detalj.
...
* Du använder _egenskaper_ för att definiera metadata som associeras med enheten. Det finns två kategorier med egenskaper:
    ...
    * Du använder _enhetsegenskaper_ för göra så att en enhet kan skicka egenskapsvärden till programmet. De här egenskaperna kan bara ändras av enheten. För operatörer är enhetsegenskaper skrivskyddade. I det här scenariot med en ansluten luftkonditioneringsenhet är versionen av inbyggd programvara och enhetens serienummer enhetsegenskaper som rapporteras av enheten.
    ...
    Mer information finns i [Egenskaper](howto-set-up-template-experimental.md#properties) i instruktionerna om hur du konfigurerar en enhetsmall.
...
2. Du kan skapa inställningar av olika typer, till exempel nummer eller text. Klicka på **Nummer** för att lägga till en nummerinställning till enheten.
...
3. För att konfigurera inställningen **Ange temperatur** använder du informationen i följande tabell:
    ...
    ![Konfigurera inställningen Ange temperatur](./media/tutorial-define-device-type-experimental/configuresetting.png)
...
4. Du kan anpassa layouten för fliken **Inställningar** genom att flytta och ändra storlek på inställningspanelerna:
    ...
    ![Anpassa layout för inställningar](./media/tutorial-define-device-type-experimental/settingslayout.png)
...
1. Du kan skapa olika typer av enhetsegenskaper, till exempel nummer eller text. Om du vill lägga till en platsegenskap i enhetsmallen väljer du **Location**. För att konfigurera platsegenskapen använder du informationen i följande tabell:
    ...
    ![Konfigurera enhetsegenskaperna](./media/tutorial-define-device-type-experimental/configureproperties.png)
...
1. För att konfigurera egenskapen för senaste servicedatum använder du informationen i följande tabell:
    ...
    ![Konfigurera enhetsegenskaperna](./media/tutorial-define-device-type-experimental/configureproperties2.png)
...
1. För att konfigurera version av inbyggd programvara använder du informationen i följande tabell:
    ...
    ![Konfigurera version av inbyggd programvara](./media/tutorial-define-device-type-experimental/configureproperties3.png)
...
1. För att konfigurera serienumret använder du informationen i följande tabell:
    ...
    ![Konfigurera serienummer](./media/tutorial-define-device-type-experimental/configureproperties4.png)
...
1. Klicka på **+ Nytt kommando** för att lägga till ett kommando på enheten och börja konfigurera det nya kommandot.
...
1. Använd informationen i följande tabell för att konfigurera det nya kommandot:
    ...
    ![Förbereda för att lägga till en inställning](./media/tutorial-define-device-type-experimental/commandsecho1.png)
...
1. Klicka på **Linjediagram** för att lägga till komponenten på **Instrumentpanelen**.
...
1. Konfigurera komponenten **Linjediagram** med hjälp av informationen i följande tabell:
    ...
    ![Inställningar för linjediagram](./media/tutorial-define-device-type-experimental/linechartsettings.png)

    Klicka sedan på **Spara**.

1. Klicka på komponenten **Händelsehistorik** med hjälp av informationen i följande tabell:

    | Inställning      | Värde       |
    | ------------ | ----------- |
    | Rubrik        | Fläktmotorhändelser |
    | Tidsintervall   | Senaste 30 minuterna |
    | Mått     | Fel på fläktmotor (klicka på **Synlighet** bredvid **Fel på fläktmotor**) |

    ![Inställningar för linjediagram](./media/tutorial-define-device-type-experimental/dashboardeventchartsetting.png)
...
1. Konfigurera komponenten **Tillståndshistorik** med hjälp av informationen i följande tabell:
    ...
    | Rubrik        | Fläktläge |
    | Tidsintervall   | Senaste 30 minuterna |
    | Mått | Fläktläge (klicka på **Synlighet** bredvid **Fläktläge**) |

    ![Inställningar för linjediagram](./media/tutorial-define-device-type-experimental/dashboardstatechartsetting.png)

    Klicka sedan på **Spara**.

1. Om du vill lägga till enhetens inställningar och egenskaper på instrumentpanelen väljer du **Inställningar och egenskaper**. Klicka på **Lägg till/ta bort** för att lägga till de inställningar eller egenskaper som du vill ha på instrumentpanelen.
...
1. Konfigurera komponenten **Inställningar och egenskaper** med hjälp av informationen i följande tabell:
    ...
    ![Ange inställningar för temperaturegenskap](./media/tutorial-define-device-type-experimental/propertysettings4.png)
...
1. Du kan nu se simulerade data för din anslutna luftkonditionering på instrumentpanelen. Du kan redigera paneler och layout för instrumentpanelen:
    ...
    ![Visa instrumentpanel](./media/tutorial-define-device-type-experimental/dashboard.png)
