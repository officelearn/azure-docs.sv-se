...
1. Förbered din miljö för fel sökning enligt kraven för ditt utvecklings språk, ange en Bryt punkt i modulen och välj den fel söknings konfiguration som ska användas:
   - **C #**
     ...
     - Navigera till vyn Visual Studio Code debug genom att välja **visa > fel sökning**. Välj fel söknings konfiguration ** * &lt;ditt Modulnamn&gt; * lokal fel sökning (.net Core)** i list rutan.
        ...
        > [!NOTE]
        > Om din .NET Core `TargetFramework` inte stämmer överens med din program `launch.json`Sök väg i måste du manuellt uppdatera program Sök vägen i `launch.json` så att den `TargetFramework` matchar i din. CSPROJ-fil så att Visual Studio Code kan starta programmet.
...
1. I Visual Studio Code Integrated Terminal kör du följande kommando för att skicka ett **Hello World** meddelande till modulen. Detta är kommandot som visas i föregående steg när du konfigurerar IoT Edge Simulator.
   ...
   > [!TIP]
   > Du kan också använda [Postman](https://www.getpostman.com/) eller andra API-verktyg för att skicka meddelanden via `curl`i stället för.
...
>
> För moduler som skrivits i C#, inklusive Azure Functions, baseras det här exemplet på fel söknings `Dockerfile.amd64.debug`versionen av, som innehåller .net Core-KOMMANDORADEN (VSDBG) i behållar avbildningen när den skapas. När du har felsöker dina C#-moduler rekommenderar vi att du direkt använder Dockerfile utan VSDBG för produktion färdiga IoT Edge moduler.
...
- **Node.js**
  ...
  - På din utvecklings dator öppnar du Visual Studio Code och redigerar `launch.json` sedan så att adressprefixet för ** * &lt;ditt Modulnamn&gt; * (Node. js)** -profil (eller ** * &lt;modulens namn&gt; * fjärrfelsökning (Node. js i Windows-behållare)** visas om modulen körs som en Windows-behållare) är IP-adressen för den dator som ska felsökas.
...
- **Python**
  ...
  - På din `launch.json` utvecklings dator öppnar du Visual Studio Code och redigerar sedan så att `host` värdet för `localhost` ** * &lt;din&gt; * -profil för Fjärrhjälp (python)** använder IP-adressen för mål datorn i stället för.
...
1. Öppna modul filen för ditt utvecklings språk och Lägg till en Bryt punkt:
   ...
   - **Python**: Lägg till din Bryt punkt i `main.py`filen i callback-metoden där du lade `ptvsd.break_into_debugger()` till raden.
