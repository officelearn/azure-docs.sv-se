...
1. På fliken **vägar** definierar du hur meddelanden ska skickas mellan moduler och IoT Hub. Meddelanden skapas med hjälp av namn/värde-par. Ersätt standard `route` och `upstream` namn och värden med de par som visas i följande tabell, följande namn/värde-par, och ersätter instanserna av _{Modulnamn}_ med namnet på din Azure Stream Analytics-modul.
