...
| Enheten protokollstöd |Stöder MQTT MQTT över WebSockets, AMQP, AMQP över WebSockets och HTTP. Dessutom IoT-hubb fungerar med den [Azure IoT-protokollgatewayen][lnk-azure-protocol-gateway], en anpassningsbara protokollimplementering gateway att stödja anpassade protokoll. |Stöder AMQP AMQP över WebSockets och HTTP. |
...
| Enheten SDK |Ger [enheten SDK] [ Azure IoT SDKs] för en stor mängd olika plattformar och språk, förutom direkt MQTT, AMQP och HTTP-APIs. |Stöds på .NET, Java och C, dessutom AMQP och HTTP send-gränssnitt. |
