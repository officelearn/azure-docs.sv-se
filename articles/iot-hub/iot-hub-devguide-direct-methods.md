...
Anropa direkt metoder med hjälp av en modul stöds ID i C# Förhandsgranska SDK (tillgänglig [här](https://www.nuget.org/packages/Microsoft.Azure.Devices/1.16.0-preview-004)).

För detta ändamål använder den `ServiceClient.InvokeDeviceMethodAsync()` metod och skicka den `deviceId` och `moduleId` som parametrar.
