...
Enhets identiteter kan också exporteras och importeras från en IoT Hub via tjänst-API: et via [REST API](/rest/api/iothub/service/createimportexportjob) eller någon av IoT Hub [tjänst-SDK](/azure/iot-hub/iot-hub-devguide-sdks#azure-iot-hub-service-sdks): er.
...
| Factor |valfritt |Ett sammansatt objekt som innehåller autentiseringsinformation och säkerhets material. |
| auth. symkey |valfritt |Ett sammansatt objekt som innehåller en primär och en sekundär nyckel, lagrad i base64-format. |
| status |kunna |En åtkomst indikator. Kan **aktive ras** eller **inaktive**ras. Om **aktive rad**tillåts enheten att ansluta. Om den är **inaktive rad**kan den här enheten inte komma åt någon enhets slut punkt. |
| statusReason |valfritt |En 128 tecken lång sträng som lagrar orsaken till enhetens identitets status. Alla UTF-8-tecken tillåts. |
...
> [!NOTE]
> För närvarande stöder inte enhets-SDK: erna användning av `+` och `#` tecken i **deviceId**.
...
> [!NOTE]
> För närvarande stöder inte enhets-SDK: erna användning av `+` och `#` tecken i **deviceId** och **moduleId**.
