...
I allmänhet används alla egenskaper på alla gränssnitt med denna REST API-mall där `{device-id}` är identifieraren för enheten:
...
Du kan anropa kommandon för IoT Plug and Play Device direkt. Om `Thermostat` gränssnittet i `t-123` enheten har ett `restart` kommando kan du anropa det med ett REST API post-anrop:
