...
1. När du anslöt _enheten_ i terminalen var ett av utdatafilerna följande meddelande för att ange onlinestatus. Egenskapen `state` som används för att ange om enheten är online, är _True_:
...
1. Bläddra till `environmentalSensor`-komponenten i _tjänstens_ Terminal-utdata. Du ser att `state`-egenskapen har rapporter ATS som _Sant_:
    ```JSON
    "environmentalSensor" : {
