...
1. Det här terminalfönstret används nu som din _enhets_ Terminal. Gå till den klonade lagrings platsen och navigera till mappen **/Azure-IoT-samples-Node/Digital-Twins/Quickstarts/Device** . Installera alla beroenden genom att köra följande kommando:
...
1. Bläddra till `environmentalSensor`-komponenten i _tjänstens_ Terminal-utdata. Du ser att `state`-egenskapen har rapporter ATS som _online_:
...
1. I början av filen finns en uppsättning konstanter definierade med plats hållare för versaler. Ersätt `<DEVICE_ID_GOES_HERE>` plats hållaren med det faktiska enhets-ID: t, uppdatera de återstående konstanterna med följande värden och spara filen:
