...
Du behöver en Azure IoT-hubb i din Azure-prenumeration för att slutföra den här snabb starten. Om du inte har en Azure-prenumeration kan du skapa ett [kostnadsfritt konto](https://azure.microsoft.com/free/?WT.mc_id=A261C142F) innan du börjar.
...
Kör följande kommando för att skapa en enhets identitet för en enhet som kallas `mypnpdevice` i din IoT-hubb. Ersätt `{YourIoTHubName}` plats hållaren med namnet på din IoT-hubb:

```azurecli-interactive
az iot hub device-identity create --hub-name {YourIoTHubName} --device-id mypnpdevice
```

Kör följande kommandon för att hämta _enhets anslutnings strängen_ för enheten som du just har registrerat. Du behöver den här anslutnings strängen senare i den här snabb starten:
...
1. Skapa en `pnp_app` katalog i din arbets katalog. Du använder den här mappen för enhets modellens filer och enhets koden stub.
...
1. Välj den **SampleDevice. capabilitymodel. JSON** -fil som ska användas för att generera enhets kodens stub.
...
1. Välj **via källkod** som metod för att inkludera SDK: n.
...
1. Skapa en **cmake** build-mapp för **sample_device** -programmet:
...
> [Självstudie: skapa en testa en enhets kapacitets modell med Visual Studio Code](tutorial-pnp-visual-studio-code.md)
