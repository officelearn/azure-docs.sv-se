...
1. Ersätt innehållet i den här filen med följande JSON och `{your name}` Ersätt i `@id` fältet och i `EnvironmentalSensor` gränssnittet med samma värde som du använde i filen **EnvironmentalSensor. Interface. JSON** . Gränssnitts-ID: t måste vara unikt för att spara gränssnittet i databasen:
