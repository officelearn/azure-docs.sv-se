...
Kom igång med klient biblioteket för Azure Key Vault-certifikat för .NET. [Azure Key Vault](../general/overview.md) är en moln tjänst som tillhandahåller en säker lagring för certifikat. Du kan lagra nycklar, lösen ord, certifikat och andra certifikat på ett säkert sätt. Du kan skapa och hantera Azure-nyckelvalv via Azure Portal. I den här snabb starten lär du dig att skapa, hämta och ta bort certifikat från ett Azure Key Vault med hjälp av .NET-klient biblioteket
...
I det här exemplet kan du för enkelhetens skull använda självsignerade certifikat med standard utgivnings princip. För den här uppgiften använder du metoden [StartCreateCertificateAsync](/dotnet/api/azure.security.keyvault.certificates.certificateclient.startcreatecertificateasync) . Metodens första parameter godkänner ett certifikat namn[och certifikat principen.](https://docs.microsoft.com/dotnet/api/azure.security.keyvault.certificates.certificatepolicy)
