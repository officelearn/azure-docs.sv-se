...
När du använder Azure Key Vault kan du med extra garantier importera eller generera nycklar i HSM: er (Hardware Security modules) som aldrig lämnar HSM-gränser. Det här scenariot kallas ofta för att *ta med din egen nyckel*eller BYOK. HSM-modulerna är FIPS 140-2 Level 2-verifierade. Azure Key Vault använder hjälp programmet nCipher nshield maskinvarusäkerhetsmodul-serien HSM: er för att skydda dina nycklar.
...
> [!NOTE]
> Mer information om Azure Key Vault finns i [Vad är Azure Key Vault?](key-vault-whatis.md)  
...
* Nyckel utbytes nyckeln (KEK) som används för att kryptera nyckeln genereras i Azure Key Vault HSM: er och kan inte exporteras. HSM: er tvingar fram att det inte går att ta bort en tydlig version av KEK utanför HSM: er. Dessutom innehåller verktygs uppsättningen attestering från hjälp programmet nCipher att KEK inte kan exporteras och genererades i en äkta HSM som tillverkats av hjälp programmet nCipher.
...
Hjälp programmet nCipher Security är en ledande global leverantör av data kryptering och cyberhot säkerhetslösningar för finansiella tjänster, hög teknik, tillverkning, myndigheter och teknik sektorer. Med en spårning på 40 år som skyddar företags-och myndighets information används hjälp programmet nCipher säkerhets kryptografiska lösningar av fyra av de fem största energi-och flyg företagen. Deras lösningar används också av 22 NATO-länder/-regioner och skyddar över 80 per procent av globalt betalnings transaktioner.

Microsoft har samarbetat med hjälp programmet nCipher Security för att förbättra den tekniska situationen för HSM: er. Dessa förbättringar gör att du kan få de vanligaste fördelarna med värdbaserade tjänster utan att behöva lämna kontroll över dina nycklar. Dessa förbättringar gör det möjligt för Microsoft att hantera HSM: er så att du inte behöver. Som en moln tjänst kan Azure Key Vault skalas upp på kort sätt för att uppfylla din organisations användnings toppar. På samma tid skyddas din nyckel i Microsofts HSM: er: Du behåller kontrollen över nyckel livs cykeln eftersom du genererar nyckeln och överför den till Microsofts HSM: er.

## <a name="implementing-bring-your-own-key-byok-for-azure-key-vault"></a>Implementera BYOK (ta med din egen nyckel) för Azure Key Vault
...
Kopiera paketet till en USB-enhet eller annan bärbar lagrings enhet.
...
Se till att hjälp programmet nCipher-verktygen finns i din sökväg ( **%nfast_home%\Bin**). Skriv till exempel följande:
...
Kopiera BYOK verktygs paketet från USB-enheten eller annan flyttbar lagring och gör sedan följande:

1. Extrahera filerna från det hämtade paketet till en mapp.
2. Från den mappen kör du vcredist_x64. exe.
...
* Säkerhetskopiera World-filen. Skydda och skydda World-filen, administratörs korten och deras PIN-filer och se till att ingen enskild person har åtkomst till mer än ett kort.
...
* Nyckel utbytes nyckeln kan inte exporteras.
...
1. Kör verifykeypackage.py-skriptet genom att skriva något av följande, beroende på din geografiska region eller Azure-instans:
   ...
   * För Schweiz:
     ...
     >
     >
2. Bekräfta att du ser följande, vilket indikerar att verifieringen lyckades: **Medför RESULTATET**

Det här skriptet validerar den som loggar in till rot nyckeln för nshield maskinvarusäkerhetsmodul. Hash-värdet för den här rot nyckeln är inbäddat i skriptet och dess värde ska vara **59178a47 de508c3f 291277ee 184f46c4 f1d9c639**. Du kan också bekräfta det här värdet separat genom att besöka [Hjälp programmet nCipher-webbplatsen](https://www.ncipher.com/products/key-management/cloud-microsoft-azure/validation).
...
Säkerhetskopiera nyckel filen för denna token på en säker plats.
...
När du kör det här kommandot ersätter du *contosokey* med samma värde som du angav **i steg 3,5: Skapa en ny nyckel** från steget [generera ditt nyckel](#step-3-generate-your-key) steg.

Du uppmanas att ansluta till ditt Security World admin-kort.

När kommandot har slutförts visas **resultatet: Klar** och kopian av din nyckel med minskad behörighet finns i filen med namnet key_xferacId_\<contosokey >.
...
* kmfile-dump.exe:
  ...
  När du kör de här kommandona ersätter du contosokey med samma värde som du **angav i steg 3,5: Skapa en ny nyckel** från steget [generera ditt nyckel](#step-3-generate-your-key) steg.
...
* Ersätt *contosokey* med den identifierare som du använde för att generera nyckeln i **steg 3,5: Skapa en ny nyckel** från steget [generera ditt nyckel](#step-3-generate-your-key) steg.
