...
Det första du ska notera är att typen för resursen inte `Microsoft.Authorization/roleAssignments` är lika med en resurs grupp.  Resurs typen följer i stället mönstret `{provider-namespace}/{resource-type}/providers/roleAssignments`. I det här fallet är resurs typen `Microsoft.DevTestLab/labs/providers/roleAssignments`.
...
Roll Definitions-ID: t är sträng identifieraren för den befintliga roll definitionen. Roll-ID: t är i `/subscriptions/{subscription-id}/providers/Microsoft.Authorization/roleDefinitions/{role-definition-id}`formatet. 

Prenumerations-ID: t hämtas `subscription().subscriptionId` med hjälp av funktionen mall.  
...
Roll-ID: t definieras i avsnittet Variables och `devTestLabUserRoleId`namnges. I mallen är roll-ID: t inställt på: 111111111-0000-0000-11111111111111111. 
...
För att ange den resurs som behörigheter beviljas till kan anges med en kombination `ResourceName`av, `ResourceType` `ResourceGroup` eller av `scope` parametern. Oavsett vilken kombination av parametrar som används, ger du tillräckligt med information till cmdleten för att unikt identifiera Active Directory-objektet (användare, grupp eller tjänstens huvud namn), omfång (resurs grupp eller resurs) och roll definition.

## <a name="use-azure-command-line-interface-cli"></a>Använda kommando rads gränssnittet i Azure (CLI)
I Azure CLI gör du det `az role assignment create` genom att lägga till en labb användare i ett labb med hjälp av kommandot. Mer information om Azure CLI-cmdlets finns i [Hantera åtkomst till Azure-resurser med RBAC och Azure CLI](../role-based-access-control/role-assignments-cli.md).

Objektet som beviljas åtkomst kan anges av parametrarna `objectId`, `signInName`,. `spn` Labbet som `scope` objektet beviljas åtkomst till kan identifieras av URL: en eller en kombination av parametrarna `resource-name`, `resource-type`och. `resource-group`
