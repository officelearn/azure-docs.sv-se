...
| parameters |Inga |Värden som anges när kommandot artefakt installation körs på en dator. Detta hjälper dig att anpassa din artefakt. |
...
Deklarera hemligheter som säkra strängar. Här är syntaxen för att deklarera en säker sträng-parameter i `parameters` avsnittet i **artifactfile. JSON** -filen:
...
Det fullständiga exemplet artifactfile. JSON och artefakt. ps1 (PowerShell-skript) finns i [det här exemplet på GitHub](https://github.com/Azure/azure-devtestlab/tree/master/Artifacts/windows-test-paramtypes).
...
4. Lagra artefakten i den [offentliga git-lagringsplatsen för DevTest Labs](https://github.com/Azure/azure-devtestlab/tree/master/Artifacts) eller [din egen git-lagringsplats](devtest-lab-add-artifact-repo.md). I det offentliga lagret kan du Visa artefakter som delas av andra som du kan använda direkt eller anpassa dem efter dina behov.
   ...
   2. Lagra artefakt definitions filen (artifactfile. JSON) i den katalog som du har skapat.
