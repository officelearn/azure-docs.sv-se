...
1. Innan du skapar Resource Manager-mall lägger du till den [WinRM artefakt](https://github.com/Azure/azure-devtestlab/tree/master/Artifacts/windows-winrm) som en del av den virtuella datorn. Distributions uppgifter som *Azure File Copy* och *PowerShell på mål datorer* behöver WinRM-åtkomst. WinRM-artefakten kräver ett värdnamn som parameter, vilket bör vara det fullständigt kvalificerade domän namnet (FQDN) för den virtuella datorn. 
   
   > [!NOTE]
   > När du använder WinRM med delade IP-adress, måste du lägga till en NAT-regel för att mappa en extern port till WinRM-port. Du behöver inte NAT-regeln om du skapar den virtuella datorn med en offentlig IP-adress.
