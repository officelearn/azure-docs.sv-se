...
En belastnings Utjämnings regel definierar klient delens IP-konfiguration för den inkommande trafiken och backend-poolen för att ta emot trafiken, tillsammans med den begärda käll-och mål porten. Skapa en regel för belastnings Utjämnings *myinboundlbrule* med [AZ Network lb Rule Create](https://docs.microsoft.com/cli/azure/network/lb/rule?view=azure-cli-latest) för att lyssna på port 80 i frontend-poolens *myfrontendinbound* och skicka belastningsutjämnad nätverks trafik till Server delens *adresspool som också använder* port 80. 
