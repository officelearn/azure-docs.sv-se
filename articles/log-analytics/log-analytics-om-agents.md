...
description: För att underhålla din investering i System Center Operations Manager och använder utökade funktioner med logganalys, kan du integrera Operations Manager med ditt arbetsområde.
...
För att underhålla din investering i System Center Operations Manager och använder utökade funktioner med logganalys, kan du integrera Operations Manager med logganalys-arbetsytan.  På så sätt kan du utnyttja möjligheter i logganalys medan du använder Operations Manager till:

* Övervaka hälsotillståndet för din IT-tjänster med Operations Manager
...
Integrering med System Center Operations Manager tillför värde till din strategi för tjänsten åtgärder med hjälp av hastighet och prestanda för Log Analytics i att samla in, lagra och analysera data från Operations Manager.  Logga Analytics hjälper till att korrelera och arbeta mot identifiera fel på problem och visa repetitioner stöd för din befintliga problem-hanteringen.  Möjlighet att sökmotor för att granska prestanda, händelser och avisering data med omfattande instrumentpaneler och rapporteringsfunktioner att exponera informationen på ett meningsfullt sätt visar styrkan logganalys ger i complimenting Operations Manager.

Agenter som rapporterar till Operations Manager-hanteringsgruppen samla in data från dina servrar baserat på logganalys-datakällor och lösningar som du har aktiverat i arbetsytan.  Beroende på lösningar aktiverat sina data antingen skickas direkt från en hanteringsserver för Operations Manager till tjänsten eller på grund av mängden data som samlas in på datorn hanteras med agent, skickas direkt från agenten till logganalys. Management-servern vidarebefordrar data direkt till tjänsten; den skrivs aldrig till den operativa eller data warehouse-databasen.  När en hanteringsserver förlorar anslutningen till logganalys, cachelagrar data lokalt tills kommunikation har återupprättats med logganalys.  Om management-servern är offline på grund av planerat underhåll eller oplanerade avbrott, fortsätter anslutningen till logganalys med en annan hanteringsserver i hanteringsgruppen.  

Följande diagram visar anslutningen mellan hanteringsservrarna och agenterna i en hanteringsgrupp för System Center Operations Manager och logganalys, inklusive riktning och portar.   

![oms-operations-manager-integration-diagram](./media/log-analytics-om-agents/oms-operations-manager-connection.png)

Om din IT-säkerhetsprinciper inte tillåter datorer i nätverket för att ansluta till Internet, kan hanteringsservrar konfigureras för att ansluta till OMS-Gateway för att ta emot information om konfiguration och skicka insamlade data beroende på de lösningar som är aktiverad.  Mer information och anvisningar om hur du konfigurerar din Operations Manager-hanteringsgrupp att kommunicera via en OMS-Gateway till Log Analytics-tjänsten finns [ansluta datorer till OMS med OMS-gatewayen](log-analytics-oms-gateway.md).  

## <a name="system-requirements"></a>Systemkrav
Granska följande information för att verifiera förutsättningar vara uppfyllda innan du börjar.

* Log Analytics stöder bara UR6 för System Center Operations Manager 1801, Operations Manager 2016 Operations Manager 2012 SP1 och senare, och Operations Manager 2012 R2 UR2 och större.  Stöd för proxy har lagts till i Operations Manager 2012 SP1 UR7 och Operations Manager 2012 R2 UR3.
...
* Logganalys-arbetsytan.  Mer information finns [Kom igång med logganalys](log-analytics-get-started.md).

### <a name="network"></a>Nätverk
Informationen nedan lista över proxy- och brandväggsinställningarna configuration information som krävs för Operations Manager-agenten, hanteringsservrar och driftkonsolen att kommunicera med logganalys.  Trafik från varje komponent är utgående från nätverket till Log Analytics-tjänsten.     
...
## <a name="connecting-operations-manager-to-log-analytics"></a>Ansluta Operations Manager till logganalys
Utför följande steg för att konfigurera hanteringsgruppen för Operations Manager för att ansluta till en av dina arbetsytor logganalys-serien.

Om det här är första gången registrerar din Operations Manager-hanteringsgrupp med en logganalys-arbetsytan och hanteringsservrarna måste kommunicera med tjänsten via en proxy- eller OMS Gateway-servern, ange proxykonfiguration för den hanteringsgruppen är inte tillgänglig i Operations-konsolen.  Hanteringsgruppen har har registrerats med tjänsten innan det här alternativet är tillgängligt.  Du behöver uppdatera proxykonfigurationen system med Netsh på systemet kör driftkonsolen från att konfigurera integration och alla hanteringsservrar i hanteringsgruppen.  
...
När du har slutfört följande steg för att integrera med Log Analytics kan du ta bort konfigurationen genom att köra `netsh winhttp reset proxy` och sedan använda den **konfigurera proxyservern** alternativet i Operations-konsolen för att ange proxy- eller OMS Gateway-servern. 
...
5. När du har autentiserats, på den **guiden Operations Management Suite Onboarding: Välj arbetsyta** sidan uppmanas du för att välja logganalys-arbetsytan.  Om du har mer än en arbetsytan väljer du arbetsytan som du vill registrera med Operations Manager-hanteringsgruppen från den nedrullningsbara listan och klicka sedan på **nästa**.
   
   > [!NOTE]
   > Operations Manager stöder bara en logganalys-arbetsytan i taget. Anslutningen och de datorer som registrerats till logganalys med arbetsytan tidigare tas bort från logganalys.
   > 
   > 
...
4. I den **datorsökning** dialogrutan som du kan söka efter datorer eller grupper som övervakas av Operations Manager. Välj datorer eller grupper som ska publiceras till logganalys, klicka på **Lägg till**, och klicka sedan på **OK**.
...
Utför följande steg om en intern proxyserver mellan hanteringsgruppen och Log Analytics-tjänsten.  De här inställningarna hanteras från hanteringsgruppen och distribueras till agenthanterade datorer som ingår i omfånget för att samla in data för Log Analytics centralt.  Detta är fördelaktigt för när vissa lösningar kringgå hanteringsservern och skicka data direkt till tjänsten.
...
När anslutningen har skapats och du konfigurerar vilka agenter som samlar in och rapportera data till logganalys, används följande konfiguration i hanteringsgruppen, inte nödvändigtvis i ordning:
...
Om du vill fortsätta efter befintliga Ändra kontroll processen för att styra management pack-versioner i hanteringsgruppen för produktion kan du inaktivera reglerna och aktivera dem vid specifika tidpunkter när uppdateringar är tillåtna. Om du har en utvecklings- eller QA hanteringsgruppen i din miljö och den är ansluten till Internet, kan du konfigurera den hanteringsgruppen med logganalys-arbetsytan för att stödja det här scenariot.  På så sätt kan du granska och utvärdera iterativ versioner av hanteringspaket logganalys innan du släpper dem till hanteringsgruppen för produktion.

## <a name="switch-an-operations-manager-group-to-a-new-log-analytics-workspace"></a>Växla en Operations Manager-gruppen till en ny Log Analytics-arbetsyta
1. Logga in på Azure Portal på [https://portal.azure.com](https://portal.azure.com).
2. I Azure Portal klickar du på knappen **Fler tjänster** längst upp till vänster. I listan över resurser skriver du **Log Analytics**. När du börjar skriva filtreras listan baserat på det du skriver. Välj **logganalys** och sedan skapa en arbetsyta.  
3. Öppna Operations Manager-konsolen med ett konto som är medlem i rollen Operations Manager-administratörer och välj den **Administration** arbetsytan.
4. Expandera Operations Management Suite och välj **anslutningar**.
5. Välj den **konfigurera åtgärden Management Suite** länk i mitten-sida av fönstret.
6. Följ den **guiden Operations Management Suite Onboarding** och ange e-postadress eller telefonnummer antal och lösenordet för administratörskontot som är associerad med den nya logganalys-arbetsytan.
...
## <a name="validate-operations-manager-integration-with-log-analytics"></a>Kontrollera Operations Manager Integration with Log Analytics
Det finns några olika sätt som du kan verifiera att Log Analytics för integrering av Operations Manager har lyckats.

### <a name="to-confirm-integration-from-the-azure-portal"></a>Bekräfta integration från Azure-portalen
...
2. Välj arbetsytan tillämpliga i din lista över logganalys arbetsytor.  
3. Välj **avancerade inställningar**väljer **anslutna källor**, och välj sedan **System Center**. 
4. Du bör se namnet på hanteringsgruppen som visas i listan med antalet agenter och status när data togs emot senast i tabellen i avsnittet System Center Operations Manager.
...
4. Från den **övervakning** visa, navigera till den **Operations hanteringstillstånd Suite\Health** vyn.  Välj en hanteringsserver under den **Hanteringsservertillstånd** fönstret och i den **detaljvy** rutan Bekräfta värdet för egenskapen **URI för autentiseringstjänst** matchar Log Analytics arbetsyte-ID.
   
   ![oms-opsmgr-mg-authsvcuri-property-ms](./media/log-analytics-om-agents/oms-opsmgr-mg-authsvcuri-property-ms.png)

## <a name="remove-integration-with-log-analytics"></a>Ta bort Integration med logganalys
Det finns flera steg som krävs för att ta bort anslutning och konfiguration korrekt i hanteringsgruppen när du inte längre behöver integrering mellan din Operations Manager-hanteringsgrupp och logganalys-arbetsytan. Följande procedur har du uppdatera logganalys-arbetsytan genom att ta bort referensen till hanteringsgruppen, ta bort logganalys-kopplingar och ta sedan bort hanteringspaket som har stöd för integrering med tjänsten.   

Hanteringspaketen för lösningarna som du har aktiverat som integreras med Operations Manager och de hanteringspaket som krävs för att stödja integration med Log Analytics-tjänsten kan inte enkelt tas bort från hanteringsgruppen.  Det beror på att vissa logganalys hanteringspaket har beroenden i andra relaterade management packs.  Hämta skriptet för att ta bort hanteringspaket med ett beroende på andra hanteringspaket [ta bort ett hanteringspaket med beroenden](https://gallery.technet.microsoft.com/scriptcenter/Script-to-remove-a-84f6873e) från TechNet Script Center.  
...
I framtiden om du planerar att ansluta din hanteringsgrupp till en logganalys-arbetsyta, måste du importera den `Microsoft.SystemCenter.Advisor.Resources.\<Language>\.mpb` management pack-filen.  Du kan hitta den här filen på följande plats beroende på vilken version av System Center Operations Manager som har distribuerats i din miljö:

* På källmediet under den `\ManagementPacks` mapp för System Center 2016 - Operations Manager och högre.
* Från den senaste uppdateringen tillämpas på din hanteringsgrupp.  För Operations Manager 2012 källmappen är` %ProgramFiles%\Microsoft System Center 2012\Operations Manager\Server\Management Packs for Update Rollups` och för 2012 R2 finns i `System Center 2012 R2\Operations Manager\Server\Management Packs for Update Rollups`.
