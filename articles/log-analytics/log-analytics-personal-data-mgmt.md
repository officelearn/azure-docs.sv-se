...
* *Enhets-ID*: som användar-ID, enhets-ID ibland betraktas som ”privat”. Använd samma metod som anges ovan för användar-ID för att identifiera tabeller där det kan vara ett problem. 
* *Anpassade data*: logganalys möjliggör insamling i flera olika sätt: anpassade loggar och anpassade fält, de [HTTP Data Collector API](log-analytics-data-collector-api.md) , och anpassade data som samlas in som en del av systemets händelseloggar. Alla dessa kan vara utsatta för som innehåller privata data och bör undersökas för att kontrollera om några sådana data finns.
...
* [Publicera Rensa] (https://docs.microsoft.com/rest/api/loganalytics/workspaces%202015-03-20/purge) – tar ett objekt som att ange parametrar för att ta bort och returnerar en referens-GUID 
...
- Läs mer om hur Log Analytics-data som samlas in, bearbetas och skyddas i [Log Analytics-datasäkerhet](log-analytics-data-security.md).
