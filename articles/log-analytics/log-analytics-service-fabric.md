...
Den här artikeln beskriver hur du använder Service Fabric-lösning i Log Analytics för att identifiera och felsöka problem i Service Fabric-klustret. Det kan du se hur utför Service Fabric-noder och hur dina program och tjänster micro körs.

Service Fabric-lösningen använder Azure-diagnostik data från din Service Fabric virtuella datorer genom att samla in data från Azure BOMULLSTUSS tabellerna. Logganalys läser sedan följande Service Fabric framework händelser:

- **Händelser för tillförlitlig tjänst**
- **Aktören händelser**
- **Operativa händelser**
- **Anpassade ETW-händelser**

Service Fabric-lösningen instrumentpanelen visar viktiga problem och relevanta händelser i Service Fabric-miljö.

## <a name="installing-and-configuring-the-solution"></a>Installera och konfigurera lösningen
Följ dessa tre enkla steg för att installera och konfigurera lösningen:

1. Koppla Azure-prenumeration som du använde för att skapa alla resurser i klustret, inklusive lagringskonton med din arbetsyta. Se [Kom igång med logganalys](log-analytics-get-started.md) information om hur du skapar en logganalys-arbetsytan.
2. Konfigurera Log Analytics för att samla in och visa loggar för Service Fabric.
3. Aktivera Service Fabric-lösningen i din arbetsyta.

## <a name="configure-log-analytics-to-collect-and-view-service-fabric-logs"></a>Konfigurera Log Analytics för att samla in och visa loggar för Service Fabric
Lär dig hur du konfigurerar Log Analytics för att hämta Service Fabric-loggar i det här avsnittet. Loggarna kan du visa, analysera och felsöka problem i ditt kluster eller i program och tjänster som körs i klustret, med hjälp av Azure portal.

> [!NOTE]
> Konfigurera Azure-diagnostik-tillägget för att ladda upp loggar för storage-tabeller. Tabellerna måste matcha ser Log Analytics för. Mer information finns i [hur du samlar in loggar med Azure-diagnostik](../service-fabric/service-fabric-diagnostics-how-to-setup-wad.md). Konfiguration av inställningar exemplen i den här artikeln visar bör du vilka namnen på storage-tabeller. När diagnostik ställs in på klustret och laddar upp loggar till ett lagringskonto, är nästa steg att konfigurera Log Analytics för att samla in dessa loggar.
>
>

Se till att du uppdaterar den **EtwEventSourceProviderConfiguration** under den **template.json** fil att lägga till poster för de nya EventSources innan du använder konfigurationen uppdatera genom att köra **deploy.ps1**. Tabellen för överföring är samma som (ETWEventTable). För tillfället logganalys endast läsa programmet ETW-händelser från den *WADETWEventTable* tabell.

Följande verktyg används för att utföra vissa åtgärder i det här avsnittet:

* Azure PowerShell
* [Log Analytics](log-analytics-overview.md)

### <a name="configure-a-log-analytics-workspace-to-show-the-cluster-logs"></a>Konfigurera en logganalys-arbetsyta om du vill visa loggarna kluster

Konfigurera arbetsytan om du vill dra loggar från Azure storage-tabeller när du har skapat en logganalys-arbetsytan. Kör följande PowerShell-skript:
...
När du aktiverar lösningen, Service Fabric-panel har lagts till Log Analytics *översikt* sidan. På sidan visas en vy över anmärkningsvärda problem, till exempel runAsync fel och avbryta som inträffade under de senaste 24 timmarna.
...
Klicka på den **Service Fabric** öppna Service Fabric-instrumentpanelen. Instrumentpanelen innehåller kolumnerna i tabellen nedan. Varje kolumnen visas de översta 10 händelserna efter antal som matchar den kolumnen villkoren för det angivna tidsintervallet. Du kan köra en sökning i logg som innehåller hela listan genom att klicka på **se alla** längst ned rätt i varje kolumn, eller genom att klicka på kolumnrubriken.

| **Service Fabric-händelse** | **Beskrivning** |
| --- | --- |
| Anmärkningsvärda problem | Visar problem, inklusive RunAsyncFailures, RunAsynCancellations och noden används. |
| Operativa händelser | Visar viktiga operativa händelser, inklusive uppgradering av programmet och distributioner. |
| Händelser för tillförlitlig tjänst | Visar viktiga tillförlitlig tjänsthändelser, inklusive Runasyncinvocations. |
| Aktören händelser | Visar viktiga aktören händelser som genererats av dina micro-tjänster. Händelser innehåller undantag som utlöses av en aktörsmetod, aktören aktiveringar och avaktiveringar och så vidare. |
| Programhändelser | Visar alla anpassade ETW händelser som genererats av dina program. |
...
> [!NOTE]
> Ändra omfattningen för händelser med **Data baserat på senaste sju dagarna** överst på instrumentpanelen. Du kan också visa händelser som genererats under de senaste sju dagarna, en dag eller sex timmar. Du kan också välja **anpassade** att ange ett anpassat datumintervall.
>
>

## <a name="troubleshoot-your-service-fabric-and-log-analytics-configuration"></a>Felsökning av konfigurationen av Service Fabric och logganalys
Om du behöver verifiera din konfiguration för logganalys eftersom det inte går att visa händelsedata i Log Analytics använder du följande skript. Följande åtgärder utförs:

1. Läser konfigurationen Service Fabric-diagnostik
2. Söker efter data som skrivs till register
3. Kontrollerar att logganalys är konfigurerad för att läsa från tabeller
