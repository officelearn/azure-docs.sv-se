...
Den här artikeln visar hur du konfigurerar båda typerna av hanterade identiteter för din Logic app. Mer information finns i följande avsnitt:
...
1. På menyn Logic app, under **Inställningar**, väljer du **identitet**. Välj **system som tilldelats** > **på** > **Spara**. När du blir ombedd att bekräfta väljer du **Ja**.
...
* Azure PowerShell
  ...
  * [Lägg till roll tilldelning](../active-directory/managed-identities-azure-resources/howto-assign-access-powershell.md)
* Azure CLI
  * [Skapa användare tilldelad identitet](../active-directory/managed-identities-azure-resources/how-to-manage-ua-identity-cli.md)
  * [Lägg till roll tilldelning](../active-directory/managed-identities-azure-resources/howto-assign-access-cli.md)
* Azure-REST API
  ...
  * [Lägg till roll tilldelning](../role-based-access-control/role-assignments-rest.md)

<a name="azure-portal-user-identity"></a>

#### <a name="create-user-assigned-identity-in-the-azure-portal"></a>Skapa användare tilldelad identitet i Azure Portal

1. I rutan [Azure Portal](https://portal.azure.com), i sökrutan på vilken sida som helst, anger du `managed identities`och väljer **hanterade identiteter**.
...
1. Ange information om din hanterade identitet och välj sedan **skapa**, till exempel:
   ...
   |----------|----------|-------|-------------|
   | **Resursnamn** | Ja | <*User-Assigned-Identity-name*> | Namnet för att ge din användar tilldelnings identitet. I det här exemplet används "Fabrikam-User-Assigned-Identity". |
   | **Prenumeration** | Ja | <*Azure-prenumerationsnamn*> | Namnet på den Azure-prenumeration som ska användas |
...
1. På menyn Logic app, under **Inställningar**, väljer du **identitet**och väljer sedan **tilldelad användare** > **Lägg till**.
...
Om du vill automatisera skapandet och distributionen av Azure-resurser, till exempel Logic Apps, kan du använda [Azure Resource Manager mallar](../logic-apps/logic-apps-azure-resource-manager-templates-overview.md)som stöder [användarspecifika identiteter för autentisering](../active-directory/managed-identities-azure-resources/how-to-manage-ua-identity-arm.md). I mallens `resources`-avsnitt kräver din Logi Kap par resurs definition följande objekt:

* Ett `identity`-objekt med egenskapen `type` inställd på `UserAssigned`

* Ett underordnat `userAssignedIdentities`-objekt som anger identitetens resurs-ID, vilket är ett annat underordnat objekt som har egenskaperna `principalId` och `clientId`

Det här exemplet visar en resurs definition för en logisk app för en HTTP-begäran och innehåller ett icke-parametriserade `identity`-objekt. Svaret på Request-begäran och efterföljande GET-åtgärd har också det här `identity`-objektet:
...
1. På menyn Logic app, under **Inställningar**, väljer du **identitet**och följer sedan stegen för din identitet:
   ...
   * Välj **system som tilldelats** > **på** > **Spara**. När du blir ombedd att bekräfta väljer du **Ja**.
...
Om du har skapat logik appens hanterade identitet med hjälp av en Azure Resource Manager mall ställer du in `identity` objektets `type` underordnade egenskap `None`. För den systemhanterade identiteten tar den här åtgärden även bort ägar-ID från Azure AD.
