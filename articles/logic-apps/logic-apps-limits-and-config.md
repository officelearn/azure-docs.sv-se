...
| Namn | Multi-tenant limit | Integration service environment limit | Anteckningar |
|------|--------------------|---------------------------------------|-------|
| Run duration | 90 dagar | 365 days | To change the default limit, see [change run duration](#change-duration). |
...
| Minimum recurrence interval | 1 sekund | 1 sekund ||
...
| Namn | Multi-tenant limit | Integration service environment limit | Anteckningar |
|------|--------------------|---------------------------------------|-------|
| Meddelandestorlek | 100 MB | 200 MB | To work around this limit, see [Handle large messages with chunking](../logic-apps/logic-apps-handle-large-messages.md). However, some connectors and APIs might not support chunking or even the default limit. |
...
| Namn | Gräns | Anteckningar |
| ---- | ----- | ----- |
| Antal återförsök | 90 | The default is 4. To change the default, use the [retry policy parameter](../logic-apps/logic-apps-workflow-actions-triggers.md). |
...
| Retry min delay | 5 sekunder | To change the default, use the [retry policy parameter](../logic-apps/logic-apps-workflow-actions-triggers.md). |
||||
...
| Namn | Multi-tenant limit | Integration service environment limit | Anteckningar |
|------|--------------------|---------------------------------------|-------|
| Number of custom connectors | 1 000 per Azure-prenumeration | 1 000 per Azure-prenumeration ||
...
| -------- | ----- | ----- |
| Sammansättning | 8 MB | To upload files larger than 2 MB, use an [Azure storage account and blob container](../logic-apps/logic-apps-enterprise-integration-schemas.md). |
...
| Runtime endpoint | Gräns | Anteckningar |
|------------------|-------|-------|
