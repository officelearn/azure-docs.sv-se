---
title: Automatisera uppgifter för företagsintegrering
description: Lär dig mer om hur du automatiserar arbetsflöden som integrerar appar, data, tjänster och system med minimal kod för företagsintegrering med hjälp av Azure Logic Apps.
services: logic-apps
...
Om du vill skapa företagsintegrationslösningar med Azure Logic Apps kan du välja från ett växande galleri med [hundratals färdiga kopplingar](../connectors/apis-list.md), som omfattar tjänster som Azure Service Bus, Azure Functions, Azure Storage, SQL Server, Office 365, Dynamics, Salesforce, BizTalk, SAP, Oracle DB, filresurser med mera. [Anslutningsappar](#logic-app-concepts) tillhandahåller [utlösare](#logic-app-concepts), [åtgärder](#logic-app-concepts) eller både och för att skapa logikappar som får säker åtkomst till och bearbetar data i realtid.

> [!VIDEO https://channel9.msdn.com/Blogs/Azure/Introducing-Azure-Logic-Apps/player]

## <a name="how-does-logic-apps-work"></a>Hur fungerar Logic Apps? 

Varje logikapps arbetsflöde börjar med en utlösare, som utlöses när en viss händelse sker eller när nya tillgängliga data uppfyller specifika villkor. Många utlösare som tillhandahålls av kopplingarna i Logic Apps innehåller grundläggande schemaläggningsfunktioner så att du kan ställa in hur regelbundet dina arbetsbelastningar körs. För mer komplexa schemaläggning eller avancerade upprepningar kan du använda en återkommande utlösare som det första steget i ett arbetsflöde. Läs mer om [schemabaserade arbetsflöden](../logic-apps/concepts-schedule-automated-recurring-tasks-workflows.md).

Varje gång utlösaren körs skapar Logic Apps-motorn en logikappinstans som kör åtgärderna i arbetsflödet. Dessa åtgärder kan också omfatta datakonverteringar och arbetsflödeskontroller, till exempel villkorssatser, växelsatser, loopar och förgrening. Den här logikappen till exempel börjar med en Dynamics 365-utlösare med det inbyggda villkoret ”När en post uppdateras”. Om utlösaren upptäcker en händelse som matchar villkoret utlöses utlösaren och kör arbetsflödets åtgärder. Här omfattar åtgärderna XML-transformering, datauppdateringar, beslutsbranchning och e-postmeddelanden.
...
* Bearbeta [platta filer](../logic-apps/logic-apps-enterprise-integration-flatfile.md)
...
Skapa dina logikappar som Azure Resource Manager-mallar så att du kan [automatisera logikappdistribution](../logic-apps/logic-apps-azure-resource-manager-templates-overview.md) över flera miljöer och regioner.

### <a name="access-resources-inside-azure-virtual-networks"></a>Komma åt resurser i virtuella Azure-nätverk

Logikappar kan komma åt säkra resurser, till exempel virtuella datorer (VMs) och andra system eller tjänster som finns i ett [virtuellt Azure-nätverk](../virtual-network/virtual-networks-overview.md) när du skapar en [ *integrationstjänstmiljö* (ISE).](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md) En ISE är en isolerad instans av Logic Apps-tjänsten som använder dedikerade resurser och körs separat från den "globala" Logic Apps-tjänsten för flera innehavare.
...
* Ökade gränser för körningstid, lagringskvarhållning, dataflöde, HTTP-begärande- och svarstidsgränser, meddelandestorlekar och anpassade anslutningsbegäranden. Mer information finns i [Gränser och konfiguration för Azure Logic Apps](../logic-apps/logic-apps-limits-and-config.md).
...
## <a name="how-does-logic-apps-differ-from-functions-webjobs-and-power-automate"></a>Hur skiljer sig Logic Apps från Funktioner, WebJobs och Power Automate?

Alla dessa tjänster hjälper dig att ”sammanlänka” och ansluta olika system till varandra. Varje tjänst har sina fördelar, så om du snabbt vill skapa ett skalbart, komplett integreringssystem är det bästa sättet att kombinera deras funktioner. Mer information finns i [Välj mellan logikappar, funktioner, webbjobbar och Power Automate](../azure-functions/functions-compare-logic-apps-ms-flow-webjobs.md).
