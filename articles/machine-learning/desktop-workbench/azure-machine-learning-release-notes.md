...
+ Snabbt bygga och driftsätta mycket exakta machine learning och modeller för djupinlärning med [anpassade Azure Machine Learning-paket](../service/reference-python-package-overview.md) för följande domäner:
...
- Azure Machine Learning Workbench stöder nu köra dina skript internt på din egen python eller pyspark-miljö. För den här funktionen användaren skapar och hanterar sina egna miljö på Virtuellt fjärrdatorn, och använda Azure Machine Learning Workbench för att köra sina skript på målet. Se [konfigurerar Azure Machine Learning Experimentation Service](experimentation-service-configuration.md) 

### <a name="model-management"></a>Modellhantering
- Stöd för att anpassa de behållare som distribueras: gör det möjligt att anpassa behållaravbildningen genom att tillåta installation av externa bibliotek med apt-get etc. Det är inte längre begränsad till pip-installation-bibliotek. Se den [dokumentation](model-management-custom-container.md) för mer information.
...
- [Stöd för SQL Server och Azure SQL DB som en datakälla](data-prep-appendix2-supported-data-sources.md#types) 
...
Läs mer om hur du skapar beräkningsmål [konfigurerar Azure Machine Learning Experimentation Service](experimentation-service-configuration.md)
...
- [_Avancerade Dataförberedelser med hjälp av Bikeshare-Data_ ](./tutorial-bikeshare-dataprep.md) exempelprojektet ”hantera fel Value” steg har åtgärdats.
...
- [_Distribuerade finjustering av Hyperparametrar_ ](./scenario-distributed-tuning-of-hyperparameters.md) exempelprojektet`docker.runconfig` format uppdateras från JSON till YAML.
- Ny exempelprojektet [ _klassificering av avbildning med hjälp av CNTK_](./scenario-image-classification-using-cntk.md).
