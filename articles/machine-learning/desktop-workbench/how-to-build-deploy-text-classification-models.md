---
title: Skapa och distribuera en modell för klassificering av text med hjälp av Azure Machine Learning-paket för textanalys.
description: Lär dig mer om att skapa, träna, testa och distribuera en modell för klassificering av text med hjälp av Azure Machine Learning-paketet för textanalys.
...
I den här artikeln får du lära dig hur du träna och distribuera en modell för klassificering av text med **Azure Machine Learning-paket för textanalys** (AMLPTA). Målet med text klassificering är att tilldela en del av texten till en eller flera fördefinierade klasser eller kategorier. Den här texten kan till exempel vara ett dokument, nyhetsartikel, sökfråga, e-post, tweets, supportärenden.

Det finns omfattande program av text klassificering som: 
+ Kategorisera tidningen artiklar och nyheter under överföring innehåll till ämnen
+ Ordna webbsidor i hierarkiska kategorier 
+ Filtrering av skräppost e-post
+ Sentimentanalys
+ Förutse användaravsikt från sökfrågorna
+ Routning supportärenden
+ Analysera kundfeedback 

Text klassificering modell att bygga och distribuera arbetsflödet för en modell med AMLPTA är följande:
...
Läs den [paketera referensdokumentation](https://aka.ms/aml-packages/text) för detaljerade referenser för varje modul och klass.
...
1. Följande konton och programmet måste ställas in och installerad:
   - Azure Machine Learning-experimenteringskonto 
...
1. Azure Machine Learning-paket för textanalys måste installeras. Lär dig hur du [installera det här paketet här](https://aka.ms/aml-packages/text).
...
Prova själv. Hämta anteckningsboken och kör den själv.

> [!div class="nextstepaction"]
> [Hämta Jupyter-anteckningsbok](https://aka.ms/aml-packages/text/notebooks/text_classification_sentiment_data)

### <a name="download-and-explore-the-sample-data"></a>Ladda ned och utforska exempeldata
I följande exempel används den [20 diskussionsgrupper datauppsättning](http://qwone.com/~jason/20Newsgroups/) som är tillgänglig via scikit-Läs biblioteket för att demonstrera hur du skapar en text-klassificerare med Azure Machine Learning-paket för textanalys. 

20 diskussionsgrupper datauppsättningen har cirka 18 000 diskussionsgrupper inlägg på 20 olika ämnen innefattar två: en för utbildning och det andra för utvärdering av resultaten. Dela i träna och testa baseras på varje meddelande efter datum om du före eller efter ett visst datum.
...
### <a name="set-the-location-of-the-data"></a>Ange platsen för dina data
Ange platsen där du har hämtat data i data dir-parametern. Du kan också använda dina egna data, den inkommande datauppsättningen måste vara ett *.tsv filformat.
...
Du kan nu skapa en preliminär utforskning ritytans histogram för klassen frekvensen i utbildning och testa datauppsättningar. 
...
När du kör Jupypter anteckningsboken visas diagrammen när det föregående kodblocket körs.


## <a name="train-the-model"></a>Träna modellen

### <a name="specify-scikit-learn-algorithm-and-define-the-text-classifier"></a>Ange scikit-Läs algoritm och definiera text-klassificerare

Det här steget ska utbildning en scikit-Läs text klassificeringsmodellen med hjälp av en jämfört med Rest learning Logistic Regression-algoritmen.

En fullständig lista över erfarenheter, finns det [Scikit-inlärning](http://scikit-learn.org/stable/supervised_learning) dokumentation.
...
Använd standardparametrarna för paketet. Som standard extraherar text-klassificerare
...
Vid träning, måste du ha både text och etiketten kolumner. Även om endast textkolumnen krävs för förutsägelser. 

### <a name="examine-and-set-the-parameters-of-the-different-pipeline-steps"></a>Kontrollera och ställa in parametrarna för de olika pipeline-steg
    
Normalt kan ange du parametrarna innan du anpassar en modell. 

***Exemplet som visas med text_word_ngrams*** 
...
Därefter kontrollera parametervärdena för ”text_char_ngrams”:
...
Om det behövs kan du ändra standardparametrar.  Du kan ändra intervallet för extraherat tecken n-gram från (4,4) till (3,4) att extrahera både tecken tre-g och 4 g med följande kod:
...
### <a name="export-the-parameters-to-a-file"></a>Exportera parametrarna till en fil
Om det behövs kan du optimera modellprestanda genom att köra modellen passning steg med ändrade parametrar:
...
Tillämpa tränade text-klassificerare mot testdatauppsättningen att generera klassen förutsägelser:
...
Felmatris utan normalisering
    
När du kör anteckningsboken visas felmatrisen



Rita normaliserade felmatrisen för visualisering.
...
## <a name="save-the-pipeline"></a>Spara pipelinen
Spara klassificering pipeline i en zip-fil. Dessutom spara n-gram word ngrams och tecken som textfiler.
...
## <a name="load-the-pipeline"></a>Läsa in pipelinen
Läs in de klassificering pipeline och word-ngrams och tecknet n-gram för inferensjobb:
...
Om du vill utvärdera en testdatamängd gäller inlästa text klassificering pipelinen:
...
I det här avsnittet ska du distribuera text klassificering pipelinen som en Azure Machine Learning web service med hjälp av [Azure Machine Learning CLI](https://docs.microsoft.com/azure/machine-learning/desktop-workbench/cli-for-azure-machine-learning). Sedan kan använda du webbtjänsten för träning och bedömning.
...
**Skapa och ange en distributionsmiljö**
...
1. Se till att din Azure Machine Learning-miljö, konto för modellhantering och resursgruppen finns i samma region.
...
1. Uppdatera konfigurationsfilen för distribution som du laddade ned för att återspegla dina resurser:
...
1. Med hänsyn till att den tränade modellen har distribuerats, anropa bedömnings webbtjänst på ny datauppsättning:
...
1. Läs in webbtjänsten när som helst med hjälp av dess namn:
...
1. Testa webbtjänsten med brödtexten i två e-postmeddelanden som tagits från 20 newsgrpoups datauppsättningen:
...
Läs mer om Azure Machine Learning-paket för textanalys i de här artiklarna:

+ Läs den [paketera översikt](https://aka.ms/aml-packages/text).

+ Utforska den [referensdokumentation](https://aka.ms/aml-packages/text) för det här paketet.
