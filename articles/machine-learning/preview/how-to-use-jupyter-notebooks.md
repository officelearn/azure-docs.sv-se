...
Mer information hittar du officiellt [Jupyter dokumentationen](http://jupyter.readthedocs.io/en/latest/architecture/how_jupyter_ipython_work.html). Fowllowing är ett diagram som illustrerar hur den här klienten, servern och kernel-arkitektur mappas till komponenterna i Azure ML.
...
Du kan komma åt många olika kärnor i Azure ML-arbetsstationen genom att helt enkelt konfigurera kör konfigurationer och beräkna mål i den `aml_config` mappen i projektet. Lägg till en ny beräkning mål genom att utfärda `az ml computetarget attach` kommandot är detsamma som att lägga till en ny kernel.
