...
Automatiserad machine learning hämtar en algoritm för dig och genererar en modell som är klar för distribution. Modellen kan laddas ned för att anpassas ytterligare samt. Det finns flera alternativ som du kan använda för att konfigurera automatiserade maskininlärningsexperiment. I den här guiden kommer du lära dig hur du definierar olika konfigurationsinställningar.

Exempel på en automatiserad maskininlärning finns [självstudie: automatiskt tränar en modell för klassificering](tutorial-auto-train-models.md) eller [träna modeller i molnet automatiskt](how-to-auto-train-remote.md).
...
* `AutoML` inställningar för experiment
* Kör `AutoML` experimentera
...
Innan du börjar experimentet måste bestämma du vilken typ av machine learning-problem som du vill lösa. Automatiserad maskininlärning stöder två typer av övervakad inlärning: klassificerings- och Regressionsmodeller. Automatiserad maskininlärning stöder följande algoritmer under automation och justera processen. Som en användare finns det inget behov av att ange algoritmen.
...
## <a name="data-source-and-format-for-automl-experiment"></a>Datakällan och format för `AutoML` experimentera
`AutoML` har stöd för data som finns på din lokala dator eller i molnet i Azure Blob Storage. Data kan läsas in i scikit-Läs stöds dataformat. Du kan läsa data till 1) Numpy matriser X (funktioner) och y (målvariabel eller så kallade etikett) eller 2) Pandas-dataframe. 
...
Om du använder en fjärransluten beräkning för att köra experimentet AutoML Datahämtningen måste inneslutas i en separat python-skriptet `GetData()`. Det här skriptet körs i den fjärranslutna beräkningen där AutoML experimentet har körts. `GetData` eliminerar behovet av att hämta data i rörelse för varje iteration. Utan `GetData`, experimentet misslyckas när du kör på fjärranslutna beräkning.

Här är ett exempel på `GetData`:
...
I din `AutoMLConfig` objekt kan du ange den `data_script` parametern och ange sökvägen till den `GetData` skriptfilen ungefär som nedan:

```python
automl_config = AutoMLConfig(****, data_script=project_folder + "./get_data.py", **** )
```

`GetData` skriptet kan returnera följande:
...
Därefter fastställer där modellen ska tränas. En automatiserad machine learning-träningsexperiment körs på ett beräkningsmål som du äger och hanterar. 
...
3.  Azure Batch AI-kluster – A hanterade kluster som du kan ställa in för att skala AutoML iterationer av och kan köras parallellt. 


## <a name="configure-your-experiment-settings"></a>Konfigurera inställningarna för experiment

Det finns flera rattar som du kan använda för att konfigurera experimentet AutoML. Dessa parametrar anges av instansiera en `AutoMLConfig` objekt.
...
## <a name="run-automl-experiment"></a>Kör `AutoML` experimentera
...
Du kan visa dina resultat i en widget eller en infogad om du är på en bärbar dator. Se information om att ”spåra och utvärdera modeller”. (kontrollera AML-innehållet innehåller relevant information till AutoML)
