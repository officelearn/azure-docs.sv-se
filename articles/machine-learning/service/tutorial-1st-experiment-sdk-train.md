...
I den här delen av självstudien kör du koden i exemplet Jupyter Notebook `tutorials/tutorial-1st-experiment-sdk-train.ipynb` öppnades i slutet av del ett. Den här artikeln vägleder dig genom samma kod som finns i antecknings boken.
...
Importera klassen `Workspace` och Läs in din prenumerations information från filen `config.json` med hjälp av funktionen `from_config().` det här söker efter JSON-filen i den aktuella katalogen som standard, men du kan också ange en Sök vägs parameter som pekar på filen med `from_config(path="your/file/path")`. I en molnbaserad Notebook-Server är filen automatiskt i rot katalogen.
...
1. För varje alpha-värde i matrisen `alphas` skapas en ny körning i experimentet. Alfa-värdet loggas för att skilja mellan varje körning.
...
När utbildningen har slutförts anropar du variabeln `experiment` för att hämta en länk till experimentet i portalen.
