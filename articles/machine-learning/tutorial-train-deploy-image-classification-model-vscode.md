...
1. Namnge din beräkning "TeamWkspc-com" och tryck på **Retur** för att skapa din beräkning.
    ...
    En fil visas i VS-kod med innehåll som liknar den nedan:
...
1. Välj **Azure-ikonen** i aktivitetsfältet Visual Studio-kod. Vyn Azure Machine Learning visas. 
1. Expandera prenumerationsnoden. 
1. Expandera **teamworkspace-noden > beräkning.** 
1. Högerklicka på beräkningsnoden **TeamWkspc-com** under beräkningsnoden och välj **Skapa körningskonfiguration**.
...
1. Namnge din miljö "MNIST-env" och tryck på **Retur**.
1. Välj **Conda-beroenden** i listan.
1. Tryck **på Retur** för att bläddra i conda-beroendefilen. I det här fallet är `env.yml` beroendefilen `vscode-tools-for-ai/mnist-vscode-docs-sample` filen i katalogen.
...
Körningskonfigurationen `MNIST-rc` läggs till under beräkningsnoden *TeamWkspc-com* och miljökonfigurationen `MNIST-env` läggs till under noden *Miljöer.*
