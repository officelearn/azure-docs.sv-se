---
title: Själv testa klienten att kontrollera ett Azure SaaS-program | Microsoft Docs
description: Så här skapar du en själv testa klient för att verifiera inför ett Azure SaaS-program.
...
---
# <a name="create-a-self-test-client-to-pre-validate-an-azure-saas-application"></a>Skapa en själv testa klient för att kontrollera ett Azure SaaS-program
...
|          DNS-namn          |         DNS-namnet på den virtuella datorn som du ska testa.          |
|          Användare          |          Användarnamn för att ansluta till och logga in på den virtuella datorn.         |
|           Lösenord         |         Lösenord för att ansluta till och logga in på den virtuella datorn.          |
|          Operativsystem          |         Operativsystemet på den virtuella datorn, ”Linux” eller ”Windows”.          |
|         PortNo           |         Öppna portnumret för att ansluta till den virtuella datorn. Portnumret är vanligtvis ”22” för Linux och ”5986” för Windows.          |
...
Följande skärmdump visar ett exempel på JSON-testresultaten som visas i en online JSON-visningsprogram.
...
3. I det vänstra navigeringsfältet, väljer **alla tjänster** och välj sedan **Azure Active Directory**.
   ...
   I följande steg ska behöva klientnamn (eller katalognamnet) eller klient-ID (eller katalog-ID).
...
3. Under **skapa**, ange den information som krävs för följande fält:
   ...
   - **Inloggnings-URL** – typen ”https://isvapp.azurewebsites.net/selftest”
...
20. Skapa en hemlig nyckel genom att välja nyckeln **beskrivning** textrutan. Konfigurera följande fält:
    ...
    - Under **värdet**, kopiera nyckeln.
     ...
     >[!Important]
     >Du kommer inte att kunna se nyckelvärdet när du har avslutat formuläret nycklar.
...
När du har har testat ditt Azure SaaS-program, kan du [publicera erbjudandet](./cpp-publish-offer.md)