...
- `List` och behörigheten `Read` räcker. Ange *inte* `Write` eller @no__t 2-åtkomst.
...
3. Redigera filen för att ange följande parameter värden.  Datum ska anges i UTC-format för UTC, till exempel `2016-10-25T00:00:00Z`.
...
9. Redigera text filen med SAS-anslutningssträngen från steg 6.  Du kommer att skapa hela SAS-URI: n med följande format:
    ...
    Om namnet på VDH till exempel är `TestRGVM2.vhd`, skulle den resulterande SAS-URI: n vara:
...
6. Dialog rutan **signatur för delad åtkomst** visas. Ange värden för följande fält:
   ...
   - **Behörigheter** – Välj behörigheterna `Read` och @no__t 2. 
...
9. Redigera text filen. Infoga ditt VHD-namn efter `vhds`-sträng i SAS-URL: en (inklusive ett inledande snedstreck).  Den sista SAS-URI: n ska ha formatet:
...
- @No__t-0 visas i mitten av URI: n. Den här strängen anger att `Read` och `List`-åtkomst har angetts.
- Efter den punkten visas även `sr=c`. Den här strängen anger att åtkomst på behållare nivå har angetts.
