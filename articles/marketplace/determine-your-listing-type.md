---
title: Fastställa listtyp i Azure Marketplace
...
>[!NOTE]
>Du kan begränsa identifiering och distribution av den virtuella datorn till en specifik uppsättning kunder genom att visa bilden och priser som ett privat erbjudande. Privata erbjudanden Lås möjligheten för utgivare skapa exklusiva erbjudanden för kunderna närmaste och erbjuda anpassade programvara och villkor för dem. Dessa anpassade villkor aktivera utgivare att lysa upp en mängd olika scenarier, inklusive ledde fältet behandlar särskilda priser och villkor samt snabb åtkomst till begränsad version programvara. Detta gör utgivare för att ge specifik prissättning eller produkter till en begränsad uppsättning kunder genom att skapa en ny SKU med dessa uppgifter. Lär dig mer om [privata erbjudanden](https://azure.microsoft.com/en-us/blog/private-offers-on-azure-marketplace/).

Använd en *Azure-programmet lösningsmall* när en lösning kräver ytterligare distribution och konfiguration av automation utöver den virtuella installationen. Lösningsmallar kan automatisera etablering av en eller flera virtuella datorresurser och etablera nätverk och lagringsresurser. Lösningsmallar ger automation fördelar på samma virtuella datorer och hela lösningen för IaaS-baserade miljöer. Lär dig mer om hur du skapar lösningsmallar för [GitHub](https://github.com/MicrosoftDocs/azure-docs).
...
SaaS-baserade, tekniska lösningar använder den *SaaS app* tillhandahåller typ om du vill aktivera Azure kunderna att köpa prenumerationen för din lösning via Azure Marketplace. Om du vill använda den här funktionen utgivare måste pris- och debiterar service med plan, månatliga hastighet och ger möjlighet att uppgradera eller avbryta tjänsten när som helst. Precis som virtuella datorer, Microsoft hyser commerce-transaktion och debiterar kunden för utgivaren. Om du vill använda faktura en SaaS-App som en prenumeration, aktivera utgivare sin egen prenumeration management service API att kommunicera direkt med Azure Marketplace etablering API: er till stöd för etablering av tjänst, uppgraderingar och annullering från Azure-portalen.

Använd *behållare* att publicera din lösning som en bild med Docker behållaren ska etableras till en Kubernetes-baserad Azure-behållaren-tjänst som Azure Kubernetes Service eller Azure Behållarinstanser användarens val av Kubernetes-baserade behållaren runtime. Microsoft stöder för närvarande lediga och BYOL-aktiverade licensmodeller med flera commerce funktioner kommer över nästa flera månader, inklusive förbrukningsbaserad fakturering alternativ och prenumeration.
