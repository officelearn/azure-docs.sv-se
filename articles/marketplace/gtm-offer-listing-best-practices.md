---
title: Go-To-Market Services - Erbjudande notering bästa praxis | Azure
description: Go-To-Market Services – i det här avsnittet beskrivs de bästa metoderna för ett erbjudande
author: qianw211
ms.service: marketplace
ms.subservice: partnercenter-marketplace-publisher
ms.topic: conceptual
ms.date: 12/21/2018
ms.author: dsindona
ms.openlocfilehash: 3cb625e613d8dbe9398e947925a8dc574a277614
ms.sourcegitcommit: 2ec4b3d0bad7dc0071400c2a2264399e4fe34897
ms.translationtype: MT
ms.contentlocale: sv-SE
ms.lasthandoff: 03/28/2020
ms.locfileid: "80286259"
---
# <a name="offer-listing-best-practices"></a>Metodtips för erbjudandelistor

Tabellerna i det här avsnittet innehåller förslag på hur du skapar och engagerar marketplace-erbjudanden. Om du vill skriva och konfigurera ditt marknadsföringsmaterial går du till [Cloud Partner-portalen](https://cloudpartner.azure.com/#insights). 

## <a name="storefront-offer-details"></a>Information om skyltfönster erbjuder

| Inställningar för skyltfönster | Regelverk |
|:--- |:--- |  
| Erbjudandets namn | Ge en stark titel:<br> <ul> <li> Ditt appnamn bör kommunicera kärnvärdet för dina kunder. </li> <li> Inkludera sökord i appens titel för att få högre rankning i sökmotorresultat. </li> </ul> Följ ett inställt titelformat för konsulttjänster:<br>[Erbjudandenamn: [Varaktighet] [Erbjudandetyp] (till exempel *Contoso: 2-veckors implementering*) <br><br> Följ varumärkesriktlinjerna:<br>Följ [Microsofts riktlinjer för varumärken och varumärken](https://www.microsoft.com/en-us/legal/intellectualproperty/trademarks/usage/general.aspx) och andra relevanta, produktspecifika riktlinjer när du hänvisar till Microsofts varumärken och namnen på Microsofts programvara, produkter och tjänster. |
| Beskrivning av erbjudandet | Ge en tydlig beskrivning av erbjudandet:<br> <ul> <li> Beskriv tydligt erbjudandets värdeerbjudande i de första meningarna i din beskrivning. </li> <li> Tänk på att de första meningarna kan visas i sökmotorresultat. </li> <li> Lita inte på funktioner och funktioner för att sälja din produkt. Fokusera istället på det värde du levererar. </li> <li> Använd branschspecifik vokabulär eller förmånsbaserad formulering så mycket som möjligt.</li> <li> Dessutom måste Consulting Service-listor tydligt ange den professionella service du tillhandahåller. </li></ul> Centrala komponenter i ditt värdeerbjudande bör innehålla följande information:<ul><li>Beskrivning av produkten.</li><li>Typ av användare som drar nytta av produkten.</li><li>Kunden behöver eller smärta som produkten adresserar.</li></ul> |

Tabellerna i det här avsnittet innehåller förslag på hur du skapar och engagerar marketplace-erbjudanden. Om du vill skriva och konfigurera ditt marknadsföringsmaterial går du till [Cloud Partner-portalen](https://cloudpartner.azure.com/#insights). 

## <a name="storefront-listing-details"></a>Information om skyltfönster

| Inställningar för skyltfönster | Regelverk |
|:--- |:--- |  
| Branscher: AppSource-appar och konsulttjänster endast | Välj branscher om ditt erbjudande tillgodoser branschspecifika behov: <ul><li> Ropa ut branschspecifika funktioner i erbjudandebeskrivningen.</li> </ul> **Obs:** Det maximala antalet alternativ varierar beroende på erbjudandetyp och anges i [Cloud Partner-portalen](https://cloudpartner.azure.com/#insights). |
| Kategorier | Välj kategorier som bäst stämmer överens med ditt erbjudande:<br> <ul> <li> Välj kategorierna noggrant eftersom kunderna använder dem för att söka efter relevanta appar. </li> <li> Var noga med att ropa ut hur ditt erbjudande stöder kategorispecifika behov i erbjudandebeskrivningen. </li> </ul> **Obs:** Det maximala antalet alternativ varierar beroende på erbjudandetyp och anges i [Cloud Partner-portalen](https://cloudpartner.azure.com/#insights). |
| Produkter som din app fungerar med (max 3): Endast AppSource-appar | Lista ytterligare produkter och tekniker som din lösning använder eller utökar. Azure IoT Hub eller Azure Machine Learning. |
| Sökord (3 max) | Sökord kan hjälpa företagsanvändare att hitta ditt erbjudande när de söker <ul> <li> på Azure Marketplace </li> <li> på AppSource </li> <li> använda sökmotorer </li> </ul> För att dina uppgifter ska visas i dessa sökningar måste du: <ul> <li> Identifiera de tre bästa sökorden för ditt erbjudande. </li> <li> införliva sökorden i ditt erbjudande sammanfattning och beskrivning; </li> <li> lista sökorden här. </li> </ul> |

### <a name="category"></a>Kategori

Azure Marketplace och Microsoft AppSource är skyltfönster som betjänar olika och unika kundkrav. Välj rätt skyltfönster och kategorier för varje erbjudande att rikta dina kunder efter roll. 

Du kan välja upp till två kategorier och två underkategorier för ett erbjudande. De kategorier och underkategorier som anges nedan finns i båda skyltfönster.  Kontrollera att ditt kategorival är i linje med den avsedda målgruppen, erbjudandebeskrivningen och värdeerbjudandet för ditt erbjudande.

| **Bransch**    | **Subindustri**  | 
| :------------------- | :-------------------|
| *Automotive (Automotive)     | *Automotive (Automotive) |
| Jordbruk     | *Övrigt - Ej segmenterad |
| Distribution    | * Partihandel <br> *Paket & paketleverans |
| Utbildning     | *Högre utbildning <br> *Grundsko &leutbildning / K-12 <br> *Bibliotek & museer |
| Ekonomiska tjänster     | *Bank & kapitalmarknaderna <br> *Försäkring |
| Government    | * Defense & Intelligence (brukade kallas nationell och allmän säkerhet) <br> * Allmän säkerhet & rättvisa <br> *Civil regering |
| Hälso- och sjukvård (brukade kallas hälsa)   | * Hälsa Betalare <br> *Vårdgivare <br> * Läkemedel |
| Tillverkning & resurser (brukade kallas tillverkning)   | *Kemisk & Agrokemi <br> *Diskret tillverkning <br> *Energi |
| Detaljhandel & konsumentvaror (brukade kallas Detaljhandel)  | *Konsumentvaror <br> *Återförsäljare |
| * Media & Communications (brukade kallas Media och Underhållning)  | *Media & Underhållning <br> *Telekommunikation |
| Professionella tjänster  | *Lagligt <br> *Partner professionella tjänster |
| * Arkitektur & Konstruktion (brukade kallas Arkitektur Engineering)  | *Övrigt - Ej segmenterad |
| * Gästfrihet & Resor  | *Hotell & Fritid <br> * Resor & Transport <br> * Restauranger & Food Services |
| *Övriga branscher inom den offentliga sektorn  | *Skogsbruk & fiske <br> *Ideella organisationer |
| * Fastigheter  | *Övrigt - Ej segmenterad |

### <a name="industries"></a>Branscher

Branschval gäller endast för erbjudanden som publiceras på AppSource och Consulting Services som publiceras på Azure Marketplace. Välj branscher och/eller vertikaler om ditt erbjudande tillgodoser branschspecifika behov och ropar branschspecifika funktioner i erbjudandebeskrivningen. Du kan välja upp till två (2) branscher och två (2) vertikaler per vald bransch.

### <a name="applicable-products"></a>Tillämpliga produkter

Välj de produkter som appen arbetar med för att erbjudandet ska visas under utvalda produkter i AppSource.

### <a name="search-keywords"></a>Sök nyckelord

Sökord kan hjälpa kunderna att hitta ditt erbjudande när de söker. Identifiera de bästa sökorden för ditt erbjudande, införliva dem i ditt erbjudande sammanfattning och beskrivning samt i sökordet i avsnittet erbjudande lista detaljer.

## <a name="storefront-marketing-details"></a>Information om marknadsföring i skyltfönster
| Inställningar för skyltfönster | Regelverk |
|:--- |:--- |  
| Erbjudandelogotyp (PNG-format, 48&nbsp;&times;&nbsp;48): söksida | Designa och optimera din logotyp för ett digitalt medium:<br>Ladda upp logotypen i PNG-format till söksidan i ditt erbjudande. |
| Erbjudandelogotyp (PNG-format,&nbsp;&times;&nbsp;216 216): appinformationssida | Designa och optimera din logotyp för ett digitalt medium:<br>Ladda upp logotypen i PNG-format till appinformationssidan för ditt erbjudande. |
| "Läs mer" dokument | Inkludera stödjande försäljnings- och marknadsföringstillgångar under "Läs mer" några exempel är: <ul> <li> Vitböcker </li> <li> Broschyrer </li> <li> checklistor, eller </li> <li> PowerPoint-presentationer.</li> </ul>Spara alla filer i PDF-format. Ditt mål här bör vara att utbilda kunder, inte sälja till dem. <br><br>Lägg till en länk till appens målsida i alla dina dokument och lägg till URL-parametrar som hjälper dig att spåra besökare och utvärderingsversioner. |
| Videor: AppSource, konsulttjänster och SaaS erbjuder endast | De starkaste videorna kommunicerar värdet av ditt erbjudande i berättande form:<ul> <li> Gör din kund, inte ditt företag, hjälten i historien. </li> <li> Din video bör ta itu med de viktigaste utmaningarna och målen för din målgrupp. </li> <li> Rekommenderad längd: 60-90 sekunder.</li> <li> Införliva nyckelord som använder namnet på videorna. </li> <li> Överväg att lägga till ytterligare videor, till exempel en instruktioner, komma igång eller kundomdömar. </li> </ul> |
| Skärmdumpar (1280&nbsp;&times;&nbsp;720) | Lägg till upp till fem skärmdumpar:<br>Införliva nyckelord i filnamnen. |

## <a name="link-to-your-offer-page-from-your-website"></a>Länk till din erbjudandesida från din webbplats

När du länkar från AppSource- eller Azure Marketplace-märket på din webbplats till dina uppgifter på marketplace kan du stödja stark analys och rapportering genom att inkludera följande frågeparametrar i slutet av webbadressen:
* **src**: Inkludera källan från vilken trafiken dirigeras till AppSource (till exempel webbplats, LinkedIn eller Facebook).
* **mktcmpid**: Ditt marknadsföringskampanj-ID, som kan innehålla upp till 16 tecken i valfri kombination av bokstäver, siffror, understreck och bindestreck (till exempel *blogpost_12*).

Följande exempel-URL innehåller båda föregående frågeparametrar:`https://appsource.microsoft.com/product/dynamics-365/mscrm.04931187-431c-415d-8777-f7f482ba8095?src=website&mktcmpid=blogpost_12`

Genom att lägga till parametrarna i appsource-URL:en kan du granska hur effektiv kampanjen är i instrumentpanelen för analys i [Cloud Partner-portalen](https://cloudpartner.azure.com/#insights).

## <a name="next-steps"></a>Nästa steg

Läs mer om dina [kommersiella marknadsplatsfördelar.](https://docs.microsoft.com/azure/marketplace/gtm-your-marketplace-benefits)

Logga in på [Cloud Partner Portal](https://cloudpartner.azure.com) för att skapa och konfigurera ditt erbjudande.

---
