...
Ange klient-ID, klient-ID och nyckel för ditt program som du hämtade från Partner Center i föregående avsnitt för *tenant_id* värde i `POST URI` och *client_id* och *client_secret* parametrar. För *resurs* parametern måste du ange `https://api.partner.microsoft.com`.
