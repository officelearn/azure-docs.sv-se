...
ms.locfileid: "94918228"
---
<!--Create a copyAllNonInterleave transform with REST-->

Följande Azure REST-kommando skapar en transformering med `copyAllBitrateNonInterleaved` från för `#Microsoft.Media.BuiltInStandardEncoderPreset` inställningen. Ersätt värdena `subscriptionID` , `resourceGroup` och `accountName` med värden som du håller på att arbeta med. Ge transformeringen ett namn genom att ange `transformName` . 

Om du vill visa alla REST API transformeringar som är tillgängliga, se [Transforms-Create eller Update](https://docs.microsoft.com/rest/api/media/transforms/createorupdate#definitions).
