...
description: Självstudiekurs som visar hur du laddar upp en fil, kodar video och streamar innehåll med Azure Media Services v3.
...
> * Undersök koden som överför, kodar och strömmar.
> * Kör appen.
> * Testa webbadressen för direktuppspelning.
> * Rensa resurser.
...
Klona en GitHub-databas som har det strömmande .NET-exemplet till datorn med följande kommando:  
...
### <a name="start-using-media-services-apis-with-net-sdk"></a><a id="start_using_dotnet" />Börja använda Media Services-API:er med .NET SDK

Om du vill börja använda API:er för Media Services med .NET, måste du skapa ett **AzureMediaServicesClient**-objekt. Om du vill skapa objektet måste du ange autentiseringsuppgifter som krävs för att klienten ska kunna ansluta till Azure med Azure AD. I den kod som du har klonat i början av artikeln skapar funktionen **GetCredentialsAsync** objektet ServiceClientCredentials baserat på de autentiseringsuppgifter som anges i den lokala konfigurationsfilen.
...
Funktionen **CreateInputAsset** skapar en ny [indatatillgång](https://docs.microsoft.com/rest/api/media/assets) och överför den angivna lokala videofilen till den. Den här **tillgången** används som indata till ditt kodningsjobb. I Media Services v3 kan indata till ett **jobb** antingen vara en **tillgång** eller ett innehåll som du gör tillgängligt för ditt Media Services-konto via HTTPS-url:er. Mer information om hur du kodar från en HTTPS-URL finns i [den här](job-input-from-http-how-to.md) artikeln.
...
[Utdatatillgången](https://docs.microsoft.com/rest/api/media/assets) lagrar resultatet av kodningsjobbet. Projektet definierar funktionen **DownloadResults** som laddar ner resultaten från den här utdatatillgången till mappen ”utdata”, så att du kan se vad du har fått.
...
**Jobb** har vanligtvis följande tillstånd: **Schemalagd**, **I kö**, **Bearbetas**, **Slutförd** (slutlig status). Om jobbet har påträffat ett fel visas tillståndet **Fel**. Om jobbet håller på att avbrytas får du **avbryta** och **avbryta när** det är klart.
...
När kodningen är klar är nästa steg att göra videon i utdatatillgången tillgänglig för uppspelning av klienterna. Du kan göra den tillgänglig i två steg: först skapa en [streaming locator](https://docs.microsoft.com/rest/api/media/streaminglocators)och för det andra, bygga strömmande webbadresser som klienter kan använda.

Processen att skapa en **positionerare för direktuppspelning** kallas för publicering. Som standard är **strömningspositioneraren** giltig direkt efter att du har ringt API-anropen och varar tills den tas bort, såvida du inte konfigurerar de valfria start- och sluttiderna.

När du skapar en [StreamingLocator](https://docs.microsoft.com/rest/api/media/streaminglocators)måste du ange önskat **StreamingPolicyName**. I det här exemplet ska du strömma in-the-clear (eller icke-krypterat innehåll) så den fördefinierade principen för rensad direktuppspelning **(PredefinedStreamingPolicy.ClearStreamingOnly)** används.

> [!IMPORTANT]
> Om du använder en anpassad [strömningsprincip](https://docs.microsoft.com/rest/api/media/streamingpolicies) bör du skapa en begränsad uppsättning av sådana principer för ditt Media Service-konto, och återanvända dem för dina StreamingLocators när samma krypterings- och protokollalternativ krävs. Media Service-kontot har en kvot för antalet strömningsprincipposter. Du bör inte skapa en ny streamingprincip för varje streamingpositionerare.
...
I allmänhet bör du rensa upp allt utom objekt som du planerar att återanvända (vanligtvis återanvänder du Transforms och du kommer att spara StreamingLocators osv.). Om du vill att kontot ska vara rent när du har experimenterat tar du bort de resurser som du inte tänker återanvända. Följande kod tar till exempel bort jobb:
...
![Exempel på utdata som visar webbadresser för direktuppspelade video för Media Services](./media/stream-files-tutorial-with-api/output.png)
...
2. I rutan **URL:** klistrar du in ett av de url-värden för direktuppspelning som du fick när du körde appen.
3. Välj **Uppdatera spelare**.

Azure Media Player kan användas för testning men bör inte användas i en produktionsmiljö.
...
Azure Media Services v3 SDK:er är inte trådsäkra. När du utvecklar en app med flera trådar bör du generera och använda ett nytt AzureMediaServicesClient-objekt per tråd.
