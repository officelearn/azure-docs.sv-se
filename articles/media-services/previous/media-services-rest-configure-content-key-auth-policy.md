---
title: Konfigurera en princip för auktorisering av innehållsnyckel med REST - Azure | Microsoft Docs
description: Lär dig hur du konfigurerar en auktoriseringsprincip för en innehållsnyckel med Media Services REST API.
...
- [Integrera en app med Azure Media Services OWIN MVC-baserade med Azure Active Directory och begränsa viktiga innehållsleverans baserat på JWT anspråk](http://www.gtrifonov.com/2015/01/24/mvc-owin-azure-media-services-ad-integration/)

### <a name="some-considerations-apply"></a>Vissa förutsättningar gäller
* Om du vill använda dynamisk paketering och dynamisk kryptering, kontrollera att den strömningsslutpunkt från vilken du vill strömma ditt innehåll är i tillståndet ”körs”.
...
> 
> När du har åtkomst till entiteter i Media Services måste du ange specifika namn på huvudfält och värden i HTTP-begäranden. Mer information finns i [installationsprogrammet för Media Services REST API-utveckling](media-services-rest-how-to-use.md).
> 
> 
> 
...
I följande exempel skapas en auktoriseringsprincip med en token begränsning. I det här exemplet presenterar klienten en token som innehåller signeringsnyckeln (VerificationKey), en token utfärdare och nödvändiga anspråk.
...
När du skyddar ditt innehåll med PlayReady en av de saker som du måste ange i principen för auktorisering är en XML-sträng som definierar den [PlayReady-licensmall](media-services-playready-license-template-overview.md). 
...
### <a name="token-restriction"></a>Tokenbegränsningar
Om du vill konfigurera alternativet tokenbegränsningar som du behöver använda en XML för att beskriva behörighetskraven för token. Konfigurationen av tokenbegränsningar XML måste motsvara det XML-schemat visas i avsnittet ”[Token begränsning schemat](#schema)”.
...
> [!NOTE]
> IP-begränsning för auktorisering av innehållsnyckel principer är ännu inte tillgänglig i tjänsten.
...
Nu när du har konfigurerat en innehållsnyckel auktoriseringsprincip finns [konfigurera tillgångsleveransprincip](media-services-rest-configure-asset-delivery-policy.md).
