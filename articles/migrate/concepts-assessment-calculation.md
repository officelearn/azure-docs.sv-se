...
--- | ---
**Målplats** | Anger den Azure-plats som du vill migrera till.<br/><br/>Server utvärderingen stöder för närvarande dessa mål regioner: östra Australien, sydöstra Australien, södra Brasilien, centrala Kanada, Östra Kanada, centrala Indien, centrala USA, Kina, östra, Kina, norra, Asien, östra, östra USA, östra 2; USA, centrala Tyskland, Tyskland nordöstra, Östra Japan, västra Japan, centrala Korea, centrala Korea, norra centrala USA, norra Europa, södra centrala USA, Sydostasien, södra Indien, Storbritannien, södra, Storbritannien, västra, US Gov, Arizona, US Gov, Texas, US Gov, Virginia, västra centrala USA, Västeuropa, västra Indien, västra USA och västra 2; USA.
...
En utvärdering i Azure Migrate Server-utvärderingen beräknas med hjälp av de metadata som samlats in om de lokala servrarna. Om identifierings källan är en import med hjälp av en. CSV-fil beräknas utvärderingen med hjälp av de metadata som tillhandahålls av användaren om servrarna. Bedömnings beräkningen hanteras i tre steg. För varje-server börjar bedömnings beräkningen med en Azure-undersöknings analys, följt av storlek och slutligen en uppskattning per månad. En server flyttas bara till ett senare steg om den tidigare har passerat. Om en server till exempel inte uppfyller Azures lämpligaste kontroll, är den markerad som olämplig för Azure, och storleks sortering och kostnads redovisning görs inte för den servern.
...
**Lagrings disk** | Den allokerade storleken på en disk måste vara 32 TB eller mindre. Även om Azure har stöd för 64 TB-diskar med Ultra SSD diskar Azure Migrate: Server utvärderingen kontrollerar 32 TB som disk storleks gränser eftersom den inte stöder Ultra SSD än. <br/><br/> Antalet diskar som är anslutna till datorn måste vara 65 eller mindre, inklusive OS-disken. | Redo om det är inom gränserna.
...
**Operativ system** | **Information** | **Status för Azure-beredskap**
--- | --- | ---
...
- Om utvärderingen använder *prestandabaserade storleks*ändringar, förbrukar Azure Migrate datorns prestanda historik för att identifiera VM-storleken och disk typen i Azure. Om det gäller servrar med identifierings källa som import, anses de prestanda användnings värden som anges av användaren. Den här metoden är särskilt användbar om du har överallokerat den lokala virtuella datorn, men användningen är låg och du vill ha rätt storlek på den virtuella datorn i Azure för att spara kostnader. Den här metoden hjälper dig att optimera storlekarna under migreringen.
...
1. För virtuella VMware-datorer samlar Azure Migrate-installationen in en exempel punkt i real tid med varje 20-sekunders intervall. För virtuella Hyper-V-datorer samlas exempel punkten i real tid in med varje 30 sekunders intervall. För fysiska servrar samlas exempel punkten i real tid in med var femte minut. 
...
4. Det här värdet multipliceras med den praktiska faktorn för att få en effektiv prestanda användnings data för varje mått (processor belastning, minnes användning, disk-IOPS (läsning och skrivning), disk data flöde (läsning och skrivning) och nätverks data flöde (in och ut) som enheten samlas in.

När det effektiva användning svärdet har fastställts hanteras lagring, nätverk och beräknings storlek enligt följande.

> [!NOTE]
> För servrar som läggs till via import används de prestanda data som tillhandahålls av användaren direkt för höger storlek.
...
> [!NOTE]
> Det finns för närvarande inte stöd för att ange antalet nätverkskort för importerade servrar

**Beräknings storlek**: när den beräknar kraven på lagring och nätverk tar Server utvärderingen hänsyn till processor-och minnes krav för att hitta en lämplig VM-storlek i Azure.
