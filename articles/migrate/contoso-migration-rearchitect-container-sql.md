...
I den här artikeln migrerar Contoso tvålagers-Windows WPF, XAML forms SmartHotel360-app som körs på virtuella VMware-datorer till Azure. Om du vill använda den här appen tillhandahålls som öppen källkod och du kan ladda ned det från [GitHub](https://github.com/Microsoft/SmartHotel360).
...
**DevOps** | Contoso vill flytta till en DevOps-modellen med Azure DevOps-tjänsterna för kod som skapar och släpper pipeline.
...
- Contoso har valt för webbnivån app konvertera den till Windows-behållare med Azure DevOps-tjänsterna.
...
- För att implementera en pipeline för DevOps, använder Contoso Azure DevOps för källa kod management (SCM), med Git-lagringsplatser.  Automatiserade versioner och versioner används för att skapa kod och distribuera den till Azure Container Registry och Azure Service Fabric.
...
2. Det etablerar en Azure SQL-instans och migrerar SmartHotel360-databasen till den.
...
2. I **grunderna**, de tillhandahåller ett unikt DS-namn för klustret och autentiseringsuppgifter för åtkomst till den lokala virtuella datorn. De placera resursen i hanteringsgruppen för produktion-resurs (**ContosoRG**) i den primära regionen östra USA 2.
...
4. De Lägg nu till ett certifikat som kommer att bli ett klientcertifikat för administratör på klustret. På så sätt kan Azure DevOps-tjänsterna att ansluta till klustret för appdistribution i versionspipelinen. För att göra de, de öppna KeyVault i portalen och välj **certifikat** > **generera/importera**.
...
Contoso-administratörer kan nu migrera SmartHotel360-databasen med hjälp av DMA.
...
[Lär dig mer om](https://docs.microsoft.com/azure/sql-database/sql-database-firewall-configure) skapa och hantera brandväggsregler för Azure SQL Database.
...
## <a name="step-6-set-up-azure-devops-services"></a>Steg 6: Konfigurera Azure DevOps-tjänsterna

Contoso behöver skapa DevOps-infrastrukturen och pipelines för programmet.  Detta gör Contoso administratörer skapa ett nytt Azure DevOps-projekt, importera sin kod, skapa och release-pipelines.

1.   De skapa ett nytt projekt i Contoso Azure DevOps-konto (**ContosoSmartHotelRearchitect**), och välj **Git** för versionskontroll.
...
10. Hunnit checka in den uppdaterade koden och push-Överför till Azure DevOps-tjänsterna.

    ![Checka in](./media/contoso-migration-rearchitect-container-sql/container9.png)

## <a name="step-8-build-and-release-pipelines-in-azure-devops-services"></a>Steg 8: Skapa och släppa pipelines i Azure DevOps-tjänsterna

Contoso-administratörer kan nu konfigurera Azure DevOps-tjänsterna för att skapa build-version och versionen processen för att åtgärden DevOps-metoder.

1. I Azure DevOps-tjänsterna, de klickar på **skapa och släpp** > **ny pipeline**.

    ![Ny pipeline](./media/contoso-migration-rearchitect-container-sql/pipeline1.png)

2. De väljer **Azure DevOps Services Git** och relevanta lagringsplatsen.
...
4. De ändrar åtgärdsetiketten avbildningar till **skapa en avbildning**, och konfigurera aktiviteten om du vill använda den etablerade ACR.

     ![Register](./media/contoso-migration-rearchitect-container-sql/pipeline4.png)

5. I den **skicka avbildningar** aktivitet, de konfigurera avbildningen som ska skickas till ACR och välj för att inkludera den senaste taggen.
...
8. När den har byggts flyttar de till versionspipelinen. I Azure DevOps-tjänsterna de klickar på **versioner** > **ny pipeline**.

    ![Releasepipeline](./media/contoso-migration-rearchitect-container-sql/pipeline6.png)    

9. De väljer den **distribution av Azure Service Fabric** mallen och namn på scenen (**SmartHotelSF**).

    ![Miljö](./media/contoso-migration-rearchitect-container-sql/pipeline7.png)

10. Du får ett namn på pipeline (**ContosoSmartHotel360Rearchitect**). För steg, de klickar på **1 jobb, 1 aktivitet** att konfigurera Service Fabric-distribution.
...
12. I **lägga till Service Fabric-tjänstanslutning**, de konfigurerar anslutningen och inställningarna för autentisering som ska användas av Azure DevOps-tjänsterna för att distribuera appen. Klusterslutpunkten kan finnas i Azure portal och de lägger till **tcp: / /** som ett prefix.
...
18. När distributionen är klar körs SmartHotel360 nu Service Fabric.

    ![Publicera](./media/contoso-migration-rearchitect-container-sql/publish4.png)

19. För att ansluta till appen, dirigera de trafik till offentliga IP-adressen för Azure-belastningsutjämnaren framför Service Fabric-noder.
...
När den SmartHotel360-app och databasen körs i Azure, vill Contoso utöka appen.
...
- Uppdatera interna dokumentation för att visa de nya platserna för SmartHotel360-appen. Visa databasen som körs i Azure SQL-databas och klientdelen som körs i Service Fabric.
...
I den här artikeln omstrukturerade Contoso SmartHotel360-app i Azure genom att migrera app-klientdelens VM till Service Fabric. App-databasen har migrerats till en Azure SQL database.
