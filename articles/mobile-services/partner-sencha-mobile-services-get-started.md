---
title: Komma igång med Azure Mobile Services och Sencha
...
3. Klicka på **Create TodoItems table** (Skapa TodoItems-tabell) för att skapa en tabell för lagring av appdata.
   ...
   > [!NOTE]
   > Ladda INTE ned HTML-appen från den klassiska Azure-portalen. Vi skapar i stället en Sencha Touch-app manuellt i avsnittet nedan.
   > 
   > 
4. Notera **appKey** och **appUrl** i den klassiska Azure-portalen. Du behöver dem i andra avsnitt i den här kursen.
   
    ![appKey](./media/partner-sencha-mobile-services-get-started/mobile-app-key-portal.png)
5. Välj fliken **Configure** och kontrollera att `localhost` redan finns i listan **Allow requests from host names** (Tillåt begäranden från värdnamn) under **Cross-Origin Resource Sharing (CORS)**. Om den inte gör det skriver du `localhost` i fältet **Host name** (Värdnamn) och klickar på **Save** (Spara).
...
> [!NOTE]
> Du kan bara använda den här metoden när du har genererat appen med kommandot <code>sencha generate app</code>.
> 
> 
