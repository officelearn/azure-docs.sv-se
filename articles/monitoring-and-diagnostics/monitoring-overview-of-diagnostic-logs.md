...
* Analysera dem med [logganalys](../log-analytics/log-analytics-azure-storage.md)
...
* Där resursen diagnostikloggar och mått som skickas (Storage-konto, Event Hubs och/eller logganalys).
...
### <a name="enable-collection-of-resource-diagnostic-logs-via-azure-cli-20"></a>Aktivera insamling av resursen diagnostikloggar via Azure CLI 2.0

Om du vill aktivera insamling av resursen diagnostikloggar via Azure CLI 2.0, som du använder den [az diagnostik-inställningar för övervakning av skapa](/cli/azure/monitor/diagnostic-settings#az-monitor-diagnostic-settings-create) kommando.
...
Strömning av diagnostiska loggar till en händelsehubb:
...
Regel-ID är en sträng med formatet: `{Service Bus resource ID}/authorizationrules/{key name}`.

Att skicka diagnostikloggar till logganalys-arbetsyta:
...
Med ett kommando du kan lägga till ytterligare kategorier diagnostiska loggen genom att lägga till ordlistor JSON-matris som skickades i `--logs` parameter. Du kan kombinera den `--storage-account`, `--event-hub`, och `--workspace` parametrar för att aktivera flera alternativ för utdata.
