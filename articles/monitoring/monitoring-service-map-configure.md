...
I den här artikeln ska vi refererar till alla agenter--om Linux- eller Windows, om ansluten till en System Center Operations Manager-hanteringsgrupp eller direkt till Log Analytics--som den *OMS-Agent*. Vi använder endast det specifika distributionsnamnet på agenten om det behövs för sammanhanget.
...
Om du använder direkt OMS-agenten måste du konfigurera OMS själva agenten att ansluta till logganalys eller till din OMS-Gateway. OMS-Gateway kan hämtas från den [Microsoft Download Center](https://www.microsoft.com/download/details.aspx?id=52666). Mer information om hur du distribuerar och konfigurerar OMS-Gateway finns [ansluta datorer utan Internetåtkomst med OMS-Gateway](../log-analytics/log-analytics-oms-gateway.md).  
...
1.  Installera OMS-agenten efter någon av metoderna som beskrivs i [samla in data från datorer i din miljö med logganalys](../log-analytics/log-analytics-concept-hybrid.md).
...
Du kan enkelt distribuera agenten beroende på din virtuella Azure-datorer med en [Azure VM-tillägget](https://docs.microsoft.com/azure/virtual-machines/windows/extensions-features).  Du kan distribuera beroende agenten till din virtuella dator via ett PowerShell-skript eller direkt i Azure Resource Manager-mall för den virtuella datorn med Azure VM-tillägg.  Det är ett tillägg för både Windows (DependencyAgentWindows) och Linux (DependencyAgentLinux).  Om du distribuerar via Azure VM-tillägget kan dina agenter uppdateras automatiskt till de senaste versionerna.
...
Ett enklare sätt att kontrollera agenten beroende på var och en av dina virtuella datorer är att inkludera agenten i Azure Resource Manager-mallen.  Observera att beroende agenten fortfarande beroende OMS-Agent därför [OMS-agenten VM-tillägget](../virtual-machines/extensions/oms-linux.md) måste distribueras först.  Följande fragment av JSON kan läggas till i *resurser* i mallen.
...
- Lär dig hur du [använder Tjänstkarta]( monitoring-service-map.md) när den har distribuerats och konfigurerats.
