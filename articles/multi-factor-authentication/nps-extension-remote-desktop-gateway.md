...
Om du vill installera NPS som du behöver veta GUID för Azure AD. Instruktioner för att hitta GUID för Azure AD finns nedan.
...
När ett konto har aktiverats för MFA, du kan inte logga i till resurser som omfattas av MFA policy förrän du har konfigurerat en betrodd enhet ska användas för andra autentiseringsfaktor har autentiseras med tvåstegsverifiering.
...
4. Granska licensvillkoren för programvara i NPS-tillägget för Azure MFA dialogrutan Kontrollera **jag godkänner licensvillkoren och villkor**, och klicka på **installera**.
 
  ![Azure MFA-inställningar](./media/nps-extension-remote-desktop-gateway/image2.png)

5. Klicka på Stäng i NPS-tillägget för Azure MFA-dialogruta. 
...
Om du vill använda dina egna certifikat måste du koppla allmänheten om ditt certifikat från tjänsten principen på Azure AD och så vidare.
...
4. I dialogrutan Egenskaper väljer du den **auktoriseringsprincip för Fjärrskrivbordsanslutning** Store-fliken.
...
8. I den **delad hemlighet** dialogrutan, ange en delad hemlighet och klicka sedan på **OK**. Se till att du registrerar den här delad hemlighet och lagra posten på ett säkert sätt.
 ...
 >[!NOTE]
 >Delad hemlighet används för att upprätta förtroende mellan RADIUS-servrar och klienter. Skapa ett långa och komplexa lösenord.
 >
...
1. På servern för fjärrskrivbordsgateway i Serverhanteraren klickar du på **verktyg**, och klicka sedan på **Network Policy Server**. 
...
Återkalla NPS-servern med Azure MFA-tillägget är avsedda centrala principarkivet för anslutningen auktorisering princip (CAP). Du måste därför att implementera ett tak för NPS-servern att godkänna giltiga anslutningar begäranden.  
...
4. I den **kopia av anslutningar till andra servrar** dialogrutan i principens namn anger du ett lämpligt namn som **RDG_CAP**. Kontrollera **principen aktiverad**, och välj **bevilja åtkomst**. Alternativt, Välj typ av nätverksåtkomst **fjärrskrivbordsgateway**, eller så kan du lämna den som **Ospecificerad**.
...
Om du vill verifiera konfigurationen som du behöver logga in på servern för fjärrskrivbordsgateway med en lämplig RDP-klient. Se till att använda ett konto som tillåts av din anslutning auktoriseringsprinciper och är aktiverat för Azure MFA. 
...
Fråga lyckad inloggning-händelser i Gateway-arbetsloggarna _(händelse loggboken\program- och tjänster Logs\Microsoft\Windows\TerminalServices-Gateway\Operational_, Använd följande kommandon:
