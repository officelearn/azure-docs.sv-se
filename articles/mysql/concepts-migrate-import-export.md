...
- När du behöver selektivt välja några tabeller som ska importeras från en befintlig MySQL-databas till Azure MySQL-databasen, är det bäst att använda import-och export metoden.  Genom att göra det kan du utelämna eventuella tabeller som inte behövs från migreringen för att spara tid och resurser. Använd exempelvis växeln `--include-tables` eller `--exclude-tables` med [mysqlpump](https://dev.mysql.com/doc/refman/5.7/en/mysqlpump.html#option_mysqlpump_include-tables) och växeln `--tables` med [mysqldump](https://dev.mysql.com/doc/refman/5.7/en/mysqldump.html#option_mysqldump_tables).
...
3. Välj mål tabellen (ny eller befintlig) och markera eller avmarkera kryss rutan **trunkera tabellen innan du importerar** . Klicka på **Next**.
