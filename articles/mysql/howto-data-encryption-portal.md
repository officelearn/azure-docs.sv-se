...
description: Lär dig hur du konfigurerar och hanterar data kryptering för din Azure Database for MySQL med hjälp av Azure Portal
author: kummanish
ms.author: manishku
ms.service: mysql
ms.topic: conceptual
...
2. När återställningen är klar krypteras den nya servern med den nyckel som används för att kryptera den primära servern. Funktionerna och alternativen på-servern är dock inaktiverade och servern har marker ATS i ett **otillgängligt** tillstånd. Detta är för att förhindra data manipulation eftersom den nya serverns identitet fortfarande har fått behörighet att komma åt Key Vault.
