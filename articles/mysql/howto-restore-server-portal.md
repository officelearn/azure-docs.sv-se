---
title: Säkerhetskopiering och återställning - Azure portal - Azure Database för MySQL
description: I den här artikeln beskrivs hur du återställer en server i Azure Database for MySQL med Azure-portalen.
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: conceptual
...
---
# <a name="how-to-backup-and-restore-a-server-in-azure-database-for-mysql-using-the-azure-portal"></a>Säkerhetskopiering och återställning av en server i Azure Database for MySQL med Azure-portalen
...
Mer information om hur du anger dessa värden under skapa finns i [snabbstarten för Azure Database for MySQL-servern](quickstart-create-mysql-server-database-using-azure-portal.md).

Lagringsperioden för säkerhetskopiering kan ändras på en server genom följande steg:
1. Logga in på [Azure-portalen](https://portal.azure.com/).
2. Välj din Azure-databas för MySQL-server. Den här åtgärden öppnar sidan **Översikt.**
...
Med Azure Database for MySQL kan du återställa servern till en tidpunkt och till en ny kopia av servern. Du kan använda den nya servern för att återställa dina data eller låta klientprogrammen peka på den nya servern.
...
2. Välj **Återställ**i verktygsfältet på serverns **översiktssida** .
   ...
   ![Knappen Azure Database för MySQL – Översikt - Återställ](./media/howto-restore-server-portal/2-server.png)

3. Fyll i formuläret Återställ med den information som behövs:

   ![Azure Database för MySQL - Återställ information](./media/howto-restore-server-portal/3-restore.png)
...
1. Välj knappen **Skapa en resurs** (+) i det övre vänstra hörnet av portalen. Välj **Databaser** > **Azure Database för MySQL**.
