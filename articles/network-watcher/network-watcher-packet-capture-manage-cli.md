...
Kör den `az network watcher packet-capture show-status` cmdlet, hämtar status för en paketinsamling som körs eller har slutförts.

```azurecli
az network watcher packet-capture show-status --name packetCaptureName --location {networkWatcherLocation}
```

Följande är exempel på utdata från den `az network watcher packet-capture show-status` cmdlet. I följande exempel är när avbildningen har stoppats, med en StopReason TimeExceeded. 
