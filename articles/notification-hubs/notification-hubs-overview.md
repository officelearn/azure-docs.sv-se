...
Smartmobiler och surfplattor har en funktion för att ”avisera” användare när en händelse inträffar. Dessa meddelanden förekommer i en rad olika former.
...
Push-meddelanden levereras via plattformsspecifika infrastrukturer som kallas för _plattformsspecifika meddelandesystem_ (Platform Notification Systems, PNS). Ett PNS-system har barebones-funktioner (det vill säga, det har inget stöd för sändning eller anpassning) och det har inget gemensamt gränssnitt. Om ett meddelande till exempel ska skickas till en Windows Store-app, måste en utvecklare kontakta WNS (Windows Notification Service). För att skicka ett meddelande till en iOS-enhet måste samma utvecklare sedan kontakta APNS (Apple Push Notification Service) och skicka meddelandet ytterligare en gång. Azure Notification Hubs gör processen enklare genom att tillhandahålla ett gemensamt gränssnitt, tillsammans med andra funktioner som stöder push-meddelanden för en rad olika plattformar.
...
- **Skalning.** Skalningen av denna infrastruktur består av två delar:
    ...
    + De flesta PNS-system kan inte användas för att sända meddelanden till flera enheter. Detta gör att en sändning till miljontals enheter även kräver miljontals anrop till PNS-system. Att kunna skala dessa förfrågningar är inget litet jobb eftersom apputvecklarna normalt vill hålla den totala svarstiden nere (till exempel att den sista enhet som tar emot meddelandet inte ska ta emot meddelandet 30 minuter efter att meddelandet har skickats; detta för att det i många fall skulle motverka själva syftet med push-meddelandena).
...
- **Stor uppsättning av leveransmönster**:
    ...
    Varje enhet kan ange en eller flera _taggar_ när du skickar dess handtag till en meddelandehubb. Följ länken för mer information om [taggar]. Taggar måste inte etableras i förväg eller placeras. Taggar ger dig ett enkelt sätt att skicka meddelanden till användare eller intressegrupper. Eftersom taggar kan innehålla alla typer av appspecifika identifierare (som användar- eller grupp-ID:n), gör användningen av dem att belastningen på appens serverdel reduceras eftersom den inte behöver lagra och hantera enhetshandtag.
...
För att underlätta för en sömlös och enhetlig användarupplevelse av alla Azure-tjänster har [Mobilappar i Apptjänst] inbyggt stöd för push-meddelanden som skickas via Notification Hubs. [Mobilappar i Apptjänst] är en mycket skalbar, globalt tillgänglig plattform för mobilappsutveckling tänkt att användas av utvecklare av företagsprogram och systemintegrerare. Den ger mobilutvecklare tillgång till ett stort utbud av funktioner.
...
+ [Microsoft.ServiceBus.Notifications]
  ...
  [Mobilappar i Apptjänst]: https://azure.microsoft.com/en-us/documentation/articles/app-service-mobile-value-prop/
  [mallar]: notification-hubs-templates.md
  [Azure Portal]: https://portal.azure.com
  [taggar]: (http://msdn.microsoft.com/library/azure/dn530749.aspx)
