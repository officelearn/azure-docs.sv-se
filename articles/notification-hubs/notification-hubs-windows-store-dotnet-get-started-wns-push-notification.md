...
description: Luta dig åt hur du använder Azure Notification Hubs för att skicka meddelanden till ett Windows Universal Platform-program.
...
---
# <a name="tutorial-send-notifications-to-universal-windows-platform-apps-using-azure-notification-hubs"></a>Självstudiekurs: Skicka meddelanden till universella Windows Platform-appar med Azure Notification Hubs

[!INCLUDE [notification-hubs-selector-get-started](../../includes/notification-hubs-selector-get-started.md)]

I den här självstudiekursen kommer du att skapa en meddelandehubb för att skicka push-meddelanden till en Windows Universal Platform-app (UWP). Du skapar en tom Windows Store-app som tar emot push-meddelanden med hjälp av Windows Push Notification Service (WNS). Sedan använder du meddelandehubben för att sända push-meddelanden till alla enheter som kör appen.
...
- Microsoft Visual Studio 2017 eller senare. I exemplet i den här självstudien används [Visual Studio 2019](https://www.visualstudio.com/products).
...
- Bekräfta att **hämta meddelanden från appar och andra avsändare** är aktiverad. 
    - Fönstret **Starta inställningar** på datorn.
    - Välj **panelen System.**
...
3. Expandera **produkthantering,** välj **WNS/MPNS**och välj sedan **Live Services-webbplats**. Logga in på ditt Microsoft-konto. Sidan för registrering av ansökan öppnas på en ny flik. Alternativt kan du navigera direkt till sidan [Mina program](https://apps.dev.microsoft.com) och välja ditt programnamn för att komma till den här sidan.

    ![WNS MPNS-sida](./media/notification-hubs-windows-store-dotnet-get-started/wns-mpns-page.png)
4. Observera lösenordet **för Application Secrets** och SID **(Package Security Identifier).**
...
2. Gör så här i dialogrutan **Skapa ett nytt projekt:**

    1. Skriv **Windows Universal**i sökrutan högst upp .
    2. I sökresultaten väljer du **Tom app (Universella Fönster)** och väljer sedan **Nästa**.

       ![Dialogrutan Nytt projekt](./media/notification-hubs-windows-store-dotnet-get-started/new-project-dialog.png)

    3. Ange ett **projektnamn**och en **plats** för projektfilerna i dialogrutan **Konfigurera det nya projektet.**
    4. Välj **Skapa**.
...
10. Leta reda på `App.xaml.cs` klassen i `App` projektets fil `InitNotificationsAsync` och lägg till följande metoddefinition. Ersätt `<your hub name>` med namnet på meddelandehubben som du skapade `<Your DefaultListenSharedAccessSignature connection string>` i `DefaultListenSharedAccessSignature` Azure-portalen och ersätt med anslutningssträngen från sidan **Åtkomstpoliser** i meddelandehubben:
