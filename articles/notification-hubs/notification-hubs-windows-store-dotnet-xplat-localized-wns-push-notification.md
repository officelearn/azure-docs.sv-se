...
- Serverdelsappen skickar meddelanden med hjälp av **tagg**- och **mall**funktionerna i Azure Notification Hubs.
...
Mer information om mallar finns i artikeln [Mallar](notification-hubs-templates-cross-platform-push-messages.md). 
...
2. Uppdatera **MainPage.xaml** så att den inkluderar en lokaliserad kombinationsruta:
    ...
    ```
2. Lägg till en språkparameter till metoderna **StoreCategoriesAndSubscribe** och **SubscribeToCateories** i klassen **Meddelanden**.
...
3. Hämta det lagrade språket genom att lägga till följande metod:
...
4. Uppdatera din knappklickningshanterare i **MainPage.xaml.cs** genom att hämta det aktuella värdet för språkkombinationsrutan och vidarebefordra det till anropet till klassen Meddelanden enligt följande:
    ...
    ```
4. Var slutligen noga med att uppdatera din `InitNotificationsAsync`-metod i filen App.xaml.cs så att du kan hämta språket och använda det när du prenumererar:
...
Detta enkla anrop levererar de lokaliserade nyheterna till **alla** dina enheter, oavsett plattform, eftersom din meddelandehubb skapar och levererar korrekt intern nyttolast för alla enheter som prenumererar på en specifik tagg.
...
1. Välj **kategorier** och **språk** och klicka på **Prenumerera**. Appen konverterar de valda kategorierna till taggar och begär en ny enhetsregistrering för de valda taggarna från meddelandehubben.

    ![Mobilprogram](./media/notification-hubs-windows-store-dotnet-xplat-localized-wns-push-notification/mobile-app.png)
2.  En **bekräftelse** om **prenumerationerna** visas. 
...
1. När du tagit emot en bekräftelse kör du **konsolappen** för att skicka meddelanden för varje kategori och i varje språk som stöds. Kontrollera att du bara får ett meddelande i de kategorier som du prenumererar på och meddelandet gäller för det språk du valt. 
