---
title: Skapa och hantera Azure Database för PostgreSQL-VNet-tjänstslutpunkter och regler med hjälp av Azure CLI | Microsoft Docs
...
az login
```
Installera CLI-tillägg för Azure Database för PostgreSQL-VNet-tjänstslutpunkter med hjälp av den `az extension add --name rdbms-vnet` kommando. 
```azurecli-interactive
az extension add --name rdbms-vnet
```

Kör den `az extension list` kommandot för att kontrollera installationen av CLI-tillägg.
```azurecli-interactive
az extension list
```
Utdata från kommandot visar alla installerade tillägg. Azure Database for PostgreSQL CLI-tillägg är:

 {”extensionType”: ”whl”, ”name”: ”rdbms-vnet”, ”version”: ”10.0.0”}

> [!NOTE]
> Avinstallera CLI-tillägg som kör den `az extension remove -n rdbms-vnet` kommando. 
...
Virtuella nätverk och Azure-tjänstresurser kan finnas i samma eller olika prenumerationer. Om VNet och Azure-tjänstresurser finns i olika prenumerationer bör resurserna finnas under samma Active Directory (AD)-klient vid tidpunkten för den här förhandsversionen.
