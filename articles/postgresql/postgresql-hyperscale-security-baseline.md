...
**Vägledning**: ej tillämpligt; Azure Database for PostgreSQL-Scale (citus) kan inte kopplas till ett virtuellt nätverk, undernät eller en nätverks säkerhets grupp.
...
**Vägledning**: ej tillämpligt; slut punkterna som används av Azure Database for PostgreSQL-storskalig (citus) hanteras av Microsoft. Du ansvarar för eventuella ytterligare kontroller som du vill distribuera till dina lokala system.
...
**Vägledning**: Microsoft underhåller den tids källa som används för Azure-resurser, t. ex. Azure Database for PostgreSQL-storskalig skalning (citus) för tidsstämplar i loggarna.
...
**Vägledning**: ej tillämpligt; Azure Database for PostgreSQL-Scale (citus) bearbetar eller skapar inte relaterade loggar mot skadlig kod.
...
**Vägledning**: Logga in på Azure Database for PostgreSQL-storskalig (citus) stöds både med användar namn/lösen ord konfigurerat direkt i databasen, samt användning av en Azure Active Directory (AD)-identitet och användning av en Azure AD-token för att ansluta. När du använder en Azure AD-token stöds olika metoder, till exempel en Azure AD-användare, en Azure AD-grupp eller ett Azure AD-program som ansluter till databasen.
...
**Vägledning**: granska och Stäm av åtkomst för båda användare som har åtkomst till den lokala databasen samt via Azure Active Directory.

Granska Azure Active Directory (AD)-loggarna för att identifiera inaktuella konton som kan innehålla de administrativa rollerna med citus-funktioner. Dessutom kan du använda Azure Identity Access-granskningar för att effektivt hantera grupp medlemskap, åtkomst till företags program som kan användas för att få åtkomst till citus (storskalig) och roll tilldelningar. Användar åtkomsten bör granskas regelbundet, till exempel var 90: e dag och se till att endast rätt användare har fortsatt åtkomst.
...
**Vägledning**: Microsoft hanterar den underliggande infrastrukturen för Azure Database for PostgreSQLd skalning (citus) och har implementerat strikta kontroller för att förhindra förlust eller exponering av kund information.
...
**Vägledning**: funktionerna för data identifiering, klassificering och förlust av förlust är inte tillgängliga ännu för skalning (citus).
...
**Vägledning**: Använd rollbaserad åtkomst kontroll i Azure (Azure RBAC) för att styra åtkomsten till citus-kontroll planet (t. ex. Azure Portal). RBAC påverkar inte användar behörigheter i databasen.
...
Microsoft hanterar den underliggande infrastrukturen för storskaliga (citus) och har implementerat strikta kontroller för att förhindra förlust eller exponering av kund information.
...
**Vägledning**: Microsoft utför sårbarhets hantering på underliggande system som stöder Azure Database for PostgreSQL-storskalig skalning (citus). För sårbarheter som rör konfigurationen av databas klustret och relaterade resurser använder du ett gemensamt risk bedömnings program (t. ex. vanliga sårbarhets bedömnings system) eller standard risk klassificeringen som tillhandahålls av ditt genomsöknings verktyg från tredje part.
...
**Vägledning**: ej tillämpligt; Du kan hantera ditt databas administratörs lösen ord på ett säkert sätt med hjälp av Azure Key Vault. Tilldela rätt begränsade begränsade behörigheter för Key Vault hemligt begränsa åtkomsten till databasens lösen ord. Undvik att lagra anslutnings strängar eller databas hemligheter i dina kod databaser.

* [Så här ansluter du till din PostgreSQL-databas](https://docs.microsoft.com/azure/postgresql/quickstart-create-hyperscale-portal#connect-to-the-database-using-psql)
...
Microsofts program mot skadlig kod har Aktiver ATS på den underliggande värden som har stöd för Azure-tjänster, till exempel Scale (citus), men det körs inte på kund innehåll.
