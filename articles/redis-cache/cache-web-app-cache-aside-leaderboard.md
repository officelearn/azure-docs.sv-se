---
title: Självstudiekurs för att skapa en webbapp med Redis Cache som använder cache-aside-mönstret | Microsoft Docs
description: Lär dig hur du skapar en webbapp med Redis Cache som använder cache-aside-mönstret
...
I den här kursen uppdaterar du ASP.NET-webbappen *ContosoTeamStats* som har skapats i [ASP.NET-snabbstarten för Azure Redis Cache](cache-web-app-howto.md), så att den inkluderar en resultattavla som använder [cache-aside-mönstret](https://docs.microsoft.com/azure/architecture/patterns/cache-aside) med Azure Redis Cache. Exempelprogrammet som visar en lista med teamstatistik från en databas och olika sätt att använda Azure Redis Cache för att lagra och hämta data från cacheminnet. När du slutför självstudien har du en webbapp som läser och skriver till en databas, samt som är optimerad med Azure Redis Cache och värdbaserad i Azure.

I den här guiden får du lära dig att:

> [!div class="checklist"]
> * Förbättra dataflödet och minskar databasbelastningen genom att lagra och hämta data med Azure Redis Cache.
...
* Kursen fortsätter där du slutade i [ASP.NET-snabbstarten för Azure Redis Cache](cache-web-app-howto.md). Följ snabbstarten först, om du inte redan har gjort det.
...
1. Öppna i Visual Studio den *ContosoTeamStats*-lösning som du skapade i [ASP.NET-snabbstarten för Azure Redis Cache](cache-web-app-howto.md).
...
1. I avsnittet `body` lägger du till följande nya `Html.ActionLink`-uttryck för *Contoso Team Stats* under länken för *Azure Redis Cache Test*.
...
## <a name="configure-the-app-for-redis-cache"></a>Konfigurera appen för Redis Cache
...
I det här testet körs både programmet och databasen lokalt. Men Redis Cache fjärrhanteras i Azure. Därför kommer cachen troligen underprestera något. För bästa prestanda bör klientprogrammet och Azure Redis Cache-instansen finnas på samma plats. I nästa avsnitt distribuerar du alla resurser till Azure för att se den förbättrade prestandan från att använda en cache.
...
> [Skala Azure Redis Cache](./cache-how-to-scale.md)