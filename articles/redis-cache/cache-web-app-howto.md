---
title: Skapa en ASP.NET-webbapp med Azure Redis Cache | Microsoft Docs
description: I den här snabbstarten lär du dig hur du skapar en ASP.NET-webbapp med Azure Redis Cache
...
ms.locfileid: "38674894"
---
# <a name="quickstart-create-an-aspnet-web-app"></a>Snabbstart: Skapa en ASP.NET-webbapp 

## <a name="introduction"></a>Introduktion

I den här snabbstarten får du veta hur du skapar och distribuerar en ASP.NET-webbapp till Azure App Service med hjälp av Visual Studio 2017. Exempelappen ansluter till en Azure Redis Cache för att lagra och hämta data från cachen. När du är klar med snabbstarten har du en fungerande webbapp, som finns i Azure och som läser och skriver till Azure Redis Cache.
...
3. NuGet-paketet hämtar och lägger till de nödvändiga sammansättningsreferenserna för klientprogrammet för att få åtkomst till Azure Redis Cache med cacheklienten StackExchange.Redis. Om du vill använda en starkt krypterad version av `StackExchange.Redis` klientbiblioteket ska du installera paketet `StackExchange.Redis.StrongName`.
...
2. I webbläsaren väljer du **Azure Redis Cache Test** i navigeringsfältet.

3. I exemplet nedan ser du att `Message`-nyckeln tidigare hade ett cachelagrat värde som angavs med Azure Redis Cache-konsolen i portalen. Appen uppdatera det cachelagrade värdet. Appen körde även kommandona `PING` och `CLIENT LIST`.
...
Välj **Azure Redis Cache-test** i navigeringsfältet för att testa cacheåtkomsten.
...
I nästa kurs får du använda Azure Redis Cache i ett mer realistiskt scenario för att förbättra prestandan för en app. Du uppdaterar appen på cachens resultattavla med hjälp av cache-aside-mönstret med ASP.NET och en databas.
