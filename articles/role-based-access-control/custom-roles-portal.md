...
1. Klicka på **Lägg** till för att lägga till behörigheten i behörighets listan.
    ...
    Behörigheten läggs till som en `Actions` eller en `DataActions`.

    ![Tillagd behörighet](./media/custom-roles-portal/permissions-list-add.png)

1. Om du vill ta bort behörigheter klickar du på ikonen Ta bort i slutet av raden. I det här exemplet, eftersom en användare inte behöver kunna skapa support biljetter, kan `Microsoft.Support/*` behörighet tas bort.

### <a name="add-wildcard-permissions"></a>Lägg till behörigheter för jokertecken

Beroende på hur du har valt att starta kan du ha behörigheter med jokertecken (\*) i listan med behörigheter. Ett jokertecken (\*) utökar en behörighet till allt som matchar den sträng som du anger. Anta till exempel att du vill lägga till alla behörigheter som är relaterade till Azure Cost Management och export. Du kan lägga till alla dessa behörigheter:
...
Om din roll har en behörighet som jokertecken (\*) och du vill undanta eller subtrahera vissa behörigheter från den jokertecken kan du undanta dem. Anta till exempel att du har följande behörighet för jokertecken:
...
På fliken **JSON** visas din anpassade roll som är FORMATERAD i JSON. Om du vill kan du redigera JSON direkt. Om du vill lägga till en jokertecken (\*) måste du använda den här fliken.
