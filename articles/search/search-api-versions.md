---
title: 'API-versions hantering för .NET SDK och REST API: er – Azure Search'
description: 'Versions princip för Azure Search REST-API: er och klient biblioteket i .NET SDK.'
author: brjohnstmsft
manager: nitinme
services: search
ms.service: search
...
ms.locfileid: "70182354"
---
# <a name="api-versions-in-azure-search"></a>API-versioner i Azure Search
Azure Search samlar ut funktions uppdateringar regelbundet. Ibland, men inte alltid, kräver de här uppdateringarna en ny version av API: n för att bevara bakåtkompatibilitet. Genom att publicera en ny version kan du styra när och hur du integrerar Sök tjänst uppdateringar i din kod.

Som regel publicerar Azure Search-teamet nya versioner bara när det behövs, eftersom det kan omfatta viss ansträngning för att uppgradera din kod för att använda en ny API-version. En ny version krävs bara om någon aspekt av API: et har ändrats på ett sätt som bryter mot bakåtkompatibilitet. Sådana ändringar kan inträffa på grund av korrigeringar av befintliga funktioner eller på grund av nya funktioner som ändrar befintliga API-Surface-områden.

Samma regel gäller för SDK-uppdateringar. Azure Search SDK följer reglerna för [semantisk version](https://semver.org/) , vilket innebär att dess version har tre delar: Major, mindre och build-nummer (till exempel 1.1.0). En ny huvud version av SDK släpps bara för ändringar som bryter mot bakåtkompatibilitet. Icke-brytande funktions uppdateringar kommer att öka den lägre versionen, och fel korrigeringarna ökar bara versions versionen.
...
Nedan visas en ögonblicks bild av de aktuella versionerna av alla programmerings gränssnitt som Azure Search.


| Gränssnitt | Senaste huvud version | State |
| --- | --- | --- |
...
Azure Search alltid för hands versioner av experiment funktioner via REST API först, sedan till för hands versioner av .NET SDK.
...
För .NET SDK: Vägledning för kod migrering hittar [du i uppgradera .NET SDK](search-dotnet-sdk-migration-version-9.md).

Allmän tillgänglighet innebär att Azure Search nu omfattas av service nivå avtalet (SLA). Service avtalet finns i [Azure Search Service nivå avtal](https://azure.microsoft.com/support/legal/sla/search/v1_0/).
