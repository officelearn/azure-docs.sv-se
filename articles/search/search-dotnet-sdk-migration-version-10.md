---
title: Uppgradera till Azure Search .NET SDK version 10
...
ms.locfileid: "72793043"
---
# <a name="upgrade-to-azure-search-net-sdk-version-10"></a>Uppgradera till Azure Search .NET SDK version 10
...
Version 10 av Azure Search .NET SDK riktar sig till den senaste allmänt tillgängliga versionen av Azure Search REST API (`2019-05-06`) med följande uppdateringar:
...
1. Uppdatera din NuGet-referens för `Microsoft.Azure.Search` med hjälp av NuGet Package Manager-konsolen eller genom att högerklicka på dina projekt referenser och välja "hantera NuGet-paket..." i Visual Studio.
...
Modellen för `WebApiSkill` angivna `HttpHeaders` som en objekt egenskap som _innehåller_ en ord lista. Om du skapar en färdigheter med en `WebApiSkill` konstruerad på det här sättet kan det leda till ett undantag eftersom REST API skulle anse att frågan är felaktigt utformad. Det här problemet har korrigerats, genom att göra `HttpHeaders` **en dictionary-egenskap på översta nivån** i själva `WebApiSkill`-modellen, som betraktas som en giltig begäran från REST API.

Om du till exempel tidigare försökte instansiera en `WebApiSkill` på följande sätt:
...
Formaren-kunskaper kan nu tillåta inkapsling av inkapslade kontexter. För att aktivera den här ändringen ändrade vi `InputFieldMappingEntry` så att de kan instansieras genom att bara ange en `Source` egenskap eller både `SourceContext`-och `Inputs` egenskaperna.
...
- Att skapa en `InputFieldMappingEntry` där endast `Source` initieras är giltigt.
- Att skapa en `InputFieldMappingEntry` där endast `SourceContext` och `Inputs` initieras är giltigt.
...
> [!NOTE]
> Även om den underliggande `InputFieldMappingEntry`s modellen har ändrats för att tillåta konsolidering från kapslade kontexter, är användningen endast giltig inom definitionen av en formaren-färdighet. Om du använder den här funktionen i andra färdigheter, medan den är giltig vid kompileringen, leder det till ett verifierings fel vid körning.

## <a name="skills-can-be-identified-by-a-name"></a>Kunskaper kan identifieras med ett namn

Varje färdighet i en färdigheter har nu en ny egenskap `Name`, som kan initieras i din kod för att identifiera kunskapen. Detta är valfritt – om inget anges (vilket är standard, om ingen explicit kod ändring gjorts), tilldelas den ett standard namn med det 1-baserade indexet för kunskapen i färdigheter, med prefixet #. Till exempel i följande färdigheter-definition (de flesta initierarna hoppades över för det kortfattat):
...
Om du väljer att identifiera färdigheter med ett anpassat namn, måste du först uppdatera alla instanser av dina klienter till version 10 av SDK. I annat fall finns det en möjlighet att en klient som använder en äldre version av SDK: n kan `null` ut egenskapen `Name` för en färdighet, vilket gör att klienten går tillbaka till standard namngivnings schemat.
