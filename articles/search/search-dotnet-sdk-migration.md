...
Metoden `Indexes.GetClient` har en ny returtyp. Tidigare returnerade den `SearchIndexClient`, men detta ändrades till `ISearchIndexClient` i version 2,0 – för hands version och denna ändring förflyttar sig till version 3. Detta är att stödja kunder som vill modellera `GetClient`-metoden för enhets test genom att returnera en modell implementering av `ISearchIndexClient`.
...
Det finns många typer i Azure Search .NET SDK som härleds från `ExtensibleEnum`. Tidigare var dessa typer alla implicit konverterbara till typ `string`. Men en bugg identifierades i `Object.Equals`-implementeringen för dessa klasser och korrigerar den bugg som krävs för att inaktivera denna implicita konvertering. Explicit konvertering till `string` tillåts fortfarande.
...
- Om du använder egenskapen `ScoringParameter.Value` använder du egenskapen `ScoringParameter.Values` eller `ToString` metoden i stället.
- Om du använder egenskapen `SearchRequestOptions.RequestId` använder du i stället egenskapen `ClientRequestId`.

### <a name="removed-preview-features"></a>Borttagna för hands versions funktioner

Om du uppgraderar från version 2,0 – för hands version till version 3, bör du tänka på att stöd för JSON-och CSV-parsning för BLOB-indexerare har tagits bort eftersom dessa funktioner fortfarande är i för hands version. Mer specifikt har följande metoder för klassen `IndexingParametersExtensions` tagits bort:
