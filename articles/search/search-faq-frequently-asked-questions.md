...
+ Azure Search är en fullständigt hanterad moln tjänst med 99,9% service avtal (SLA) som tillhandahålls med tillräcklig redundans (2 repliker för Läs behörighet, 3 repliker för Läs-och Skriv behörighet).
...
### <a name="backup-and-restore-or-download-and-move-indexes-or-index-snapshots"></a>Vill du säkerhetskopiera och återställa (eller ladda ned och flytta) index eller index ögonblicks bilder?

Även om du kan [Hämta en index definition](https://docs.microsoft.com/rest/api/searchservice/get-index) när som helst, finns det ingen index extrahering, ögonblicks bild eller funktionen för återställning av säkerhets kopiering för att ladda ned ett ifyllt index som körs i molnet till ett lokalt system, eller flytta den till en annan Azure Search-tjänst.

Indexen skapas och fylls i från kod som du skriver och körs bara på Azure Search i molnet. Kunder som vill flytta ett index till en annan tjänst gör det vanligt vis genom att redigera sin kod för att använda en ny slut punkt och sedan köra indexeringen igen. Om du vill kunna ta en ögonblicks bild eller säkerhetskopiera ett index, omvandla en röst på [användarens röst](https://feedback.azure.com/forums/263029-azure-search/suggestions/8021610-backup-snapshot-of-index).
...
Nej, du kan inte återställa index eller tjänster. Om du tar bort ett Azure Search-index är åtgärden slutgiltig och indexet kan inte återställas. När du tar bort en Azure Search-tjänst tas alla index i tjänsten bort permanent. Om du tar bort en Azure-resurs grupp som innehåller en eller flera Azure Search tjänster tas alla tjänster bort permanent.  

Att återställa resurser som index, indexerare, data källor och färdighetsuppsättningar kräver att du återskapar dem från kod. Om det gäller index måste du indexera om data från externa källor. Därför rekommenderar vi starkt att du behåller en huvud kopia eller en säkerhets kopia av original data i ett annat data lager, till exempel Azure SQL Database eller Cosmos DB.
...
Det vanligaste fallet är att du inte vet att varje frågetyp stöder olika Sök beteenden och nivåer av språkliga analyser. Full texts ökning, som är den dominerande arbets belastningen, innehåller en språk analys fas som avbryter termerna till rot formulären. Den här aspekten av frågans parsing skickar ett bredare netto över möjliga matchningar, eftersom den token termen matchar ett större antal varianter.

Jokertecken, fuzzy och regexfrågor analyseras dock inte som vanliga term-eller fras frågor och kan leda till dåligt återkallande om frågan inte överensstämmer med den analyserade formen av ordet i Sök indexet. Mer information om fråga om parsning och analys av frågor finns i [fråga om arkitektur](https://docs.microsoft.com/azure/search/search-lucene-query-architecture).
