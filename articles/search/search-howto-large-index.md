...
ms.locfileid: "69648166"
---
# <a name="how-to-index-large-data-sets-in-azure-search"></a>Så här indexerar du stora data mängder i Azure Search

Allteftersom data volymer växer eller bearbetningen behöver ändras kan du se att standard indexerings strategier inte längre är praktiska. För Azure Search finns det flera metoder för att få större data mängder, från hur du strukturerar en begäran om data uppladdning, för att använda en produktspecifik indexerare för schemalagda och distribuerade arbets belastningar.
...
## <a name="use-indexers"></a>Använda indexerare

[Indexerare](search-indexer-overview.md) används för att crawla externa data källor för sökbart innehåll. Även om det är särskilt avsett för storskalig indexering är flera indexerings funktioner särskilt användbara för att kunna använda större data uppsättningar:
...
+ Att partitionera data i mindre enskilda data källor möjliggör parallell bearbetning. Du kan dela upp en stor data uppsättning i mindre data uppsättningar och sedan skapa flera definitioner av data källor som kan indexeras parallellt.
...
2. I **Inställningar** > **skala** [ökar](search-capacity-planning.md) du repliker för parallell bearbetning: ytterligare en replik för varje Indexer-arbetsbelastning. Lämna ett tillräckligt antal för den befintliga frågesträngen. Att offra frågor mot arbets belastningar för indexering är inte ett utmärkt kompromiss.
