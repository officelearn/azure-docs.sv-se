...
+ **`https://azs-playground.search.windows.net/`** är en tjänst i begränsat läge som underhålls av Azure kognitiv sökning Development-teamet. 
...
För det kortfattat är frågan bara avsedd för fältet *business_title* och anger bara affärs titlar. **SearchFields** -parametern begränsar frågekörningen till bara fältet business_title och **väljer** anger vilka fält som ska ingå i svaret.
...
Se till att placera flera strängar inom citat tecken om du vill att båda strängarna ska utvärderas som en enda entitet, som i det här fallet söker efter två olika platser i fältet `state`. Se också till att operatorn är kapitaliserad som du ser med inte och och.
...
```
En mer lättläst version av ovanstående fråga är `search=business_title:computer analyst^2`. För en fungerande fråga kodas `^2` som `%5E2`, vilket är svårare att se.
...
Du kan använda en allmänt identifierad syntax för flera (\*) eller enkla (?) Character-sökningar. Observera att funktionen Lucene Query parser stöder användningen av dessa symboler med en enda term och inte en fras.
