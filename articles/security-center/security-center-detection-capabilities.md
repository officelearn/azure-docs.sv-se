...
## Hur Azure Security Center identifierar och svarar på hot<a name="asc-detects"></a>
...
* **Utgående kommunikation till en skadlig IP-adress**: utgående trafik till en känd botnät eller Darknet tyder troligt vis på att din resurs har komprometterats och en angripare försöker köra kommandon på systemet eller stjäla data. Azure Security Center jämför nätverkstrafik med Microsofts globala hotdatabas och varnar dig om kommunikation till en skadlig IP-adress identifieras.
...
* **Misstänkt process körning**: Angripare använder flera tekniker för att köra skadlig program vara utan identifiering. Till exempel kan en angripare ge skadlig kod samma namn som legitima systemfiler, men placera filerna på en annan plats, använda ett namn som liknar namnet på en oskadlig fil eller maskera filens egentliga filnamnstillägg. Security Center-modellerna bearbetar beteenden och övervakar körningen av processer för att identifiera avvikelser som dessa.  
* **Dolda skadliga program och användnings försök**: Avancerad skadlig kod kan undvika traditionella program för program mot skadlig kod genom att antingen aldrig skriva till disk eller kryptera program varu komponenter som lagras på disk.  Sådan skadlig kod kan dock identifieras med minnesanalyser eftersom den skadliga koden måste lämna spår i minnet för att kunna fungera. När programvara kraschar fångar en kraschdumpfil en del av minnet vid tidpunkten för kraschen.  Genom att analysera minnet i kraschdumpen kan Azure Security Center identifiera tekniker som utnyttjar sårbarheter i programvara, som kommer åt känsliga data och som ligger gömda på en komprometterad dator, utan att påverka datorns prestanda.
* **Lateral förflyttning och internt rekognosering**: För att bevaras i ett komprometterat nätverk och hitta/skörda värdefulla data försöker angripare ofta flytta senare från den komprometterade datorn till andra inom samma nätverk. Security Center övervakar process- och inloggningsaktiviteter för att identifiera försök att expandera en angripares fäste i nätverket, till exempel kommandokörning via fjärranslutning, nätverksavsökning och kontouppräkning.
* **Skadliga PowerShell-skript**: PowerShell används av angripare för att köra skadlig kod på virtuella mål datorer i en mängd olika syften. Security Center inspekterar PowerShell-aktivitet för att hitta tecken på misstänkt aktivitet.
* **Utgående attacker**: Angripare riktar ofta moln resurser med målet att använda dessa resurser för att montera ytterligare attacker. Komprometterade virtuella datorer kan till exempel användas för att köra råstyrkeattacker mot andra virtuella datorer, skicka skräppost eller söka av öppna portar och andra enheter på Internet. Genom att använda maskininlärning för nätverkstrafiken kan Security Center upptäcka avvikande utgående nätverkskommunikation. När det gäller skräppost kan Security Center även korrelera ovanlig e-posttrafik med information fån Office 365 för att avgöra om e-posten är skräppost eller en legitim e-postkampanj.  
