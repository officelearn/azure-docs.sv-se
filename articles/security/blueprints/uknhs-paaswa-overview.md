---
title: PaaS-webbprogram för Storbritannien NHS Handlingsplan för säkerhet och efterlevnad i Azure
description: PaaS-webbprogram för Storbritannien NHS Handlingsplan för säkerhet och efterlevnad i Azure
services: security
author: jomolesk
...
Den här lösningen ger en referens arkitektur för ett PaaS-webbprogram med en Azure SQL Database Server del. Webb programmet finns i ett isolerat Azure App Service-miljön, vilket är en privat, dedikerad miljö i ett Azure-datacenter. Miljö belastningen balanserar trafiken för webb programmet på virtuella datorer som hanteras av Azure. Alla externa anslutningar kräver TLSv 1.2. Den här arkitekturen omfattar även nätverks säkerhets grupper, en Application Gateway, Azure DNS och Load Balancer.
...
För ökad säkerhet hanteras alla resurser i lösningen som en resurs grupp via Azure Resource Manager. Azure Active Directory rollbaserad åtkomst kontroll används för att kontrol lera åtkomsten till distribuerade resurser och nycklar i Azure Key Vault. System hälsan övervakas via Azure Security Center och Azure Monitor. Kunderna konfigurerar både övervaknings tjänster för att avbilda loggar och Visa system hälsan på en enda, enkel portalerna instrument panel. Azure Application Gateway har kon figurer ATS som en brand vägg i skydds läge och inte tillåter trafik som inte är TLSv 1.2. Lösningen använder Azure Application Service Environment v2 för att isolera webb nivån i en miljö som inte har flera innehavare.

**Microsoft rekommenderar att du konfigurerar en VPN-eller ExpressRoute-anslutning för hantering och data import till referens arkitektur under nätet.**

![PaaS-webbprogram för NHS referens arkitektur diagram](images/uknhs-paaswa-architecture.png?raw=true "PaaS-webbprogram för NHS referens arkitektur diagram")
...
**App Service-miljön v2**: Azure App Service-miljön är en App Service-funktion som ger en helt isolerad och dedikerad miljö för säker körning av App Service program i stor skala.

App Service-miljön är isolerad för att bara köra ett enda program och distribueras alltid till ett virtuellt nätverk. Den här isolerings funktionen gör att referens arkitekturen har fullständig klient isolering och tar bort den från Azures miljö för flera innehavare. Den här isolerings funktionen krävs för att uppfylla kraven i UK NHS-princip 3. Kunderna har detaljerad kontroll över både inkommande och utgående program nätverks trafik, och program kan upprätta snabba säkra anslutningar över virtuella nätverk till lokala företags resurser. Kunder kan skala automatiskt med App Service-miljön baserat på inläsnings mått, tillgänglig budget eller ett definierat schema.


Användning av App Service-miljön för den här arkitekturen möjliggör följande kontroller och konfigurationer:
...
Data flödes diagrammet för den här referens arkitekturen är tillgängligt för [hämtning](https://aka.ms/uknhs-paaswa-tm) eller finns nedan. Den här modellen kan hjälpa kunder att förstå punkter av potentiell risk i system infrastrukturen när de gör ändringar.

![PaaS webb program för NHS hot modell för Storbritannien](images/uknhs-paaswa-threat-model.png?raw=true "PaaS webb program för NHS hot modell för Storbritannien")
...
[Handlingsplan för säkerhet och efterlevnad i Azure – Storbritannien NHS PaaS Web Application Implementation Matrix](https://aka.ms/uknhs-paaswa-cim) innehåller information om vilka Storbritannien NHS-krav som hanteras av PaaS-webbappens arkitektur, inklusive detaljerade beskrivningar av hur implementeringen uppfyller kraven för varje princip som omfattas.
