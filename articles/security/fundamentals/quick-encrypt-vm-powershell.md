...
- Installera eller uppdatera till den [senaste versionen av modulen Azure PowerShell](/powershell/azure/install-az-ps)
    - AZ-modulens version måste vara 1.0.0 eller högre. Använd `Get-Module Az -ListAvailable | Select-Object -Property Name,Version,Path` för att kontrol lera versionen.
...
![Azure Disk Encryption-skript om förhandskrav som körs i PowerShell ISE](./media/quick-encrypt-vm-powershell/ade-prereq-script.PNG)
...
Nu när du har skapat och konfigurerat ett nyckel valv och en virtuell dator kan du kryptera disken med cmdleten **set-AzVmDiskEncryptionExtension** . 
...
1. När krypteringen är klar kan du kontrollera att disken är krypterad med följande cmdlet: 
     ...
     ```
    ![Get-AzVmDiskEncryptionStatus-utdata](./media/quick-encrypt-vm-powershell/ade-get-encryption-status.PNG)
