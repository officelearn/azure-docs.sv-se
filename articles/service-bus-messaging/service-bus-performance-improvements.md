...
`MessageReceiver`-objektet instansieras med anslutnings strängen, könamnet och ett gransknings se mottagnings läge. Sedan används `receiver`-instansen för att registrera meddelande hanteraren.
...
`MessagingFactory` skapar ett `factory`-objekt från anslutnings strängen. Med `factory`-instansen instansieras en `MessageReceiver`. Sedan används `receiver`-instansen för att registrera meddelande hanteraren.

---

## <a name="receive-mode"></a>Mottagnings läge

När du skapar en kö eller en prenumerations klient kan du ange ett mottagnings läge: *Peek-lock* eller *ta emot och ta bort*. Standard mottagnings läget är `PeekLock`. Vid användning i standard läget skickar klienten en begäran om att ta emot ett meddelande från Service Bus. När klienten har tagit emot meddelandet skickar den en förfrågan om att slutföra meddelandet.

När du ställer in mottagnings läget på `ReceiveAndDelete`kombineras båda stegen i en enskild begäran. De här stegen minskar det övergripande antalet åtgärder och kan förbättra det övergripande meddelande data flödet. Den här prestandan ger risk för att förlora meddelanden.
...
> [!NOTE]
> Inställningen `BatchFlushInterval` garanterar att batchen är implicit från programmets perspektiv. dvs. programmet gör `SendAsync` och `CompleteAsync` samtal och utför inga specifika batch-anrop.
>
...
När du använder standard låsets utgångs datum på 60 sekunder är ett utmärkt värde för `PrefetchCount` 20 gånger den maximala bearbetnings takten för alla mottagare av fabriken. Till exempel skapar en fabrik tre mottagare och varje mottagare kan bearbeta upp till 10 meddelanden per sekund. För hämtnings antalet får inte överstiga 20 × 3 X 10 = 600. Som standard är `PrefetchCount` inställd på 0, vilket innebär att inga ytterligare meddelanden hämtas från tjänsten.
