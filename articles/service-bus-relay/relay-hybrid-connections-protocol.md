...
Lyssnare som bifogas till Hybridanslutningar med HTTP-stöd måste `request` hantera gesten. En lyssnare som inte hanterar `request` och orsakar därför upprepade timeout-fel vid anslutning kan vara svartlistad av tjänsten i framtiden.
...
2. Om begärandehuvuden innehåller ett `ServiceBusAuthorization` huvud är huvud uttrycket alltid kvar från huvud samlingen.
   Den kommer att utvärderas om relä auktorisering är aktiverat.
3. Endast om Relay-auktorisering har Aktiver ATS, och om begärandehuvuden innehåller ett `Authorization` huvud, och inget av de tidigare uttrycken finns, kommer rubriken att utvärderas och tas bort. Annars skickas alltid `Authorization`den som den är.
...
##### <a name="rejecting-the-socket"></a>Avvisar socketen
 ...
 Att avvisa socketen efter att ha kontrollerat `accept` att meddelandet kräver en liknande hand skakning så att status koden och status beskrivningen som skickar orsaken till avvisningen kan flöda tillbaka till avsändaren.
...
* **requestTarget** – sträng. Den här egenskapen innehåller ["mål för begäran" (RFC7230, Section 5,3)](https://tools.ietf.org/html/rfc7230#section-5.3) i begäran. Detta inkluderar frågesträngen, som inte är lika med alla `sb-hc-` prefix som har fastställts för fasta parametrar.
...
* **responseHeaders** – HTTP-huvuden som ska anges i ett externt http-svar.
  Som med måste `request`RFC7230-definierade huvuden inte användas.
...
När token-token håller på att gå ut kan den ersätta den genom att skicka ett text Rute meddelande till tjänsten via den etablerade kontroll kanalen. Meddelandet innehåller ett JSON-objekt med `renewToken`namnet, som definierar följande egenskap för tillfället:
...
_Namn områdets adress_ är det fullständigt kvalificerade domän namnet för Azure Relay namn området som är värd för Hybrid anslutningen, vanligt vis av `{myname}.servicebus.windows.net`formuläret.
...
Eftersom tjänsten fungerar effektivt som en proxy, även om den inte är en sann HTTP-proxy, lägger den till `Via` ett sidhuvud eller kommenterar den `Via` befintliga rubriken som [är kompatibel med RFC7230, avsnitt 5.7.1](https://tools.ietf.org/html/rfc7230#section-5.7.1).
Tjänsten lägger till relä namn områdets värdnamn `Via`i.
