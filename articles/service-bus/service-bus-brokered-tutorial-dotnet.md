...
1. Anteckna den primära nyckeln som är kopplad till principen **RootManagerSharedAccessKey** eller kopiera den till Urklipp. Du använder det här värdet senare i självstudiekursen.
...
1. Skapa sedan en funktion som tar emot och lagrar namnområdet för tjänsten och SAS-nyckeln. Lägg till den här metoden utanför `Main()`. Till exempel: 
...
1. För att den här självstudiekursen ska bli tydligare placeras alla köåtgärder i en annan metod. Skapa en asynkron `Queue()`-metod i `Program`-klassen efter `Main()`-metoden. Till exempel:
