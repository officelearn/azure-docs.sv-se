...
När du skapar ett program som använder Service Bus måste du lägga till en referens till Service Bus-sammansättningen och inkludera motsvarande namnområden. Det enklaste sättet att göra det är att hämta ett lämpligt NuGet-paket.

## Hämta Service Bus-NuGet-paketet

[Service Bus-NuGet-paketet](https://www.nuget.org/packages/WindowsAzure.ServiceBus) är det enklaste sättet att komma åt Service Bus-API:er och att konfigurera ditt program med alla nödvändiga Service Bus-beroenden. Om du vill installera NuGet-paketet i ditt program ska du göra följande:
...
Mekanismen för tjänstekonfiguration gör att du på ett dynamiskt sätt kan ändra konfigurationsinställningarna från den [klassiska Azure-portalen][] utan att omdistribuera ditt program. Du kan till exempel lägga till en `Setting`-etikett till tjänstedefinitionsfilen (***.csdef**), som visas i nästa exempel.
...
Använd SAS-nyckelnamnet (signatur för delad åtkomst) och de nyckelvärden som hämtades från portalen, enligt beskrivningen i det förra avsnittet.
...
Använd det SAS-namn och de nyckelvärden som du hämtade från [klassiska Azure-portalen][], enligt beskrivningen i det förra avsnittet.
