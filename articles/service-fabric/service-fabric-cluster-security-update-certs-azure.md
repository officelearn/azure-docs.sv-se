...
Sekundära kluster certifikat kan inte läggas till via Azure-portalen. Du måste använda Azure powershell för den. Processen beskrivs senare i det här dokumentet.

## <a name="swap-the-cluster-certificates-using-the-portal"></a>Växla klustercertifikat med hjälp av portalen

När du har distribuerat ett sekundärt kluster-certifikat har om du vill byta ut de primära och sekundära, navigera till bladet för säkerhet och alternativet 'Växlingen med primära' på snabbmenyn för att växla sekundära cert med primära certifikat.
...
Ta bort ett sekundärt certifikat används för klustret säkerhet, navigera till bladet för säkerhet och väljer alternativet ”Ta bort' snabbmenyn på sekundärt certifikat.
...
**Steg 1:** öppna Resource Manager-mallen som du använde för att distribuera kluster. (Om du har hämtat provet från ovan lagringsplatsen kan sedan distribuera ett kluster för säker med hjälp av 5-VM-1-nodetypes får-Secure_Step1.JSON och öppna sedan mallen).

**Steg 2:** Lägg till **två nya parametrar** ”secCertificateThumbprint” och ”secCertificateUrlValue” för typ ”sträng” i parameteravsnittet i mallen. Du kan kopiera följande kodavsnitt och lägga till den i mallen. Beroende på källan till mallen kanske du redan dessa definieras om därför gå vidare till nästa steg. 
...
**Steg 3:** göra ändringar i den **Microsoft.ServiceFabric/clusters** resurs, leta upp resursdefinitionen ”Microsoft.ServiceFabric/clusters” i mallen. Under Egenskaper för den definitionen av du hittar ”certifikat” JSON-tagg som ska se ut ungefär som följande kodavsnitt i JSON:
...
Om du vill **förnyelse cert**, ange det nya certifikatet som primär och flytta den aktuella primärt som sekundär. Detta resulterar i förnyelsen av ditt aktuella primära certifikat till det nya certifikatet i ett steg i distributionen.
...
**Steg 4:** göra ändringar i **alla** den **Microsoft.Compute/virtualMachineScaleSets** resursdefinitionerna - hitta Microsoft.Compute/virtualMachineScaleSets resursdefinitionen. Bläddra till ”publisher”: ”Microsoft.Azure.ServiceFabric” under ”virtualMachineProfile”.

Du bör se ut så här i service fabric publisher inställningar.
...
Om du vill **förnyelse cert**, ange det nya certifikatet som primär och flytta den aktuella primärt som sekundär. Detta resulterar i förnyelsen av ditt aktuella certifikat till det nya certifikatet i ett steg i distributionen. 
...
**Steg 5:** göra ändringar i **alla** den **Microsoft.Compute/virtualMachineScaleSets** resursdefinitionerna - hitta Microsoft.Compute/virtualMachineScaleSets resursdefinitionen. Bläddra till ”vaultCertificates”:, under ”OSProfile”. Det bör se ut ungefär så här.
...
> [!NOTE]
> Kontrollera att du har upprepas steg 4 och 5 för alla definitioner för Nodetypes/Microsoft.Compute/virtualMachineScaleSets-resurs i mallen. Om du missar någon av dem får certifikatet inte installeras på att VMSS och du kan få oväntade resultat i klustret, inklusive klustret gå (om du får inga giltiga certifikat som klustret kan använda för säkerhet. Så kontrollera double, innan du fortsätter.
> 
> 
...
Du kan använda samma steg som beskrivs i steg 5 ovan för att få certifikat distribueras från en keyvault till noder. Du behöver bara ange och använda olika parametrar.
...
1. Navigera till bladet för säkerhet och markera den ”+ autentisering” knappen ovanpå bladet säkerhet.
2. Välj 'Autentiseringstypen' - 'skrivskyddad klient- eller Admin-klienten på bladet Lägg till autentisering
...
Ta bort ett sekundärt certifikat används för klustret säkerhet, navigera till bladet för säkerhet och väljer alternativet ”Ta bort' snabbmenyn på specifika certifikat.
