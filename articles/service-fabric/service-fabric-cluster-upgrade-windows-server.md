...
Om du vill ange att klustret ska hämta uppdateringar av Service Fabric när Microsoft släpper en ny version, anger du `fabricClusterAutoupgradeEnabled` kluster konfigurationen till *True*. Om du vill välja en version av Service Fabric som du vill att klustret ska vara på manuellt, ställer du in `fabricClusterAutoupgradeEnabled` kluster konfigurationen på *falskt*.
