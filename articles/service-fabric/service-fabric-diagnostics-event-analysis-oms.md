...
I `Summary`visas paneler i form av en graf för var och en av de lösningar som är aktiverade, inklusive en för Service Fabric. Fortsätt till Service Fabric-analys-lösningen genom att klicka på **Service Fabric** graf.
...
Pålitliga aktörs händelser kan visas på liknande sätt. Om du vill konfigurera mer detaljerade händelser för tillförlitliga aktörer måste du ändra `scheduledTransferKeywordFilter` i konfig for Diagnostic-tillägget (visas nedan). Information om värdena för dessa finns i [händelse referens för pålitliga aktörer](service-fabric-reliable-actors-diagnostics.md#keywords).
