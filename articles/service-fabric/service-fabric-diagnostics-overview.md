...
* Service Fabric fungerar korrekt, när det gäller att placera dina program och nätverksbelastning lösning klustret? 
* Användarens åtgärder vidtas på ditt kluster bekräftas och köras på som förväntat? T.ex. Skalning, växling vid fel, distributioner
* Är Service Fabric att hålla reda på vilka noder som ingår i klustret och informera mig när ett problem i något?
...
Händelserna skickas via standard-kanaler i både Windows och Linux och kan läsas av alla verktyg som stöder dessa. Azure Monitor-lösningen är Log Analytics. Gärna veta mer om vår [Log Analytics-integrering](service-fabric-diagnostics-event-analysis-oms.md) som innehåller en anpassad operativa instrumentpanel för ditt kluster och vissa exempelfrågor som du kan skapa aviseringar. Fler begrepp för kluster-övervakning finns på [genereringen av plattform på händelse- och log](service-fabric-diagnostics-event-generation-infra.md).
...
Azure Monitor-lösningen för att samla in dessa är Log Analytics som plattform på övervakning. Du bör använda den [Log Analytics-agenten](service-fabric-diagnostics-oms-agent.md) att samla in lämpliga prestandaräknarna och visa dem i Log Analytics.
...
* Klusterövervakning med [Diagnostikagenten](service-fabric-diagnostics-event-aggregation-wad.md) och [Log Analytics](service-fabric-diagnostics-oms-setup.md)
* Övervakning av infrastruktur med [Log Analytics](service-fabric-diagnostics-oms-agent.md)

Du kan också använda och ändra exemplet ARM-mallen finns [här](service-fabric-diagnostics-oms-setup.md#deploy-log-analytics-with-azure-resource-manager) att automatisera distributionen av alla nödvändiga resurser och agenter. 
...
* Konfigurera Log Analytics-integrering med Service Fabric på [konfigurera Log Analytics för ett kluster](service-fabric-diagnostics-oms-setup.md)
* Lär dig hur du ställer in Log Analytics för övervakning av behållare – [övervakning och diagnostik för Windows-behållare i Azure Service Fabric](service-fabric-tutorial-monitoring-wincontainers.md).
