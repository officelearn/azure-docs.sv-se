...
1. Kontrollera att du har den senaste Eclipse **Neon**-versionen och den senaste Buildship-versionen (1.0.17 eller senare) installerat. Du kan kontrollera vilka versioner de installerade komponenterna har genom att välja ``Help => Installation Details``. Du kan uppdatera Buildship genom att följa instruktionerna [här][buildship-update]. Om du vill kontrollera och uppdatera om Eclipse Neon-versionen är den senaste går du till ``Help => Check for Updates``.

2. För att installera Service Fabric-plugin-programmet väljer du ``Help => Install New Software...``.
...
4. Ange tjänstens namn och/eller relevanta uppgifter om tjänsten på den här sidan och klicka på ``Finish``.

    ![Service Fabric, ny projektsida 4][create-application/p4]

5. När du skapar ditt första Service Fabric-projekt får du frågan om du vill ställa in Service Fabric-perspektivet. Välj ``yes`` för att fortsätta.
...
* Högerklicka på Service Fabric-programmet du just skapade ovan. Välj alternativ ``Service Fabric`` på snabbmenyn. Nu visas en undermeny med flera alternativ. som ser ut som –
  ...
  - ``Build Application`` skapar programmet utan rensning
  - ``Rebuild Application`` gör en rensad version av programmet
  - ``Clean Application`` rensar programmet på skapade artefakter


* Du kan välja att distribuera, ta bort eller publicera programmet på den här menyn.
  - ``Deploy Application`` distribuerar till ditt lokala kluster
  - ``Publish Application...`` frågar vilken publiceringsprofil du vill välja mellan ``Local.json`` och ``Cloud.json``. De här JSON-filerna används för att lagra information (som anslutningsslutpunkter och säkerhetsinformation) som krävs för att ansluta till ett lokalt kluster eller ett molnkluster (Azure).
...
* Du kan distribuera Service Fabric-programmet på ett annat sätt med Eclipse Run Configurations.
  ...
  1. Välj ``Run => Run Configurations``. Markera ``ServiceFabricDeployer``Run configuration (Kör konfiguration) under ``Grade Project``.
  2. Under fliken ``Arguments`` i den högra rutan anger du **lokal** eller **moln** som ``publishProfile``. Standardinställningen är **lokal**. Om du distribuerar till ett fjärr-/molnkluster väljer du **moln**.
  ...
  4. Kontrollera att ``Working Directory`` på den högra rutan under ``Grade Project`` pekar på programmet du vill distribuera. Om inte klickar du bara på knappen ``Workspace...`` och väljer önskat program.
...
1. Högerklicka på projektet du vill lägga till en tjänst till för att öppna snabbmenyn och välja alternativet ”Service Fabric”. Nu visas en undermeny med flera alternativ.

    ![Service Fabric, lägg till tjänst, sida 1][add-service/p1]

2. Om du väljer alternativet `Add ServiceFabric Service` blir du guidad genom nästa uppsättning steg för att lägga till en tjänst till projektet.
...
1. Välj ``Run => Run Configurations``. Klicka på den lilla pilen till vänster som ``Grade Project`` i den vänstra rutan.
2. Högerklicka på ``ServiceFabricDeployer`` och välj ``Duplicate``. Ge konfigurationen ett namn, till exempel ``ServiceFabricUpgrader``.
3. Under fliken ``Arguments`` i den högra rutan ändrar du ``-Pconfig='deploy'`` till ``-Pconfig=upgrade`` och klickar på ``Apply``.
4. Nu har du skapat och sparat en körningskonfiguration för att uppgradera programmet, som du kan ``Run`` när du vill. Då får du den senast uppdaterade programtypversionen från programmanifestfilen.
