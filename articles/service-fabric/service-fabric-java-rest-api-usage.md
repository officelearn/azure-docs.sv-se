...
5. Kör följande AutoRest-kommando för att generera Java-klient koden.
   ...
   Följande kommando tar ``servicefabric.json`` Specifikations filen som indata och genererar Java-klientens kod i ``java-rest-api-     code`` mapp och omsluter koden i ``servicefabricrest`` namn område. Efter det här steget hittar du två mappar ``models``, ``implementation`` och två filer ``ServiceFabricClientAPIs.java`` och ``package-info.java`` genereras i ``java-rest-api-code``-mappen.
...
För varje API kommer du att se fyra överlagringar av implementeringen. Om det finns valfria parametrar kan du se fyra fler variationer, inklusive de valfria parametrarna. Överväg till exempel API-``removeReplica``.
 ...
 3. **offentligt observerbar\<annullera > removeReplicaAsync (String nodnamn, UUID partitionId, String replicaId)**
