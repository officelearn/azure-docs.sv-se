...
* Se till att `IComparable<TKey>` implementationen är korrekt. Systemet använder beroende av `IComparable<TKey>` för att sammanfoga kontroll punkter och rader.
...
* Standardtoken för annullering är `CancellationToken.None` i alla Reliable Collection-API: er.
* Nyckel typs parametern (*TKey*) för en tillförlitlig ord lista måste implementera `GetHashCode()` och `Equals()`korrekt. Nycklar måste vara oföränderliga.
