...
* Beständiga: data sparas på disk för tålighet mot storskaliga avbrott (till exempel ett data centers strömavbrott).
...
* Inga out-parametrar: använder `ConditionalValue<T>` för att returnera ett `bool` och ett värde i stället för parametrar. `ConditionalValue<T>` är som `Nullable<T>` men behöver inte vara en STRUCT.
