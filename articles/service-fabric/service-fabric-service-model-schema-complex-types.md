...
Om behållaren har hälsokontroll aktiverad och docker rapporter health_status händelsen för den här behållaren, Service Fabric tas detta som en del av system rapporterade hälsa. När health_status är felfri rapporterar Service Fabric en varning om hälsa. Som standard är den inställd på true.
|Attribut|Värde|
|---|---|
...
Om behållaren har hälsokontroll aktiverad och docker rapporter health_status händelsen för den här behållaren, Service Fabric startar om behållaren när health_status som rapporterats av docker är felfri.
Som standard anges till false.
|Attribut|Värde|
|---|---|
...
Isoleringsläge för behållaren. Giltiga värden är standard, process eller hyperv (stöds bara för Windows-behållare) på Windows 10-värdar, bara Hyper-V-isoleringsläge stöds, så behållaren som körs i Hyper-V-isoleringsläge oavsett dess inställningen för läge för isolering.
|Attribut|Värde|
|---|---|
|namn|Isolering|
|Använd|valfri|
...
Källmappen som kan vara en mapp på den virtuella datorn som är värd för behållarna eller en fjärransluten beständiga arkivet.
|Attribut|Värde|
|---|---|
|namn|Källa|
|Använd|obligatorisk|
...
Den plats där källan är mappad till i behållaren körs. Därför måste ditt mål för en plats som redan finns i en behållare.
|Attribut|Värde|
|---|---|
|namn|Mål|
|Använd|obligatorisk|
...
#### <a name="driver"></a>Drivrutin
Drivrutinsnamn för plugin-programmet Azure Files volym är ”sfazurefile”.
|Attribut|Värde|
|---|---|
|namn|Drivrutin|
|Använd|valfri|
...
Drivrutiner som ska skickas till drivrutinen. Azure Files volym plugin-programmet stöder följande alternativ: resursnamn (Azure Files filresursen som innehåller volymen för behållaren), storageAccountName (Azure storage-kontot som innehåller filresursen Azure-filer), storageAccountKey ( Åtkomstnyckeln för Azure storage-konto som innehåller filresursen Azure-filer).
...
Namnet på den slutpunkt som måste deklareras i avsnittet resurser i service manifest.  När du använder HTTPS, Använd inte samma port och certifikat för olika tjänstinstanser (oberoende av programmet) distribueras till samma nod. Uppgradera två olika tjänster som använder samma port i annat fall leder en uppgraderar.
|Attribut|Värde|
|---|---|
|namn|EndpointRef|
...
#### <a name="os"></a>OS
Skapa version av Operativsystemet. Versionen av Windows Server version 1709 är till exempel 16299.
|Attribut|Värde|
|---|---|
|namn|OS|
|typ|Xs:String|
...
Begränsar de resurser som kan användas på värden och deklarerar gränserna för ett paket för service-kod.
...
Minnesgränserna i MB. Måste vara ett positivt heltal. Kodpaket (behållare eller processer) kan inte tilldela mer minne än den här gränsen, och försök att göra detta leder till undantag utanför minnet.
|Attribut|Värde|
|---|---|
|namn|MemoryInMB|
|typ|Xs:String|
...
Den totala mängden minne i byte som kan användas i MB. Måste vara ett positivt heltal.
|Attribut|Värde|
|---|---|
|namn|MemorySwapInMB|
|typ|Xs:String|
...
Mjuka gräns (i MB) för styrning av minne som används endast när konkurrens om minne har identifierats på noden. Måste vara ett positivt heltal.
|Attribut|Värde|
|---|---|
...
Användbar procentandelen tillgängliga processorer (endast Windows). Måste vara ett positivt heltal. Den här parametern ignoreras effektivt om CPU-gränser har angetts för tjänstepaketet.
|Attribut|Värde|
|---|---|
|namn|CpuPercent|
|typ|Xs:String|
...
Maximala i/o hastighet (läsning och skrivning) som IOps som kan användas. Måste vara ett positivt heltal.
|Attribut|Värde|
|---|---|
|namn|MaximumIOps|
|typ|Xs:String|
...
Block-i/o vikt i förhållande till andra paket i koden. Måste vara ett positivt heltal mellan 10 och 1000.
|Attribut|Värde|
|---|---|
|namn|BlockIOWeight|
|typ|Xs:String|
...
Filen ServiceManifest kontrollsummevärde.
|Attribut|Värde|
|---|---|
...
Beskriver anpassade inställningar för en ServiceComponent eller ett program. Det består av ett eller flera avsnitt med nyckel-värdepar.
