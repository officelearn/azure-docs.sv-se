...
Anger information om X509 certifikat att utsättas för behållaren-miljö. Certifikatet måste vara installerad i arkivet LocalMachine på alla klusternoder.
När programmet startas körningsmiljön läser certifikatet och genererar en PFX-fil och lösenord (Windows) eller en PEM-fil (på Linux).
PFX-filen och lösenordet är tillgängliga i behållaren med hjälp av miljövariablerna Certificates_ServicePackageName_CodePackageName_CertName_PFX och Certificates_ServicePackageName_CodePackageName_CertName_Password. PEM-filen är tillgänglig i behållaren med hjälp av miljövariablerna Certificates_ServicePackageName_CodePackageName_CertName_PEM och Certificates_ServicePackageName_CodePackageName_CertName_PrivateKey.
...
Standard admin rollen klientcertifikatet används för att säkra kommunikationen mellan klient-server.
...
|typ|[endpointType](service-fabric-service-model-schema-complex-types.md#endpointtype-complextype)|
|innehåll|0-element, 0 attribut|
|definierats|lokalt i slutpunkter element, DigestedEndpoint element|
|namn|Slutpunkt|
|maxOccurs|unbounded|
...
## <a name="entrypoint-element-type-xsstring"></a>EntryPoint element (typen xs:string) 
|Attribut|Värde|
|---|---|
|typ|Xs:String|
|innehåll|0-element, 0 attribut|
...
Behållaren bilden motsvarar OS build-versionsnummer som ska startas. Om Os-attributet inte anges behållaren avbildningen antas för att fungera i alla versioner av Operativsystemet och åsidosätter den avbildning som angetts i tjänstmanifestet.
...
Lagringsplatsen och bild på https://hub.docker.com eller Azure-behållare registret.
...
Windows Server-behållare får inte vara kompatibla mellan olika versioner av Operativsystemet.  Du kan ange flera operativsystemsavbildningar per behållare och tagga dem med build-versioner av Operativsystemet. Hämta OS-versionen genom att köra ”winver” i en kommandotolk i Windows. Om det underliggande Operativsystemet är version version 16299 (Windows Server version 1709), Service Fabric hämtar behållaren avbildningen märkta med Os = ”16299”. En behållare för ej taggade avbildning antas fungera över alla versioner av Operativsystemet och åsidosätter den avbildning som angetts i tjänstmanifestet.
...
Användare kan läggas till en befintlig medlemskapsgrupp, så den kan ärvas alla egenskaper och säkerhetsinställningar för i Medlemskapsgruppen. I Medlemskapsgruppen kan användas för att skydda externa resurser som ska användas av olika tjänster eller samma tjänst (på en annan dator).
...
|definierats|lokalt i nodetypes får element|
|namn|NodeType|
|maxOccurs|unbounded|
...
## <a name="parameter-element-defined-in-section"></a>Parameterelement (anges i punkt) 
...
|typ|anonym complexType|
|innehåll|0-element, 1 attribut|
|definierats|lokalt i avsnittselement|
|namn|Parameter|
|minOccurs|0|
|maxOccurs|unbounded|
...
Om värdet är true är värdet för den här parametern krypterad. Programutvecklaren ansvarar för att skapa ett certifikat och använda cmdleten Invoke-ServiceFabricEncryptSecret för att kryptera känslig information. Information för certifikat som används för att kryptera värdet har angetts i avsnittet certifikat.
|Attribut|Värde|
|---|---|
|namn|isEncrypted|
|typ|Xs:Boolean|
...
## <a name="policies-element-type-applicationpoliciestype"></a>Principer för element (typ ApplicationPoliciesType) 
...
## <a name="policies-element-type-servicemanifestimportpoliciestype"></a>Principer för element (typ ServiceManifestImportPoliciesType) 
...
Anger vilken resurs som slutpunkten ska bindas till exponerade behållaren port.
...
Anger gränserna för ett kodpaket.

|Attribut|Värde|
|---|---|
|typ|[ResourceGovernancePolicyType](service-fabric-service-model-schema-complex-types.md#resourcegovernancepolicytype-complextype)|
|innehåll|0-element, 0 attribut|
|definierats|lokalt i [ServiceManifestImportPoliciesType complexType](service-fabric-service-model-schema-complex-types.md#servicemanifestimportpoliciestype-complextype), DigestedCodePackage element, DigestedEndpoint-element|
|namn|ResourceGovernancePolicy|
|minOccurs|0|
...
Beskriver resurs åsidosättningar för slutpunkter i service manifest resurser.
...
## <a name="roles-element-defined-in-windowsazure"></a>Roller element (definieras i WindowsAzure) 
|Attribut|Värde|
|---|---|
|typ|anonym complexType|
|innehåll|1-element, 0 attribut|
|definierats|lokalt i WindowsAzure element|
|namn|Roller|
...
## <a name="section-element-defined-in-settingsoverridestype"></a>Avsnittselement (definieras i SettingsOverridesType) 
...
## <a name="section-element-defined-in-settingstype"></a>Avsnittselement (definieras i SettingsType) 
En användardefinierad namnet avsnitt.
...
|typ|[SecurityAccessPolicyType](service-fabric-service-model-schema-complex-types.md#securityaccesspolicytype-complextype)|
|innehåll|0-element, 0 attribut|
|definierats|lokalt i [ServiceManifestImportPoliciesType complexType](service-fabric-service-model-schema-complex-types.md#servicemanifestimportpoliciestype-complextype), SecurityAccessPolicies element, DigestedEndpoint-element|
|namn|SecurityAccessPolicy|
|minOccurs|0|
...
## <a name="settings-element-type-settingsoverridestype"></a>Inställningar för element (typ SettingsOverridesType) 
Förklarar konfigurationsinställningar i en tjänstmanifestet ska åsidosättas. Det består av ett eller flera avsnitt med nyckel-värdepar. Parametervärden kan krypteras med hjälp av cmdleten Invoke-ServiceFabricEncryptSecret.
...
## <a name="settings-element-type-settingstype"></a>Inställningar för element (typ SettingsType) 
...
En privilegierad startpunkt som standard körs med samma autentiseringsuppgifter som Service Fabric (vanligtvis kontot NetworkService) innan andra startpunkt. Förekomsten av en separat installationsprogrammet startpunkt slipper du köra tjänstvärden med höga privilegier för längre tid. I programmanifestet, kan du ändra säkerhetsbehörigheter för att kunna köra startskriptet under ett lokalt systemkonto eller ett administratörskonto.
...
|typ|[StatefulServiceType](service-fabric-service-model-schema-complex-types.md#statefulservicetype-complextype)|
|innehåll|0-element, 0 attribut|
|definierats|lokalt i [ServiceTemplatesType complexType](service-fabric-service-model-schema-complex-types.md#servicetemplatestype-complextype), tjänstelementet|
|namn|StatefulService|
...
Standard användaren rollen klientcertifikatet används för att säkra kommunikationen mellan klient-server.
