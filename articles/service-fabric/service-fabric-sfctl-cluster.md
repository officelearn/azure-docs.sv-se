...
| --ca | Absolut sökväg till katalogen CA-certifikat för att behandla som giltig eller CA-paketfil. Om du använder en katalog med CA- `c_rehash <directory>` certifikat som tillhandahålls av OpenSSL måste köras först för att beräkna certifikat-hashar och skapa lämpliga symboliska länkar. Detta används för att kontrol lera att certifikatet som returneras av klustret är giltigt. |
