...
Om du ansluter till säkert kluster, kan du ange en absolut sökväg till ett certifikat (.crt) och nyckelfilen (.key) eller en enda fil med både (.pem). Ange inte båda. Du kan också om ansluta till ett säkert kluster kan även ange en absolut sökväg till en CA paket fil eller katalog för betrodda CA-certifikat. Om du använder en katalog med CA-certifikat, `c_rehash <directory>` tillhandahålls av OpenSSL måste köras först för att beräkna certifikatet-hashar och skapa lämpliga symbolics länkar.
