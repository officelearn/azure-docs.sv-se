---
title: Lär dig mer om Azure Service Fabric terminologi | Microsoft Docs
...
**Nod**: En dator eller en virtuell dator som ingår i ett kluster kallas för en *nod*. Varje nod har tilldelats ett nodnamn (en sträng). Noder har egenskaper, till exempel placerings egenskaper. Varje dator eller virtuell dator har en Windows-tjänst som startar `FabricHost.exe`automatiskt, som börjar köras vid start och sedan startar två körbara `Fabric.exe` filer `FabricGateway.exe`: och. Dessa två körbara filer utgör noden. För testnings scenarier kan du vara värd för flera noder på en enskild dator eller virtuell dator genom att `Fabric.exe` köra `FabricGateway.exe`flera instanser av och.

## <a name="application-and-service-concepts"></a>Program-och tjänst koncept

**Service Fabric nätprogram**: Service Fabric nätappar beskrivs av resurs modellen (YAML-och JSON-resursfiler) och kan distribueras till alla miljöer där Service Fabric körs.
...
**Nätverk**: En nätverks resurs skapar ett privat nätverk för dina program och är oberoende av de program eller tjänster som kan referera till den. Flera tjänster från olika program kan vara en del av samma nätverk. Nätverk är distributions bara resurser som refereras till av program.
...
**Repair Manager tjänst**: Detta är en valfri system tjänst som gör det möjligt att utföra reparations åtgärder på ett kluster på ett sätt som är säkert, kan automatiseras och transparent. Reparations hanteraren används i:
...
**Behållare**:  Service Fabric stöder distribution av Docker-behållare på Linux-och Windows Server-behållare på Windows Server 2016, tillsammans med stöd för isolerings läget för Hyper-V. I Service Fabric [program modellen](service-fabric-application-model.md)representerar en behållare en program värd där flera tjänst repliker placeras. Service Fabric kan köra alla behållare, och scenariot liknar scenariot för gäst körning, där du paketerar ett befintligt program i en behållare. Dessutom kan du [köra Service Fabric tjänster i behållare](service-fabric-services-inside-containers.md) också.

**Körbara gäst program**: Du kan köra vilken typ av kod som helst, till exempel Node. js, python, Java C++ eller Azure Service Fabric som en tjänst. Service Fabric avser de här typerna av tjänster som körbara gäst program, som behandlas som tillstånds lösa tjänster. Fördelarna med att köra en körbar gäst fil i ett Service Fabric kluster är hög tillgänglighet, hälso övervakning, hantering av program livs cykel, hög densitet och identifiering.
...
Service Fabric är en plattforms teknik med öppen källkod som flera olika tjänster och produkter baseras på. Microsoft tillhandahåller följande alternativ:
 ...
 - **Azure Service Fabric**: Azure Hosted Service Fabric Cluster-erbjudandet. Den ger integration mellan Service Fabric och Azure-infrastrukturen, tillsammans med uppgradering och konfigurations hantering av Service Fabric-kluster.
