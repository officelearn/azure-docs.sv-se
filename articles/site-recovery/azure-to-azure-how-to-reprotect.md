...
2. Granska resurs grupp, nätverk, lagring och tillgänglighets uppsättningar. Klicka sedan på **OK**. Om det finns resurser som marker ATS som nya skapas de som en del av återställnings processen.
...
4. Endast ändringar mellan käll disken och mål disken synkroniseras. Skillnaderna beräknas genom att jämföra både diskarna och sedan överföras. Se nedan för att hitta den beräknade tiden för att slutföra återskyddet.
5. När synkroniseringen är klar börjar delta-replikeringen och en återställnings punkt skapas i linje med replikeringsprincipen.
...
4. När synkroniseringen är klar börjar delta-replikeringen och en återställnings punkt skapas i linje med replikeringsprincipen.
...
1.  Om de virtuella käll datorerna tas bort, skadas eller inte går att komma åt på grund av någon anledning, t. ex. en resurs grupp ändring/ta bort, sker en fullständig Site Recovery när det inte finns några tillgängliga data i käll regionen som ska användas.
2.  Om de virtuella käll datorerna är tillgängliga, beräknas bara differentiella genom att jämföra diskarna och sedan överföra dem. Kontrol lera tabellen nedan för att få den uppskattade tiden.

|Exempel situation | Åtgången tid för att skydda |
|---|---|
|Käll regionen har 1 virtuell dator med 1 TB standard disk<br/>– Endast 127 GB data används och resten av disken är tom<br/>– Disktyp är standard med 60 MiB/S-genomflöde<br/>-Ingen data ändring efter redundans| Ungefärlig tid: 45 minuter – 1,5 timmar<br/> – Under omskyddet fyller Site Recovery i kontroll summan för alla data som tar 127 GB/45 MB ~ 45 minuter<br/>– Viss omkostnader krävs för att Site Recovery automatisk skalning: ca. 20-30 minuter<br/>– Inga utgående avgifter |
|Käll regionen har 1 virtuell dator med 1 TB standard disk<br/>– Endast 127 GB data används och resten av disken är tom<br/>– Disktyp är standard med 60 MiB/S-genomflöde<br/>– 45 GB data ändringar efter redundans| Ungefärlig tid: 1 timmar – 2 timmar<br/>– Under omskyddet fyller Site Recovery i kontroll summan för alla data som tar 127 GB/45 MB ~ 45 minuter<br/>– Överförings tid för att tillämpa ändringar på 45 GB som är 45 GB/45 Mbit/s ~ 17 minuter<br/>-Utgående kostnader skulle vara för 45 GB data ändringar, inte för kontroll summan |
 
## <a name="next-steps"></a>Nästa steg

När den virtuella datorn är skyddad kan du starta en redundansväxling. Redundansväxlingen stänger den virtuella datorn i den sekundära regionen och skapar och startar om den virtuella datorn i den primära regionen, med lite stillestånds tid under den här processen. Vi rekommenderar att du väljer lämplig tid för den här processen och att du kör ett redundanstest innan du påbörjar en fullständig redundans till den primära platsen. [Läs mer](site-recovery-failover.md) om Azure Site Recovery redundans.
