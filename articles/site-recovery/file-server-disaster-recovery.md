...
- [Skydda Azure IaaS-filservrar](#disaster-recovery-recommendation-for-azure-iaas-virtual-machines)
- [Skydda lokala filservrar](#replicate-an-onpremises-file-server-using-azure-site-recovery)
...
Ger en grafiska representation syftar övergång ut beslutet av vilken strategi för din servermiljö och filen under diagrammet.

![beslutsträdet](media/site-recovery-file-server/decisiontree.png)


### <a name="factors-to-consider-while-making-disaster-recovery-decision"></a>Faktorer att tänka på när du fattar beslut för disaster recovery
...
**Plats-till-plats-anslutning**: direkt anslutning mellan lokal plats och Azure-nätverk måste införas för att tillåta kommunikation mellan servrar.  Detta kan säkerställas genom en säker plats-till-plats VPN-anslutning till ett Windows Azure-nätverk som ska användas som DR-plats.  
...
Om du konfigurerar och hanterar katastrofåterställning av filservrar som finns på virtuella Azure IaaS-datorer kan du välja mellan två alternativ baserat på om du vill flytta till [Azure Files](https://docs.microsoft.com/azure/storage/files/storage-files-introduction).
...
**Azure Files** kan användas för att helt ersätter eller kompletterar traditionella lokala filservrar eller NAS-enheter. Azure-filresurser kan också replikeras med Azure-filsynkronisering till Windows-servrar, antingen lokalt eller i molnet, för högpresterande och distribuerad cachelagring av data där den används. Detaljerat DR-rekommendation för Azure virtuella datorer som utför samma funktion som traditionella filservrar nedanstående steg:
1.  Skydda datorer med hjälp av Azure Site Recovery med hjälp av stegen som nämns här
2.  Använd Azure filsynkronisering replikera filer från den virtuella datorn som fungerar som en filserver till molnet.
3.  Använda Azure Site Recovery återställning plan för att lägga till skript till [montera filresursen Azure](https://docs.microsoft.com/azure/storage/files/storage-how-to-use-files-windows) och få åtkomst till resursen i den virtuella datorn.

De beskrivs kortfattat nedanstående steg för hur du använder Azure filen Sync-tjänsten:

1. [Skapa ett lagringskonto i Azure](https://docs.microsoft.com/azure/storage/common/storage-create-storage-account?toc=%2fazure%2fstorage%2ffiles%2ftoc.json). Om du väljer geo-redundant lagring med läsbehörighet (RA-GRS) för dina lagringskonton får du tillgång till läsåtkomst till dina data från den sekundära regionen vid en katastrof. Mer information finns i [strategi för Azure File share katastrofåterställning](https://docs.microsoft.com/azure/storage/common/storage-disaster-recovery-guidance?toc=%2fazure%2fstorage%2ffiles%2ftoc.json) för ytterligare information.
...
6.  Vid katastrofåterställning i din lokala miljö, utföra som redundans med en återställningsplan och lägga till skript till [montera filresursen Azure](https://docs.microsoft.com/azure/storage/files/storage-how-to-use-files-windows) och få åtkomst till resursen i den virtuella datorn.
...
Om du har lokala klienter som ansluter till den virtuella datorn på filen utföra IaaS första 2 stegen, samt annan gå vidare till steg 3.
...
Den under steg detalj replikering för en VMware-VM steg för att replikera en virtuell dator i Hyper-V, finns här.
...
1. [Skapa ett lagringskonto i Azure](https://docs.microsoft.com/en-us/azure/storage/common/storage-create-storage-account?toc=%2fazure%2fstorage%2ffiles%2ftoc.json). Om du väljer geo-redundant lagring med läsbehörighet (RA-GRS) (rekommenderas) för dina lagringskonton, kommer du har läsbehörighet till dina data från den sekundära regionen vid en katastrof. Mer information finns i [strategi för Azure File share katastrofåterställning](https://docs.microsoft.com/en-us/azure/storage/common/storage-disaster-recovery-guidance?toc=%2fazure%2fstorage%2ffiles%2ftoc.json) för ytterligare information.
...
6.  Utför som redundans med en återställningsplan vid en katastrof i din lokala miljö och Lägg till skript för att montera filresursen för Azure och få åtkomst till resursen i den virtuella datorn.
...
Mer information om hur du utför redundanstestet [här](site-recovery-failover.md).
