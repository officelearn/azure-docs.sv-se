...
Högerklicka på `app\java\<PackageName>` -> **ny**->**Java-klass**. Ange **namnet** _MyFirstApp_och **superklass** till _Android. app. Application_. Lämna de andra alternativen som de är. Klicka på **OK**. En fil som `MyFirstApp.java` heter kommer att skapas. Lägg till följande import:
...
Slutligen är det dags att koppla ihop allt. Lägg till `handleTap()` följande kod i metoden. Den anropar `uploadCloudAnchorAsync()` metoden så snart din sfär skapas. När metoden returnerar utför koden nedan en slutgiltig uppdatering av din sfär och ändrar färgen till blå.
