...
Hantering av arbetsbelastning ger dig möjlighet att optimera prestanda i alla frågor. En väl utformad arbetsbelastning körs frågor och läsa in åtgärder effektivt, oavsett om de är beräkningsintensiva eller i/o-intensiva. SQL Data Warehouse ger funktioner för hantering av arbetsbelastning för miljöer med flera användare. Ett informationslager är inte avsedd för arbetsbelastningar för flera innehavare.
...
- Dynamiska resursklasser som lämpar sig för datauppsättningar som växer i storlek och behöver bättre prestanda eftersom servicenivån skalas upp.
...
Resursklasser implementeras genom att tilldela användare till databasroller. När en användare kör en fråga körs frågan med användarens resursklass. Om en användare är medlem i rollen staticrc10, till exempel köras deras frågor med små mängder minne. Om en databasanvändare som är medlem av databasrollerna xlargerc eller staticrc80, kör deras frågor med stora mängder minne.

Använda för att öka en användares resursklass [sp_addrolemember](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/sp-addrolemember-transact-sql) att lägga till användaren till en databasroll i en stor resursklass.  Den lägger till en användare till rollen largerc under.  Varje begäran hämtar 22% av systemminnet.

```sql
EXEC sp_addrolemember 'largerc', 'loaduser';
```

Om du vill minska resursklassen använda [sp_droprolemember](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/sp-droprolemember-transact-sql).  Om 'loaduser' inte är en medlem eller andra resursklasser, går de du till standard smallrc resursklassen med en minnestilldelningen på 3%.  
...
Vi rekommenderar att du skapar en användare som är dedikerade för att köra en viss typ av fråga eller läsa in igen. Ger användaren en permanent resursklass i stället för att ändra resursklass regelbundet. Statiska resursklasser råd större övergripande kontroll på arbetsbelastningen, så vi föreslår att du använder statiska resursklasser innan dynamiska resursklasser.

### <a name="resource-classes-for-load-users"></a>Resursklasser för load-användare

`CREATE TABLE` använder klustrade columnstore-index som standard. Genom att komprimera data i ett columnstore index är en minneskrävande åtgärd och minnesbelastning kan minska index kvalitet. Minnesbelastning kan leda till att du behöver en högre resursklass vid inläsning av data. För att säkerställa belastningar har tillräckligt med minne, kan du skapa en användare som är avsedda att köra inläsningar och tilldela användaren till en högre resursklass.
...
- Använda en dynamisk resursklass när tabellen minneskrav varierar mycket. Inläsningar kan kräva mer minne än den aktuella DWU eller cDWU nivå ger. Skala informationslagret lägger till mer minne belastningen åtgärder, vilket gör inläsningar att utföra snabbare.

### <a name="resource-classes-for-queries"></a>Resursklasser för frågor

Vissa frågor är beräkningsintensiva och vissa inte.  
...
- Välj en statisk resursklass när resursen förväntningar varierar under dagen. Till exempel fungerar en statisk resursklass bra när datalagret efterfrågas av många personer. När skalning datalagret, ändras inte mängden minne som allokerats till användaren. Fler frågor kan därför köras parallellt på systemet.

Rätt minne beviljar beror på många faktorer, till exempel hur mycket data som efterfrågas, typen av tabellscheman, och olika kopplingar, markera och gruppera predikat. I allmänhet allokera mer minne kan frågor för att slutföra snabbare, men minskar den övergripande samtidigheten. Om samtidighet inte är ett problem, skadar över minnesallokering inte dataflöde.
...
Du kan använda följande angivna lagrade procedur för [Gen1](#stored-procedure-definition-for-gen1) eller [Gen2](#stored-procedure-definition-for-gen2)-för att ta reda på bevilja samtidighet och minne per resursklass vid en viss SLO och den bästa resursklassen för minne beräkningsintensiva CCI åtgärder för icke-partitionerad CCI tabellen i en viss resursklass:
...
2. För att se den bästa resursklassen för minnesintensiva CCI åtgärder (belastning, kopiera tabellen, återskapa indexet osv.) för icke-partitionerad CCI tabellen i en viss resursklass. Den lagrade proceduren använder tabellens schema för att ta reda på den obligatoriska minnestilldelningen.
...
- Den här lagrade proceduren tar emot inte minneskraven hänsyn för den VALDA delen av CTAS/INSERT-välja och förutsätter att det är en väljer.
...
- Den här lagrade proceduren beror på befintliga samtidighet gränsen erbjudanden och om de ändrar sedan den här lagrade proceduren fungerar inte korrekt.  
- Den här lagrade proceduren är beroende av befintlig klass resurserbjudanden och om de ändrar sedan den här lagrade proceduren fungerar inte korrekt.  

>[!NOTE]  
>Om du inte får utdata när du kör lagrad procedur med parametrar som tillhandahålls, kan kommer det finnas två fall.
>
>1. Antingen DW-parametern innehåller ett ogiltigt värde för Servicenivåmål
>2. Eller, det finns inga matchande resursklassen för CCI åtgärden för tabellen.
>
>Till exempel på DW100, den högsta minnestilldelningen är 400 MB, och om tabellschemat är tillräckligt bred för att mellan krav på 400 MB.
...
### <a name="stored-procedure-definition-for-gen1"></a>Lagrad Procedurdefinition för Gen1
...
### <a name="stored-procedure-definition-for-gen2"></a>Lagrad Procedurdefinition för Gen2
