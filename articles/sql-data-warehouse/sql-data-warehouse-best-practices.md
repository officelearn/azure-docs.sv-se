...
Se även [Pausa beräkningsresurser][Pausa beräkningsresurser], [Återuppta beräkningsresurser][Återuppta beräkningsresurser], [Skala beräkningsresurser][Skala beräkningsresurser]
...
Se även [Förstå transaktioner][Förstå transaktioner], [Optimera transaktioner][Optimera transaktioner]
...
Se även [Hantera tabellstatistik][Hantera tabellstatistik], [SKAPA STATISTIK][SKAPA STATISTIK], [UPPDATERA STATISTIK][UPPDATERA STATISTIK]
...
Se även [Läsa in data][Läsa in data], [Guide för att använda PolyBase][Guide för att använda PolyBase], [Azure SQL Data Warehouse läsa in mönster och strategier][Azure SQL Data Warehouse läsa in mönster och strategier], [Läsa in Data med Azure Data Factory][Läsa in Data med Azure Data Factory], [Flytta data med Azure Data Factory][Flytta data med Azure Data Factory], [SKAPA EXTERNA FILFORMAT][SKAPA EXTERNA FILFORMAT], [Skapa en tabell som Välj (CTAS)][Skapa en tabell som Välj (CTAS)]
...
Se även [Guide för att använda PolyBase][Guide för att använda PolyBase]
...
Se även [Tabellöversikt][Tabellöversikt], [Tabelldistribution][Tabelldistribution], [Välja tabelldistribution][Välja tabelldistribution], [CREATE TABLE][CREATE TABLE], [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]

## <a name="do-not-overpartition"></a>Överpartitionera inte
...
Se även [Tabellpartitionering][Tabellpartitionering]
...
Se även [Förstå transaktioner][Förstå transaktioner], [Optimera transaktioner][Optimera transaktioner], [Tabellpartitionering][Tabellpartitionering], [TRUNKERA TABELL][TRUNKERA TABELL], [ÄNDRA TABELL][ÄNDRA TABELL], [Skapa en tabell som Välj (CTAS)][Skapa en tabell som Välj (CTAS)]
...
Se även [Tabellöversikt][Tabellöversikt], [Tabelldatatyper][Tabelldatatyper], [SKAPA TABELL][SKAPA TABELL]
...
Se även [Tabellindex][Tabellindex], [Guide för kolumnlagringsindex][Guide för kolumnlagringsindex], [Återskapa kolumnlagringsindex ][Återskapa kolumnlagringsindex ]
...
Se även [Hantering av samtidighet och arbetsbelastning][Hantering av samtidighet och arbetsbelastning]
...
Se även [Hantering av samtidighet och arbetsbelastning][Hantering av samtidighet och arbetsbelastning], [sys.dm_pdw_waits][sys.dm_pdw_waits]
...
Se även [Övervaka din arbetsbelastning med DMV:er][Övervaka din arbetsbelastning med DMV:er], [ETIKETT][ETIKETT], [ALTERNATIV][ALTERNATIV], [sys.dm_exec_sessions][sys.dm_exec_sessions], [sys.dm_pdw_exec_requests][sys.dm_pdw_exec_requests], [sys.dm_pdw_request_steps][sys.dm_pdw_request_steps], [sys.dm_pdw_sql_requests][sys.dm_pdw_sql_requests], [sys.dm_pdw_dms_workers], [DBCC PDW_SHOWEXECUTIONPLAN][DBCC PDW_SHOWEXECUTIONPLAN], [sys.dm_pdw_waits][sys.dm_pdw_waits]

## <a name="other-resources"></a>Andra resurser
Information om vanliga problem och lösningar finns i vår artikel [Felsökning][Felsökning].

Om du inte hittar det du letar efter i den här artikeln provar du att använda ”Sök efter dokument” till vänster på den här sidan för att söka igenom alla Azure SQL Data Warehouse-dokument.  [Azure SQL Data Warehouse-forumet på MSDN][Azure SQL Data Warehouse-forumet på MSDN] är en plats där du kan ställa frågor till andra användare och till SQL Data Warehouse-produktgruppen.  Vi övervakar aktivt detta forum för att kontrollera att dina frågor besvaras antingen av en annan användare eller av någon av oss.  Om du föredrar att ställa dina frågor i Stack Overflow finns det även ett [Stack Overflow-forum för Azure SQL Data Warehouse][Stack Overflow-forum för Azure SQL Data Warehouse].
...
[Concurrency and workload management]: ./sql-data-warehouse-develop-concurrency.md (Hantering av samtidiga körningar och arbetsbelastningar)
[Create table as select (CTAS)]: ./sql-data-warehouse-develop-ctas.md
[Table overview]: ./sql-data-warehouse-tables-overview.md (Tabellöversikt)
[Table data types]: ./sql-data-warehouse-tables-data-types.md (Datatyper för tabeller)
[Table distribution]: ./sql-data-warehouse-tables-distribute.md (Tabelldistribution)
[Table indexes]: ./sql-data-warehouse-tables-index.md (Tabellindex)
[Causes of poor columnstore index quality]: ./sql-data-warehouse-tables-index.md#causes-of-poor-columnstore-index-quality (Orsaker till låg columnstore-indexkvalitet)
[Rebuilding columnstore indexes]: ./sql-data-warehouse-tables-index.md#rebuilding-indexes-to-improve-segment-quality (Återskapa columnstore-index)
[Table partitioning]: ./sql-data-warehouse-tables-partition.md (Tabellpartitionering)
[Manage table statistics]: ./sql-data-warehouse-tables-statistics.md (Hantera tabellstatistik)
[Temporary tables]: ./sql-data-warehouse-tables-temporary.md (Temporära tabeller)
[Guide for using PolyBase]: ./sql-data-warehouse-load-polybase-guide.md (Guide för att använda PolyBase)
[Load data]: ./sql-data-warehouse-overview-load.md (Läsa in data)
[Move data with Azure Data Factory]: ../data-factory/data-factory-azure-sql-data-warehouse-connector.md (Flytta data med Azure Data Factory)
[Load data with Azure Data Factory]: ./sql-data-warehouse-get-started-load-with-azure-data-factory.md (Läsa in data med Azure Data Factory)
[Läs in data med BCP]: ./sql-data-warehouse-load-with-bcp.md
[Load data with PolyBase]: ./sql-data-warehouse-get-started-load-with-polybase.md (Läsa in data med PolyBase)
[Monitor your workload using DMVs]: ./sql-data-warehouse-manage-monitor.md (Övervaka arbetsbelastningen med datahanteringsvyer)
[Pause compute resources]: ./sql-data-warehouse-manage-compute-overview.md#pause-compute-bk (Pausa beräkningsresurser)
[Resume compute resources]: ./sql-data-warehouse-manage-compute-overview.md#resume-compute-bk (Återuppta beräkningsresurser)
[Scale compute resources]: ./sql-data-warehouse-manage-compute-overview.md#scale-performance-bk (Skala beräkningsresurser)
[Understanding transactions]: ./sql-data-warehouse-develop-transactions.md (Förstå transaktioner)
[Optimizing transactions]: ./sql-data-warehouse-develop-best-practices-transactions.md (Optimera transaktioner)
[Troubleshooting]: ./sql-data-warehouse-troubleshoot.md (Felsökning)
...
[Columnstore indexes guide]: https://msdn.microsoft.com/library/gg492088.aspx (Guide för columnstore-index)

<!--Other Web references-->
[Selecting table distribution]: https://blogs.msdn.microsoft.com/sqlcat/2015/08/11/choosing-hash-distributed-table-vs-round-robin-distributed-table-in-azure-sql-dw-service/ (Välja tabelldistribution)
[Feedback om Azure SQL Data Warehouse]: https://feedback.azure.com/forums/307516-sql-data-warehouse
[Azure SQL Data Warehouse-forum på MSDN]: https://social.msdn.microsoft.com/Forums/sqlserver/home?forum=AzureSQLDataWarehouse
[Stack Overflow-forum för Azure SQL Data Warehouse]:  http://stackoverflow.com/questions/tagged/azure-sqldw
[Azure SQL Data Warehouse loading patterns and strategies]: https://blogs.msdn.microsoft.com/sqlcat/2016/02/06/azure-sql-data-warehouse-loading-patterns-and-strategies (Azure SQL Data Warehouse: inläsningsmönster och strategier)
