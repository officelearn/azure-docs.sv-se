...
Se även [hantera tabellstatistik][Manage table statistics], [CREATE STATISTICS][CREATE STATISTICS], [UPDATE STATISTICS][UPDATE STATISTICS]
...
Se även [Webbsidesinläsning][Load data], [Guide for using PolyBase][Guide for using PolyBase], [Azure SQL Data Warehouse inläsningsmönster och strategier][Azure SQL Data Warehouse inläsningsmönster och strategier], [ Läs in Data med Azure Data Factory][Load Data with Azure Data Factory], [Move data with Azure Data Factory][Move data with Azure Data Factory], [CREATE EXTERNAL FILE FORMAT][CREATE EXTERNAL FILE FORMAT], [skapa en tabell som Välj (CTAS)][Create table as select (CTAS)]

## <a name="load-then-query-external-tables"></a>Läsa in och sedan fråga externa tabeller
Även om Polybase, även kallat externa tabeller, kan vara det snabbaste sättet att läsa in data, är det inte optimalt för frågor. Polybase-tabeller i SQL Data Warehouse har för närvarande endast stöd för Azure-blobfiler och Azure Data Lake-lagring. Dessa filer har inte några beräkningsresurser som backar upp dem.  Det betyder att SQL Data Warehouse inte kan avlasta detta arbete och därför måste läsa hela filen genom att läsa in den till tempdb för att kunna läsa data.  Om du har flera frågor som ska köras mot dessa data är det därför bättre att läsa in dem en gång och låta frågorna använda den lokala tabellen.
...
Se även [tabellöversikt][Table overview], [Table distribution][Table distribution], [välja tabelldistribution][välja tabelldistribution], [CREATE TABLE][CREATE TABLE], [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]
...
Se även [förstå transaktioner][Understanding transactions], [Optimizing transactions][Optimizing transactions], [Tabellpartitionering][Tabellpartitionering], [TRUNCATE TABLE][TRUNCATE TABLE] , [ALTER TABLE][ALTER TABLE], [Skapa en tabell som Välj (CTAS)][skapa en tabell som Välj (CTAS)]
...
Se även [tabellöversikt][Table overview], [Table data types][Table data types], [CREATE TABLE][CREATE TABLE]
...
Se även [Tabellindex][Table indexes], [Columnstore indexes guide][Columnstore indexes guide], [återskapa kolumnlagringsindex][återskapa kolumnlagringsindex]
