...
Granskning för [Azure SQL Database](sql-database-technical-overview.md) och [Azure Synapse Analytics](../synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is.md) har stöd för skrivning av databas händelser till ett [Azure Storage konto](../storage/common/storage-account-overview.md) bakom ett virtuellt nätverk och en brand vägg. 

I den här artikeln beskrivs två sätt att konfigurera Azure SQL Server och Azure Storage-kontot för det här alternativet. Det första använder Azure Portal, den andra använder REST.
...
Anslut till [Azure Portal](https://portal.azure.com) med din prenumeration. Navigera till resurs gruppen och Azure SQL Database-servern.
...
|`<sql database server>`| Namn på Azure SQL Database-Server|
|`<administrator login>`| Administratörs konto för SQL Database |
...
2. Öppna [Azure-portalen](https://portal.azure.com). Navigera till ditt lagringskonto. Leta upp **Access Control (IAM)** och klicka på **Lägg till roll tilldelning**. Tilldela RBAC-rollen **Storage BLOB data Contributor** till Azure SQL Server som värd för din Azure SQL-databas som du registrerade med Azure Active Directory (Azure AD) som i föregående steg.
...
3. Konfigurera [Azures gransknings princip för Azure SQL Server](/rest/api/sql/server%20auditing%20settings/createorupdate), utan att ange en *storageAccountAccessKey*:
...
- [Använd PowerShell för att skapa en tjänst slut punkt för virtuellt nätverk och sedan en regel för virtuella nätverk för Azure SQL Database.](sql-database-vnet-service-endpoint-rule-powershell.md)
- [Virtual Network regler: åtgärder med REST API: er](/rest/api/sql/virtualnetworkrules)
- [Använd tjänst slut punkter och regler för virtuella nätverk för databas servrar](sql-database-vnet-service-endpoint-rule-overview.md)
