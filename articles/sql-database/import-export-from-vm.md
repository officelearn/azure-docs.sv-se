...
description: Importera eller exportera en Azure SQL-databas utan att ge Azure-tjänster åtkomst till servern.
...
---
# <a name="import-or-export-an-azure-sql-database-without-allowing-azure-services-to-access-the-server"></a>Importera eller exportera en Azure SQL-databas utan att ge Azure-tjänster åtkomst till servern

Den här artikeln visar hur du importerar eller exporterar en Azure SQL-databas när *Tillåt att Azure-tjänster* är inställt på Azure SQL-servern. Arbets flödet använder en virtuell Azure-dator för att köra SqlPackage för att utföra import-eller export åtgärden.
...
7. Ange användarnamnet och lösenordet du angav när du skapar den virtuella datorn och välj sedan **OK**.
...
Lägg till den virtuella datorns offentliga IP-adress i SQL Database Server-brandväggen.
...
5. Välj **Spara**. En regel för IP-brandvägg på server nivå skapas för den virtuella datorns offentliga IP-adress som öppnar port 1433 på SQL Database-servern.
...
Följande SqlPackage-kommando importerar **AdventureWorks2017** -databasen från lokal lagring till en Azure SQL Database-Server. Den skapar en ny databas med namnet **myMigratedDatabase** med en **Premium** tjänstnivå och en **P6** Tjänstmål. Ändra dessa värden som passar din miljö.
