---
title: Kör analysfrågor
description: Analysfrågor för flera innehavare med hjälp av data som extraherats från flera Azure SQL Database-databaser i en app för flera innehavare.
...
---
# <a name="cross-tenant-analytics-using-extracted-data---multi-tenant-app"></a>Analys över flera innehavare med extraherade data – app för flera innehavare
 
I den här självstudien går du igenom ett komplett analysscenario för en implementering med flera data. Scenariot visar hur analyser kan göra det möjligt för företag att fatta smarta beslut. Med hjälp av data som extraheras från fragmenterad databas använder du analyser för att få insikter om klientbeteende, inklusive deras användning av exempelprogrammet Wingtip Tickets SaaS.Using data extracted from sharded database, you use analytics to gain insights into tenant behavior, including their use of the sample Wingtip Tickets SaaS application. Det här scenariot omfattar tre steg: 
...
![arkitekturÖversikt](media/saas-multitenantdb-tenant-analytics/architectureOverview.png)

## <a name="offline-tenant-analytics-pattern"></a>Analysmönster för offlineklienter

SaaS-program som du utvecklar har åtkomst till en stor mängd klientdata som lagras i molnet. Data ger en rik källa till insikter om hur du använder ditt program och om klienternas beteende. Dessa insikter kan vägleda funktionsutveckling, användbarhetsförbättringar och andra investeringar i appen och plattformen.

Det är enkelt att komma åt data för alla klienter när alla data finns i bara en databas med flera innehavare. Men åtkomsten är mer komplex när den distribueras i stor skala över tusentals databaser. Ett sätt att tämja komplexiteten är att extrahera data till en analysdatabas eller ett informationslager. Du frågar sedan informationslagret för att samla in insikter från biljettdata för alla klienter.

Den här självstudien visar ett fullständigt analysscenario för det här exemplet SaaS-programmet. Först används elastiska jobb för att schemalägga extrahering av data från varje klientdatabas. Data skickas till ett analyslager. Analysarkivet kan antingen vara en SQL-databas eller ett SQL Data Warehouse. För storskalig dataextrahering får [Azure Data Factory](../data-factory/introduction.md) beröm.

Därefter strimlas de aggregerade data till en uppsättning [stjärnschematabeller.](https://www.wikipedia.org/wiki/Star_schema) Tabellerna består av en central faktatabell plus relaterade dimensionstabeller:
...
![Stjärnschema](media/saas-multitenantdb-tenant-analytics/StarSchema.png)

Slutligen efterfrågas stjärnschematabellerna. Frågeresultaten visas visuellt för att markera insikter om klientbeteende och deras användning av programmet. Med det här stjärnschemat kan du köra frågor som hjälper dig att upptäcka objekt som följande:
...
Att förstå hur konsekvent varje klient använder tjänsten ger en möjlighet att skapa serviceplaner för att tillgodose deras behov. Den här självstudien innehåller grundläggande exempel på insikter som kan hämtas från klientdata.
...
- Wingtip-biljetterna SaaS Multi-tenant Database-programmet distribueras. Information om hur du distribuerar på mindre än fem minuter finns i [Distribuera och utforska Wingtip Tickets SaaS-databasprogrammet för flera innehavare](saas-multitenantdb-get-started-deploy.md)
- Wingtip SaaS-skripten och [programkällakoden](https://github.com/Microsoft/WingtipTicketsSaaS-MultiTenantDB) hämtas från GitHub. Var noga med att *låsa upp zip-filen* innan du extraherar dess innehåll. Kolla in den [allmänna vägledningen](saas-tenancy-wingtip-app-guidance-tips.md) för steg för att ladda ner och låsa upp Wingtip Tickets SaaS skript.
- Power BI Desktop är installerat. [Ladda ner Power BI Desktop](https://powerbi.microsoft.com/downloads/)
- Batchen med ytterligare klienter har etablerats, se [**självstudien för tillhandahållande klienter**](saas-multitenantdb-provision-and-catalog.md).
- En jobbagent- och jobbagentdatabas har skapats. Se lämpliga steg i [**självstudiekursen för schemahantering**](saas-multitenantdb-schema-management.md#create-a-job-agent-database-and-new-job-agent).
...
Ofta finns det många transaktionella fragmenterade databaser som tillsammans innehåller alla klientdata. Du måste aggregera klientdata från den fragmenterade databasen i ett analysarkiv. Aggregeringen möjliggör effektiv fråga för data. I den här självstudien används en Azure SQL Database-databas för att lagra aggregerade data.
...
2. Ange $DemoScenario variabel i skriptet så att den matchar ditt val av analytics Store. I utbildningssyfte rekommenderas SQL-databas utan columnstore.
    - Om du vill använda SQL-databas utan columnstore anger du **$DemoScenario** = **2**
    - Om du vill använda SQL-databas med columnstore ställer du in **$DemoScenario** = **3**  
...
Nu när du har distribuerat programmet och fyllt den med intressanta klientdata, använd [SQL Server Management Studio (SSMS)](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms) för att ansluta **tenants1-mt-\<Användare\> ** och **katalog-mt-\<Användarservrar\> ** med Login = *utvecklare*, Lösenord = *P\@ssword1*.

![arkitekturÖversikt](media/saas-multitenantdb-tenant-analytics/ssmsSignIn.png)

Gör följande i Objektutforskaren:

1. Expandera *\<klienterna1-mt-\> Användarserver.*
2. Expandera noden Databaser och se *tenants1-databasen* som innehåller flera klienter.
3. Expandera *katalog-mt-\<\> Användarserver.*
...
- Den **sp_ShredRawExtractedData** lagrade proceduren används för att fylla i stjärnschematabellerna från rådatatabellerna.

![tenantAnalytics (tenantAnalytics)](media/saas-multitenantdb-tenant-analytics/tenantAnalytics.png)

## <a name="data-extraction"></a>Data extrahering 

### <a name="create-target-groups"></a>Skapa målgrupper 

Innan du fortsätter kontrollerar du att du har distribuerat jobbkontot och jobbkontodatabasen. I nästa steg används elastiska jobb för att extrahera data från den fragmenterade klientdatabasen och för att lagra data i analysarkivet. Sedan strimlar det andra jobbet data och lagrar den i tabeller i stjärnschemat. Dessa två jobb körs mot två olika målgrupper, nämligen **TenantGroup** och **AnalyticsGroup**. Extrahera-jobbet körs mot TenantGroup, som innehåller alla klientdatabaser. Strimlingsjobbet körs mot AnalyticsGroup, som bara innehåller analysarkivet. Skapa målgrupperna med hjälp av följande steg:

1. I SSMS ansluter du till **jobaccount-databasen** \<i\>katalog-mt- Användare .
2. Öppna *...\Utbildningsmoduler\Operational Analytics\Tenant Analytics\ TargetGroups.sql i* SSMS 
3. Ändra @User variabeln högst upp i `<User>` skriptet och ersätt med det användarvärde som användes när du distribuerade Wingtip Tickets SaaS Multi-tenant Database-programmet.
...
1. I SSMS ansluter du till **jobaccount-databasen** \<i\> katalog-mt- Användarserver.
2. Öppna *...\Utbildningsmoduler\Operational Analytics\Tenant Analytics\ExtractTickets.sql*i SSMS .
3. Ändra @User högst upp i skriptet `<User>` och ersätt med det användarnamn som användes när du distribuerade Wingtip Tickets SaaS Multi-tenant Database-programmet. 
...
![biljettUttredar](media/saas-multitenantdb-tenant-analytics/ticketExtracts.png)
...
Nästa steg är att strimla extraherade rådata i en uppsättning tabeller som är optimerade för analysfrågor. Ett stjärnschema används. En central faktatabell innehåller individuella biljettförsäljningsposter. Dimensionstabeller fylls med data om platser, händelser, kunder och inköpsdatum. 
...
![fragmenteringJob](media/saas-multitenantdb-tenant-analytics/shreddingJob.PNG)
...
4. I databasinloggningsfönstret anger du servernamnet\<(catalog-mt- Användare\>.database.windows.net). Välj **Importera** för **dataanslutningsläge**och klicka sedan på OK. 
...
5. Välj **Databas** i den vänstra rutan och ange sedan användarnamn = *utvecklare*och ange lösenord = *P\@ssword1*. Klicka på **Anslut**.  
    ...
    ![DatabasSignIn](media/saas-multitenantdb-tenant-analytics/databaseSignIn.PNG)
...
![FörsäljningVersusDate](media/saas-multitenantdb-tenant-analytics/SaleVersusDate.PNG)
...
![FörsäljningDayDistribution](media/saas-multitenantdb-tenant-analytics/SaleDistributionPerDay.PNG)
...
![ContosoSales (ContosoSales)](media/saas-multitenantdb-tenant-analytics/EventSaleTrends.PNG)
...
Du har observerat trender i klientdata från Wingtip Tickets SaaS Multi-tenant Database-programmet. Du kan tänka över andra sätt som appen kan informera affärsbeslut för SaaS-programleverantörer. Leverantörer kan bättre tillgodose behoven hos sina hyresgäster. Förhoppningsvis har den här självstudien utrustat dig med verktyg som krävs för att utföra analyser av klientdata för att göra det möjligt för dina företag att fatta datadrivna beslut.
...
- [Analys över flera innehavare med extraherade data – app för en innehavare](saas-tenancy-tenant-analytics.md) 