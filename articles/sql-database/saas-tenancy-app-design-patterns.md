...
- **Anpassningsbarhet:** &nbsp; enkelt stöd för schema anpassningar som antingen är klient-eller klient klass-/regionsspecifika.
...
*Data:* &nbsp; en databas med flera klient organisationer vill ha nödvändigt vis klient isolering.  Data för flera klienter lagras tillsammans i en databas.  Under utvecklingen ser du till att frågor aldrig visar data från fler än en klient.  SQL Database stöder [säkerhet på radnivå][docu-sql-svr-db-row-level-security-947w], som kan genomdriva att data som returneras från en fråga besvaras till en enda klient.

*Bearbetning:* &nbsp; en databas för flera klienter delar beräknings-och lagrings resurser över alla klienter.  Databasen som helhet kan övervakas för att säkerställa att den godkänns.  Men Azure-systemet har inget inbyggt sätt att övervaka eller hantera användningen av dessa resurser av en enskild klient.  Därför medför databasen för flera innehavare en ökad risk för att stöta på problem med störningar, där arbets belastningen för en överaktiv klient påverkar prestanda upplevelsen för andra klienter i samma databas.  Ytterligare övervakning på program nivå kan övervaka prestanda på klient nivå.
