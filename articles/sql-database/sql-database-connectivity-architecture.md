...
- **Standard:** Detta är anslutnings principen som används på alla servrar när den har skapats, såvida du inte uttryckligen ändrar anslutnings principen `Proxy` till `Redirect`antingen eller. Standard principen är`Redirect` för alla klient anslutningar som har sitt ursprung i Azure (till exempel från en virtuell Azure-dator) och `Proxy`för alla klient anslutningar som kommer från utsidan (till exempel anslutningar från din lokala arbets Station).
...
Om du ansluter inifrån Azure får anslutningarna `Redirect` som standard en anslutnings princip. En princip på `Redirect` innebär att när TCP-sessionen har upprättats till Azure SQL Database omdirigeras sedan klientsessionen till rätt databas kluster med en ändring av den virtuella mål-IP-adressen från den Azure SQL Database gatewayen till klustret. Därefter flödar alla efterföljande paket direkt till klustret och hoppar över Azure SQL Database Gateway. Följande diagram illustrerar det här trafikflödet.
...
| Japan, östra           | 13.78.61.196, 40.79.184.8, 191.237.240.43, 40.79.192.5 | 
