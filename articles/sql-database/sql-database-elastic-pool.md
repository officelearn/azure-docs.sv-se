...
För tillgängliga tjänst nivåer och begränsningar för varje resurs modell, se den [DTU-baserade inköps modellen](sql-database-service-tiers-dtu.md) eller den [vCore-baserade inköps modellen](sql-database-service-tiers-vcore.md).
...
> [!IMPORTANT]
> Om antalet databaser i en pool närmar sig det högsta antal som stöds, se till att du funderar på [resurs hantering i kompakta elastiska pooler](sql-database-elastic-pool-resource-management.md).
> 
...
- **Geo-återställning**
  ...
  Geo-återställning tillhandahåller standard återställnings alternativet när en databas inte är tillgänglig på grund av en incident i den region där databasen finns. Se [återställa en Azure SQL Database eller redundans till en sekundär](sql-database-disaster-recovery.md)
...
Starta övervakningen av den elastiska poolen genom att söka efter och öppna en elastisk pool i portalen. Först ser du en skärm som ger dig en översikt över statusen för den elastiska poolen. Du måste bland annat:
