...
- Ett tillstånds löst beräknings lager som kör `sqlservr.exe` processen och bara innehåller temporära och cachelagrade data, till exempel tempdb, modell databaser på anslutna SSD och planera cache, resurspool och columnstore-pool i minnet. Den här tillstånds lösa noden drivs av Azure-Service Fabric som `sqlservr.exe`initierar, kontrollerar nodens hälsa och utför redundans till en annan nod vid behov.
