...
- Om du vill köra exempel koden i de här övningarna måste du först ha [Azure SQL Database med Machine Learning Services (med R)](sql-database-machine-learning-services-overview.md) aktiverat.
...
Det går vanligt vis att hitta svaret med hjälp av `str()` R-kommandot. Lägg till funktionen `str(object_name)` var som helst i R-skriptet om du vill att data schemat för det angivna R-objektet ska returneras som ett informations meddelande. Du kan visa meddelandena på fliken **meddelanden** i SSMS.

För att ta reda på varför exempel 1 och exempel 2 har sådana olika resultat, infogar `str(OutputDataSet)` du linjen i slutet av `@script` variabel definitionen i varje instruktion, så här:
...
> [!TIP]
> Du kan också använda R Identity-funktioner, till `is.matrix`exempel `is.vector`, för att returnera information om den interna data strukturen.
...
- Ange kolumner i din indata-datakälla individuellt, i stället `SELECT *`för att använda, och se hur varje kolumn ska hanteras.
