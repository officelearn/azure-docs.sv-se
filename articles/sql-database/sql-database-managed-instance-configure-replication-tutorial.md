...
- Namnet på den hanterade instansen av distributör `sql-mi-distributor` ska vara: (tillsammans med några tecken för slumpmässig het) och bör finnas _i samma virtuella nätverk som utgivarens hanterade instans_.
...
1. Ange ett godtyckligt namn för DNS-zonen, `repldns.com`till exempel. 
...
1. Upprepa de här stegen för att lägga till en länk till det virtuella prenumeranten för prenumerant, med `Sub-link`ett namn som. 
...
När anslutningen har upprättats och du har en exempel databas kan du konfigurera distributionen på `sql-mi-distributor` din hanterade instans. Det gör du på följande sätt:
