...
> [!NOTE]
> DTU-gränsen för en databas bestäms av CPU, läsningar, skrivningar och minne som är tillgängligt för databasen. Men eftersom den SQL Server databas motorn vanligt vis använder allt tillgängligt minne för Datacachen för att förbättra prestandan `avg_memory_usage_percent` ligger värdet normalt nära 100% oavsett den aktuella databas belastningen. Det innebär att även om minnet påverkar DTU-gränsen, används den inte i formeln DTU-användning.
>
