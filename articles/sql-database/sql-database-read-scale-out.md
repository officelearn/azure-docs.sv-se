...
När du är ansluten till en skrivskyddad replik kan du komma åt prestanda måtten med hjälp `sys.dm_db_resource_stats` av DMV. Använd `sys.dm_exec_query_stats`- `sys.dm_exec_query_plan` och `sys.dm_exec_sql_text` DMV: er för att få åtkomst till statistik för frågeplan.
