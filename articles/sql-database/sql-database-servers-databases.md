---
title: Azure SQL-logiska servrar och enskilda databaser | Microsoft Docs
description: Läs mer om logiska Azure SQL Database-server och databasbegrepp och deras resurser.
...
ms.locfileid: "39091804"
---
# <a name="azure-sql-database-logical-servers-and-single-databases-and-their-resources"></a>Azure SQL Database logiska servrar och enskilda databaser och deras resurser
...
> [!TIP]
> Giltigt resurs-grupp och servernamnen, se [namngivningsregler och begränsningar](https://docs.microsoft.com/azure/architecture/best-practices/naming-conventions).
>
...
- En enkel databas som skapats i en [Azure-resursgrupp](../azure-resource-manager/resource-group-overview.md) med en [kombineras uppsättning beräknings- och lagringsresurser](sql-database-service-tiers-dtu.md) eller en [oberoende skala beräknings- och lagringsresurser](sql-database-service-tiers-vcore.md). En Azure SQL database är associerad med en logisk server från Azure SQL-databas som har skapats i en specifik Azure-region.
- En databas som skapats som en del av en [pool med databaser](sql-database-elastic-pool.md) inom en [Azure-resursgrupp](../azure-resource-manager/resource-group-overview.md) med en [kombineras uppsättning beräknings- och lagringsresurser (DTU-baserade)](sql-database-service-tiers-dtu.md) eller en [oberoende skala beräknings- och lagringsresurser (vCore-baserade)](sql-database-service-tiers-vcore.md) som ska delas mellan alla databaser i poolen. En Azure SQL database är associerad med en logisk server från Azure SQL-databas som har skapats i en specifik Azure-region.

> [!IMPORTANT]
> SQL Database Managed Instance är för närvarande i förhandsversion, är en [instans av en SQLServer](sql-database-managed-instance.md) (en hanterad instans) som skapats i en [Azure-resursgrupp](../azure-resource-manager/resource-group-overview.md) med en definierad uppsättning beräknings- och resurser för alla databaser på den serverinstansen. En hanterad instans innehåller både system- och användardatabaserna. Hanterad instans har utformats för att aktivera databasen – flytta till en helt hanterad PaaS, utan att göra om programmet. Managed Instance tillhandahåller hög kompatibilitet med programmeringsmodellen för en lokal SQL Server och har stöd för en majoritet av SQL Server-funktioner och tillhörande verktyg och tjänster. Mer information finns i [SQL Database Managed Instance](sql-database-managed-instance.md). Resten av den här artikeln gäller inte för hanterad instans.

## <a name="tds-and-tcpip-connections"></a>TDS- och TCP/IP-anslutningar

Microsoft Azure SQL Database stöder tabular data stream (TDS) protokollet klientversion 7.3 eller senare och tillåter endast krypterade TCP/IP-anslutningar.

## <a name="azure-sql-databases-protected-by-sql-database-firewall"></a>Azure SQL-databaser skyddas av SQL Database-brandvägg

För att skydda dina data, en [SQL Database-brandvägg](sql-database-firewall-configure.md) förhindra all åtkomst till din databasserver eller någon av dess databaser från utanför din anslutning till servern direkt via din Azure-prenumeration-anslutning. Om du vill aktivera ytterligare anslutning, måste du [skapa en eller flera brandväggsregler](sql-database-firewall-configure.md#creating-and-managing-firewall-rules). Skapa och hantera elastiska pooler finns i [elastiska pooler](sql-database-elastic-pool.md).
