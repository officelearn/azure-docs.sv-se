...
- Ett tillstånds löst beräknings lager som kör `sqlservr.exe` processen och endast innehåller tillfälliga och cachelagrade data (till exempel plan cache, resurspool, Column Store pool). Den här tillstånds lösa SQL Server-noden drivs av Azure Service Fabric som initierar process, styr hälsan för noden och utför redundans till en annan plats vid behov.
