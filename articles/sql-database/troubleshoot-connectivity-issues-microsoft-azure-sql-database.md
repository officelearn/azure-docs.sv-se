...
description: Felsöka anslutnings problem med Microsoft Azure SQL Database
...
Problemet beror på att programmet inte kan ansluta till servern.

Lös problemet genom att prova stegen (i ordning) i avsnittet nedan, med rubriken **steg för att åtgärda vanliga anslutnings problem**.
...
### <a name="error-10053-a-transport-level-error-has-occurred-when-receiving-results-from-the-server"></a>Fel 10053: Ett fel på transport nivå har uppstått när resultat togs emot från servern.

**Detaljerat fel**

``10053: A transport-level error has occurred when receiving results from the server. (Provider: TCP Provider, error: 0 - An established connection was aborted by the software in your host machine)``

De här problemen uppstår eftersom programmet inte kan ansluta till servern.
...
Lös problemet genom att [Konfigurera brand Väggs inställningar på SQL Database med hjälp av Azure Portal.](https://docs.microsoft.com/azure/sql-database/sql-database-firewall-configure)
...
## <a name="unable-to-login-to-the-server-errors-18456-40531"></a>Det går inte att logga in på servern (fel 18456, 40531)
...
Kontakta tjänst administratören för att få ett giltigt användar namn och lösen ord för SQL för att lösa problemet.

Tjänstadministratören kan normalt använda följande steg för att lägga till inloggningen:

1. Logga in på servern med SQL Server Management Studio (SSMS).
2. Kontrollera om inloggningsnamnet är inaktiverat med hjälp av följande SQL-fråga:
...
4. Om SQL-inloggningens användar namn inte finns skapar du den med hjälp av SSMS:

   1. Dubbelklicka på **Säkerhet** för att expandera. 
   2. Högerklicka på **Inloggningar** och välj sedan **Ny inloggning**. 
   3. I det här genererade skriptet med platshållare kan du redigera och köra följande SQL-fråga:
...
6. Välj databasen du vill bevilja användaren behörighet till.
7. Dubbelklicka på **Säkerhet**. 
8. Högerklicka på **Användare** och välj **Ny användare**. 
9. I det här genererade skriptet med platshållare kan du redigera och köra följande SQL-fråga: 
...
### <a name="systemdatasqlclientsqlexception-0x80131904-connection-timeout-expired"></a>System. data. SqlClient. SqlException (0x80131904): Tids gränsen för anslutningen har gått ut.
...
### <a name="systemdataentitycoreentityexception-the-underlying-provider-failed-on-open"></a>System. data. Entity. Core. EntityException: Det gick inte att öppna den underliggande providern.
...
Dessa undantag kan inträffa antingen på grund av problem med anslutningen eller frågan. Du kan kontrol lera att det här felet beror på anslutnings problem genom att kontrol lera avsnittet nedan med rubriken **bekräfta om felet beror på ett anslutnings problem**:

Tids gränsen för anslutningen inträffar eftersom programmet inte kan ansluta till servern. Lös problemet genom att prova stegen (i ordning) i avsnittet nedan, med rubriken **steg för att åtgärda vanliga anslutnings problem**.
...
Det här problemet uppstår på grund av ett tillfälligt fel som påträffats under en omkonfiguration/redundans på Server delen.

Lös problemet genom att vänta en kort period och försök igen. Inget support ärende krävs om inte problemet förblir beständigt.

Vi rekommenderar att du ser till att omprövnings logiken är på plats. Mer information om logiken för omprövning finns i [Felsöka tillfälliga fel och anslutnings fel till SQL Database](https://docs.microsoft.com/azure/sql-database/sql-database-connectivity-issues).

## <a name="connection-terminated-due-to-hitting-some-system-defined-limit"></a>Anslutningen avbröts på grund av en viss systemdefinierad gräns
...
* Kontrol lera om det finns tids krävande frågor:
  ...
  > [!NOTE]
  > Det här är en minimalist metod som kanske inte löser problemet nödvändigt vis.

  1. Kontrol lera vyn [sys. DM _exec_requests](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-requests-transact-sql) för att se eventuella spärrnings begär Anden genom att köra följande SQL-fr åga:
...
* Överväg att uppgradera till en av de nya för hands versionerna (till exempel [standard-eller Premium-versioner](https://azure.microsoft.com/pricing/details/sql-database/)) om databasen ständigt når sin gräns trots att blockera och långvariga frågor.
...
Följande steg kan hjälpa dig med antingen att lösa problemet eller ge dig ytterligare alternativ som du kan tänka på.

1. Kontrol lera databasens aktuella storlek med hjälp av instrument panelen i Azure Portal.

   > [!NOTE]
   > Du kan använda följande SQL-fråga för att identifiera vilka tabeller som förbrukar mest utrymme och potentiella kandidater för rensning:
...
3. Om databas storleken redan har passerat den maximala storleken som stöds för din utgåva kan du göra något av följande:
   1. Utför normal databas rensnings aktiviteter (rensar bort oönskade data genom att använda trunkera/ta bort osv. eller flytta data ut med SSIS, BCP osv.)
   2. Partitionera eller ta bort data, släpp index eller Läs om möjliga lösningar i dokumentationen. 
...
Om du stöter på det här fel meddelandet upprepade gånger kan du försöka lösa problemet på följande sätt: 

1. Kontrol lera vyn sys. DM _exec_requests för att se alla öppna sessioner som har ett högt värde för kolumnen total_elapsed_time genom att köra följande SQL-skript:

   ```
   SELECT * FROM dm_exec_requests
   ```
2. Fastställ indatabufferten för den fråga som körs länge. 
3. Finjustera frågan.
...
Följ dessa metoder för att lösa problemet: 

* Problemet beror på åtgärderna INSERT, Update eller Delete. Försök att minska antalet rader som körs omedelbart genom att implementera batchbearbetning eller dela upp i flera mindre transaktioner.
* Problemet beror på åtgärderna för att återskapa index. Kontrol lera att du följer följande formel: antalet rader som påverkas i tabell * (genomsnittlig storlek på fält som uppdateras i byte + 80) < 2 GB

  > [!NOTE]
  > För att indexera om indexet, ska den genomsnittliga storleken för det fält som uppdateras ersättas med den genomsnittliga index storleken.
...
Det här problemet uppstår eftersom kontot inte har åtkomst behörighet till Master-databasen. SQL Server Management Studio (SSMS) försöker dock ansluta till Master-databasen som standard.

Följ dessa anvisningar för att lösa problemet:

1. På inloggnings skärmen på SSMS klickar du på **alternativ**och sedan på **anslutnings egenskaper**. 
2. I **Anslut till databas**anger du användarens standard databas namn som standard inloggnings databas och klickar sedan på **Anslut**.
...
## <a name="confirm-whether-an-error-is-due-to-a-connectivity-issue"></a>Bekräfta om ett fel beror på ett anslutnings problem

Du kan kontrol lera om ett fel beror på ett anslutnings problem genom att granska stack spårningen för ramar som visar anrop för att öppna en anslutning som följande (Observera referensen till **SQLConnection** -klassen):
...
När undantaget uppstår på grund av problem med frågan, kommer du att märka en anrops stack som liknar följande (Observera referensen till klassen **SqlCommand** ). I dessa scenarier kan [du finjustera dina frågor](http://blogs.msdn.com/b/sqlblog/archive/2013/11/01/is-my-query-running-fine-in-the-cloud.aspx).
...
```
För ytterligare vägledning om fin justering av prestanda, se följande:
...
1. Kontrol lera att TCP IP är aktiverat som ett klient protokoll på program servern. Mer information finns i [Konfigurera klient protokoll](https://docs.microsoft.com/sql/database-engine/configure-windows/configure-client-protocols). På program servrar där du inte har installerat SQL Server verktyg, kontrollerar du att TCP IP är aktiverat genom att köra **cliconfg. exe** (SQL Server klient nätverks verktyg). 
2. Kontrol lera programmets anslutnings sträng för att kontrol lera att den är korrekt konfigurerad. Kontrol lera till exempel att anslutnings strängen anger rätt port (1433) och det fullständigt kvalificerade Server namnet.
...
3. Försök att öka **tids gränsen**för anslutningen. Microsoft rekommenderar att du använder en tids gräns för anslutning på minst 30 sekunder. 
4. Testa anslutningen mellan program servern och Azure SQL-databasen med hjälp av [SQL Server Management Studio (SSMS)](https://docs.microsoft.com/azure/sql-database/sql-database-connect-query-ssms), en UDL-fil, ping och Telnet. Mer information finns i [fel sökning av problem med SQL Server anslutning](https://support.microsoft.com/help/4009936/solving-connectivity-errors-to-sql-server) och [diagnostik för anslutnings problem](https://docs.microsoft.com/azure/sql-database/sql-database-connectivity-issues#diagnostics).

   > [!NOTE]
   > Som fel söknings steg kan du också försöka testa anslutningen på en annan klient dator.

5. Vi rekommenderar att du ser till att omprövnings logiken är på plats. Mer information om logiken för omprövning finns i [Felsöka tillfälliga fel och anslutnings fel till SQL Database](https://docs.microsoft.com/azure/sql-database/sql-database-connectivity-issues).

Om föregående steg inte löser problemet försöker du samla in mer data och kontakta supporten. Aktivera loggning om ditt program är en moln tjänst. Det här steget returnerar en tidsstämpel för UTC-tid för felet. Dessutom returnerar SQL Azure spårnings-ID. [Microsofts kund support tjänster](http://azure.microsoft.com/support/options/) kan använda den här informationen. 

Mer information om hur du aktiverar loggning finns i [Aktivera diagnostikloggning för Web Apps i Azure App Service](https://azure.microsoft.com/documentation/articles/web-sites-enable-diagnostic-log/).

**Relaterade dokument**

* [Anslutningsarkitektur för Azure SQL](https://docs.microsoft.com/azure/sql-database/sql-database-connectivity-architecture)<br>
