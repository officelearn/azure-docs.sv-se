...
I det här `my-directory` exemplet flyttas en `my-directory-2` katalog `my-subdirectory`med namnet till en underkatalog med namnet . 
...
I följande exempel `ACL` `Permissions`visas `Group`egenskaperna `Owner` , , och för varje objekt i katalogen. Parametern `-FetchProperty` krävs för att `ACL` hämta värden för egenskapen. 
