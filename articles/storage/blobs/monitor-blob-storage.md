...
**Översikts** sidan i Azure Portal för varje Blob Storage-resurs innehåller en kort översikt över resursanvändningen, till exempel begär Anden och fakturering per timme. Den här informationen är användbar, men bara en liten mängd av övervaknings data är tillgänglig. En del av dessa data samlas in automatiskt och är tillgängliga för analys så fort du skapar resursen. Du kan aktivera ytterligare typer av data insamling med en del konfiguration.

## <a name="what-is-azure-monitor"></a>Vad är Azure Monitor?
Azure Blob Storage skapar övervaknings data med hjälp av [Azure Monitor](../../azure-monitor/overview.md), som är en fullständig stack övervaknings tjänst i Azure. Azure Monitor innehåller en fullständig uppsättning funktioner för att övervaka dina Azure-resurser och resurser i andra moln och lokalt. 
...
Om du vill samla in resurs loggar måste du skapa en diagnostisk inställning. När du skapar inställningen väljer du **BLOB** som den typ av lagring som du vill aktivera loggar för. Ange sedan en av följande kategorier av åtgärder som du vill samla in loggar för. 
...
> [!NOTE]
> Data Lake Storage Gen2 visas inte som lagrings typ. Det beror på att Data Lake Storage Gen2 är en uppsättning funktioner som är tillgängliga för Blob Storage. 
...
5. Klicka på **Lägg till diagnostisk inställning**.
   ...
   > [!div class="mx-imgBorder"]
   > ![Sidan resurs loggar](media/monitor-blob-storage/diagnostic-logs-page.png)
...
1. Markera kryss rutan **arkivera till ett lagrings konto** och klicka sedan på knappen **Konfigurera** .
   ...
   > [!div class="mx-imgBorder"]   
   > ![Sidan diagnostikinställningar Arkiv Arkiv lagring](media/monitor-blob-storage/diagnostic-logs-settings-pane-archive-storage.png)
...
2. I fönstret **Välj en händelse hubb** väljer du namn område, namn och princip namn för den händelsehubben som du vill strömma loggarna till. 
   ...
   > [!div class="mx-imgBorder"]
   > ![Händelse hubb för sidan diagnostikinställningar](media/monitor-blob-storage/diagnostic-logs-settings-pane-event-hub.png)
...
1. Markera kryss rutan **Skicka till Log Analytics** , Välj en Log Analytics-arbetsyta och klicka sedan på knappen **Spara** .
   ...
   > [!div class="mx-imgBorder"]   
   > ![Sid logg analys för diagnostikinställningar](media/monitor-blob-storage/diagnostic-logs-settings-pane-log-analytics.png)
...
Ersätt `<storage-service-resource--id>` plats hållaren i det här kodfragmentet med resurs-ID för BLOB service. Du hittar resurs-ID: t i Azure Portal genom att öppna sidan **Egenskaper** för ditt lagrings konto.
...
![Skärm bild av åtkomst till mått i Azure Portal](./media/monitor-blob-storage/access-metrics-portal.png)
...
![Skärm bild av åtkomst till mått med dimension i Azure Portal](./media/monitor-blob-storage/access-metrics-portal-with-dimension.png)
...
En lista över alla Azure Monitor support mått, som innehåller Azure Blob Storage, finns i [Azure Monitor mått som stöds](https://docs.microsoft.com/azure/azure-monitor/platform/metrics-supported).
...
I de här exemplen ersätter du `<resource-ID>` plats hållaren med resurs-ID för hela lagrings kontot eller Blob Storage-tjänsten. Du hittar dessa resurs-ID: n på **egenskaps** sidorna för ditt lagrings konto i Azure Portal.
...
Du kan visa mått definitionen för ditt lagrings konto eller Blob Storage-tjänsten. Använd cmdleten [Get-AzMetricDefinition](https://docs.microsoft.com/powershell/module/az.monitor/get-azmetricdefinition) .

I det här exemplet ersätter du `<resource-ID>` plats hållaren med resurs-ID för hela lagrings kontot eller resurs-ID: t för Blob Storage-tjänsten.  Du hittar dessa resurs-ID: n på **egenskaps** sidorna för ditt lagrings konto i Azure Portal.
...
Du kan läsa mått värden på konto nivå för ditt lagrings konto eller Blob Storage-tjänsten. Använd cmdleten [Get-AzMetric](https://docs.microsoft.com/powershell/module/Az.Monitor/Get-AzMetric) .
...
Du kan visa mått definitionen för ditt lagrings konto eller Blob Storage-tjänsten. Använd kommandot [AZ Monitor Metric List-definitions](https://docs.microsoft.com/cli/azure/monitor/metrics#az-monitor-metrics-list-definitions) .
...
Du kan läsa mått värden för ditt lagrings konto eller Blob Storage-tjänsten. Använd kommandot [AZ Monitor Metric List](https://docs.microsoft.com/cli/azure/monitor/metrics#az-monitor-metrics-list) .
...
En detaljerad referens för de fält som visas i dessa loggar finns i [referens för Azure Blob Storage övervaknings data](monitor-blob-storage-reference.md).
...
Begär Anden som görs av Blob Storage-tjänsten, till exempel när loggen skapas eller tas bort, loggas inte. En fullständig lista över de loggade data som loggas finns i [lagrings loggade åtgärder och status meddelanden](/rest/api/storageservices/storage-analytics-logged-operations-and-status-messages) och [logg format för lagring](monitor-blob-storage-reference.md).
...
Data lagras i tabellen **StorageBlobLog** . Loggar för Data Lake Storage Gen2 visas inte i en dedikerad tabell. Det beror på att Data Lake Storage Gen2 inte är tjänst. Det är en uppsättning funktioner som du kan aktivera i ditt lagrings konto. Om du har aktiverat dessa funktioner kommer loggar att fortsätta visas i StorageBlobLogs-tabellen. 

#### <a name="sample-kusto-queries"></a>Exempel på Kusto-frågor

Här följer några frågor som du kan ange i **logg Sök** fältet för att hjälpa dig att övervaka Blob Storage. Dessa frågor fungerar med det [nya språket](https://docs.microsoft.com/azure/azure-monitor/log-query/log-query-overview).
