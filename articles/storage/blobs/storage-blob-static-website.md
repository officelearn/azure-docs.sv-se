...
Du kan använda en anpassad domän som värd för ditt webbinnehåll. Om du vill göra det, följer du riktlinjerna i [konfigurera ett anpassat domännamn för Azure Storage-kontot](storage-custom-domain-name.md). Du hittar din webbplats på ett anpassat domännamn via HTTPS i [använda Azure CDN för att få åtkomst till blobar med anpassade domäner över HTTPS](storage-https-custom-domain-cdn.md).
