...
> 
> [!NOTE]
> Mappningen fungerar endast för underdomäner (t.ex. www\.contoso.com). Om du vill ha din web-slutpunkt som är tillgängliga på rotdomänen (t.ex. contoso.com) kommer du behöva [använda Azure CDN med anpassade domäner](storage-https-custom-domain-cdn.md)
...
| Webb |  http:\//mystorageaccount. [ Zone].Web.Core.Windows.NET/$Web/[indexdoc] eller http:\//mystorageaccount. [ Zone].Web.Core.Windows.NET/[indexdoc] eller http:\//mystorageaccount. [ Zone].Web.Core.Windows.NET/$Web eller http:\//mystorageaccount. [ Zone].Web.Core.Windows.NET/ | http:\//www.contoso.com/$ webb- eller http:\//www.contoso.com/ eller http:\//www.contoso.com/$ web / [indexdoc] eller http:\//www.contoso.com/ [indexdoc] |

> [!NOTE]  
> I följande avsnitt visas gäller även alla exempel för blob service-slutpunkt för web service-slutpunkt.

## <a name="direct-vs-intermediary-cname-mapping"></a>Direct jämfört med mellanliggande CNAME-mappning

Du kan ange din anpassade domän som föregås av en underdomän (till exempel www\.contoso.com) till blob-slutpunkten för ditt lagringskonto i något av två sätt: 
...
1. Skapa en ny CNAME-post, ange en underdomän alias som **www** eller **foton** (underdomän krävs, rotdomäner stöds inte), och sedan ange ett värdnamn.  
...
Ta bort en anpassad domän-registrering med den [az storage-konto update](https://docs.microsoft.com/cli/azure/storage/account) CLI kommandot och ange en tom sträng (`""`) för den `--custom-domain` argumentvärde.
