...
För att kontrol lera att det nya lagrings kontot använder konto krypterings nyckeln anropar du kommandot för Azure CLI- [AZ lagrings konto](/cli/azure/storage/account#az-storage-account-show) . Det här kommandot returnerar en lista över lagrings konto egenskaper och deras värden. Leta efter egenskapen `keyType` och kontrol lera att den är inställd på `Account`.
