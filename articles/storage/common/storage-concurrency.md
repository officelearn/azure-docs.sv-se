...
description: Hantera samtidighet för BLOB-, kö-, tabell-och fil tjänster
services: storage
author: jasontang501
ms.service: storage
ms.devlang: dotnet
...
## <a name="azure-storage--simplifies-cloud-development"></a>Azure Storage – fören klar moln utveckling
...
Du kan välja att använda antingen optimistisk eller pessimistisk samtidighets modell för att hantera åtkomst till blobbar och behållare i Blob-tjänsten. Om du inte uttryckligen anger en strategi senast skrivs WINS som standard.  
...
Följande C# fragment (med 4.2.0) visar ett enkelt exempel på hur du skapar en **If-Match-AccessCondition** baserat på etag-värdet som nås från egenskaperna för en blob som tidigare har hämtats eller sitter. Den använder sedan **AccessCondition** -objektet när det uppdaterar blobben: **AccessCondition** -objektet lägger till **If-Match-** huvudet i begäran. Om en annan process har uppdaterat blobben returnerar Blob-tjänsten ett HTTP-412 (Precondition Failed) status meddelande. Du kan hämta hela exemplet här: [Hantera samtidighet med hjälp av Azure Storage](https://code.msdn.microsoft.com/Managing-Concurrency-using-56018114).  
...
Lagrings tjänsten innehåller också stöd för ytterligare villkorliga huvuden som **If-Modified-sedan**, **IF-Unmodified-** on och **If-None-Match** och kombinationer av dessa. Mer information finns i [ange villkorliga huvuden för BLOB service-åtgärder](https://msdn.microsoft.com/library/azure/dd179371.aspx) på MSDN.  
...
Om du vill låsa en BLOB för exklusiv användning kan du skaffa ett [lån](https://msdn.microsoft.com/library/azure/ee691972.aspx) på den. När du skaffar ett lån anger du hur länge du behöver lånet: Detta kan vara mellan 15 och 60 sekunder eller oändlig, vilket är ett exklusivt lås. Du kan förnya ett begränsat lån för att utöka det och du kan släppa eventuella lån när du är färdig med det. Blob-tjänsten släpper automatiskt begränsade lån när de upphör att gälla.  
...
Följande C# fragment visar ett exempel på att förvärva ett exklusivt lån i 30 sekunder på en BLOB, uppdatera innehållet i blobben och sedan släppa lånet. Om det redan finns ett giltigt lån i blobben när du försöker hämta ett nytt lån returnerar Blob-tjänsten status resultatet "HTTP (409) konflikt". I följande kodfragment används ett **AccessCondition** -objekt för att kapsla in låne informationen när en begäran om att uppdatera bloben i lagrings tjänsten görs.  Du kan hämta hela exemplet här: [Hantera samtidighet med hjälp av Azure Storage](https://code.msdn.microsoft.com/Managing-Concurrency-using-56018114).
...
## <a name="managing-concurrency-in-the-table-service"></a>Hantera samtidighet i tabell tjänsten
Tabell tjänsten använder optimistisk concurrency-kontroller som standard beteende när du arbetar med entiteter, till skillnad från BLOB service där du uttryckligen måste välja att utföra optimistiska samtidighets kontroller. Den andra skillnaden mellan tabellerna och blob-tjänsterna är att du bara kan hantera samtidighet i entiteter med Blob-tjänsten. du kan hantera samtidigheten för både behållare och blobbar.  
...
5. Om det aktuella ETag-värdet för entiteten är detsamma som ETag i det obligatoriska **If-Match-** huvudet i begäran eller **om IF-match-** huvudet innehåller jokertecknet (*), utför tjänsten den begärda åtgärden och uppdaterar aktuell etag värdet för entiteten för att visa att den har uppdaterats.  

Observera att i motsats till BLOB Service kräver tabell tjänsten att klienten inkluderar ett **If-Match-** huvud i Update-begäranden. Det är dock möjligt att tvinga en ovillkorlig uppdatering (senaste skrivarens WINS-strategi) och kringgå samtidighets kontroller om klienten anger **If-Match-** huvudet till jokertecknet (*) i begäran.  

I följande C# kodfragment visas en kundentitet som tidigare har skapats eller hämtats med e-postadressen uppdaterad. Den inledande INSERT-eller hämtnings åtgärden lagrar ETag-värdet i objektet kund, och eftersom exemplet använder samma objekt instans när den utför ersättnings åtgärden skickar den automatiskt ETag-värdet tillbaka till tabell tjänsten, vilket gör att tjänsten kan Sök efter samtidiga överträdelser. Om en annan process har uppdaterat entiteten i Table Storage, returnerar tjänsten ett HTTP 412-status meddelande (Precondition Failed).  Du kan hämta hela exemplet här: [Hantera samtidighet med hjälp av Azure Storage](https://code.msdn.microsoft.com/Managing-Concurrency-using-56018114).
...
## <a name="managing-concurrency-in-the-file-service"></a>Hantera samtidighet i fil tjänsten
...
Microsoft Azure Storages tjänsten har utformats för att uppfylla behoven hos de mest komplexa online-programmen utan att utvecklare kan kompromettera eller komma åt viktiga design antaganden, till exempel samtidighet och data konsekvens som de har kommit att vidta för låta.  
