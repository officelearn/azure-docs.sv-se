...
* **Geo-replikering:** Din Azure Storage kan geo-replikeras. Även om detta ger dig geografisk återställning och dataredundans, innebär en växling till den geo-replikerade platsen en avsevärd försämring av prestandan och kan medföra ytterligare kostnader. Så rekommenderar vi att du väljer geo-replikering klokt och bara om värdet i data är motiverar den extra kostnaden.
* **Livscykelhantering för data:** alla data i alla filsystem passerar sin egen livscykeln för är mycket viktig och används ofta via mindre värde, mindre nås via att arkivera eller tas bort. Azure Storage tillhandahåller data skiktning och livscykel principer för hantering som tjänstnivån data på lämpligt sätt för dess steg i livscykeln.
...
> [!NOTE]
> De flesta HDFS-kommandon (till exempel `ls`, `copyFromLocal` och `mkdir`) fungerar fortfarande som förväntat. Endast de kommandon som är specifika för den interna HDFS-implementeringen (som kallas DFS), t.ex `fschk` och `dfsadmin`, visa olika beteenden i Azure-lagring.
...
> [!NOTE]
> Under förhandsversion av Data Lake lagring Gen2 endast `--type LRS` stöds. Ytterligare redundansalternativ blir tillgängliga i hela förhandsgranskningsprogrammet.
...
Den &lt;FileSystem&gt; identifierar sökvägen för lagring av Azure Data Lake i filsystemet.
Den &lt;AccountName&gt; identifierar namnet på Azure Storage-kontot. Ett fullständigt kvalificerat domännamn (FQDN) krävs.

Om värden för &lt;FileSystem&gt; eller &lt;AccountName&gt; har angetts används standardfilsystemet. För filer i filsystemet kan du använda en relativ sökväg eller en absolut sökväg. Till exempel den *hadoop-mapreduce-examples.jar* fil som medföljer HDInsight-kluster kan refereras till genom att använda någon av följande sökvägar:
...
&lt;Sökvägen&gt; är filens eller katalogens HDFS-sökvägsnamn.
