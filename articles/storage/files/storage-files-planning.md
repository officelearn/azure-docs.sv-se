...
För att hjälpa oss att prioritera nya regioner och funktioner, Fyll i det här [undersökningen](https://aka.ms/azurefilesatscalesurvey).

### <a name="steps-to-onboard"></a>Steg för att publicera

Om du vill registrera din prenumeration på större filförhandsgranskning för resurser som du behöver använda Azure PowerShell. Du kan använda [Azure Cloud Shell](https://shell.azure.com/) eller installera den [lokalt Azure PowerShell-modulen](https://docs.microsoft.com/powershell/azure/install-Az-ps?view=azps-2.4.0) att köra följande PowerShell-kommandon:

Kontrollera först den prenumeration som du vill registrera i förhandsversionen har valts:
...
Registrera sedan i förhandsversionen med hjälp av följande kommandon:
