...
Om du vill skapa ett lagrings konto med hjälp av PowerShell använder vi `New-AzStorageAccount`-cmdleten. Denna cmdlet har många alternativ; endast de alternativ som krävs visas. Mer information om avancerade alternativ finns i dokumentationen för [`New-AzStorageAccount` cmdlet](/powershell/module/az.storage/new-azstorageaccount).
...
Vi använder följande kommando för att skapa ett lagrings konto som kan lagra standard Azure-filresurser. Parametern `-SkuName` relaterar till den typ av redundans som önskas. Om du vill ha ett Geo-redundant eller Geo-redundant lagrings konto måste du också ta bort `-EnableLargeFileShare`-parametern.
...
För att kunna skapa ett lagrings konto som kan lagra Premium Azure-filresurser, kommer vi att använda följande kommando. Observera att parametern `-SkuName` har ändrats till att inkludera både `Premium` och önskad redundans nivå av lokalt redundant (`LRS`). Parametern `-Kind` `FileStorage` i stället för `StorageV2` eftersom Premium-filresurser måste skapas i ett FileStorage lagrings konto i stället för ett GPv2-lagrings konto.
...
Vi använder följande kommando för att skapa ett lagrings konto som kan lagra standard Azure-filresurser. Parametern `--sku` relaterar till den typ av redundans som önskas. Om du vill ha ett Geo-redundant eller Geo-redundant lagrings konto måste du också ta bort `--enable-large-file-share`-parametern.
...
För att kunna skapa ett lagrings konto som kan lagra Premium Azure-filresurser, kommer vi att använda följande kommando. Observera att parametern `--sku` har ändrats till att inkludera både `Premium` och önskad redundans nivå av lokalt redundant (`LRS`). Parametern `--kind` `FileStorage` i stället för `StorageV2` eftersom Premium-filresurser måste skapas i ett FileStorage lagrings konto i stället för ett GPv2-lagrings konto.
...
# <a name="powershell"></a>[PowerShell](#tab/azure-powershell)
Du kan skapa Azure-filresursen med [`New-AzRmStorageShare`](/powershell/module/az.storage/New-AzRmStorageShare) -cmdleten. Följande PowerShell-kommandon förutsätter att du har angett variablerna `$resourceGroupName` och `$storageAccountName` som definieras ovan i avsnittet Skapa ett lagrings konto med Azure PowerShell. 

> [!Important]  
> För Premium-filresurser refererar `-QuotaGiB`-parametern till fil resursens allokerade storlek. Den allokerade storleken på fil resursen är den mängd som du debiteras för, oavsett användning. Standard fil resurser faktureras baserat på användning i stället för en etablerad storlek.
...
# <a name="azure-cli"></a>[Azure CLI](#tab/azure-cli)
Innan vi kan skapa en Azure-filresurs med Azure CLI måste du skaffa en lagrings konto nyckel för att auktorisera fil resursens skapa-åtgärd med. Detta kan göras med kommandot [`az storage account keys list`](/cli/azure/storage/account/keys) :
...
När du har lagrings konto nyckeln kan du skapa Azure-filresursen med kommandot [`az storage share create`](/cli/azure/storage/share) . 

> [!Important]  
> För Premium-filresurser refererar `--quota`-parametern till fil resursens allokerade storlek. Den allokerade storleken på fil resursen är den mängd som du debiteras för, oavsett användning. Standard fil resurser faktureras baserat på användning i stället för en etablerad storlek.
...
Det här kommandot kommer inte att fungera om lagrings kontot finns i ett virtuellt nätverk och datorn som du anropar kommandot från inte ingår i det virtuella nätverket. Du kan undvika den här begränsningen genom att använda Azure PowerShell `New-AzRmStorageShare`-cmdleten enligt beskrivningen ovan, eller genom att köra Azure CLI från en dator som är en del av det virtuella nätverket, inklusive via en VPN-anslutning.
