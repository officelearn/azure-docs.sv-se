...
[Azure Files](storage-files-introduction.md) är Microsofts lättanvända filsystem i molnet. Azure file shares can be mounted in Linux distributions using the [SMB kernel client](https://wiki.samba.org/index.php/LinuxCIFS). This article shows two ways to mount an Azure file share: on-demand with the `mount` command and on-boot by creating an entry in `/etc/fstab`.
...
In order to mount an Azure file share on Linux, port 445 must be accessible. Många organisationer blockerar port 445 på grund av säkerhetsrisker med SMB 1. SMB 1, also known as CIFS (Common Internet File System), is a legacy file system protocol included with many Linux distributions. SMB 1 är ett inaktuellt, ineffektivt och framför allt oskyddat protokoll. The good news is that Azure Files does not support SMB 1, and starting with Linux kernel version 4.18, Linux makes it possible to disable SMB 1. We always [strongly recommend](https://aka.ms/stopusingsmb1) disabling the SMB 1 on your Linux clients before using SMB file shares in production.
...
Once you have unmounted all SMB file shares, it's safe to unload the module. Det kan du göra med kommandot `modprobe`:
