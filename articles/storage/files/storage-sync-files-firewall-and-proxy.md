...
Om &ast;. one.microsoft.com är för breda, du kan begränsa serverns kommunikation genom att tillåta att endast explicit regionala instanser av Azure Files Sync-tjänsten. Vilka instanser att välja beror på regionen som den Lagringssynkroniseringstjänst som du har distribuerat och registrerat servern. Det är den region som du vill tillåta för servern. Snart kommer att fler URL: er för att aktivera nya funktioner för affärskontinuitet. 

| Region | Azure File Sync regionala slutpunkts-URL |
|--------|---------------------------------------|
...
> [!Important]
> Om du definierar dessa detaljerad brandväggsregler, kontrollera det här dokumentet ofta och uppdatera brandväggsreglerna för att undvika avbrott i tjänsten på grund av inaktuella eller ofullständig URL-listor i inställningarna för brandväggen.

## <a name="summary-and-risk-limitation"></a>Sammanfattning och risk begränsning
Listorna tidigare i det här dokumentet innehåller URL: er med Azure File Sync för närvarande kommunicerar med. Brandväggar måste kunna tillåta trafik utgående trafik till dessa domäner som svar från dem. Microsoft strävar efter att den här listan har uppdaterats.

Konfigurera domänen att begränsa brandväggsregler kan vara ett mått för att förbättra säkerheten. Om dessa brandväggskonfigurationer används måste en ha i åtanke att URL: er kommer läggs till och ändras med tiden. Därför är det ett ansvarsfull mått för att kontrollera tabellerna i det här dokumentet som en del av en process för ändringshantering från en version av Azure File Sync-agenten till en annan på en test-distribution av den senaste agenten. På så sätt kan du se till att brandväggen är konfigurerad för att tillåta trafik till domäner senaste agenten kräver.

## <a name="next-steps"></a>Nästa steg
- [Planera för distribution av Azure File Sync](storage-sync-files-planning.md)
- [Distribuera Azure File Sync (förhandsversion)](storage-sync-files-deployment-guide.md)
