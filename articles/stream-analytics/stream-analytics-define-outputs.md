...
| Azure Blob Storage | Ja | Använd {date} och {time}-token från händelsen fälten i sökväg-mönster. Välj datumformat, till exempel ÅÅÅÅ-MM/DD, DD/MM/ÅÅÅÅ-MM-DD-ÅÅÅÅ. HH används för tidsformat. Som en del av den [preview](https://aka.ms/ASAPreview), blob utdata kan partitioneras av en enda anpassade händelsen attributet {fieldname}. | Följer inkommande partitionering för [fullständigt parallell frågor](stream-analytics-scale-jobs.md). | 
