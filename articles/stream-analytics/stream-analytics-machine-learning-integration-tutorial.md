...
ms.locfileid: "73467890"
---
# <a name="perform-sentiment-analysis-with-azure-stream-analytics-and-azure-machine-learning-studio-classic-preview"></a>Utföra sentiment-analys med Azure Stream Analytics och Azure Machine Learning Studio (klassisk) (för hands version)
...
1. Klicka på **skapa en resurs** > **lagring** > **lagrings konto**i Azure Portal.
...
4. I Azure Portal väljer du lagrings kontot. I bladet lagrings konto klickar du på **behållare** och klickar sedan på **+&nbsp;behållare** för att skapa Blob Storage.
...
8. På bladet **Ladda upp BLOB** laddar du upp den **sampleinput. csv** -fil som du laddade ned tidigare. För **Blob-typ**väljer du **blockera BLOB** och ställer in block storleken på 4 MB, vilket är tillräckligt för den här självstudien.
...
3. Namnge jobbet `azure-sa-ml-demo`, ange en prenumeration, ange en befintlig resurs grupp eller skapa en ny och välj platsen för jobbet.
...
2. I bladet **indata** klickar du på **Lägg till Stream-indata** >**Blob Storage**
...
3. Fyll i bladet **Blob Storage** med följande värden:
   ...
   |Fält  |Värde  |
   |---------|---------|
   |**Inmatat alias** | Använd namnet `datainput` och välj **Blob Storage från din prenumeration**       |
   |**Lagringskonto**  |  Välj det lagrings konto som du skapade tidigare.  |
   |**Fönster**  | Välj den behållare som du skapade tidigare (`azuresamldemoblob`)        |
...
2. Klicka på **Lägg till** >**Blob Storage**i bladet **utdata** och Lägg sedan till utdata med aliaset `datamloutput`. 

3. Fyll i bladet **Blob Storage** med följande värden:

   |Fält  |Värde  |
   |---------|---------|
   |**Alias för utdata** | Använd namnet `datamloutput` och välj **Blob Storage från din prenumeration**       |
   |**Lagringskonto**  |  Välj det lagrings konto som du skapade tidigare.  |
   |**Fönster**  | Välj den behållare som du skapade tidigare (`azuresamldemoblob`)        |
   |**Händelseserialiseringsformat**  |  Välj **CSV**       |
...
2. Navigera till bladet jobb > **funktioner** >  **+ Lägg till** > **azureml**
...
4. Klicka på **Spara** för att spara frågan.
...
2. Om du har ett verktyg som du normalt använder för att undersöka innehållet i Blob Storage, använder du verktyget för att undersöka `azuresamldemoblob` containern. Du kan också utföra följande steg i Azure Portal:

    1. Leta upp `samldemo` lagrings konto i portalen och leta upp `azuresamldemoblob` container i kontot. Du ser två filer i behållaren: filen som innehåller exempel-tweets och en CSV-fil som genereras av Stream Analyticss jobbet.
