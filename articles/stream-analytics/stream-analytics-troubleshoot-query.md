...
2.  [Felsöka frågor steg för steg lokalt med hjälp av jobbdiagram](debug-locally-using-job-diagram.md) i Azure Stream Analytics-verktyg för Visual Studio. Jobbdiagrammet är att visa hur data flödar från indatakällor (Event Hub, IoT Hub, etc.) genom flera frågesteg och slutligen utdata till sänkor. Varje frågesteg mappas till en tillfällig resultatuppsättning som definieras i skriptet med HJÄLP av WITH-satsen. Du kan visa data samt mått i varje frågesteg i varje mellanliggande resultatuppsättning för att hitta källan till problemet.
...
## <a name="job-is-consuming-too-many-streaming-units"></a>Jobbet förbrukar för många streamingenheter
...
I realtid databehandling, att veta hur data ser ut i mitten av frågan kan vara till hjälp. Eftersom indata eller steg i ett Azure Stream Analytics-jobb kan läsas flera gånger kan du skriva extra SELECT INTO-satser. Om du gör det matas mellanliggande data ut i lagring och du kan kontrollera att data är korrekta, precis som *klockvariabler* gör när du felsöker ett program.
