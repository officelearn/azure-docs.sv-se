...
1. Skriver över referensdatablobar inte orsakar Stream Analytics för att läsa in bloben och i vissa fall kan det orsaka jobb misslyckas. Det rekommenderade sättet att ändra referensdata är att lägga till en ny blob med hjälp av mönstret för samma behållare och sökväg som har definierats i jobbindata och använda ett datum/tid **större** än den som angetts av senaste blob i sekvensen.
2. Referensdatablobar är **inte** sorteras efter blobens ”senast ändrad” tid men endast av tid och datum som anges i blob namn med hjälp av den {date} och {time} ersättning.
