---
title: Självstudiekurs - Kör Azure-funktioner i Azure Stream Analytics-jobb
...
---
# <a name="tutorial-run-azure-functions-from-azure-stream-analytics-jobs"></a>Självstudiekurs: Kör Azure-funktioner från Azure Stream Analytics-jobb 
...
Följ anvisningarna i självstudien [Upptäck bedrägerier i realtid](stream-analytics-real-time-fraud-detection.md) och skapa en händelsehubb. Starta händelsegeneratorprogrammet och skapa Stream Analytics-jobbet. Hoppa över stegen för att skapa frågan och utdata. I stället, se följande avsnitt för att ställa in en Azure Functions-utdata.
...
1. Läs avsnittet [Skapa en funktionsapp](../azure-functions/functions-create-first-azure-function.md#create-a-function-app) i dokumentationen till Functions. I det här avsnittet får du lära dig hur du skapar en funktionsapp och en [HTTP-utlöst funktion i Azure Functions](../azure-functions/functions-create-first-azure-function.md#create-function)med hjälp av CSharp-språket.  

2. Gå till funktionen **run.csx**. Uppdatera den med nedanstående kod. Ersätt **\<" din Azure Cache for\>Redis-anslutningssträng går hit "** med den primära anslutningssträngen för Azure Cache for Redis som du hämtade i föregående avsnitt. 
...
2. Bläddra till din funktion och välj > **Översiktutdata** > **Lägg till**. **Overview** Om du vill lägga till en ny utdatakanal väljer du **Azure-funktion** för kanalmottagaralternativet. Utdataadaptern för Functions har följande egenskaper:  
...
4. Öppna Stream Analytics-jobbet och uppdatera frågan till följande. Om du inte har namngivningen för utdatamottagaren **saop1**bör du komma ihåg att ändra den i frågan.  
...
2. Verifiera att dina data finns i Azure Cache for Redis genom att använda [Azure Cache for Redis-kommandon](https://redis.io/commands). (Kommandot tar formatet Hämta {key}.) Till exempel:
...
Om ett fel inträffar när händelser skickas till Azure Functions, försöker Stream Analytics de flesta åtgärder. Alla http undantag görs på nytt tills framgång med undantag för http fel 413 (entiteten för stor). Ett för stort fel behandlas som ett datafel som är föremål för [principen för återförsök eller släpp.](stream-analytics-output-error-policy.md)

> [!NOTE]
> Tidsgränsen för HTTP-begäranden från Stream Analytics till Azure Functions är inställd på 100 sekunder. Om din Azure Functions-app tar mer än 100 sekunder att bearbeta en batch, felar Stream Analytics.
...
Användningen av [HTTP-routning](https://docs.microsoft.com/sandbox/functions-recipes/routes?tabs=csharp) på dina Azure-funktioner stöds för närvarande inte av Stream Analytics.

Stöd för att ansluta till Azure-funktioner som finns i ett virtuellt nätverk är inte aktiverat.
