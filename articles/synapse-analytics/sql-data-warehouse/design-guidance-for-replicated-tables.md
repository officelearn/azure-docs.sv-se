...
ms.locfileid: "80582228"
---
# <a name="design-guidance-for-using-replicated-tables-in-synapse-sql"></a>Designvägledning för användning av replikerade tabeller i Synapse SQL

Den här artikeln innehåller rekommendationer för att utforma replikerade tabeller i Synapse SQL-schemat. Använd dessa rekommendationer för att förbättra frågeprestanda genom att minska dataflyttning och frågekomplexitet.

> [!VIDEO https://www.youtube.com/embed/1VS_F37GI9U]

## <a name="prerequisites"></a>Krav

Den här artikeln förutsätter att du är bekant med datadistribution och datarörelsebegrepp i Synapse SQL.Mer information finns [architecture](massively-parallel-processing-mpp-architecture.md) i arkitekturartikeln. 
...
- Har jag fakta- och dimensionstabeller i en Synapse SQL-databas?   
...
Följande diagram visar en replikerad tabell som är tillgänglig för varje beräkningsnod. I Synapse SQL kopieras den replikerade tabellen helt till en distributionsdatabas på varje beräkningsnod. 
...
- Tabellstorleken på disken är mindre än 2 GB, oavsett antalet rader. Om du vill hitta storleken på en tabell kan du `DBCC PDW_SHOWSPACEUSED('ReplTableCandidate')`använda kommandot [DBCC PDW_SHOWSPACEUSED:](/sql/t-sql/database-console-commands/dbcc-pdw-showspaceused-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest) . 
- Tabellen används i kopplingar som annars skulle kräva dataflyttning. När du sammanfogar tabeller som inte distribueras i samma kolumn, till exempel en hash-distribuerad tabell till en round-robin-tabell, krävs dataförflyttning för att slutföra frågan.  Om en av tabellerna är liten bör du överväga en replikerad tabell. Vi rekommenderar att du använder replikerade tabeller i stället för round-robin-tabeller i de flesta fall. Om du vill visa dataflyttningar i frågeplaner använder [du sys.dm_pdw_request_steps](/sql/relational-databases/system-dynamic-management-views/sys-dm-pdw-request-steps-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest).  BroadcastMoveOperation är den typiska dataflyttningsåtgärden som kan elimineras med hjälp av en replikerad tabell.  
...
- Synapse SQL-databasen skalas ofta. Skalning av en databas ändrar antalet beräkningsnoder, vilket medför att återskapa den replikerade tabellen.
- Tabellen har ett stort antal kolumner, men dataåtgärder kommer vanligtvis bara åt ett litet antal kolumner. I det här fallet, i stället för att replikera hela tabellen, kan det vara mer effektivt att distribuera tabellen och sedan skapa ett index på de kolumner som används ofta. När en fråga kräver dataflyttning flyttas endast data för de begärda kolumnerna.
...
I det här exemplet används [CTAS](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest) för att ändra dimsalesterritorytabellen till en replikerad tabell. Det här exemplet fungerar oavsett om DimSalesTerritory är hash-distribuerad eller round-robin.
...
En replikerad tabell implementeras genom att behålla en huvudversion av tabellen. Den kopierar huvudversionen till en distributionsdatabas på varje beräkningsnod. När det sker en ändring uppdateras huvudtabellen först. Därefter återskapas tabellen på varje beräkningsnod. En ombyggnad av en replikerad tabell inkluderar att kopiera tabellen till varje beräkningsnod och sedan skapa indexen.  En replikerad tabell på en DW400 har till exempel 5 kopior av data.  En huvudkopia och en fullständig kopia på varje beräkningsnod.  Alla data lagras i distributionsdatabaser för att stödja snabbare dataändringssatser och flexibla skalningsåtgärder. 
...
Standardindexeringsmetoder gäller för replikerade tabeller. Varje replikerat tabellindex återskapas som en del av en indexrekning. Använd endast index när resultatökningen uppväger kostnaden för att återskapa indexen.  
...
Den här frågan använder [sys.pdw_replicated_table_cache_state](/sql/relational-databases/system-catalog-views/sys-pdw-replicated-table-cache-state-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest) DMV för att lista de replikerade tabeller som har ändrats, men inte återskapats.
...
- [SKAPA TABELL](/sql/t-sql/statements/create-table-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
- [SKAPA TABELL SOM SELECT](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest)
