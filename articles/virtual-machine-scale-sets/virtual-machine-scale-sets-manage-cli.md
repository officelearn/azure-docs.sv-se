...
Om du vill visa ytterligare information om en speciell VM-instans lägger du till parametern `--instance-id` till [AZ VMSS get-instance-View](/cli/azure/vmss) och anger en instans som ska visas. Följande exempel visar information om den virtuella dator instansen *0* i skalnings uppsättningen med namnet *myScaleSet* och resurs gruppen *myResourceGroup* . Ange dina egna namn enligt följande:
...
## <a name="stop-and-start-vms-in-a-scale-set"></a>Stoppa och starta virtuella datorer i en skalnings uppsättning
...
## <a name="restart-vms-in-a-scale-set"></a>Starta om virtuella datorer i en skalnings uppsättning
...
Om du vill ta bort en eller flera virtuella datorer i en skalnings uppsättning använder du [AZ VMSS Delete-instances](/cli/azure/vmss). Med parametern `--instance-ids` kan du ange en eller flera virtuella datorer som ska tas bort. Om du anger * för instans-ID, tas alla virtuella datorer i skalnings uppsättningen bort. Om du vill ta bort flera virtuella datorer separerar du varje instans-ID med ett blank steg.
