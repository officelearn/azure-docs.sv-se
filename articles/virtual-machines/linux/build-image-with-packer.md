...
Nu kan du skapa en virtuell dator från avbildningen med [az vm skapa](/cli/azure/vm#create). Ange den avbildning som du skapade med den `--image` parameter. I följande exempel skapas en virtuell dator med namnet *myVM* från *myPackerImage* och genererar SSH-nycklar, om de inte redan finns:
...
Nu kan du öppna en webbläsare och ange `http://publicIpAddress` i adressfältet. Ange dina egna offentliga IP-adress från den virtuella datorn skapa processen. Sidan NGINX visas som i följande exempel:
