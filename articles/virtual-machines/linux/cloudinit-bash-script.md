...
Skapa ett enkelt bash-skript för att testa om du vill se den här funktionen i åtgärden. T.ex. moln-init `#cloud-config` filen skriptet måste vara lokal för där du ska köra kommandona AzureCLI att etablera den virtuella datorn.  I det här exemplet skapar du filen i molnet Shell inte på den lokala datorn. Du kan använda valfri redigerare som du vill. Ange `sensible-editor simple_bash.sh` att skapa filen och se en lista över tillgängliga redigerare. Välj #1 att använda den **nano** editor. Se till att hela molnet init-filen har kopierats korrekt, särskilt den första raden.  
...
Nu ska du skapa en virtuell dator med [az vm skapa](/cli/azure/vm#create) och ange bash-skriptfil med `--custom-data simple_bash.sh` på följande sätt:
