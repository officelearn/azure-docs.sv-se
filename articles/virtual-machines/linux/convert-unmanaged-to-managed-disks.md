---
title: Konvertera en virtuell Linux-dator i Azure från ohanterade diskar till hanterade diskar – Azure Managed Disks | Microsoft Docs
...
1. Frigör den virtuella datorn genom att använda [AZ VM](/cli/azure/vm)-frigörning. I följande exempel avallokeras den virtuella datorn `myVM` som heter i resurs gruppen `myResourceGroup`med namnet:
...
2. Frigör alla virtuella datorer genom att använda [AZ VM](/cli/azure/vm)-frigörning. I följande exempel avallokeras den virtuella datorn `myVM` som heter i resurs gruppen `myResourceGroup`med namnet:
