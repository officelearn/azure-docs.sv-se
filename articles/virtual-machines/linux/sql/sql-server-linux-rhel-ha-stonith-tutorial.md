...
Använd följande kommando för att skapa en resurs grupp `<resourceGroupName>` i en region. Ersätt `<resourceGroupName>` med ett namn som du väljer. Vi använder `East US 2` för den här självstudien. Mer information finns i följande [snabb start](../quick-create-cli.md).
...
1. Hämta en lista över virtuella dator avbildningar som erbjuder RHEL med HA:
    ...
    I den här självstudien väljer vi avbildningen `RedHat:RHEL-HA:7.6:7.6.2019062019`.
...
> [!IMPORTANT]
> Standard avbildningen som skapas med kommandot ovan skapar en 32 GB OS-disk som standard. Du kan eventuellt ta slut på utrymme med den här standard installationen. Du kan använda följande parameter som har lagts till i ovanstående `az vm create`-kommando för att skapa en OS-disk med 128 GB som exempel: `--os-disk-size-gb 128`.
>
...
1. Använd följande kommando för att öppna hosts-filen och konfigurera värd namns matchning. Mer information finns i [Konfigurera AG](/sql/linux/sql-server-linux-availability-group-configure-ha#prerequisites) på Konfigurera värd filen.
    ...
    I redigeraren för **vi** skriver du `i` för att infoga text och på en tom rad lägger du till den **privata IP-adressen** för motsvarande virtuella dator. Lägg sedan till namnet på den virtuella datorn efter ett blank steg bredvid IP-adressen. Varje rad måste ha en separat post.
...
1. På den primära noden kör du följande kommandon för att konfigurera klustret.
    ...
    - När du kör kommandot `pcs cluster auth` för att autentisera klusternoderna uppmanas du att ange ett lösen ord. Ange lösen ordet för **hacluster** -användaren som skapades tidigare.
...
### <a name="register-a-new-application-in-azure-active-directory"></a>Registrera ett nytt program i Azure Active Directory
 ...
 2. Öppna [bladet Azure Active Directory](https://ms.portal.azure.com/#blade/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/Properties). Gå till egenskaper och anteckna Directory-ID. Detta är `tenant ID`
...
Tilldela den anpassade roll `Linux Fence Agent Role-<username>` som skapades i det sista steget till tjänstens huvud namn. Använd inte ägarrollen längre!
 
1. Gå till https://portal.azure.com
2. Öppna [bladet alla resurser](https://ms.portal.azure.com/#blade/HubsExtension/BrowseAll)
3. Välj den virtuella datorn från den första noden i klustret
4. Klicka på **åtkomst kontroll (IAM)**
5. Klicka på **Lägg till en roll tilldelning**
6. Välj roll `Linux Fence Agent Role-<username>` i **roll** listan
...
> [!NOTE] 
> För enkelhetens skull lägger du till/opt/MSSQL-tools/bin/i miljövariabeln PATH. På så sätt kan du köra verktygen utan att ange den fullständiga sökvägen. Kör följande kommandon för att ändra sökvägen för både inloggnings-och sessioner med interaktiva/icke-inloggningssession:</br></br>
...
1. Anslut till den primära repliken med SSMS eller SQL CMD. Nedanstående kommandon skapar ett certifikat på `/var/opt/mssql/data/dbm_certificate.cer` och en privat nyckel på `var/opt/mssql/data/dbm_certificate.pvk` på den primära SQL Server repliken:
...
1. Kopiera de två filerna som har skapats till samma plats på alla servrar som ska vara värdar för tillgänglighets repliker.
    ...
    > [!NOTE]
    > Du behöver inte köra `sudo -i`, vilket ger rot miljön. Du kan bara köra kommandot `sudo` framför varje kommando som vi tidigare gjorde i den här självstudien.
...
1. Kör följande kommando på mål servern:
    ...
    - Kommandot `mv` flyttar filer eller kataloger från en plats till en annan.
...
- Ersätt `ag1` med önskat tillgänglighets grupp namn.
...
- Ersätt `<password>` med ett eget komplext lösen ord.
...
Om `synchronization_state_desc`s listan är SYNKRONISERAd för `db1`innebär det att replikerna är synkroniserade. Sekundärerna visar `db1` i den primära repliken.
...
1. Skapa den virtuella IP-resursen med hjälp av följande kommando:
    ...
    - Ersätt `<availableIP>` värde nedan med en oanvänd IP-adress.
...
Du kan testa STONITH genom att köra följande kommando. Försök köra kommandot nedan från `<VM1>` för `<VM3>`.
