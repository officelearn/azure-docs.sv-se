...
1. Ta bort den virtuella käll datorn. De virtuella diskarna bevaras.

2. Koppla den virtuella käll datorns OS-disk till en annan virtuell dator på samma plats i din Azure-prenumeration. Den här virtuella datorn kallas för den virtuella fel söknings datorn.

3. Med hjälp av den virtuella datorn för fel sökning skapar du några konfigurationsfiler på den virtuella käll datorns OS-disk.

4. Koppla bort den virtuella datorns OS-disk från den virtuella fel söknings datorn.

5. Använd en Resource Manager-mall för att skapa en virtuell dator med hjälp av den ursprungliga virtuella disken.

6. När den nya virtuella datorn startas uppdaterar config-filerna som du skapar lösen ordet för den begärda användaren.

> [!NOTE]
> Du kan automatisera följande processer:
>
> - Skapar den virtuella fel söknings datorn
> - Ansluta OS-disken
> - Återskapar den ursprungliga virtuella datorn
> 
> Det gör du med hjälp av [återställnings skript för virtuella Azure-datorer](https://github.com/Azure/azure-support-scripts/blob/master/VMRecovery/ResourceManager/README.md). Om du väljer att använda Azures återställnings skript för virtuella datorer kan du använda följande process i avsnittet detaljerade steg:
> 1. Hoppa över steg 1 och 2 genom att använda skripten för att koppla operativ system disken för den berörda virtuella datorn till en virtuell dator för återställning.
> 2. Följ steg 3 – 6 för att tillämpa åtgärds begränsningarna.
> 3. Hoppa över steg 7 – 9 med skripten för att återskapa den virtuella datorn.
> 4. Följ steg 10 och 11.

## <a name="detailed-steps-for-resource-manager"></a>Detaljerade steg för Resource Manager
...
1. Ta bort den virtuella datorn som påverkas i Azure Portal. Att ta bort den virtuella datorn tar bara bort metadata, referens för den virtuella datorn i Azure. De virtuella diskarna bevaras när den virtuella datorn tas bort:
   ...
   * Välj den virtuella datorn i Azure Portal, klicka på *ta bort*:
     
     ![Ta bort befintlig virtuell dator](./media/reset-local-password-without-agent/delete-vm.png)
...
2. Anslut den virtuella käll datorns OS-disk till den virtuella fel söknings datorn. Den virtuella fel söknings datorn måste finnas i samma region som den virtuella käll datorns OS- `West US`disk (till exempel):
   ...
   1. Välj den virtuella fel söknings datorn i Azure Portal. Klicka på *diskar* | *bifoga befintliga*:
     ...
     ![Bifoga befintlig disk](./media/reset-local-password-without-agent/disks-attach-existing.png)
     
   2. Välj *VHD-fil* och välj sedan det lagrings konto som innehåller den virtuella käll datorn:
     
     ![Välj lagringskonto](./media/reset-local-password-without-agent/disks-select-storage-account.png)
     
   3. Välj käll container. Käll behållaren är vanligt vis *virtuella hård diskar*:
     
     ![Välj lagrings behållare](./media/reset-local-password-without-agent/disks-select-container.png)
     
   4. Välj den virtuella OS-disk som ska bifogas. Klicka på *Välj* för att slutföra processen:
     
     ![Välj virtuell käll disk](./media/reset-local-password-without-agent/disks-select-source-vhd.png)
...
3. Anslut till fel söknings datorn med hjälp av fjärr skrivbord och se till att den virtuella käll datorns OS-disk är synlig:
   ...
   3. Leta efter den datadisk som du har anslutit i Utforskaren. Om den virtuella datorns virtuella hård disk är den enda datadisk som är ansluten till den virtuella fel söknings datorn ska den vara enheten F:
     ...
     ![Visa ansluten datadisk](./media/reset-local-password-without-agent/troubleshooting-vm-file-explorer.png)

4. Skapa `gpt.ini`på den virtuella käll hård disken (om GPT. ini finns, Byt namn till GPT. ini. bak): `\Windows\System32\GroupPolicy`
...
6. Skapa `FixAzureVM.cmd` `<username>` `<newpassword>` i `\Windows\System32` med följande innehåll och ersätt och med dina egna värden:
...
7. Ta bort disken från den virtuella fel söknings datorn i Azure Portal:
   ...
   2. Välj den data disk som är ansluten i steg 2, klicka på *Koppla från*:
     
     ![Koppla från disk](./media/reset-local-password-without-agent/detach-disk.png)

8. Innan du skapar en virtuell dator hämtar du URI: n till käll-OS-disken:
   
   1. Välj lagrings kontot i Azure Portal och klicka på *blobbar*.

   2. Välj behållaren. Käll behållaren är vanligt vis *virtuella hård diskar*:
     
     ![Välj Storage-kontots BLOB](./media/reset-local-password-without-agent/select-storage-details.png)
     
   3. Välj den virtuella käll datorn för VM OS och klicka på knappen *Kopiera* bredvid *URL* -namnet:
     
     ![Kopiera disk-URI](./media/reset-local-password-without-agent/copy-source-vhd-uri.png)

9. Skapa en virtuell dator från den virtuella käll datorn OS-disk:
   
   1. Använd [den här Azure Resource Manager-mallen](https://github.com/Azure/azure-quickstart-templates/tree/master/201-vm-specialized-vhd-new-or-existing-vnet) för att skapa en virtuell dator från en specialiserad virtuell hård disk. Klicka på `Deploy to Azure` knappen för att öppna Azure Portal med den mallade informationen som är ifylld för dig.

   2. Om du vill behålla alla tidigare inställningar för den virtuella datorn väljer du *Redigera mall* för att tillhandahålla ditt befintliga VNet, undernät, nätverkskort eller offentlig IP-adress.

   3. I text rutan parameter klistrar du in URI för din käll-VHD i föregående steg: `OSDISKVHDURI`
     
     ![Skapa en virtuell dator från en mall](./media/reset-local-password-without-agent/create-new-vm-from-template.png)

10. När den nya virtuella datorn har körts ansluter du till den virtuella datorn med hjälp av fjärr skrivbord med det nya lösen `FixAzureVM.cmd` ordet som du angav i skriptet.

11. Ta bort följande filer från fjärrsessionen till den nya virtuella datorn för att rensa miljön:
...
4. `gpt.ini` `gpt.ini` Skapa på den virtuella käll datorn (om den finns, Byt namn till `gpt.ini.bak`): `\Windows\System32\GroupPolicy`
