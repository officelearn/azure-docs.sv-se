...
3. Skapa `gpt.ini`på den virtuella käll hård disken (om GPT. ini finns, Byt namn till GPT. ini. bak): `\Windows\System32\GroupPolicy`
...
5. Skapa `FixAzureVM.cmd` `<username>` `<newpassword>` i `\Windows\System32` med följande innehåll och ersätt och med dina egna värden:
