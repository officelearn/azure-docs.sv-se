...
Om du inte kan ansluta till en Windows-dator (VM), kan du återställa ditt lösenord för lokal administratör eller återställa Remote Desktop Services-konfigurationen (stöds inte på Windows-domänkontrollanter). Om du vill återställa lösenordet, kan du använda Azure portal eller VM Access-tillägg i Azure PowerShell. När du har loggat in på den virtuella datorn kan du återställa lösenordet för den lokala administratören.  
...
Logga in på Azure-portalen och välj sedan **virtuella datorer** på den vänstra menyn.

### <a name="reset-the-local-administrator-account-password"></a>**Återställa lösenordet för lokala administratörskontot**

Välj din virtuella Windows-dator och därefter **Återställ lösenord** under **stöd + felsökning**. Den **Återställ lösenord** -fönstret visas.

Välj **Återställ lösenord**, ange ett användarnamn och ett lösenord och välj sedan **uppdatering**. Vill du försöka ansluta till den virtuella datorn igen.

### <a name="reset-the-remote-desktop-services-configuration"></a>**Återställ Remote Desktop Services-konfiguration**

Välj din virtuella Windows-dator och därefter **Återställ lösenord** under **stöd + felsökning**. Den **Återställ lösenord** -fönstret visas. 

Välj **Återställ endast konfiguration** och välj sedan **uppdatering**. Vill du försöka ansluta till den virtuella datorn igen.


## <a name="reset-by-using-the-vmaccess-extension-and-powershell"></a>Återställa med hjälp av VMAccess-tillägget och PowerShell
Se till att du har den [senaste PowerShell-modulen installeras och konfigureras](/powershell/azure/overview) och har loggat in på Azure-prenumerationen med hjälp av den [Connect-AzureRmAccount](https://docs.microsoft.com/powershell/module/azurerm.profile/connect-azurermaccount) cmdlet.
