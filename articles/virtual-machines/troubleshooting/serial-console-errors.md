...
Webb-socket är stängt eller kunde inte öppnas. | Du kan behöva lägga till brand Väggs `*.console.azure.com`åtkomst till. En mer detaljerad men längre metod är att tillåta brand Väggs åtkomst till [Microsoft Azure Data Center IP-intervall](https://www.microsoft.com/download/details.aspx?id=41653), som ändras relativt regelbundet.
...
Den seriella konsolens anslutning till den virtuella datorn påträffade ett fel: "förbjuden" (SubscriptionNotEnabled)-prenumerations namnet är odefinierat, ID: t \<för prenumerations-ID> är inaktiverat tillstånd är odefinierat | Det här problemet kan inträffa om prenumerationen som en användare har skapat Cloud Shell lagrings konto i har inaktiverats. Du kan åtgärda problemet genom att starta Cloud Shell och [utföra de steg som krävs](https://docs.microsoft.com/azure/cloud-shell/persisting-shell-storage#unmount-clouddrive-1) för att etablera ett lagrings konto för lagring av Cloud Shell i den aktuella prenumerationen.
