...
1. När du har startat i enanvändarläge anger `chroot /sysroot` du för att växla `sysroot` till jailbrokad.
...
Oracle Linux levereras med GRUB aktive rad. Om du vill ange grub startar du om den `sudo reboot`virtuella datorn genom att köra och trycker sedan på ESC. Den här åtgärden visar fönstret GRUB. Om fönstret grub inte visas kontrollerar du att `GRUB_TERMINAL` radens värde innehåller en *serie konsol* `GRUB_TERMINAL="serial console"`(det vill säga). Återskapa GRUB med `grub2-mkconfig -o /boot/grub/grub.cfg`.
