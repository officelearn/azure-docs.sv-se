...
> **Det gick inte att ta bort lagrings kontot StorageAccountName. Fel: Det går inte att ta bort lagrings kontot eftersom dess artefakter används.**
> 
> **Det gick inte att ta bort # av # containers (<br>s): VHD: ar: Det finns för närvarande ett lån i behållaren och inget låne-ID har angetts i begäran.**
> 
...
5. I fönstret BLOB-metadata, kontrol lera och registrera värdet för **MicrosoftAzureCompute_DiskType**. Det här värdet anger om den anslutna disken är operativ system eller data disk (se **viktigt** om fältet inte finns). 

     ![Skärm bild av portalen med fönstret för BLOB-metadata för lagring öppen](./media/troubleshoot-vhds/utd-blob-metadata-sm.png)

6. Om typen av BLOB-disken är OSDisk [följer du steg 2: Ta bort den virtuella datorn för](#step-2-delete-vm-to-detach-os-disk)att koppla från OS-disken. Annars, om blobb disk typen är **DataDisk** , följer du stegen i [steg 3: Koppla från datadisk från den](#step-3-detach-data-disk-from-the-vm)virtuella datorn. 
...
### <a name="scenario-2-deleting-a-container---identify-all-blobs-within-container-that-are-attached-to-vms"></a>Scenario 2: Ta bort en container – identifiera alla blobar i behållaren som är kopplade till virtuella datorer
1. Logga in på [Azure Portal](https://portal.azure.com).
...
### <a name="scenario-3-deleting-storage-account---identify-all-blobs-within-storage-account-that-are-attached-to-vms"></a>Scenario 3: Tar bort lagrings konto – identifiera alla blobar i lagrings kontot som är kopplade till virtuella datorer
1. Logga in på [Azure Portal](https://portal.azure.com).
