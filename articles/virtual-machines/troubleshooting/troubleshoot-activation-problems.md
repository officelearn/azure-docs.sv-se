...
1. Kör **slmgr.vbs/DLV** i en kommando tolk med förhöjd behörighet. Kontrol lera Beskrivning svärdet i utdata och ta reda på om det skapades från återförsäljar-eller volym (VOLUME_KMSCLIENT) licens medium:
...
2. Om **slmgr.vbs /dlv** visar RETAIL-kanalen kör du följande kommandon för att konfigurera [KMS-klientens konfigurationsnyckel](/windows-server/get-started/kmsclientkeys) för versionen av Windows Server som används och tvinga den att försöka aktivera igen: 
