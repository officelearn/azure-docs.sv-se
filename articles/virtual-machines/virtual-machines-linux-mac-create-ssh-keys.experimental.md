...
Nästa uppgift är att skapa virtuella Azure Linux-datorer med den nya offentliga SSH-nyckeln.  Virtuella Azure-datorer som skapas med en offentlig SSH-nyckel för inloggning är bättre skyddade än virtuella datorer som skapas med vanliga lösenordsbaserade inloggningsmetoder.  Lösenord är som standard inaktiverade för Virtuella Azure-datorer som skapas med SSH-nycklar för att undvika råstyrkeattacker som försöker gissa lösenord. Om du behöver mer hjälp för att skapa SSH-nyckelpar eller om du behöver fler certifikat, till exempel för användning med den klassiska portalen, finns [detaljerade steg för att skapa SSH-nyckelpar och certifikat](virtual-machines-linux-create-ssh-keys-detailed.md).
