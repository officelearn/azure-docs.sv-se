---
title: "Skapa SSH-nycklar för Linux och Mac | Microsoft Azure"
...
Om du inte har en `~/.ssh`-katalog skapar `ssh-keygen`-kommandot en åt dig med rätt behörighet.
...
`User myusername` = fjärranvändarkontot som ska användas när du loggar in på servern.

`PubKeyAuthentication yes` = meddelar SSH att du vill använda en SSH-nyckel för att logga in.

`IdentityFile /home/myusername/.ssh/id_id_rsa` = den privata SSH-nyckeln och motsvarande offentliga nyckel som ska användas för autentisering.
