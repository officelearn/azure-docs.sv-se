...
* [Migrera virtuella datorer i en moln tjänst (inte i ett virtuellt nätverk)](#step-61-option-1---migrate-virtual-machines-in-a-cloud-service-not-in-a-virtual-network)
* [Migrera virtuella datorer i ett virtuellt nätverk](#step-61-option-2---migrate-virtual-machines-in-a-virtual-network)
* [Migrera ett lagrings konto](#step-62-migrate-a-storage-account)
...
> [!NOTE]
> Det virtuella nätverks namnet kan skilja sig från vad som visas i den nya portalen. Det nya Azure Portal visar namnet som `[vnet-name]`, men det faktiska virtuella nätverks namnet är av typen `Group [resource-group-name] [vnet-name]`. Innan du påbörjar migreringen ska du leta upp det faktiska namnet på det virtuella nätverket genom att använda kommandot `Get-AzureVnetSite | Select -Property Name` eller Visa det i det gamla Azure Portal. 
