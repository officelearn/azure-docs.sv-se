...
Du kan distribuera och hantera Oracle shardade-databaser med två gränssnitt: Oracle Enterprise Manager Cloud control GUI och/eller kommando rads verktyget `GDSCTL`. Du kan även övervaka olika Shards för tillgänglighet och prestanda med hjälp av moln kontroll. Kommandot `GDSCTL DEPLOY` skapar automatiskt Shards och deras respektive lyssnare. Dessutom distribuerar det här kommandot automatiskt den replikeringskonfiguration som används för att Shard hög tillgänglighet som anges av administratören.
...
Även om systemhanterade horisontell partitionering är det enklaste sättet att konfigurera och hantera, användardefinierade horisontell partitionering eller sammansatt horisontell partitionering passar bra för scenarier där dina data och program är geo-distribuerade eller i scenarier där du behöver kontrol lera replikeringen för varje Shard. 
