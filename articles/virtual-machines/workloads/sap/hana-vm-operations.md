...
UltraSSD ger dig möjlighet att definiera en enskild disk som uppfyller ditt storlek, IOPS och disk dataflöde intervall. Istället för att använda logiska volymhanterare som LVM eller MDADM ovanpå Azure Premium Storage för att skapa volymer som uppfyller krav på IOPS och lagring dataflöde. Du kan köra en blandning av konfiguration mellan UltraSSD och Premiumlagring. Därmed kan du begränsa användningen av UltraSSD för kritiska /hana/data för prestanda och /hana/log volymer och täcker andra volymer med Azure Premium Storage
...
Värdena tros bara vara en startpunkt och måste utvärderas mot de verkliga behoven. Fördelen med Azure Ultra SSD är att värdena för IOPS och dataflöde kan anpassas utan att behöva stänga den virtuella datorn eller stoppat arbetsbelastningen användas i systemet.   
