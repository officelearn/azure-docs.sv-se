---
title: 'Snabb start: skapa en NAT-gateway – Azure CLI'
...
ms.locfileid: "72376401"
---
# <a name="quickstart-create-a-nat-gateway-using-azure-cli"></a>Snabb start: skapa en NAT-gateway med Azure CLI

Den här snabb starten visar hur du använder Azure NAT-tjänsten och skapar en NAT-gateway för att tillhandahålla utgående anslutning för en virtuell dator i Azure. 
...
Du kan slutföra den här självstudien med Azure Cloud Shell eller köra respektive kommando lokalt.  Om du aldrig har använt Azure Cloud Shell [loggar du in nu](https://shell.azure.com) för att gå igenom den första installationen.
...
Skapa det virtuella nätverket **myVnet** med undernätet **mySubnet** i **myResourceGroup** med [az network vnet create](https://docs.microsoft.com/cli/azure/network/vnet).  IP-adressutrymmet för det virtuella nätverket är **192.168.0.0/16** och under nätet i det virtuella nätverket är **192.168.0.0/24**.
...
Vi konfigurerar käll under nätets **undernät** i det virtuella nätverket **myVnet** för att använda en angiven NAT-gateway resurs **myNAT** med [AZ Network VNet Subnet Update](https://docs.microsoft.com/cli/azure/network/vnet/subnet).  Med det här kommandot aktive ras NAT-tjänsten på det angivna under nätet.
...
Vänta tills den virtuella datorn har slutfört distributionen och fortsätt sedan med resten av stegen.
...
I den här självstudien har du skapat en NAT-gateway och en virtuell dator som använder NAT-tjänsten. Om du vill veta mer om Azure NAT-tjänsten fortsätter du till andra självstudier för Azure NAT-tjänsten.
