...
I ett senare steg används `tracert.exe` kommandot för att testa routning. Tracert använder Internet Control Message Protocol (ICMP) som nekas via Windows-brandväggen. Aktivera ICMP via Windows-brandväggen genom att ange följande kommando från PowerShell på den virtuella datorn *myVmPrivate*:
