---
title: 'Självstudie: skapa och testa en NAT-gateway – Azure CLI'
...
Den här självstudien visar hur du använder Azure NAT-tjänsten och skapar en NAT-gateway för att tillhandahålla utgående anslutning för virtuella datorer i Azure. Om du vill testa NAT-gatewayen distribuerar du en virtuell käll-och mål dator.  Du kommer att testa NAT-gatewayen genom att göra utgående anslutningar till en offentlig IP-adress från källan till den virtuella mål datorn.  I den här självstudien distribueras källa och mål i två olika virtuella nätverk i samma resurs grupp för enkelhetens skull.
...
Du kan slutföra den här självstudien med Azure Cloud Shell eller köra respektive kommando lokalt.  Om du aldrig har använt Azure Cloud Shell bör du [Logga in nu](https://shell.azure.com) för att gå igenom den första installationen.
...
Du kan använda en eller flera offentliga IP-adressresurser eller en eller flera offentliga IP-prefix eller både och med NAT-gateway. Vi kommer att lägga till en resurs för offentliga IP-prefix i det här scenariot för att demonstrera.   Använd [AZ Network Public-IP prefix Create](https://docs.microsoft.com/cli/azure/network/public-ip-prefix) för att skapa en offentlig IP-prefixlängd som heter **myPublicIPprefixsource** i **myResourceGroupNAT**.
...
Vi vägleder dig genom konfigurationen av en fullständig test miljö och körningen av testerna i nästa steg. Vi kommer att börja med källan som använder NAT gateway-resursen som vi skapade tidigare.
...
Skapa ett virtuellt nätverk med namnet **myVnetsource** med ett undernät med namnet **mySubnetsource** i **myResourceGroupNAT** med [AZ Network VNet Create](https://docs.microsoft.com/cli/azure/network/vnet).  IP-adressutrymmet för det virtuella nätverket är **192.168.0.0/16** och under nätet i det virtuella nätverket är **192.168.0.0/24**.
...
Eftersom standard-offentliga IP-adresser är "säkra som standard", måste vi skapa en NSG för att tillåta inkommande åtkomst för SSH-åtkomst.  Eftersom NAT-tjänsten är en flödes riktning som är medveten om att den här NSG inte används för utgående trafik när NAT-gateway har kon figurer ATS i samma undernät. Använd [AZ Network NSG Create](https://docs.microsoft.com/cli/azure/network/nsg?view=azure-cli-latest#az-network-nsg-create) för att skapa en NSG-resurs med namnet **myNSGsource** i **myResourceGroupNAT**.
...
Vi skapar en regel i NSG för SSH-åtkomst till den virtuella käll datorn. Använd [AZ Network NSG Rule Create](https://docs.microsoft.com/cli/azure/network/nsg/rule?view=azure-cli-latest#az-network-nsg-rule-create) för att skapa en NSG-regel med namnet **SSH** i NSG som heter **myNSGsource** i **myResourceGroupNAT**.
...
Innan du distribuerar en virtuell dator och för målet, måste du också skapa ett virtuellt nätverk där den virtuella mål datorn kan placeras.  Detta är samma steg som för den virtuella käll datorn med några små ändringar för att exponera mål slut punkten.

Skapa ett virtuellt nätverk med namnet **myVnetdestination** med ett undernät med namnet **mySubnetdestination** i **myResourceGroupNAT** med [AZ Network VNet Create](https://docs.microsoft.com/cli/azure/network/vnet).  IP-adressutrymmet för det virtuella nätverket är **192.168.0.0/16** och under nätet i det virtuella nätverket är **192.168.0.0/24**.
...
De här kommandona kommer att uppdatera den virtuella datorn, installera nginx och skapa en 100 KByte-fil som du kan använda för att hämta från den virtuella käll datorn med hjälp av NAT-tjänsten.
...
SSH-autentiseringsuppgifterna lagras återigen i Cloud Shell. Öppna en ny flik för [Azure Cloud Shell](https://shell.azure.com) i webbläsaren.  Använd IP-adressen som hämtades i föregående steg för SSH till den virtuella datorn. 
...
Detta kommer att uppdatera den virtuella datorn, installera Go, installera [Hej](https://github.com/rakyll/hey) från GitHub och uppdatera din gränssnitts miljö.
...
Använd sväng för att hämta 100 KByte-filen.  Ersätt **\<ip-Address-destination >** i exemplet nedan med mål-IP-adressen som du tidigare har kopierat.  Parametern **--output** anger att den hämtade filen kommer att tas bort.
...
Detta genererar 100-begäranden, 10 samtidigt, med en tids gräns på 30 sekunder och utan att återanvända TCP-anslutningen.  Varje begäran kommer att hämta 100 kByte.  I slutet av körningen rapporterar **Hej** viss statistik om hur väl NAT-tjänsten utförts.
...
I den här självstudien har du skapat en NAT-gateway, skapat en källa och en virtuell måldator och testat NAT-gatewayen. Om du vill veta mer om Azure NAT-tjänsten fortsätter du till andra självstudier för Azure NAT-tjänsten.

Du kan också granska mått i Azure Monitor för att se hur NAT-tjänsten fungerar och diagnostisera problem som resurs utbelastning av tillgängliga SNAT-portar.  Resurs överbelastning av SNAT-portar kan enkelt åtgärdas genom att lägga till ytterligare offentliga IP-adressresurser eller resurser för offentliga IP-prefix eller båda.
