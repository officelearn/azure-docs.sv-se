...
Varje grupp innehåller flera [standardsäkerhetsregler](security-overview.md#default-security-rules). Den regel som följer åsidosätter en standardsäkerhetsregel som tillåter utgående åtkomst till alla offentliga IP-adresser. Den `destination-address-prefix "Internet"` alternativet nekar utgående åtkomst till alla offentliga IP-adresser. Den föregående regeln åsidosätter den här regeln på grund av dess högre prioritet, vilket ger åtkomst till de offentliga IP-adresserna i Azure Storage.
...
Följande regel tillåter SSH-trafik inkommande i undernätet från var som helst. Regeln åsidosätter en standardsäkerhetsregel som nekar all inkommande trafik från internet. SSH tillåts i undernätet så att anslutningen kan testas i ett senare steg.
