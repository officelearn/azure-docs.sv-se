...
Montera Azure-filresursen i den katalog som du skapade. Innan du kör följande kommando ersätter du `<storage-account-name>` med konto namnet och `<storage-account-key>` med den nyckel som du hämtade i [skapa ett lagrings konto](#create-a-storage-account).
