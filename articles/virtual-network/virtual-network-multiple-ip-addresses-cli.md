...
3. Logga in med kommandot `az login` från ett kommando gränssnitt och välj den prenumeration som du använder.
...
När den virtuella datorn har skapats anger du kommandot `az network nic show --name MyNic1 --resource-group myResourceGroup` för att Visa NIC-konfigurationen. Ange `az network nic ip-config list --nic-name MyNic1 --resource-group myResourceGroup --output table` för att visa en lista över de IP-konfigurationer som är kopplade till NÄTVERKSKORTet.
...
1. Öppna ett kommando gränssnitt och slutför de återstående stegen i det här avsnittet inom en enda session. Om du inte redan har installerat och konfigurerat Azure CLI slutför du stegen i artikeln [Azure CLI-installation](/cli/azure/install-az-cli2?toc=%2fazure%2fvirtual-network%2ftoc.json) och loggar in på ditt Azure-konto med kommandot `az-login`.
