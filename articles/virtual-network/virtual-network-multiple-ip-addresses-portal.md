...
Om du vill skapa en virtuell dator med flera IP-adresser eller en statisk privat IP-adress, måste du skapa den med hjälp av PowerShell eller Azure CLI. Klicka på PowerShell eller CLI alternativ överst i den här artikeln för att lära dig hur. Du kan skapa en virtuell dator med en dynamisk privat IP-adress och (valfritt) en offentlig IP-adress med hjälp av portalen genom att följa stegen i den [skapa en virtuell Windows-dator](../virtual-machines/virtual-machines-windows-hero-tutorial.md) eller [skapa ett Linux VM](../virtual-machines/linux/quick-create-portal.md) artiklar. När du har skapat den virtuella datorn kan du ändra IP-adress från dynamisk till statisk och lägga till ytterligare IP-adresser med hjälp av portalen genom att följa anvisningarna i den [Lägg till IP-adresser till en virtuell dator](#add) i den här artikeln.
...
3. I den **virtuella datorer** bladet, klickar du på den virtuella datorn som du vill lägga till IP-adresser till. Klicka på **nätverksgränssnitt** bladet som visas och välj sedan nätverksgränssnittet som du vill lägga till IP-adresser till på den virtuella datorn. I exemplet som visas i följande bild, nätverkskortet med namnet *myNIC* från den virtuella datorn med namnet *myVM* väljs:

    ![Nätverksgränssnitt](./media/virtual-network-multiple-ip-addresses-portal/figure1.png)

4. I bladet som visas för nätverkskort som du har valt klickar du på **IP-konfigurationer**.
...
2. Klicka på **Lägg till**. I den **lägga till IP-konfiguration** bladet som visas, skapa en IP-konfiguration med namnet *IPConfig 4* med *10.0.0.7* som en *statiska* privata IP-adress och klicka sedan på **OK**.

    > [!NOTE]
    > När du lägger till en statisk IP-adress, måste du ange en oanvända, giltig adress på nätverkskortet är anslutet till undernätet. Om den adress som du väljer inte är tillgängligt på portalen visas ett X för IP-adress och måste du välja en annan.

3. När du klickar på OK bladet stängs och du ser den nya IP-konfigurationen visas. Klicka på **OK** att stänga den **lägga till IP-konfiguration** bladet.
...
5. Lägga till privata IP-adresser i Virtuella operativsystem genom att slutföra stegen för operativsystemet i den [lägga till IP-adresser till ett VM-operativsystem](#os-config) i den här artikeln.
...
1. Bläddra till Azure-portalen på https://portal.azure.com och logga in, om det behövs.
3. I portalen klickar du på **ny** > **nätverk** > **offentliga IP-adressen**.
4. I den **skapa offentlig IP-adress** bladet som visas, ange en **namn**väljer en **IP-adresstilldelning** typ, en **prenumeration**, **resursgruppen**, och en **plats**, klicka på **skapa**som visas i följande bild:
...
1. Utför stegen i den [viktiga steg](#coreadd) i den här artikeln.
2. Klicka på **Lägg till**. I den **lägga till IP-konfiguration** bladet som visas, skapa en IP-konfiguration med namnet *IPConfig 4*. Aktivera den **offentliga IP-adressen** och välj en befintlig, tillgängliga offentliga IP-adressresurs från den **Välj offentlig IP-adress** bladet som visas.

    När du har markerat den offentliga IP-adressresursen, klickar du på **OK** och bladet stängs. Om du inte har en befintlig offentlig IP-adress, kan du skapa en genom att slutföra stegen i den [skapa en offentlig IP-adressresurs](#create-public-ip) i den här artikeln. 
...
3. Klicka på bladet IPConfig **IP-adress**.
4. I den **Välj offentlig IP-adress** bladet som visas, Välj en offentlig IP-adress.
5. Klicka på **spara** och bladen stängs. Om du inte har en befintlig offentlig IP-adress, kan du skapa en genom att slutföra stegen i den [skapa en offentlig IP-adressresurs](#create-public-ip) i den här artikeln.
