...
Om du vill komma åt köer i ASP.NET Core-projekt inkluderar du följande objekt i valfri C#-källfil som har åtkomst till Azure Queue Storage. Använd all den här koden framför koden i de avsnitt som följer.
...
Du kan hämta en uppskattning av antalet meddelanden i en kö. `FetchAttributes` Metoden uppmanar Queue Service att hämta attributen för kön, inklusive antalet meddelanden. `ApproximateMethodCount` Egenskapen returnerar det sista värdet som hämtades av `FetchAttributes` metoden, utan att köa tjänsten anropas.
