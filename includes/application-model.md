...
-   **Hohe Verfügbarkeit.** Hohe Verfügbarkeit bedeutet, dass Azure sicherstellt,
    ...
    und startet die Anwendung automatisch neu. Wenn die
    ...
    und führt den Code von dort aus. Hinweis: In der Reihenfolge für die Anwendung
    die Microsoft-Servicelevelvereinbarung von 99,95 % Verfügbarkeit einhalten kann,
    müssen mindestens zwei virtuelle Computer den Anwendungscode ausführen. Dieses
...
Ausführung als eine oder mehrere *Rollen.* Ein *Rolle* bezieht sich auf die Anwendung
...
Anzahl von virtuellen Computern oder *Rolleninstanzen*, zum Ausführen. Die Abbildung unten zeigt zwei
...
Rechenzentrum durch einen sogenannten eine *eingabeendpunkt*. Eine einzelne Rolle
...
zwei eingabeendpunkten: HTTP-Überwachung auf Port 80 und HTTPS-Überwachung
Port 443. Die folgende Abbildung zeigt ein Beispiel für zwei unterschiedliche Rollen
...
im Wesentlichen das Reservieren der *Präfix*. CLOUDAPP.NET-URL, damit niemand
...
*Präfix*. CLOUDAPP.NET-URL. Stattdessen kaufen Sie einen DNS-Namen von.
...
die [Datenspeicherlösungen in Azure][] Artikel
...
Anwendung aus verschiedenen Rollen bestehen, die zusammenarbeiten. Weitere
...
sollen. Nachdem die Urlaubszeit erhalten Sie möglicherweise viele Produkte zurück,
...
der Anwendung in der Stagingumgebung befindet. Das vollständige Schema für die XML-Datei ist unter folgendem Link zu finden:
...
Weitere Informationen finden Sie unter [Azure-Preise] []. Im Allgemeinen ist es
ratsam, viele kleine Rolleninstanzen anstelle weniger großer
Instanzen verwenden, damit Ihre Anwendung weniger fehleranfällig ist. Nach
...
-   **Eingabeendpunkte**. Hier geben Sie HTTP-, HTTPS- oder TCP-Endpunkte
    ...
    über die *Präfix*. cloadapp.net-URL. Wenn Azure Ihre
...
-   **Lokaler Speicher**. Dies weist ein Unterverzeichnis in der Rolleninstanz
    ...
    in der [Datenspeicherlösungen in Azure][] Artikel.
...
beschreibt, die geändert werden können, ohne die Anwendung erneut bereitzustellen. Die
...
der Anwendung in der Stagingumgebung befindet. Einige der Elemente, die Sie in der CSCFG-Datei angeben können,
werden im Folgenden beschrieben:

-   **BS-Version**. Dieses Attribut ermöglicht Ihnen die Auswahl der Betriebssystemversion,
    die für alle Rolleninstanzen verwendet werden soll, die
    Ihren Anwendungscode ausführen. Dieses Betriebssystem wird als bezeichnet die *Gastbetriebssystem*, und jede
    ...
    Informationen zu Gast-BS-Versionen finden Sie unter [Managing Upgrades to the
...
Erstellen eines gehosteten Diensts erfordert, dass Sie zuerst die [Azure Management Portal] und einen gehosteten Dienst bereitstellen, indem Sie angeben
ein DNS-Präfix und das Rechenzentrum angeben, in dem Ihr Code im Endeffekt
in. Klicken Sie dann in Ihrer Entwicklungsumgebung Sie Ihren Dienst zu erstellen
...
und startet die Instanzen. Nun wird Ihr Code ausgeführt.
...
Rollen finden Sie in der [Bereitstellen und Aktualisieren von Azure Applications][]
Artikel.<a id="Ref" name="Ref"></a>
