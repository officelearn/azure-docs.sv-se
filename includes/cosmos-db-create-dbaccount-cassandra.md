...
3. I den **nytt konto** anger du inställningarna för det nya kontot i Azure Cosmos DB. 
    ...
    API|Cassandra|API: et avgör vilken typ av konto för att skapa. Azure Cosmos-DB innehåller fem-API: er som passar bäst för ditt program: SQL (dokumentdatabasen), Gremlin (graph-databas), MongoDB (dokumentdatabasen), Azure Table och Cassandra, varje som för närvarande kräver ett särskilt konto. <br><br>Välj **Cassandra** eftersom den här snabbstarten du skapar en databas i hela kolumner som är frågbar med CQL syntax.<br><br>Om Cassandra (wide kolumn) visas inte i listan, så du behöver [tillämpa för att ansluta till](https://aka.ms/cosmosdb-cassandra-signup) förhandsgranskningsprogrammet Cassandra API.<br><br> [Mer information om Cassandra-API](../articles/cosmos-db/cassandra-introduction.md)|
