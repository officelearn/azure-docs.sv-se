...
Dieser Leitfaden beschreibt, wie Sie mithilfe der Azure-Befehlszeilentools für Mac und Linux Dienste in Azure erstellen und verwalten. Die behandelten Szenarien umfassen **Installation der Tools**, **Import Ihrer veröffentlichungseinstellungen**, **Erstellen und Verwalten von Azure-Websites**, und **Erstellen und Verwalten von Azure Virtual Machines**. Umfassendes Referenzmaterial finden Sie unter [Azure-Befehlszeilentool für Mac und Linux-Dokumentation][reference-docs]. 
...
<h2><a id="Overview"></a>Was sind die Azure-Befehlszeilentools für Mac und Linux</h2>
...
Geben Sie eine vollständige Liste der unterstützten Befehle, `azure -help` in der Befehlszeile nach der Installation der Tools finden Sie unter der [Referenzdokumentation][reference-docs].

<h2><a id="Download">Gewusst wie: Installieren der Azure-Befehlszeilentools für Mac und Linux</a></h2>

Die folgende Liste enthält Informationen zur Installation der Befehlszeilentools, abhängig von Ihrem Betriebssystem:

* **Mac**: Herunterladen der [Azure SDK Installer][mac-installer]. Öffnen Sie die heruntergeladene PKG-Datei, und führen Sie die Installationsschritte durch, zu denen Sie aufgefordert werden.

* **Linux**: Installieren der neuesten Version von [Node.js][nodejs-org] (finden Sie unter [Installieren von Node.js via Package Manager][install-node-linux]), führen Sie den folgenden Befehl:
...
* **Windows**: Führen Sie den Windows Installer (MSI-Datei), die hier verfügbar ist: [Azure-Befehlszeilentools][windows-installer].
...
<h2><a id="Account"></a>Zum Herunterladen und Importieren der veröffentlichungseinstellungen</h2>
...
Sie können alle von gespeicherten Informationen Entfernen der <code>import</code> Befehl mithilfe der <code>account clear</code> Befehl:
...
> [AZURE.NOTE] Beim Importieren von veröffentlichungseinstellungen, Anmeldeinformationen für den Zugriff auf Ihr Azure-Abonnement befinden sich innerhalb der `user` Ordner. Ihr `user`-Ordner ist durch Ihr Betriebssystem geschützt. Es wird jedoch empfohlen, dass Sie zusätzliche Schritte vornehmen, um Ihren `user`-Ordner zu verschlüsseln. Sie haben dazu folgende Möglichkeiten:    
> 
...
<h2><a id="WebSites"></a>Das Erstellen und Verwalten einer Azure-Website</h2>
...
<h2><a id="VMs"></a>Das Erstellen und Verwalten von Azure-Computer</h2>
