...
<sup>1</sup> kräver "i"- `direction` bindning i *Function. JSON* eller `FileAccess.Read` i ett C#-klass bibliotek. Du kan dock använda behållarobjektet som körningen tillhandahåller för att utföra Skriv åtgärder, till exempel att ladda upp blobar till behållaren.

<sup>2</sup> kräver "Inout"- `direction` bindning i *Function. JSON* eller `FileAccess.ReadWrite` i ett C#-klass bibliotek.
