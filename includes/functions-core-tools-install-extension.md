...
> [!NOTE]
> Om du vill registrera tillägg manuellt med hjälp av `func extensions install` måste du ha .net Core 2. x SDK installerat.

När du har uppdaterat *function.jspå* fil för att inkludera alla bindningar som din funktion behöver kör du följande kommando i projektmappen.

```bash
func extensions install
```

Kommandot läser *function.jspå* fil för att se vilka paket du behöver, installerar dem och återbygga tilläggs projektet. Den lägger till eventuella nya bindningar i den aktuella versionen men uppdaterar inte befintliga bindningar. Använd `--force` alternativet för att uppdatera befintliga bindningar till den senaste versionen när du installerar nya.
