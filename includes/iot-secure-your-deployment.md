...
* AMQP: SASL PLAIN-och AMQP-baserad säkerhet (`{policyName}@sas.root.{iothubName}` med IoT Hub-token-token, `{deviceId}` med enhets omfångs-token).

* MQTT: Anslut Packet använder `{deviceId}` som `{ClientId}``{IoThubhostname}/{deviceId}` i fältet **username** och en SAS-token i fältet **lösen ord** .
