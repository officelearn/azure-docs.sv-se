...
| Antal ämnen/köer per namnområde |Namnrymd |Efterföljande förfrågningar för att skapa ett nytt ämne eller en kö i namnområdet avvisas. Resultatet blir att, om konfigurerad via den [Azure-portalen][Azure portal], genereras ett felmeddelande. Om den anropas från ett hanteringsgränssnitt emot ett undantag av den anropande koden. |10 000 (basic/Standard-nivå). Det totala antalet ämnen och -köer i ett namnområde måste vara mindre än eller lika med 10 000. <br/><br/>1 000 per meddelanden unit(MU) på premium-nivån. Maxgränsen är 4 000. |
