---
title: storage-files-create-clean-up-portal
description: Rensa upp efter Azure Files-snabbstarter.
services: storage
author: wmgries
ms.service: storage
ms.topic: include
ms.date: 03/28/2018
ms.author: wgries
ms.custom: include file
ms.openlocfilehash: 842c08ec36730eaab1c37fe61d3855f9e2ddd245
ms.sourcegitcommit: 829d951d5c90442a38012daaf77e86046018e5b9
ms.translationtype: MT
ms.contentlocale: sv-SE
ms.lasthandoff: 10/09/2020
ms.locfileid: "91329471"
---
När du är klar kan du ta bort resursgruppen. När du tar bort resursgruppen tar du bort lagringskontot, Azure-filresursen och andra eventuella resurser som du distribuerat i resursgruppen.

1. Välj **Resursgrupper** på den vänstra menyn.
1. Högerklicka på resursgruppen och välj sedan **Ta bort resursgrupp**. Ett fönster öppnas och visar en varning om vilka resurser som tas bort med resursgruppen.
1. Ange resursgruppens namn och välj sedan **Ta bort**.