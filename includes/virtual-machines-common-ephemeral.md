...
Om du vill använda en tillfällig disk för en distribution av CLI-VM ställer du in parametern `--ephemeral-os-disk` i [AZ VM Create](/cli/azure/vm#az-vm-create) to `true` och `--os-disk-caching`-parametern för att `ReadOnly`.
...
För skalnings uppsättningar använder du samma `--ephemeral-os-disk true` parameter för [AZ-VMSS-Create](/cli/azure/vmss#az-vmss-create) och anger parametern `--os-disk-caching` till `ReadOnly`.
...
Processen för att skapa en skalnings uppsättning som använder en tillfällig OS-disk är att lägga till `diffDiskSettings`-egenskapen i `Microsoft.Compute/virtualMachineScaleSets/virtualMachineProfile` resurs typ i mallen. Dessutom måste caching-principen anges till `ReadOnly` för den tillfälliga OS-disken. 
