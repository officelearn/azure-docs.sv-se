...
Det beror på gateway-SKU. Mer information om antalet anslutningar som stöds finns i [Gateway SKU: er](../articles/vpn-gateway/vpn-gateway-about-vpngateways.md#gwsku).
...
Azure stöder tre typer av VPN-alternativ från punkt till plats:

* SSTP (Secure Socket Tunneling Protocol). SSTP är en Microsoft-egenutvecklad SSL-baserad lösning som kan penetrera brandväggar eftersom de flesta brandväggar öppnar den utgående TCP-port som 443 SSL använder.

* Openvpn. OpenVPN är en SSL-baserad lösning som kan penetrera brandväggar eftersom de flesta brandväggar öppnar den utgående TCP-porten som 443 SSL använder.

* IKEv2 VPN. IKEv2 VPN är en standardbaserad IPsec VPN-lösning som använder utgående UDP-portar 500 och 4500 och IP-protokollnr. 50. Eftersom inte alla brandväggar öppnar dessa portar kan det hända att IKEv2 VPN inte kan komma igenom proxyservrar och brandväggar.
...
### <a name="can-i-configure-a-point-to-site-client-to-connect-to-multiple-virtual-network-gateways-at-the-same-time"></a>Kan jag konfigurera en Point-to-Site-klient för att ansluta till flera virtuella nätverksgateways samtidigt?

Beroende på vilken VPN-klientprogram som används kan du kanske ansluta till flera virtuella nätverksgateways förutsatt att de virtuella nätverk som är anslutna till inte har motstridiga adressutrymmen mellan dem eller nätverket från med klienten ansluter från.  Azure VPN-klienten stöder många VPN-anslutningar, men endast en anslutning kan anslutas vid en given tidpunkt.
...
Ja, Point-to-Site-anslutningar till en virtuell nätverksgateway som distribueras i ett virtuellt nätverk som är peered med andra virtuella nätverk kan ha åtkomst till andra peer-vnets.  Förutsatt att de peer-ed virtuella nätverken använder funktionerna UseRemoteGateway / AllowGatewayTransit, kommer Point-to-Site-klienten att kunna ansluta till de peer-peer-virtuella nätverken.  För mer information hänvisas till [den här](../articles/vpn-gateway/vpn-gateway-about-point-to-site-routing.md) artikeln.
...
IKEv2 stöds på Windows 10 och Server 2016. Om du vill använda IKEv2 måste du installera uppdateringar och ange ett registreringsnyckelvärde lokalt. OS-versioner före Windows 10 stöds inte och kan endast använda SSTP eller **OpenVPN® Protocol**.
...
### <a name="how-do-i-remove-the-configuration-of-a-p2s-connection"></a><a name="removeconfig"></a>Hur tar jag bort konfigurationen av en P2S-anslutning?

En P2S-konfiguration kan tas bort med Azure CLI och PowerShell med följande kommandon:
